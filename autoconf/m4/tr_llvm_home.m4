...
AC_DEFUN([TR_LLVM_HOME],[
    AC_ARG_WITH([llvm],
                ...
                [
                 AC_PATH_PROGS(LLVM_CONFIG, llvm-config, no-llvm-config, "/usr/lib64/llvm20/bin:/usr/lib64/llvm/bin:/bin:/usr/bin:/usr/local/bin:/sw/bin:/opt/homebrew/opt/llvm/bin")
                 AS_IF([test "$ac_cv_path_LLVM_CONFIG" = "no-llvm-config"],AC_MSG_ERROR([could not find llvm-config]),[])
