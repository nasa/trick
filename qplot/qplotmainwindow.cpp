    ...
    _rubber_band(0)
{
    ...
    ui->customPlot->plotLayout()->
            addElement(0, 0, new QCPPlotTitle(ui->customPlot, "Snap!"));
...
void QPlotMainWindow::axisLabelDoubleClick(QCPAxis *axis,
                                           QCPAxis::SelectablePart part)
{
    // Set an axis label by double clicking on it
    if (part == QCPAxis::spAxisLabel) {
        // only react when the actual axis label is clicked,
        // not tick label or axis backbone
        bool ok;
...
void QPlotMainWindow::legendDoubleClick(QCPLegend *legend,
                                       QCPAbstractLegendItem *item)
{
    ...
    if (item)  {
        ...
        // (user could have clicked on border padding of legend
...
void QPlotMainWindow::selectionChanged()
{
   ...
   selectable separately, but we want the user only to be able to select the axis
   as a whole, so we tie the selected states of the tick labels
   ...
   Further, we want to synchronize the selection of the graphs with the selection
   state of the respective legend item belonging to that graph. So the user can
   select a graph by either clicking on the graph itself
  ...
  */
    ...
    if (ui->customPlot->xAxis->selectedParts().testFlag(QCPAxis::spAxis) ||
        ...
        ui->customPlot->xAxis2->
                setSelectedParts(QCPAxis::spAxis|QCPAxis::spTickLabels);
        ui->customPlot->xAxis->
                setSelectedParts(QCPAxis::spAxis|QCPAxis::spTickLabels);
    }
    ...
    if (ui->customPlot->yAxis->selectedParts().testFlag(QCPAxis::spAxis) ||
        ...
        ui->customPlot->yAxis2->
                setSelectedParts(QCPAxis::spAxis|QCPAxis::spTickLabels);
        ui->customPlot->yAxis->
                setSelectedParts(QCPAxis::spAxis|QCPAxis::spTickLabels);
    }

    // sync selection of graphs with selection of corresponding legend items:
    for (int i=0; i<ui->customPlot->graphCount(); ++i)
    {
        QCPGraph *graph = ui->customPlot->graph(i);
        QCPPlottableLegendItem *item = ui->customPlot->
                                          legend->itemWithPlottable(graph);
...
void QPlotMainWindow::mousePress(QMouseEvent* mevent)
{
    ...
    if ( mevent->button() == Qt::MidButton ){
        _origin = mevent->pos();
        if ( !_rubber_band ) {
            _rubber_band = new QRubberBand(QRubberBand::Rectangle,ui->customPlot);
        }
        ...
        if (ui->customPlot->xAxis->selectedParts().testFlag(QCPAxis::spAxis))
            ui->customPlot->axisRect()->
                    setRangeDrag(ui->customPlot->xAxis->orientation());
        else if (ui->customPlot->yAxis->selectedParts().testFlag(QCPAxis::spAxis))
            ui->customPlot->axisRect()->
                    setRangeDrag(ui->customPlot->yAxis->orientation());
        else
            ui->customPlot->axisRect()->setRangeDrag(Qt::Horizontal|Qt::Vertical);
    }
}

void QPlotMainWindow::mouseMove(QMouseEvent *mevent)
{
    if ( mevent->button() == Qt::NoButton && _rubber_band ){
        _rubber_band->setGeometry(QRect(_origin, mevent->pos()).normalized());
    }
}

void QPlotMainWindow::mouseRelease(QMouseEvent *mevent)
{
    if ( mevent->button() == Qt::MidButton && _rubber_band ) {
        QRect geom = _rubber_band->geometry();
        double gxmin = (double)geom.left();
        double gxmax = (double)geom.right();
        double xmin = ui->customPlot->xAxis->pixelToCoord(gxmin);
        double xmax = ui->customPlot->xAxis->pixelToCoord(gxmax);
        double gymin = (double)geom.top();
        double gymax = (double)geom.bottom();
        double ymin = ui->customPlot->yAxis->pixelToCoord(gymax); // screen flip
        double ymax = ui->customPlot->yAxis->pixelToCoord(gymin); // screen flip
...
void QPlotMainWindow::mouseWheel()
{
    ...
    if (ui->customPlot->xAxis->selectedParts().testFlag(QCPAxis::spAxis)) {
        ui->customPlot->axisRect()->
                setRangeZoom(ui->customPlot->xAxis->orientation());
    } else if (ui->customPlot->yAxis->selectedParts().testFlag(QCPAxis::spAxis)) {
        ui->customPlot->axisRect()->
                setRangeZoom(ui->customPlot->yAxis->orientation());
    } else {
...
{
    // make sure this slot is really called by a context menu action,
    // so it carries the data we need
    if (QAction* contextAction = qobject_cast<QAction*>(sender()))
    {
        ...
        if (ok)
        {
            ui->customPlot->axisRect()->insetLayout()->
                    setInsetAlignment(0, (Qt::Alignment)dataInt);
            ui->customPlot->replot();
        }
    }
}
...
{
    ui->statusBar->
            showMessage(QString("Clicked on graph '%1'.").
