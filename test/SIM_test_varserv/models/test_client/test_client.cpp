...
TEST_F (VariableServerTest, Binary) {
    ...
    // EXPECT_EQ(strcmp(message.variables[0].getName().c_str(), "vsx.vst.o"), 0);
    // EXPECT_EQ(message.variables[0].getType(), TRICK_STRING);
    // EXPECT_EQ(message.variables[0].getValue<std::string>(), std::string("You will rejoice to hear that no disaster has accompanied the commencement of an enterprise which you have regarded with such evil forebodings. I arrived here yesterday, and my first task is to assure my dear sister of my welfare and increasing confidence in the success of my undertaking."));
