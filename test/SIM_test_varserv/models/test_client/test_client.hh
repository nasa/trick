...
class Socket
{
    public:
        ...
        {
            // weird syntax I've never used before - since the send syscall that i'm trying to use is overloaded in this
            // class, I have to append :: to the front of it so that the compiler knows to look in the global namespace
        ...
        int operator<<(std::string message) { return send(message); }
        ...
        void operator>>(std::string& ret) { ret = receive(); }

        void operator>>(std::ostream& stream) { stream << receive(); }
