...
# truncation bounds.
mc_var = trick.MonteCarloVariableRandomNormal( "monte_carlo.x_normal_trunc[0]", 11122, 0, 5)
...
mc_var = trick.MonteCarloVariableRandomNormal( "monte_carlo.x_normal_trunc[1]", 77546, 75, 5)
mc_var.truncate(-0.5, 2, trick.MonteCarloVariableRandomNormal.StandardDeviation)
mc_var.thisown = False
monte_carlo.mc_master.add_variable(mc_var)

# should keep values greater than 90.0
mc_var = trick.MonteCarloVariableRandomNormal( "monte_carlo.x_normal_trunc[2]", 60540, 100, 5)
mc_var.truncate_low(-2, trick.MonteCarloVariableRandomNormal.StandardDeviation)
mc_var.thisown = False
monte_carlo.mc_master.add_variable(mc_var)

# should keep values less than 135.0
mc_var = trick.MonteCarloVariableRandomNormal( "monte_carlo.x_normal_trunc[3]", 77077, 125, 5)
