...
Trick::Segment::Segment(std::string in_name, std::string enter_job_class_str, std::string exit_job_class_str)
    : segment_name(in_name)
{
...
std::string Trick::Segment::get_segment_name() { return segment_name; }

void Trick::Segment::set_segment_name(std::string in_segment_name) { segment_name = in_segment_name; }
...
int Trick::Segment::enter(std::string prev_segment __attribute__((unused))) { return enter_jobs.execute_all_jobs(); }

// Call the exit_jobs when exiting this segment.
int Trick::Segment::exit(std::string next_segment __attribute__((unused))) { return exit_jobs.execute_all_jobs(); }

std::map<std::string, Trick::ScheduledJobQueue*>& Trick::Segment::get_job_queues() { return job_queues; }
