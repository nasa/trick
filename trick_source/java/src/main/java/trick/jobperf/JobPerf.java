...
public class JobPerf {
    ...
    public JobPerf(String[] args) {
        ...
        String filesDir =
                Paths.get(timeLineFileName).toAbsolutePath().getParent().toString();
        ...
        if (frameArray.length <= 1) {
            System.out.println("WARNING: Fewer than two frames were found.\n"
        ...
        for (int frameNumber = 0; frameNumber < frameArray.length; frameNumber++) {
            ...
            for (JobExecutionEvent jobExec : frame.jobEvents) {
                ...
                if (frameNumber > 0 && jobExec.start == 0) {
                    System.out.println(
                            "WARNING: Job " + jobExec.id + " in frame " + frameNumber + " has a start time of zero.");
                }

                // TEST: Job start times greater than the stop time indicates a problem.
                if (jobExec.start > jobExec.stop) {
                    System.out.println("ERROR: Job " + jobExec.id + " in frame " + frameNumber
                            + " has a start time greater than stop time.");
                }
            ...
            if (percentage > 110) {
                System.out.println("WARNING: Size of frame " + frameNumber + " is > " + percentage
                        + "% of the average frame size.");
                System.out.println(
                        "Could be a frame over-run, or perhaps the time-line was collected with itimers enabled.");
            }
            ...
            if ((frameNumber != 0) && (percentage < 90)) {
                System.out.println("WARNING: Size of frame " + frameNumber + " is < " + percentage
                        + "% of the average frame size.");
                System.out.println(
                        "Perhaps the sim user-code is calling a \"start-of-frame\" or \"end-of-frame\" job directly?");
            }
        }
     ...
     */
    private ArrayList<JobExecutionEvent> getJobExecutionEventList(
            String fileName, JobSpecificationMap jobSpecificationMap) {
        String line;
        String field[];
