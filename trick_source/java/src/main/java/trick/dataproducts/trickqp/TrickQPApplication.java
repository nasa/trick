...
public class TrickQPApplication extends DataProductsApplication {
    ...
    public void newProgramOutput() {
        if (programsNode.getChildCount() < 1) {
            ...
            String response = JOptionPane.showInputDialog(
                    getMainFrame(), "Enter Name for Output:", "Add Output", JOptionPane.QUESTION_MESSAGE);
    ...
    public void subtractFunction() {
        if (varList == null || varList.getSelectedData() == null || varList.getSelectedData().length != 2) {
            ...
            actionController.handleSubtractFunction(
                    (LogVar) varList.getSelectedData()[0], (LogVar) varList.getSelectedData()[1]);
        }
    }
    ...
    public void reverseSubtractFunction() {
        if (varList == null || varList.getSelectedData() == null || varList.getSelectedData().length != 2) {
            ...
            actionController.handleSubtractFunction(
                    (LogVar) varList.getSelectedData()[1], (LogVar) varList.getSelectedData()[0]);
        }
    }
    ...
    public void quit(ActionEvent e) {
        if ((productTree.getAllChildren(plotsNode) != null
                        && productTree.getAllChildren(plotsNode).size() > 0)
                || (productTree.getAllChildren(tablesNode) != null
                        && productTree.getAllChildren(tablesNode).size() > 0)) {
    ...
    protected JMenuBar createMenuBar() {
        ...
        String[] varsMenuActionNames = {"addVar", "expandVar", "contractVar", "changeUnits"};
        String[] runsMenuActionNames = {"addRun", "removeRun"};
        String[] plotsMenuActionNames = {
            "newPage", "removeAllPages", "---", "newPlot", "---", "newCurve", "---", "newVarcase"
        };
        String[] tablesMenuActionNames = {"newTable", "removeTable", "---", "newColumn"};
        String[] programsMenuActionNames = {"newProgram", "removeAllPrograms", "newProgramOutput"};
        String[] actionsMenuActionNames = {
            "singlePlot", "comparisonPlot", "errorPlot", "contrastPlot", "tabularData", "tabularErrorData"
        };
        String[] helpMenuActionNames = {"helpContents", "---", "showAboutBox"};
    ...
    public boolean saveDPToFile(String fullPath) {
        ...
        // TODO: Programs?
        // Since the plotting utility can show either plot or table or both based on the mode
        // attibute in Session xml file, go back to add all pages and let it handle it.
