...
TrickIntegrator::TrickIntegrator(void)
...
TrickIntegrator::TrickIntegrator(const TrickIntegrator & source, IntegratorConstructor & integ_constructor_in)
...
TrickIntegrator::TrickIntegrator(IntegratorConstructor & integ_constructor_in, bool use_deriv2_in)
    : // Except for the BaseIntegrationGroup, which is special,
      // construct parent classes with their default constructors.
      // Construct the BaseIntegrationGroup so it knows that this object
      // serves as both the integration interface and time interface.
...
// Non-default constructor.
TrickIntegrator::TrickIntegrator(IntegratorConstructor & integ_constructor_in,
                                 int state_size,
                                 double delta_t,
                                 bool use_deriv2_in)
    : // Except for the BaseIntegrationGroup, which is special,
...
// Initialize the integrator.
void TrickIntegrator::initialize_trick_integrator(int state_size, double delta_t)
{
...
void TrickIntegrator::initialize_trick_workspace(void)
{
...
void TrickIntegrator::initialize_trick_interface(void)
{
    if(use_deriv2)
    {
...
void TrickIntegrator::allocate_first_order_integrators(int state_size)
{
    first_order_integrator = trick_integ_constructor->create_first_order_ode_integrator(state_size, *integ_controls);

    if(half_state_size * 2 == state_size)
    {
        second_order_integrator = trick_integ_constructor->create_second_order_ode_integrator(half_state_size,
                                                                                              *integ_controls);
    }
    else
    {
...
void TrickIntegrator::allocate_second_order_integrators(int state_size)
{
    if(2 * half_state_size != state_size)
    {
        message_publish(MSG_ERROR, "Integrator ERROR: State size is not a multiple of two.\n");
    }

    if(trick_integ_constructor->provides(er7_utils::Integration::FirstOrderODE))
    {
        first_order_integrator = trick_integ_constructor->create_first_order_ode_integrator(state_size,
                                                                                            *integ_controls);
    }
    else
    {
...
void TrickIntegrator::deallocate_integrators(void)
{
...
void TrickIntegrator::deallocate_trick_arrays(void)
{
...
void TrickIntegrator::reset_body_integrators(void)
{
...
// Integrate state.
IntegratorResult TrickIntegrator::integrate_bodies(double dyndt, unsigned int next_stage)
{
    switch(integ_mode)
    {
        ...
        case UseFirstOrderIntegrator:
            return first_order_integrator->integrate(dyndt, next_stage, cached_derivs, cached_state);
            break;
        ...
        case UseSecondOrderIntegrator:
            return second_order_integrator->integrate(dyndt,
                                                      next_stage,
                                                      cached_accel,
                                                      cached_velocity,
                                                      cached_position);
        ...
        default:
            message_publish(MSG_ERROR, "Integrator ERROR: Improper use of integration interface.\n");
            intermediate_step = 0;
            return 1.0;
            break;
    }
