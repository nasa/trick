...
namespace er7_utils
{
    ...
    TrickIntegrator::TrickIntegrator(
        IntegratorConstructor& integ_constructor_in, int state_size, double delta_t, bool use_deriv2_in)
        : // Except for the BaseIntegrationGroup, which is special,
          // construct parent classes with their default constructors.
          // Construct the BaseIntegrationGroup so it knows that this object
          // serves as both the integration interface and time interface.
    ...
    void TrickIntegrator::allocate_first_order_integrators(int state_size)
    {
        first_order_integrator
            = trick_integ_constructor->create_first_order_ode_integrator(state_size, *integ_controls);

        if (half_state_size * 2 == state_size)
        {
            second_order_integrator
                = trick_integ_constructor->create_second_order_ode_integrator(half_state_size, *integ_controls);
        }
        else
        {
            second_order_integrator = NULL;
    ...
    IntegratorResult TrickIntegrator::integrate_bodies(double dyndt, unsigned int next_stage)
    {
        ...
        case UseSecondOrderIntegrator:
            return second_order_integrator->integrate(
                dyndt, next_stage, cached_accel, cached_velocity, cached_position);
            break;
