...
namespace er7_utils
{
    ...
    RKNystrom4SecondOrderODEIntegrator::RKNystrom4SecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
        ...
        , dtheta(NULL)
    {
        ...
        if (problem_type == Integration::GeneralizedStepSecondOrderODE)
        {
            ...
            alloc::replicate_2D_array<4>(state_size[1], src.veldot_hist, veldot_hist);
        }
    ...
    RKNystrom4SecondOrderODEIntegrator::RKNystrom4SecondOrderODEIntegrator(unsigned int position_size,
    ...
    // Non-throwing swap.
    void RKNystrom4SecondOrderODEIntegrator::swap(RKNystrom4SecondOrderODEIntegrator& other)
    {
    ...
    RKNystrom4SimpleSecondOrderODEIntegrator* RKNystrom4SimpleSecondOrderODEIntegrator::create_copy() const
    {
    ...
    RKNystrom4GeneralizedStepSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
     ...
     */
    inline void ER7_UTILS_ALWAYS_INLINE simple_rkn4_step_one(double const* ER7_UTILS_RESTRICT accel, double hdt,
        int size, double* ER7_UTILS_RESTRICT init_pos, double* ER7_UTILS_RESTRICT init_vel,
     ...
     * @param[in,out] velocity     Updated velocity vector
     */
    inline void ER7_UTILS_ALWAYS_INLINE simple_rkn4_step_three(double const* ER7_UTILS_RESTRICT init_pos,
        double const* ER7_UTILS_RESTRICT init_vel, double const* ER7_UTILS_RESTRICT accel, double delta_t, int size,
        double* ER7_UTILS_RESTRICT veldot, double* ER7_UTILS_RESTRICT position, double* ER7_UTILS_RESTRICT velocity)
    {
        for (int ii = 0; ii < size; ++ii)
        {
     ...
     * @param[in,out] velocity     Updated velocity vector
     */
    inline void ER7_UTILS_ALWAYS_INLINE simple_rkn4_step_four(double const* ER7_UTILS_RESTRICT init_pos,
        double const* ER7_UTILS_RESTRICT init_vel, double const* ER7_UTILS_RESTRICT accel,
        double const* ER7_UTILS_RESTRICT const* ER7_UTILS_RESTRICT veldot_hist, double delta_t, int size,
    ...
    // position.
    IntegratorResult RKNystrom4SimpleSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            simple_rkn4_step_one(
                accel, 0.5 * dyn_dt, state_size[0], init_pos, init_vel, veldot_hist[0], position, velocity);
            step_factor = 0.5;
            break;

        case 2:
#if 1
            integ_utils::euler_step_save_deriv(init_vel, accel, 0.5 * dyn_dt, state_size[0], veldot_hist[1], velocity);
#else
            simple_rkn4_step_three(
                init_pos, init_vel, accel, 0.5 * dyn_dt, state_size[0], veldot_hist[1], position, velocity);
#endif
            step_factor = 0.5;
            break;
        ...
        case 3:
            simple_rkn4_step_three(
                init_pos, init_vel, accel, dyn_dt, state_size[0], veldot_hist[2], position, velocity);
            step_factor = 1.0;
            break;

        case 4:
            simple_rkn4_step_four(init_pos, init_vel, accel, veldot_hist, dyn_dt, state_size[0], position, velocity);
            step_factor = 1.0;
            break;
     ...
     * @param[in,out] velocity     Updated velocity vector
     */
    inline void ER7_UTILS_ALWAYS_INLINE generalized_rkn4_step_one_velocity(double const* ER7_UTILS_RESTRICT accel,
        double hdt, int size, double* ER7_UTILS_RESTRICT init_vel, double* ER7_UTILS_RESTRICT dtheta,
        double* ER7_UTILS_RESTRICT init_veldot, double* ER7_UTILS_RESTRICT velocity)
    {
        for (int ii = 0; ii < size; ++ii)
        {
            double adt       = accel[ii] * hdt;
     ...
     * @param[in,out] velocity     Updated velocity vector
     */
    inline void ER7_UTILS_ALWAYS_INLINE generalized_rkn4_step_three_velocity(double const* ER7_UTILS_RESTRICT init_vel,
        double const* ER7_UTILS_RESTRICT accel, double delta_t, int size, double* ER7_UTILS_RESTRICT dtheta,
        double* ER7_UTILS_RESTRICT veldot, double* ER7_UTILS_RESTRICT velocity)
    {
        for (int ii = 0; ii < size; ++ii)
        {
            double adt   = accel[ii] * delta_t;
     ...
     * @param[in,out] velocity     Updated velocity vector
     */
    inline void ER7_UTILS_ALWAYS_INLINE generalized_rkn4_step_four_velocity(double const* ER7_UTILS_RESTRICT init_vel,
        double const* ER7_UTILS_RESTRICT accel, double const* ER7_UTILS_RESTRICT const* ER7_UTILS_RESTRICT veldot_hist,
        double delta_t, int size, double* ER7_UTILS_RESTRICT dtheta, double* ER7_UTILS_RESTRICT velocity)
    {
        static const double one_sixth             = 1.0 / 6.0;
        double dto6                               = delta_t * one_sixth;
        double const* ER7_UTILS_RESTRICT veldot_0 = veldot_hist[0];
    ...
    IntegratorResult RKNystrom4GeneralizedStepSecondOrderODEIntegrator::integrate(double dyn_dt,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            generalized_rkn4_step_one_velocity(
                accel, 0.5 * dyn_dt, state_size[1], init_vel, dtheta, veldot_hist[0], velocity);
      ...
      integ_utils::euler_step_save_deriv (
         ...
         veldot_hist[1], velocity);
            ...
            generalized_rkn4_step_three_velocity(
                init_vel, accel, 0.5 * dyn_dt, state_size[1], dtheta, veldot_hist[1], velocity);
        ...
        case 3:
            generalized_rkn4_step_three_velocity(
                init_vel, accel, dyn_dt, state_size[1], dtheta, veldot_hist[2], velocity);
            compute_expmap_position_step(init_pos, dtheta, position);
            step_factor = 1.0;
            break;

        case 4:
            generalized_rkn4_step_four_velocity(init_vel, accel, veldot_hist, dyn_dt, state_size[1], dtheta, velocity);
            compute_expmap_position_step(init_pos, dtheta, position);
            step_factor = 1.0;
            break;
