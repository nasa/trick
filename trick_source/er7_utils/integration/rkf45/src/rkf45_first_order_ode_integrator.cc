...
namespace er7_utils
{
    ...
    RKFehlberg45FirstOrderODEIntegrator::RKFehlberg45FirstOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    RKFehlberg45FirstOrderODEIntegrator* RKFehlberg45FirstOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Non-throwing swap.
    void RKFehlberg45FirstOrderODEIntegrator::swap(RKFehlberg45FirstOrderODEIntegrator& other)
    {
    ...
    IntegratorResult RKFehlberg45FirstOrderODEIntegrator::integrate(double dt, unsigned int target_stage,
        const double* ER7_UTILS_RESTRICT velocity, double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            integ_utils::inplace_euler_step_save_both(
                velocity, RKFehlberg45ButcherTableau::RKa[1][0] * dt, state_size, init_state, deriv_hist[0], position);
        ...
        case 2:
            integ_utils::weighted_step_save_deriv<2>(
                init_state, velocity, RKFehlberg45ButcherTableau::RKa[2], dt, state_size, deriv_hist, position);
            step_factor = RKFehlberg45ButcherTableau::RKc[2];
            break;

        case 3:
            integ_utils::weighted_step_save_deriv<3>(
                init_state, velocity, RKFehlberg45ButcherTableau::RKa[3], dt, state_size, deriv_hist, position);
            step_factor = RKFehlberg45ButcherTableau::RKc[3];
            break;

        case 4:
            integ_utils::weighted_step_save_deriv<4>(
                init_state, velocity, RKFehlberg45ButcherTableau::RKa[4], dt, state_size, deriv_hist, position);
            step_factor = RKFehlberg45ButcherTableau::RKc[4];
            break;

        case 5:
            integ_utils::weighted_step_save_deriv<5>(
                init_state, velocity, RKFehlberg45ButcherTableau::RKa[5], dt, state_size, deriv_hist, position);
        ...
        case 6:
            integ_utils::weighted_step<6>(
                init_state, velocity, deriv_hist, RKFehlberg45ButcherTableau::RKb5, dt, state_size, position);
            step_factor = RKFehlberg45ButcherTableau::RKc[5];
            break;
