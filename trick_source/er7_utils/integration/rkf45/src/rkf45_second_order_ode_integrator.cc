...
namespace er7_utils
{
    ...
    inline double ER7_UTILS_ALWAYS_INLINE rkf45_simple_intermediate_step(double dyn_dt, int state_size[2],
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        integ_utils::two_state_weighted_step_save_derivs<target_stage>(init_pos, init_vel, accel,
            RKFehlberg45ButcherTableau::RKa[target_stage], dyn_dt, state_size[0], posdot_hist, veldot_hist, position,
            velocity);
     ...
     */
    template <int target_stage>
    inline double ER7_UTILS_ALWAYS_INLINE rkf45_generalized_deriv_intermediate_step(
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        integ_utils::two_state_weighted_step_save_veldot<target_stage>(init_pos, init_vel, accel,
            RKFehlberg45ButcherTableau::RKa[target_stage], dyn_dt, state_size, posdot_hist, veldot_hist, position,
            velocity);
     ...
     */
    template <int target_stage>
    inline double ER7_UTILS_ALWAYS_INLINE rkf45_generalized_step_intermediate_step(
        ...
        double* ER7_UTILS_RESTRICT velocity, double* ER7_UTILS_RESTRICT position)
    {
        ...
        integ_utils::weighted_sum<target_stage>(
            posdot_hist, RKFehlberg45ButcherTableau::RKa[target_stage], dyn_dt, vel_size, dtheta);
        ...
        integ_utils::weighted_step_save_deriv<target_stage>(
            init_vel, accel, RKFehlberg45ButcherTableau::RKa[target_stage], dyn_dt, vel_size, veldot_hist, velocity);
    ...
    RKFehlberg45SecondOrderODEIntegrator::RKFehlberg45SecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
        ...
        , dtheta(NULL)
    {
        if (problem_type == Integration::GeneralizedStepSecondOrderODE)
        {
            ...
            alloc::replicate_2D_array<6>(state_size[1], src.posdot_hist, posdot_hist);
            alloc::replicate_2D_array<6>(state_size[1], src.veldot_hist, veldot_hist);
        }
        ...
        else if (src.init_pos != NULL)
        {
            init_pos = alloc::replicate_array(state_size[0], src.init_pos);
            init_vel = alloc::replicate_array(state_size[1], src.init_vel);
            alloc::replicate_2D_array<6>(state_size[0], src.posdot_hist, posdot_hist);
            alloc::replicate_2D_array<6>(state_size[1], src.veldot_hist, veldot_hist);
        }
    ...
    RKFehlberg45SecondOrderODEIntegrator::RKFehlberg45SecondOrderODEIntegrator(unsigned int position_size,
    ...
    // Non-throwing swap.
    void RKFehlberg45SecondOrderODEIntegrator::swap(RKFehlberg45SecondOrderODEIntegrator& other)
    {
    ...
    RKFehlberg45SimpleSecondOrderODEIntegrator* RKFehlberg45SimpleSecondOrderODEIntegrator::create_copy() const
    {
    ...
    RKFehlberg45GeneralizedDerivSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
    ...
    RKFehlberg45GeneralizedStepSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
    ...
    // position.
    IntegratorResult RKFehlberg45SimpleSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            integ_utils::inplace_two_state_euler_step_save_all(accel, RKFehlberg45ButcherTableau::RKa[1][0] * dyn_dt,
        ...
        case 2:
            step_factor = rkf45_simple_intermediate_step<2>(
                dyn_dt, state_size, init_pos, init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;

        case 3:
            step_factor = rkf45_simple_intermediate_step<3>(
                dyn_dt, state_size, init_pos, init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;

        case 4:
            step_factor = rkf45_simple_intermediate_step<4>(
                dyn_dt, state_size, init_pos, init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;

        case 5:
            step_factor = rkf45_simple_intermediate_step<5>(
                dyn_dt, state_size, init_pos, init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;
        ...
        case 6:
            integ_utils::two_state_weighted_step<6>(init_pos, init_vel, accel, posdot_hist, veldot_hist,
                RKFehlberg45ButcherTableau::RKb5, dyn_dt, state_size[0], position, velocity);
            step_factor = 1.0;
            break;
    ...
    IntegratorResult RKFehlberg45GeneralizedDerivSecondOrderODEIntegrator::integrate(double dyn_dt,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            integ_utils::inplace_two_state_euler_step_save_pos_vel_acc(posdot_hist[0], accel,
                RKFehlberg45ButcherTableau::RKa[1][0] * dyn_dt, state_size, init_pos, init_vel, veldot_hist[0],
                position, velocity);
        ...
        // Update state per RKF45 RKa[target_stage] (target_stage elements).
        case 2:
            step_factor = rkf45_generalized_deriv_intermediate_step<2>(compute_posdot, dyn_dt, state_size, init_pos,
                init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;

        case 3:
            step_factor = rkf45_generalized_deriv_intermediate_step<3>(compute_posdot, dyn_dt, state_size, init_pos,
                init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;

        case 4:
            step_factor = rkf45_generalized_deriv_intermediate_step<4>(compute_posdot, dyn_dt, state_size, init_pos,
                init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;

        case 5:
            step_factor = rkf45_generalized_deriv_intermediate_step<5>(compute_posdot, dyn_dt, state_size, init_pos,
                init_vel, accel, veldot_hist, posdot_hist, velocity, position);
            break;
        ...
        case 6:
            ...
            integ_utils::two_state_weighted_step<6>(init_pos, init_vel, posdot_hist[5], accel, posdot_hist, veldot_hist,
                RKFehlberg45ButcherTableau::RKb5, dyn_dt, state_size, position, velocity);
            step_factor = 1.0;
            break;
    ...
    // Propagate state for the general case of the generalized position derivative
    // being a function of generalized position and generalized velocity.
    IntegratorResult RKFehlberg45GeneralizedStepSecondOrderODEIntegrator::integrate(double dyn_dt,
        unsigned int target_stage, double const* ER7_UTILS_RESTRICT accel, double* ER7_UTILS_RESTRICT velocity,
        double* ER7_UTILS_RESTRICT position)
    {
        double step_factor;
        ...
        // Update state per RKF45 RKa[target_stage] (target_stage elements).
        case 2:
            step_factor = rkf45_generalized_step_intermediate_step<2>(compute_dexpinv_velocity_transform,
                compute_expmap_position_step, dyn_dt, vel_size, init_pos, init_vel, accel, veldot_hist, posdot_hist,
                dtheta, velocity, position);
            break;

        case 3:
            step_factor = rkf45_generalized_step_intermediate_step<3>(compute_dexpinv_velocity_transform,
                compute_expmap_position_step, dyn_dt, vel_size, init_pos, init_vel, accel, veldot_hist, posdot_hist,
                dtheta, velocity, position);
            break;

        case 4:
            step_factor = rkf45_generalized_step_intermediate_step<4>(compute_dexpinv_velocity_transform,
                compute_expmap_position_step, dyn_dt, vel_size, init_pos, init_vel, accel, veldot_hist, posdot_hist,
                dtheta, velocity, position);
            break;

        case 5:
            step_factor = rkf45_generalized_step_intermediate_step<5>(compute_dexpinv_velocity_transform,
                compute_expmap_position_step, dyn_dt, vel_size, init_pos, init_vel, accel, veldot_hist, posdot_hist,
                dtheta, velocity, position);
            break;
        ...
        case 6:
            ...
            // Compute the position step for this stage.
            integ_utils::weighted_sum<6>(posdot_hist, RKFehlberg45ButcherTableau::RKb5, dyn_dt, vel_size, dtheta);

            // Take the non-linear position step.
            compute_expmap_position_step(init_pos, dtheta, position);

            integ_utils::weighted_step<6>(
                init_vel, accel, veldot_hist, RKFehlberg45ButcherTableau::RKb5, dyn_dt, vel_size, velocity);
