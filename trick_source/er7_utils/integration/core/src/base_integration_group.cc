...
namespace er7_utils
{
    ...
    // rather than crash.
    BaseIntegrationGroup::BaseIntegrationGroup(void)
    ...
    BaseIntegrationGroup::BaseIntegrationGroup(const BaseIntegrationGroup& source)
        : integ_constructor(source.integ_constructor)
        , integ_interface(source.integ_interface)
        , time_interface(source.time_interface)
        , integ_controls(
              source.integ_controls ? source.integ_controls->create_copy() : BogusIntegrationControls().create_copy())
    {
    }

    // Non-throwing swap.
    void BaseIntegrationGroup::swap(BaseIntegrationGroup& other)
    {
    ...
    BaseIntegrationGroup::~BaseIntegrationGroup(void) { alloc::delete_object(integ_controls); }

    // Initialize the integration group.
    void BaseIntegrationGroup::initialize_group(void)
    {
        ...
        {
            MessageHandler::error(__FILE__, __LINE__, IntegrationMessages::invalid_request,
        ...
        integ_interface->set_first_step_derivs_flag(integ_constructor->need_first_step_derivatives());
