...
namespace er7_utils
{
    ...
    {
        MessageHandler::fail(
            __FILE__, __LINE__, IntegrationMessages::internal_error, "Integration group has not been initialized.");

        return 0;
    }

    // Clone a BogusIntegrationControls.
    BogusIntegrationControls* BogusIntegrationControls::create_copy() const { return alloc::replicate_object(*this); }
