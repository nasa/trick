...
namespace er7_utils
{
     ...
     */
    static inline double ER7_UTILS_ALWAYS_INLINE truncate_scalar(double val)
    {
     ...
     */
    static inline void ER7_UTILS_ALWAYS_INLINE truncate_vector(
        const double* ER7_UTILS_RESTRICT in, double* ER7_UTILS_RESTRICT out)
    {
     ...
     * @return ||vec||^2
     */
    static inline double ER7_UTILS_ALWAYS_INLINE square_magnitude(const double vec[3])
    {
     ...
     * @return vec1 . vec2
     */
    static inline double ER7_UTILS_ALWAYS_INLINE inner_product(const double vec1[3], const double vec2[3])
    {
     ...
     */
    template <int N>
    static inline double ER7_UTILS_ALWAYS_INLINE cross_product_element(const double* vec1, const double* vec2)
    {
     ...
     * @param[out] out    Scaled vector
     */
    static inline void ER7_UTILS_ALWAYS_INLINE scale_vector(
        const double* ER7_UTILS_RESTRICT in, const double scale, double* ER7_UTILS_RESTRICT out)
    {
        out[0] = scale * in[0];
        out[1] = scale * in[1];
        out[2] = scale * in[2];
     ...
     * @param[out] out    Scaled vector
     */
    static inline void ER7_UTILS_ALWAYS_INLINE scale_and_truncate_vector(
        const double* ER7_UTILS_RESTRICT in, const double scale, double* ER7_UTILS_RESTRICT out)
    {
...
namespace er7_utils
{
    ...
    void LeftQuaternionGeneralizedPositionDerivativeFunctions::derivative(const double* ER7_UTILS_RESTRICT quat,
        const double* ER7_UTILS_RESTRICT ang_vel, double* ER7_UTILS_RESTRICT qdot)
    {
    ...
    void LeftQuaternionGeneralizedPositionDerivativeFunctions::second_derivative(const double* ER7_UTILS_RESTRICT quat,
        const double* ER7_UTILS_RESTRICT ang_vel, const double* ER7_UTILS_RESTRICT ang_acc,
    ...
    void LeftQuaternionGeneralizedPositionStepFunctions::expmap(
        double const quat_init[4], double const dtheta[3], double quat_end[4])
    {
        ...
        quat_norm[1] = qs_init * mtheta_hat[0] + cross_product_element<0>(mtheta_hat, qv_init);
        quat_norm[2] = qs_init * mtheta_hat[1] + cross_product_element<1>(mtheta_hat, qv_init);
        quat_norm[3] = qs_init * mtheta_hat[2] + cross_product_element<2>(mtheta_hat, qv_init);
        ...
        if (h_dtheta_sq < 0.001685)
        {
            ...
            double omcos_hwdt
                = 0.5 * h_dtheta_sq * (1.0 - ONE_OVER_12 * h_dtheta_sq * (1.0 - ONE_OVER_30 * h_dtheta_sq));
            // Approximate sin(0.5*dtheta)
            double sin_hwdt = h_dtheta_mag
                * (1.0
                    - ONE_OVER_6 * h_dtheta_sq * (1.0 - ONE_OVER_20 * h_dtheta_sq * (1.0 - ONE_OVER_42 * h_dtheta_sq)));
    ...
    void LeftQuaternionGeneralizedPositionStepFunctions::dexpinv(
        double const angular_vel_in[3], double const dtheta[3], double angular_vel_out[3])
    {
        double vin[3];
        double dth[3];
        ...
        double fact = ONE_OVER_12
            * (1.0 + dtheta_sq * ONE_OVER_60 * (1.0 + dtheta_sq * ONE_OVER_42 * (1.0 + dtheta_sq * ONE_OVER_40)));
