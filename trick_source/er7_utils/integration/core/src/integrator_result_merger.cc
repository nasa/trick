...
namespace er7_utils
{
    ...
    bool IntegratorResultMerger::merge_integrator_result(
        const IntegratorResult& new_result, IntegratorResult& merged_result) const
    {
        bool success = true;
        ...
        else
        {
            ...
            if (std::abs(new_result.get_time_scale() - merged_result.get_time_scale()) > 1e-15)
            {
                MessageHandler::error(__FILE__, __LINE__, IntegrationMessages::invalid_item,
                    "Time scale mismatch between %.16f (new) and %.16f (old).", new_result.get_time_scale(),
    ...
    IntegratorResultMerger* IntegratorResultMerger::create_copy() const { return alloc::replicate_object(*this); }

    // Croak when merging an IntegratorResult into another.
    bool BogusIntegratorResultMerger::merge_integrator_result(const IntegratorResult&, IntegratorResult&) const
    {
    ...
    IntegratorResultMerger* BogusIntegratorResultMerger::create_copy() const { return alloc::replicate_object(*this); }
