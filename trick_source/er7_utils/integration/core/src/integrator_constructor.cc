...
namespace er7_utils
{
    ...
    IntegratorResultMerger* IntegratorConstructor::create_integrator_results_merger(void) const
    {
        return alloc::allocate_object<IntegratorResultMerger>();
    }

    // Create an integrator for a first order ODE.
    FirstOrderODEIntegrator* IntegratorConstructor::create_first_order_ode_integrator(
        unsigned int, IntegrationControls&) const
    {
        MessageHandler::error(__FILE__, __LINE__, IntegrationMessages::unsupported_option,
            "Unable to create a first order ODE integrator for %s.", get_class_name());

        return NULL;
    }

    // Create an integrator for a second order ODE.
    SecondOrderODEIntegrator* IntegratorConstructor::create_second_order_ode_integrator(
        unsigned int, IntegrationControls&) const
    {
        MessageHandler::error(__FILE__, __LINE__, IntegrationMessages::unsupported_option,
            "Unable to create a simple second order ODE integrator for %s.", get_class_name());
    ...
    {
        MessageHandler::error(__FILE__, __LINE__, IntegrationMessages::unsupported_option,
            "Unable to create a generalized second order ODE integrator for %s.", get_class_name());
