...
namespace er7_utils
{
    ...
    // Default constructor.
    PrimingSecondOrderODEIntegrator::PrimingSecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    // Copy constructor.
    PrimingSecondOrderODEIntegrator::PrimingSecondOrderODEIntegrator(const PrimingSecondOrderODEIntegrator& src)
        : Er7UtilsDeletable()
    ...
    PrimingSecondOrderODEIntegrator::PrimingSecondOrderODEIntegrator(unsigned int priming_size,
        ...
        , primed(false)
    {
        ...
        primer = primer_constructor.create_second_order_ode_integrator(size, controls_in);
    }

    // Non-default constructor for a PrimingSecondOrderODEIntegrator
    // for generalized position, generalized velocity.
    PrimingSecondOrderODEIntegrator::PrimingSecondOrderODEIntegrator(unsigned int priming_size,
        const IntegratorConstructor& primer_constructor, unsigned int position_size, unsigned int velocity_size,
        const GeneralizedPositionDerivativeFunctions& deriv_funs, IntegrationControls& controls_in)
        : Er7UtilsDeletable()
        , SecondOrderODEIntegrator(position_size, velocity_size, deriv_funs, controls_in)
        , controls(&controls_in)
        , primer(NULL)
        , priming_state_size(priming_size)
        , prime_counter(priming_size)
        , primed(false)
    {
        ...
        primer = primer_constructor.create_generalized_deriv_second_order_ode_integrator(
    ...
    PrimingSecondOrderODEIntegrator::PrimingSecondOrderODEIntegrator(unsigned int priming_size,
        const IntegratorConstructor& primer_constructor, unsigned int position_size, unsigned int velocity_size,
        const GeneralizedPositionStepFunctions& step_funs, IntegrationControls& controls_in)
        : Er7UtilsDeletable()
        , SecondOrderODEIntegrator(position_size, velocity_size, step_funs, controls_in)
        , controls(&controls_in)
        , primer(NULL)
        , priming_state_size(priming_size)
        , prime_counter(priming_size)
    ...
    {
        // Create the integrator used to prime the state.
        primer = primer_constructor.create_generalized_step_second_order_ode_integrator(
    ...
    // Non-throwing swap.
    void PrimingSecondOrderODEIntegrator::swap(PrimingSecondOrderODEIntegrator& other)
    {
    ...
    void PrimingSecondOrderODEIntegrator::set_controls(IntegrationControls& controls_in)
    {
        controls = &controls_in;

        PrimingIntegrationControls* priming_controls = dynamic_cast<PrimingIntegrationControls*>(controls);
    ...
    // Integrate state
    IntegratorResult PrimingSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        const double* ER7_UTILS_RESTRICT accel, double* ER7_UTILS_RESTRICT velocity,
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        if (!primed)
        {
            ...
            if (controls->in_initial_cycle())
            {
                ...
                if (prime_counter > 0)
                {
                    technique_save_derivatives(prime_counter, accel, velocity, position);
                }
                else
                {
                    primed = true;
            ...
            if (!primed)
            {
                return primer->integrate(dyn_dt, target_stage, accel, velocity, position);
            }
        }
