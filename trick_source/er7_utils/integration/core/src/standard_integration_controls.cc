...
namespace er7_utils
{
    ...
    // Non-throwing swap.
    void StandardIntegrationControls::swap(StandardIntegrationControls& other)
    {
    ...
    StandardIntegrationControls* StandardIntegrationControls::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Perform one step of the integration process.
    unsigned int StandardIntegrationControls::integrate(double starttime, double sim_dt, TimeInterface& time_interface,
        IntegratorInterface& integ_interface, BaseIntegrationGroup& integ_group)
    {
        ...
        {
            step_number = alt_controls->integrate(starttime, sim_dt, time_interface, integ_interface, integ_group);
        }
