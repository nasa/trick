...
namespace er7_utils
{
      ...
      double * ER7_UTILS_RESTRICT velocity)
   {
      ...
      for (int ii = 0; ii < size; ++ii) {
         ...
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk_initial_step(double const* ER7_UTILS_RESTRICT deriv, double deltat,
            int size, double* ER7_UTILS_RESTRICT init_state, double* ER7_UTILS_RESTRICT init_deriv,
         ...
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk_initial_step(double const* ER7_UTILS_RESTRICT deriv, double deltat,
            int size, double* ER7_UTILS_RESTRICT init_state, double* ER7_UTILS_RESTRICT state)
        {
            for (int ii = 0; ii < size; ++ii)
            {
                init_state[ii]  = state[ii];
         ...
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk_two_state_initial_step(double const* ER7_UTILS_RESTRICT posdot,
            double const* ER7_UTILS_RESTRICT veldot, double deltat, int size[2], double* ER7_UTILS_RESTRICT init_pos,
            ...
            double* ER7_UTILS_RESTRICT velocity)
        {
            rk_initial_step(posdot, deltat, size[0], init_pos, init_posdot, position);
            rk_initial_step(veldot, deltat, size[1], init_vel, init_veldot, velocity);
        }
        ...
        {
            rk_initial_step(posdot, deltat, size[0], init_pos, position);
            rk_initial_step(veldot, deltat, size[1], init_vel, init_veldot, velocity);
        }
         ...
         * @param[in,out] velocity     Updated velocity vector
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk_two_state_initial_step(double const* ER7_UTILS_RESTRICT accel,
            double deltat, int size, double* ER7_UTILS_RESTRICT init_pos, double* ER7_UTILS_RESTRICT init_vel,
        ...
        inline void ER7_UTILS_ALWAYS_INLINE rk_two_state_initial_step(double const* ER7_UTILS_RESTRICT accel,
            double deltat, int size, double* ER7_UTILS_RESTRICT init_pos, double* ER7_UTILS_RESTRICT init_vel,
            double* ER7_UTILS_RESTRICT init_acc, double* ER7_UTILS_RESTRICT position,
            double* ER7_UTILS_RESTRICT velocity)
        {
            for (int ii = 0; ii < size; ++ii)
            {
         ...
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk_intermediate_step(double const* ER7_UTILS_RESTRICT init_state,
            double const* ER7_UTILS_RESTRICT curr_deriv, double deltat, int size,
            double* ER7_UTILS_RESTRICT saved_deriv, double* ER7_UTILS_RESTRICT state)
        {
            double work_state, work_deriv;
            for (int ii = 0; ii < size; ++ii)
            {
         ...
         * @param[in,out] velocity      Updated velocity vector
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk_two_state_intermediate_step(double const* ER7_UTILS_RESTRICT init_pos,
            double const* ER7_UTILS_RESTRICT init_vel, double const* ER7_UTILS_RESTRICT posdot,
            double const* ER7_UTILS_RESTRICT veldot, double deltat, int size[2],
            double* ER7_UTILS_RESTRICT saved_veldot, double* ER7_UTILS_RESTRICT position,
            double* ER7_UTILS_RESTRICT velocity)
        {
            int lim = size[0];
        ...
        inline void ER7_UTILS_ALWAYS_INLINE rk_two_state_intermediate_step(double const* ER7_UTILS_RESTRICT init_pos,
            ...
            double* ER7_UTILS_RESTRICT saved_vel, double* ER7_UTILS_RESTRICT saved_acc,
            double* ER7_UTILS_RESTRICT position, double* ER7_UTILS_RESTRICT velocity)
        {
            for (int ii = 0; ii < size; ++ii)
            {
         ...
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk2_heun_final_step(double const* ER7_UTILS_RESTRICT init_state,
            double const* ER7_UTILS_RESTRICT deriv1, double const* ER7_UTILS_RESTRICT deriv2, double deltat, int size,
            double* ER7_UTILS_RESTRICT state)
        {
      ...
      double * ER7_UTILS_RESTRICT velocity)
   {
      ...
      for (int ii = 0; ii < size; ++ii) {
         ...
         * @param[out] state       Updated state vector
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk4_final_step(double const* ER7_UTILS_RESTRICT init_state,
            double const* ER7_UTILS_RESTRICT const* ER7_UTILS_RESTRICT deriv_hist,
            double const* ER7_UTILS_RESTRICT curr_deriv, double deltat, int size, double* ER7_UTILS_RESTRICT state)
        {
      ...
      double state_init, deriv0_i, deriv1_i, deriv2_i, deriv3_i;
...
#endif
#if 1
                state[ii] = init_state[ii] + (deriv0[ii] + 2.0 * (deriv1[ii] + deriv2[ii]) + curr_deriv[ii]) * dto6;
#else
                state[ii] = init_state[ii]
                    + (deriv_hist[0][ii] + 2.0 * (deriv_hist[1][ii] + deriv_hist[2][ii]) + curr_deriv[ii]) * dto6;
#endif
            }
        }
         ...
         * @param[in,out] velocity   Updated velocity vector
         */
        inline void ER7_UTILS_ALWAYS_INLINE rk4_two_state_final_step(double const* ER7_UTILS_RESTRICT init_pos,
            double const* ER7_UTILS_RESTRICT init_vel,
            ...
            int size[2], double* ER7_UTILS_RESTRICT position, double* ER7_UTILS_RESTRICT velocity)
        {
            static const double one_sixth = 1.0 / 6.0;
            double dto6                   = deltat * one_sixth;
            int lim                       = size[0];
            ...
            {
                position[ii] = init_pos[ii]
                    + (saved_vel[0][ii] + 2.0 * (saved_vel[1][ii] + saved_vel[2][ii]) + posdot[ii]) * dto6;
            }
            ...
            {
                velocity[ii] = init_vel[ii]
                    + (saved_acc[0][ii] + 2.0 * (saved_acc[1][ii] + saved_acc[2][ii]) + veldot[ii]) * dto6;
            }
        }
        ...
        inline void ER7_UTILS_ALWAYS_INLINE rk4_two_state_final_step(double const* ER7_UTILS_RESTRICT init_pos,
            ...
            double const* ER7_UTILS_RESTRICT const* ER7_UTILS_RESTRICT saved_vel,
            double const* ER7_UTILS_RESTRICT const* ER7_UTILS_RESTRICT saved_acc,
            double const* ER7_UTILS_RESTRICT accel, double deltat, int size, double* ER7_UTILS_RESTRICT position,
            double* ER7_UTILS_RESTRICT velocity)
        {
            static const double one_sixth = 1.0 / 6.0;
            double dto6                   = deltat * one_sixth;
            ...
            {
                position[ii] = init_pos[ii]
                    + (saved_vel[0][ii] + 2.0 * (saved_vel[1][ii] + saved_vel[2][ii]) + velocity[ii]) * dto6;
                velocity[ii] = init_vel[ii]
                    + (saved_acc[0][ii] + 2.0 * (saved_acc[1][ii] + saved_acc[2][ii]) + accel[ii]) * dto6;
            }
        }
