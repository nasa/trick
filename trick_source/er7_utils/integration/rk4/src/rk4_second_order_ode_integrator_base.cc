...
namespace er7_utils
{
    ...
    RK4SecondOrderODEIntegrator::RK4SecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    // Copy constructor.
    RK4SecondOrderODEIntegrator::RK4SecondOrderODEIntegrator(const RK4SecondOrderODEIntegrator& src)
        : Er7UtilsDeletable()
        ...
        , dtheta(NULL)
    {
        ...
        if (problem_type == Integration::GeneralizedStepSecondOrderODE)
        {
            ...
            alloc::replicate_2D_array<4>(state_size[1], src.posdot_hist, posdot_hist);
        ...
        else if (src.init_pos != NULL)
        {
            init_pos = alloc::replicate_array(state_size[0], src.init_pos);
            init_vel = alloc::replicate_array(state_size[1], src.init_vel);
            alloc::replicate_2D_array<4>(state_size[0], src.posdot_hist, posdot_hist);
            alloc::replicate_2D_array<4>(state_size[1], src.veldot_hist, veldot_hist);
        }
    ...
    RK4SecondOrderODEIntegrator::RK4SecondOrderODEIntegrator(unsigned int size, IntegrationControls& controls)
        : Er7UtilsDeletable()
    ...
    RK4SecondOrderODEIntegrator::RK4SecondOrderODEIntegrator(unsigned int position_size, unsigned int velocity_size,
    ...
    // Non-throwing swap.
    void RK4SecondOrderODEIntegrator::swap(RK4SecondOrderODEIntegrator& other)
    {
