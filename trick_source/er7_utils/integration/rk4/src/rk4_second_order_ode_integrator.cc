...
namespace er7_utils
{
    ...
    RK4SimpleSecondOrderODEIntegrator* RK4SimpleSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Clone a RK4GeneralizedDerivSecondOrderODEIntegrator.
    RK4GeneralizedDerivSecondOrderODEIntegrator* RK4GeneralizedDerivSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Clone a RK4GeneralizedStepSecondOrderODEIntegrator.
    RK4GeneralizedStepSecondOrderODEIntegrator* RK4GeneralizedStepSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
    ...
    // position.
    IntegratorResult RK4SimpleSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
         ...
         */
        case 1:
            integ_utils::inplace_two_state_euler_step_save_all(accel, 0.5 * dyn_dt, state_size[0], init_pos, init_vel,
         ...
         */
        case 2:
            rk::rk_two_state_intermediate_step(init_pos, init_vel, accel, 0.5 * dyn_dt, state_size[0], posdot_hist[1],
        ...
        case 3:
            rk::rk_two_state_intermediate_step(
                init_pos, init_vel, accel, dyn_dt, state_size[0], posdot_hist[2], veldot_hist[2], position, velocity);
            step_factor = 1.0;
            break;
        ...
        case 4:
            rk::rk4_two_state_final_step(
                init_pos, init_vel, posdot_hist, veldot_hist, accel, dyn_dt, state_size[0], position, velocity);
            step_factor = 1.0;
            break;
    ...
    IntegratorResult RK4GeneralizedDerivSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        double const* ER7_UTILS_RESTRICT accel, double* ER7_UTILS_RESTRICT velocity,
        double* ER7_UTILS_RESTRICT position)
    {
        double step_factor;
        ...
        case 1:
        {
            ...
            integ_utils::inplace_euler_step_save_state(posdot_hist[0], hdt, state_size[0], init_pos, position);
        ...
        {
            double hdt = 0.5 * dyn_dt;
            integ_utils::euler_step(init_pos, posdot_hist[1], hdt, state_size[0], position);
            integ_utils::euler_step_save_deriv(init_vel, accel, hdt, state_size[1], veldot_hist[1], velocity);
            step_factor = 0.5;
            break;
        }

        case 3:
            integ_utils::euler_step(init_pos, posdot_hist[2], dyn_dt, state_size[0], position);
            integ_utils::euler_step_save_deriv(init_vel, accel, dyn_dt, state_size[1], veldot_hist[2], velocity);
            step_factor = 1.0;
            break;

        case 4:
            rk::rk4_final_step(init_pos, posdot_hist, posdot_hist[3], dyn_dt, state_size[0], position);
            rk::rk4_final_step(init_vel, veldot_hist, accel, dyn_dt, state_size[1], velocity);
            step_factor = 1.0;
            break;
    ...
    IntegratorResult RK4GeneralizedStepSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        double const* ER7_UTILS_RESTRICT accel, double* ER7_UTILS_RESTRICT velocity,
        double* ER7_UTILS_RESTRICT position)
    {
        double step_factor;
        ...
        case 4:
        {
            ...
            integ_utils::weighted_sum<4>(velocity, posdot_hist, rk4cf_beta[0], dyn_dt, state_size[1], dtheta);
            compute_expmap_position_step(init_pos, dtheta, position);

            integ_utils::weighted_sum<4>(velocity, posdot_hist, rk4cf_beta[1], dyn_dt, state_size[1], dtheta);
