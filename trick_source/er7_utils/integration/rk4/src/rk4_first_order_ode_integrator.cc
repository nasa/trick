...
namespace er7_utils
{
    ...
    RK4FirstOrderODEIntegrator::RK4FirstOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    RK4FirstOrderODEIntegrator::RK4FirstOrderODEIntegrator(const RK4FirstOrderODEIntegrator& src)
        : Er7UtilsDeletable()
    ...
    RK4FirstOrderODEIntegrator::RK4FirstOrderODEIntegrator(unsigned int size, IntegrationControls& controls)
        : Er7UtilsDeletable()
    ...
    RK4FirstOrderODEIntegrator* RK4FirstOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Non-throwing swap.
    void RK4FirstOrderODEIntegrator::swap(RK4FirstOrderODEIntegrator& other)
    {
    ...
    IntegratorResult RK4FirstOrderODEIntegrator::integrate(double dt, unsigned int target_stage,
        const double* ER7_UTILS_RESTRICT velocity, double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            integ_utils::inplace_euler_step_save_both(
                velocity, 0.5 * dt, state_size, init_state, deriv_hist[0], position);
            step_factor = 0.5;
            break;

        case 2:
            integ_utils::euler_step_save_deriv(init_state, velocity, 0.5 * dt, state_size, deriv_hist[1], position);
            step_factor = 0.5;
            break;

        case 3:
            integ_utils::euler_step_save_deriv(init_state, velocity, dt, state_size, deriv_hist[2], position);
            step_factor = 1.0;
            break;

        case 4:
            rk::rk4_final_step(init_state, deriv_hist, velocity, dt, state_size, position);
            step_factor = 1.0;
            break;
