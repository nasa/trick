...
namespace er7_utils
{
    ...
    SymplecticEulerSecondOrderODEIntegrator::SymplecticEulerSecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    // Copy constructor.
    SymplecticEulerSecondOrderODEIntegrator::SymplecticEulerSecondOrderODEIntegrator(
    ...
    SymplecticEulerSecondOrderODEIntegrator::SymplecticEulerSecondOrderODEIntegrator(unsigned int position_size,
    ...
    // Non-throwing swap.
    void SymplecticEulerSecondOrderODEIntegrator::swap(SymplecticEulerSecondOrderODEIntegrator& other)
    {
    ...
    SymplecticEulerSimpleSecondOrderODEIntegrator* SymplecticEulerSimpleSecondOrderODEIntegrator::create_copy() const
    {
    ...
    SymplecticEulerGeneralizedDerivSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
    ...
    SymplecticEulerGeneralizedStepSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
    ...
    // position.
    IntegratorResult SymplecticEulerSimpleSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
    ...
    IntegratorResult SymplecticEulerGeneralizedDerivSecondOrderODEIntegrator::integrate(double dyn_dt,
    ...
    // Propagate state for the general case of the generalized position derivative
    // being a function of generalized position and generalized velocity.
    IntegratorResult SymplecticEulerGeneralizedStepSecondOrderODEIntegrator::integrate(double dyn_dt,
        unsigned int target_stage, double const* ER7_UTILS_RESTRICT accel, double* ER7_UTILS_RESTRICT velocity,
        double* ER7_UTILS_RESTRICT position)
    {
        if (target_stage == 1)
