...
namespace er7_utils
{
    ...
    BeemanSecondOrderODEIntegrator::BeemanSecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    BeemanSecondOrderODEIntegrator::BeemanSecondOrderODEIntegrator(const BeemanSecondOrderODEIntegrator& src)
        : Er7UtilsDeletable()
    ...
    BeemanSecondOrderODEIntegrator::BeemanSecondOrderODEIntegrator(const IntegratorConstructor& primer_constructor,
        ...
        : Er7UtilsDeletable()
        , PrimingSecondOrderODEIntegrator(2, primer_constructor, position_size, velocity_size, deriv_funs, controls_in)
        , init_vel(NULL)
        , mean_vel(NULL)
        , init_acc(NULL)
        , prev_acc(NULL)
    ...
    // Non-throwing swap.
    void BeemanSecondOrderODEIntegrator::swap(BeemanSecondOrderODEIntegrator& other)
    {
    ...
    BeemanSimpleSecondOrderODEIntegrator* BeemanSimpleSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Clone a BeemanGeneralizedDerivSecondOrderODEIntegrator.
    BeemanGeneralizedDerivSecondOrderODEIntegrator* BeemanGeneralizedDerivSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Save derivatives during priming
    void BeemanSimpleSecondOrderODEIntegrator::technique_save_derivatives(
        int, double const* ER7_UTILS_RESTRICT accel, double const*, double const*)
    {
        integ_utils::copy_array(accel, state_size[1], init_acc);
    }

    // Save derivatives during priming
    void BeemanGeneralizedDerivSecondOrderODEIntegrator::technique_save_derivatives(int,
        double const* ER7_UTILS_RESTRICT accel, double const* ER7_UTILS_RESTRICT velocity,
     ...
     */
    inline void ER7_UTILS_ALWAYS_INLINE simple_beeman_step_one(double const* ER7_UTILS_RESTRICT accel,
        double const* ER7_UTILS_RESTRICT prev_acc, double deltat, int size, double* ER7_UTILS_RESTRICT init_vel,
        double* ER7_UTILS_RESTRICT init_acc, double* ER7_UTILS_RESTRICT position, double* ER7_UTILS_RESTRICT velocity)
    {
     ...
     * @param[in,out] velocity    Updated velocity vector
     */
    inline void ER7_UTILS_ALWAYS_INLINE generalized_beeman_velocity_step_one(double const* ER7_UTILS_RESTRICT accel,
        double const* ER7_UTILS_RESTRICT prev_acc, double deltat, int size, double* ER7_UTILS_RESTRICT init_vel,
        double* ER7_UTILS_RESTRICT init_acc, double* ER7_UTILS_RESTRICT velocity)
    {
        for (int ii = 0; ii < size; ++ii)
        {
            double acc_prev  = prev_acc[ii];
     ...
     */
    inline void ER7_UTILS_ALWAYS_INLINE generalized_beeman_position_step_one(
        double const* ER7_UTILS_RESTRICT init_posddot, double const* ER7_UTILS_RESTRICT prev_posddot, double deltat,
        int size, double* ER7_UTILS_RESTRICT posdot)
    {
        static const double one_sixth = 1.0 / 6.0;
        double dto6                   = one_sixth * deltat;
     ...
     */
    inline void ER7_UTILS_ALWAYS_INLINE beeman_step_two(double const* ER7_UTILS_RESTRICT init_vel,
        double const* ER7_UTILS_RESTRICT accel, double const* ER7_UTILS_RESTRICT prev_acc,
        double const* ER7_UTILS_RESTRICT init_acc, double deltat, int size, double* ER7_UTILS_RESTRICT velocity)
    {
        static const double one_sixth = 1.0 / 6.0;
        double dto6                   = one_sixth * deltat;
        ...
        for (int ii = 0; ii < size; ++ii)
        {
            velocity[ii] = init_vel[ii] + (2.0 * accel[ii] + 5.0 * init_acc[ii] - prev_acc[ii]) * dto6;
        }
    }

    // Propagate state for the special case of velocity being the derivative of
    // position.
    IntegratorResult BeemanSimpleSecondOrderODEIntegrator::technique_integrate(double dyn_dt, unsigned int target_stage,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            simple_beeman_step_one(accel, prev_acc, dyn_dt, state_size[0], init_vel, init_acc, position, velocity);
            break;

        // Target stage 2 represents the Beeman's method corrector stage.
        case 2:
            beeman_step_two(init_vel, accel, prev_acc, init_acc, dyn_dt, state_size[0], velocity);
            break;
        }
    ...
    IntegratorResult BeemanGeneralizedDerivSecondOrderODEIntegrator::technique_integrate(double dyn_dt,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            generalized_beeman_position_step_one(init_posddot, prev_posddot, dyn_dt, state_size[0], posdot);

            // Advance position as an Euler step with the updated position derivative.
            integ_utils::euler_step(position, posdot, dyn_dt, state_size[0], position);

            // Advance velocity per the Beeman velocity predictor step.
            generalized_beeman_velocity_step_one(accel, prev_acc, dyn_dt, state_size[1], init_vel, init_acc, velocity);
            break;

        // Target stage 2 represents the Beeman's method corrector stage.
        // Velocity is corrected, but not position.
        case 2:
            beeman_step_two(init_vel, accel, prev_acc, init_acc, dyn_dt, state_size[1], velocity);
            break;
        }
