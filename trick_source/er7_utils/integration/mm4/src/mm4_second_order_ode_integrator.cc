...
namespace er7_utils
{
     ...
     */
    static inline void ER7_UTILS_ALWAYS_INLINE second_order_mm4_step_one(double const* ER7_UTILS_RESTRICT accel,
        double hdt, int size, double* ER7_UTILS_RESTRICT* ER7_UTILS_RESTRICT pos_hist,
        double* ER7_UTILS_RESTRICT* ER7_UTILS_RESTRICT vel_hist, double* ER7_UTILS_RESTRICT position,
     ...
     * @param[in,out] velocity   Generalized velocity
     */
    static inline void ER7_UTILS_ALWAYS_INLINE second_order_mm4_step_one(double const* ER7_UTILS_RESTRICT posdot,
        double const* ER7_UTILS_RESTRICT veldot, double hdt, int size[2],
     ...
     */
    static inline void ER7_UTILS_ALWAYS_INLINE second_order_mm4_step_two(double const* ER7_UTILS_RESTRICT accel,
        double dt, int size, double* ER7_UTILS_RESTRICT* ER7_UTILS_RESTRICT pos_hist,
        double* ER7_UTILS_RESTRICT* ER7_UTILS_RESTRICT vel_hist, double* ER7_UTILS_RESTRICT position,
        double* ER7_UTILS_RESTRICT velocity)
    {
        double* ER7_UTILS_RESTRICT init_pos = pos_hist[0];
     ...
     * @param[out]    velocity  Generalized velocity
     */
    static inline void ER7_UTILS_ALWAYS_INLINE second_order_mm4_step_two(double const* ER7_UTILS_RESTRICT posdot,
        double const* ER7_UTILS_RESTRICT veldot, double dt, int size[2],
        double* ER7_UTILS_RESTRICT* ER7_UTILS_RESTRICT pos_hist,
        double* ER7_UTILS_RESTRICT* ER7_UTILS_RESTRICT vel_hist, double* ER7_UTILS_RESTRICT position,
        double* ER7_UTILS_RESTRICT velocity)
    {
     ...
     */
    static inline void ER7_UTILS_ALWAYS_INLINE first_order_mm4_step_three(double const* ER7_UTILS_RESTRICT deriv,
        double const* ER7_UTILS_RESTRICT step1_state, double const* ER7_UTILS_RESTRICT step2_state, double hdt,
        int size, double* ER7_UTILS_RESTRICT state)
    {
        for (int ii = 0; ii < size; ++ii)
        {
            state[ii] = 0.5 * (step1_state[ii] + step2_state[ii] + hdt * deriv[ii]);
        }
    }
     ...
     */
    static inline void ER7_UTILS_ALWAYS_INLINE second_order_mm4_step_three(double const* ER7_UTILS_RESTRICT accel,
        double const* ER7_UTILS_RESTRICT step1_pos, double const* ER7_UTILS_RESTRICT step2_pos,
        double const* ER7_UTILS_RESTRICT step1_vel, double const* ER7_UTILS_RESTRICT step2_vel, double hdt, int size,
    ...
    MM4SecondOrderODEIntegrator::MM4SecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    // Copy constructor.
    MM4SecondOrderODEIntegrator::MM4SecondOrderODEIntegrator(const MM4SecondOrderODEIntegrator& src)
        : Er7UtilsDeletable()
    ...
    MM4SecondOrderODEIntegrator::MM4SecondOrderODEIntegrator(unsigned int size, IntegrationControls& controls)
        : Er7UtilsDeletable()
    ...
    MM4SecondOrderODEIntegrator::MM4SecondOrderODEIntegrator(unsigned int position_size, unsigned int velocity_size,
    ...
    // Non-throwing swap.
    void MM4SecondOrderODEIntegrator::swap(MM4SecondOrderODEIntegrator& other)
    {
    ...
    MM4SimpleSecondOrderODEIntegrator* MM4SimpleSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Clone a MM4GeneralizedDerivSecondOrderODEIntegrator.
    MM4GeneralizedDerivSecondOrderODEIntegrator* MM4GeneralizedDerivSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }
    ...
    // position.
    IntegratorResult MM4SimpleSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        {
        case 1:
            second_order_mm4_step_one(accel, 0.5 * dyn_dt, state_size[0], pos_hist, vel_hist, position, velocity);
            step_factor = 0.5;
            break;

        case 2:
            second_order_mm4_step_two(accel, dyn_dt, state_size[0], pos_hist, vel_hist, position, velocity);
            step_factor = 1.0;
            break;

        case 3:
            second_order_mm4_step_three(accel, pos_hist[1], pos_hist[2], vel_hist[1], vel_hist[2], 0.5 * dyn_dt,
                state_size[0], position, velocity);
            step_factor = 1.0;
            break;
    ...
    IntegratorResult MM4GeneralizedDerivSecondOrderODEIntegrator::integrate(double dyn_dt, unsigned int target_stage,
        double const* ER7_UTILS_RESTRICT accel, double* ER7_UTILS_RESTRICT velocity,
        double* ER7_UTILS_RESTRICT position)
    {
        double step_factor;
        ...
        case 1:
            ...
            second_order_mm4_step_one(posdot, accel, 0.5 * dyn_dt, state_size, pos_hist, vel_hist, position, velocity);
            step_factor = 0.5;
            break;

        case 2:
            compute_posdot(position, velocity, posdot);
            second_order_mm4_step_two(posdot, accel, dyn_dt, state_size, pos_hist, vel_hist, position, velocity);
            step_factor = 1.0;
            break;
        ...
        case 3:
        {
            ...
            first_order_mm4_step_three(accel, vel_hist[1], vel_hist[2], hdt, state_size[1], velocity);
            compute_posdot(position, velocity, posdot);
            first_order_mm4_step_three(posdot, pos_hist[1], pos_hist[2], hdt, state_size[0], position);
            step_factor = 1.0;
            break;
        }
