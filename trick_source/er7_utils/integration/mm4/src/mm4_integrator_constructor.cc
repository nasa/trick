...
namespace er7_utils
{
    ...
    IntegratorConstructor* MM4IntegratorConstructor::create_constructor(void)
    {
        return alloc::allocate_object<MM4IntegratorConstructor>();
    }

    // Create a duplicate of the constructor.
    IntegratorConstructor* MM4IntegratorConstructor::create_copy(void) const { return alloc::replicate_object(*this); }

    // Create an MM4 integration controls.
    IntegrationControls* MM4IntegratorConstructor::create_integration_controls(void) const
    {
    ...
    FirstOrderODEIntegrator* MM4IntegratorConstructor::create_first_order_ode_integrator(
    ...
    SecondOrderODEIntegrator* MM4IntegratorConstructor::create_second_order_ode_integrator(
        unsigned int size, IntegrationControls& controls) const
    {
        return integ_utils::allocate_integrator<MM4SimpleSecondOrderODEIntegrator>(size, controls);
    }

    // Create an MM4 state integrator for a second order ODE.
    SecondOrderODEIntegrator* MM4IntegratorConstructor::create_generalized_deriv_second_order_ode_integrator(
    ...
    {
        return integ_utils::allocate_integrator<MM4GeneralizedDerivSecondOrderODEIntegrator>(
