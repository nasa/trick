...
namespace er7_utils
{
    ...
    RKGill4FirstOrderODEIntegrator::RKGill4FirstOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    RKGill4FirstOrderODEIntegrator::RKGill4FirstOrderODEIntegrator(const RKGill4FirstOrderODEIntegrator& src)
        : Er7UtilsDeletable()
        ...
        , init_state(NULL)
    {
        ...
        if (src.init_state != NULL)
        {
            ...
            alloc::replicate_2D_array<double, 4>(state_size, src.deriv_hist, deriv_hist);
        }
        else
        {
            alloc::initialize_2D_array<double, 4>(deriv_hist);
    ...
    RKGill4FirstOrderODEIntegrator::RKGill4FirstOrderODEIntegrator(unsigned int size, IntegrationControls& controls)
        : Er7UtilsDeletable()
    ...
    RKGill4FirstOrderODEIntegrator* RKGill4FirstOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Non-throwing swap.
    void RKGill4FirstOrderODEIntegrator::swap(RKGill4FirstOrderODEIntegrator& other)
    {
    ...
    IntegratorResult RKGill4FirstOrderODEIntegrator::integrate(double dt, unsigned int target_stage,
        const double* ER7_UTILS_RESTRICT velocity, double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            integ_utils::inplace_euler_step_save_both(
                velocity, RKGill4ButcherTableau::RKa[1][0] * dt, state_size, init_state, deriv_hist[0], position);
            step_factor = 0.5;
            break;

        case 2:
            integ_utils::weighted_step_save_deriv<2>(
                init_state, velocity, RKGill4ButcherTableau::RKa[2], dt, state_size, deriv_hist, position);
            step_factor = 0.5;
            break;

        case 3:
            integ_utils::weighted_step_save_deriv<2>(
                init_state, velocity, RKGill4ButcherTableau::RKa[3] + 1, dt, state_size, deriv_hist + 1, position);
            step_factor = 1.0;
            break;

        case 4:
            integ_utils::weighted_step<4>(
                init_state, velocity, deriv_hist, RKGill4ButcherTableau::RKb, dt, state_size, position);
            step_factor = 1.0;
            break;
