...
namespace
{
     ...
     */
    const double scaled_predictor_weights[4] = {55.0 / 24.0, -59.0 / 24.0, 37.0 / 24.0, -9.0 / 24.0};
     ...
     */
    const double scaled_corrector_weights[4] = {9.0 / 24.0, 19.0 / 24.0, -5.0 / 24.0, 1.0 / 24.0};
}
...
namespace er7_utils
{
    ...
    ABM4SecondOrderODEIntegrator::ABM4SecondOrderODEIntegrator(void)
        : Er7UtilsDeletable()
   ...
   dtheta (NULL)
    ...
    // Copy constructor.
    ABM4SecondOrderODEIntegrator::ABM4SecondOrderODEIntegrator(const ABM4SecondOrderODEIntegrator& src)
        : Er7UtilsDeletable()
   ...
   if (problem_type == Integration::GeneralizedStepSecondOrderODE) {
      // Fill this in if/when this technique is revitalized.
        ...
        if (problem_type == Integration::GeneralizedDerivSecondOrderODE)
        {
            ...
            alloc::replicate_2D_array<double, 4>(state_size[0], src.posdot_hist, posdot_hist);
            posdot_hist_anchor = posdot_hist[0];
            alloc::replicate_2D_array<double, 4>(state_size[1], src.veldot_hist, veldot_hist);
            veldot_hist_anchor = veldot_hist[0];
        }
   ...
   dtheta (NULL)
    ...
    ABM4SecondOrderODEIntegrator::ABM4SecondOrderODEIntegrator(const IntegratorConstructor& primer_constructor,
        ...
        : Er7UtilsDeletable()
        , PrimingSecondOrderODEIntegrator(4, primer_constructor, position_size, velocity_size, deriv_funs, controls_in)
        , init_pos(NULL)
        , init_vel(NULL)
        , posdot(NULL)
        , posdot_hist_anchor(NULL)
   ...
   dtheta (NULL)
{
   ...
   alloc::deallocate_array<double> (dtheta);
    ...
    // Non-throwing swap.
    void ABM4SecondOrderODEIntegrator::swap(ABM4SecondOrderODEIntegrator& other)
    {
   ...
   std::swap (dtheta, other.dtheta);
    ...
    ABM4SimpleSecondOrderODEIntegrator* ABM4SimpleSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Clone a ABM4GeneralizedDerivSecondOrderODEIntegrator.
    ABM4GeneralizedDerivSecondOrderODEIntegrator* ABM4GeneralizedDerivSecondOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Save derivatives during priming
    void ABM4SimpleSecondOrderODEIntegrator::technique_save_derivatives(
        int, double const* ER7_UTILS_RESTRICT accel, double const* ER7_UTILS_RESTRICT velocity, double const*)
    {
        abm::rotate_history<4>(posdot_hist, veldot_hist);
        integ_utils::two_state_copy_array(velocity, accel, state_size[0], posdot_hist[0], veldot_hist[0]);
    }

    // Propagate state via ABM4, simple second order ODE.
    IntegratorResult ABM4SimpleSecondOrderODEIntegrator::technique_integrate(double dyn_dt, unsigned int target_stage,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            abm::predictor_step<4>(accel, predictor_weights, wscale, dyn_dt, state_size[0], posdot_hist, veldot_hist,
        ...
        case 2:
            ...
            abm::corrector_step<4>(init_pos, init_vel, accel, posdot_hist, veldot_hist, corrector_weights, wscale,
                dyn_dt, state_size[0], position, velocity);
            break;
        }
    ...
    // Save derivatives during priming
    void ABM4GeneralizedDerivSecondOrderODEIntegrator::technique_save_derivatives(int,
    ...
    IntegratorResult ABM4GeneralizedDerivSecondOrderODEIntegrator::technique_integrate(double dyn_dt,
        ...
        double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            abm::predictor_step<4>(posdot, accel, predictor_weights, wscale, dyn_dt, state_size, posdot_hist,
                veldot_hist, init_pos, init_vel, position, velocity);
            break;
        ...
        case 2:
            ...
            // Perform the corrector step.
            abm::corrector_step<4>(init_pos, init_vel, posdot, accel, posdot_hist, veldot_hist, corrector_weights,
                wscale, dyn_dt, state_size, position, velocity);
            break;
        }
