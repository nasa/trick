...
namespace er7_utils
{
    ...
    // Default constructor.
    ABM4FirstOrderODEIntegrator::ABM4FirstOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    // Copy constructor.
    ABM4FirstOrderODEIntegrator::ABM4FirstOrderODEIntegrator(const ABM4FirstOrderODEIntegrator& src)
        : Er7UtilsDeletable()
    ...
    ABM4FirstOrderODEIntegrator* ABM4FirstOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Non-throwing swap.
    void ABM4FirstOrderODEIntegrator::swap(ABM4FirstOrderODEIntegrator& other)
    {
    ...
    void ABM4FirstOrderODEIntegrator::technique_save_derivatives(
        int, const double* ER7_UTILS_RESTRICT velocity, const double*)
    {
    ...
    IntegratorResult ABM4FirstOrderODEIntegrator::technique_integrate(double dt, unsigned int target_stage,
        const double* ER7_UTILS_RESTRICT velocity, double* ER7_UTILS_RESTRICT position)
    {
        ...
        case 1:
            ...
            abm::predictor_step<4>(
                velocity, predictor_weights, wscale, dt, state_size, deriv_hist, init_state, position);
            break;
        ...
        case 2:
            ...
            abm::corrector_step<4>(
                init_state, velocity, deriv_hist, corrector_weights, wscale, dt, state_size, position);
            break;
        }
