...
namespace er7_utils
{
    ...
    IntegratorConstructor* ABM4IntegratorConstructor::create_constructor(void)
    {
        return alloc::allocate_object<ABM4IntegratorConstructor>();
    }

    // Create a duplicate of the constructor.
    IntegratorConstructor* ABM4IntegratorConstructor::create_copy(void) const { return alloc::replicate_object(*this); }

    // Create an ABM4 integration controls.
    IntegrationControls* ABM4IntegratorConstructor::create_integration_controls(void) const
    {
        return integ_utils::allocate_controls<PrimingIntegrationControls>(*primer_constructor, 4, 2);
    }

    // Create an ABM4 state integrator for a first order ODE.
    FirstOrderODEIntegrator* ABM4IntegratorConstructor::create_first_order_ode_integrator(
        unsigned int size, IntegrationControls& controls) const
    {
        return integ_utils::allocate_integrator<ABM4FirstOrderODEIntegrator>(*primer_constructor, size, controls);
    }

    // Create an ABM4 state integrator for a second order ODE.
    SecondOrderODEIntegrator* ABM4IntegratorConstructor::create_second_order_ode_integrator(
        unsigned int size, IntegrationControls& controls) const
    {
    ...
    // Create an ABM4 state integrator for a second order ODE.
    SecondOrderODEIntegrator* ABM4IntegratorConstructor::create_generalized_deriv_second_order_ode_integrator(
    ...
    {
        return integ_utils::allocate_integrator<ABM4GeneralizedDerivSecondOrderODEIntegrator>(
            *primer_constructor, position_size, velocity_size, deriv_funs, controls);
    }
