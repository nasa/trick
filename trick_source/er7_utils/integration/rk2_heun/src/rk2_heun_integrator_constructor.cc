...
namespace er7_utils
{
    ...
    IntegratorConstructor* RK2HeunIntegratorConstructor::create_constructor(void)
    {
        return alloc::allocate_object<RK2HeunIntegratorConstructor>();
    }

    // Create a duplicate of the constructor.
    IntegratorConstructor* RK2HeunIntegratorConstructor::create_copy(void) const
    {
        return alloc::replicate_object(*this);
    }

    // Create an RK2Heun integration controls.
    IntegrationControls* RK2HeunIntegratorConstructor::create_integration_controls(void) const
    {
    ...
    FirstOrderODEIntegrator* RK2HeunIntegratorConstructor::create_first_order_ode_integrator(
    ...
    SecondOrderODEIntegrator* RK2HeunIntegratorConstructor::create_second_order_ode_integrator(
        unsigned int size, IntegrationControls& controls) const
    {
        return integ_utils::allocate_integrator<RK2HeunSimpleSecondOrderODEIntegrator>(size, controls);
    }

    // Create an RK2Heun state integrator for a second order ODE.
    SecondOrderODEIntegrator* RK2HeunIntegratorConstructor::create_generalized_deriv_second_order_ode_integrator(
    ...
    {
        return integ_utils::allocate_integrator<RK2HeunGeneralizedDerivSecondOrderODEIntegrator>(
            position_size, velocity_size, deriv_funs, controls);
    }

    // Create an RK2Heun state integrator for a second order ODE.
    SecondOrderODEIntegrator* RK2HeunIntegratorConstructor::create_generalized_step_second_order_ode_integrator(
    ...
    {
        return integ_utils::allocate_integrator<RK2HeunGeneralizedStepSecondOrderODEIntegrator>(
