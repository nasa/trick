...
namespace er7_utils
{
    ...
    RK2HeunFirstOrderODEIntegrator::RK2HeunFirstOrderODEIntegrator(void)
        : Er7UtilsDeletable()
    ...
    // Copy constructor.
    RK2HeunFirstOrderODEIntegrator::RK2HeunFirstOrderODEIntegrator(const RK2HeunFirstOrderODEIntegrator& src)
        : Er7UtilsDeletable()
    ...
    RK2HeunFirstOrderODEIntegrator::RK2HeunFirstOrderODEIntegrator(unsigned int size, IntegrationControls& controls)
        : Er7UtilsDeletable()
    ...
    RK2HeunFirstOrderODEIntegrator* RK2HeunFirstOrderODEIntegrator::create_copy() const
    {
        return alloc::replicate_object(*this);
    }

    // Non-throwing swap.
    void RK2HeunFirstOrderODEIntegrator::swap(RK2HeunFirstOrderODEIntegrator& other)
    {
    ...
    IntegratorResult RK2HeunFirstOrderODEIntegrator::integrate(double dt, unsigned int target_stage,
        const double* ER7_UTILS_RESTRICT velocity, double* ER7_UTILS_RESTRICT position)
    {
        switch (target_stage)
        {
        case 1:
            integ_utils::inplace_euler_step_save_both(velocity, dt, state_size, init_state, init_deriv, position);
            break;

        case 2:
            rk::rk2_heun_final_step(init_state, init_deriv, velocity, dt, state_size, position);
            break;
        }
