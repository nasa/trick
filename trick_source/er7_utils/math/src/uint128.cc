...
namespace er7_utils
{
    ...
    UInt128& UInt128::operator<<=(unsigned int shift)
    {
        if (shift >= 128)
        {
            low = high = 0;
    ...
    UInt128& UInt128::operator*=(const UInt128& other)
    {
        ...
        for (int ii = 0; ii < 4; ++ii)
        {
            ...
            {
                uint64_t prod_ii_jj = fact * other_arr[jj] + prod[ii + jj] + carry;
    ...
    void UInt128::divide(UInt128 divisor, UInt128& quotient, UInt128& remainder)
    {
    ...
    UInt128::operator double() const { return double(high) * 0x1p64 + double(low); }
    ...
    std::ostream& operator<<(std::ostream& stream, const UInt128& num)
    {
