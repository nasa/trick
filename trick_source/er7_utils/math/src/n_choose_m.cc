...
namespace er7_utils
{
        ...
        , triangle_nrows(src.triangle_nrows)
    {
        ...
        {
            triangle = alloc::replicate_array<uint64_t>(row_index(triangle_nrows), src.triangle);
        }
    }

    NChooseM::~NChooseM() { alloc::deallocate_array<uint64_t>(triangle); }

    void NChooseM::resize(unsigned int new_size)
    {
        ...
        if (new_size > triangle_nrows)
        {
            ...
            if (new_size > 68u)
            {
                throw std::overflow_error("new_size exceeds 68 in NChooseM::resize");
            }

            uint64_t* new_triangle = alloc::allocate_array<uint64_t>(row_index(new_size));
            ...
            if (triangle_nrows > 0u)
            {
                ...
                std::memcpy(new_triangle, triangle, row_index(triangle_nrows) * sizeof(uint64_t));
    ...
    uint64_t NChooseM::operator()(unsigned int n, unsigned int m)
    {
    ...
    double NChooseM::compute(unsigned int n, unsigned int m)
    {
        // Special case: n choose m is zero for all m>n.
        if (m > n)
        {
        ...
        // Compute n choose m.
        double result = 1.0;
        for (double num = double(n), den = double(std::min(m, n - m)); den > 0.0; --num, --den)
        {
            result *= num / den;
        }
    ...
    void NChooseM::swap(NChooseM& other)
    {
        std::swap(triangle, other.triangle);
        std::swap(triangle_nrows, other.triangle_nrows);
    }

    void NChooseM::construct_row(unsigned int n, unsigned int known_n)
    {
