...
foreach my $technique (@techniques) {
    ...
    if ( scalar @trick_vals ) {
        ...
        warn "Results mismatch\n" if abs( $er7_vals[1] - $trick_vals[1] ) > 1e-2;
    }

    if ( $repeat_count > 0 ) {
        if ( scalar @trick_vals ) {
            printf "%-15s  %8d  %14.4f  %7.3f %7.3f  %6.2f\n",
                $technique,
                $er7_vals[0], $er7_vals[1], $er7_vals[2], $trick_vals[2],
                100 * ( 1 - $er7_vals[2] / $trick_vals[2] );
        }
        else {
            printf "%-15s  %8d  %14.4f  %7.3f  ------   -----\n", $technique, $er7_vals[0], $er7_vals[1], $er7_vals[2];
        }
    }
    else {
...
sub get_sim_values ($$) {
    ...
    my $lastx = get_sim_result $input, 'Final position:', 's/.*:\([^,]*\).*/\1/';
