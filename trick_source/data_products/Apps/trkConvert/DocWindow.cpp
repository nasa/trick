    ...
    : QMainWindow(0, 0)
{
    ...
    {
        varTable->addRecord(
            Qt::Checked, datalog->parameterName(ii), datalog->parameterType(ii), datalog->parameterUnits(ii));
    }
...
void DocWindow::load()
{
    ...
    newFileName = QFileDialog::getOpenFileName(this, tr("Open Data File"), ".", tr("Data Files (*.trk)"));
...
void DocWindow::checkAll() { varTable->checkAll(); }

void DocWindow::unCheckAll() { varTable->unCheckAll(); }
...
void DocWindow::findAgainForward() { findAgain(1); }

void DocWindow::findAgainBackward() { findAgain(-1); }
