...
void VarTableWidget::addRecord(
    Qt::CheckState checkState, std::string nameStr, std::string typeStr, std::string unitsStr)
{
...
void VarTableWidget::scrollToRecord(int index) { scrollToItem(item(index, 0)); }

bool VarTableWidget::isChecked(int index) { return (item(index, 0)->checkState() == Qt::Checked) ? true : false; }

int VarTableWidget::recordCount() { return rowCount(); }
