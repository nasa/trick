...
const char* TypeName[] = {"--0--", "char", "unsigned char", "--3--", "short", "unsigned short", "int", "unsigned int",
    "long", "unsigned long", "float", "double", "--12--", "--13--", "long long", "unsigned long long", "--16--", "bool",
...
std::string TRK_DataLog::getFileName() const { return fileName; }

int TRK_DataLog::parameterCount() const { return (int)N_params; }
    ...
    while (fread(dataRecord, 1, dataRecordSize, in_fp) == (size_t)dataRecordSize)
    {
        ...
        for (int ii = 1; ii < (int)N_params; ii++)
        {
            if (paramSelected[ii])
            {
                ...
                default:
                {
                    fprintf(stdout, "Unknown Data Type [%d] of size [%d] bytes.", paramDescriptions[ii]->dataType,
