 ...
 */
IterOverLogGroups::IterOverLogGroups(LogMultiGroup * lmg, vector<string> & runs, vector<string> & paramNames)
{
    int found;
    string runDirName;
    int tIdx, tLogIdx;
    ...
    for(lg = iterLogGroups_.begin(); lg != iterLogGroups_.end(); lg++)
    {
        ...
        {
            (*lg)->getParamIdx("sys.exec.out.time", &tIdx, &tLogIdx, (*ss).c_str(), &vIdx, &vLogIdx);
            ...
            // Weed out duplicates
            found = 0;
            for(ii = logIndices.begin(); ii != logIndices.end(); ii++)
            {
                if(vLogIdx == *ii)
                {
                    found = 1;
    ...
    listLogGroupIterators_.reserve(listLogGroupIterators_.size() + lg2log.size());
    for(ll = lg2log.begin(); ll != lg2log.end(); ll++)
    {
        key = (*ll).first;

        IterPreserveTime * iter = new IterPreserveTime(key, lg2log[key], 0.0, 0.0, DBL_MAX);
...
void IterOverLogGroups::operator++(int)
{
    ...
    for(ili = listLogGroupIterators_.begin(); ili != listLogGroupIterators_.end(); ili++)
    {
