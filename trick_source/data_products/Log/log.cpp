 ...
 *  variables NOT 6.
 */
int LogData::getNumVars() { return (nVars_); }
 ...
 */
void LogData::setNumVars(int nVars) { nVars_ = nVars; }
...
void LogData::enableStats()
{
    ...
    {
        fprintf(stderr,
            "ERROR: enableStats() because time is "
...
void LogData::getMinMaxStats(int paramIdx, double* tmin, double* min, double* tmax, double* max)
{
    if (!isEnabledStats_)
    {
        enableStats();
...
int LogData::countNumBinaryFiles(char* pathToBinaryData)
{
    ...
    while ((dp = readdir(dirp)) != NULL)
    {
        ...
        if (len_dname == len_binFileName + len_extension && !strncmp(dp->d_name, binFileName_, len_binFileName))
        {
            ...
            {
                if (!(dp->d_name[ii] >= '0' && dp->d_name[ii] <= '9'))
                {
                    binary = 0;
                }
            }
...
int LogData::openCurrBinaryFile()
{
    ...
    len  = strlen(binFilePath_) + strlen(binFileName_) + strlen(extension) + 3;
    file = new char[len];
    ...
    {
        fprintf(stderr,
            "ERROR: Search for parameter exhausted."
...
int LogData::isEOF() { return (isEOF_); }
...
int LogData::getSizeRecord()
{
    ...
    if (sizeRec <= 0)
    {
        std::cerr << "ERROR: LogData::getSizeRecord() is zero or negative, may cause divide-by-zero. Exiting."
                  << std::endl;
        exit(-1);
    }
    return (sizeRec);
 ...
 */
int LogData::getNumValRecord() { return (nVals_); }
...
void* LogData::getVal(int index)
{
    ...
    {
        fprintf(stderr,
            "ERROR: getVal attempting to grab val "
            "out of bounds.\n"
...
Var::enumType LogData::getType(int idx) { return (typeVal_[idx]); }

/** Get size in bytes for value of current record at given index */
int LogData::getSize(int idx) { return (sizeVal_[idx]); }
...
int LogData::setScaleFactor(int paramIdx, double factor)
{
    ...
    {
        fprintf(stderr,
            "ERROR: setScaleFactor received paramIdx "
            "%d out of bounds.\n",
            paramIdx);
        exit(-1);
    }
...
int LogData::setBias(int paramIdx, double bias)
{
    ...
    {
        fprintf(stderr,
            "ERROR: setBias received paramIdx %d "
            "out of bounds.\n",
            paramIdx);
        exit(-1);
    }
...
int LogData::setMax(int paramIdx, double max)
{
    ...
    {
        fprintf(stderr,
            "ERROR: setMax received paramIdx %d"
            " out of bounds.\n",
            paramIdx);
        exit(-1);
    }
...
int LogData::setMin(int paramIdx, double min)
{
    ...
    {
        fprintf(stderr,
            "ERROR: setMin received paramIdx %d "
            "out of bounds.\n",
            paramIdx);
        exit(-1);
    }
...
int LogData::setUnit(int paramIdx, std::string to_units)
{
    ...
    {
        fprintf(stderr,
            "ERROR: setUnit received paramIdx %d "
            "out of bounds.\n",
            paramIdx);
        exit(-1);
    }
 ...
 */
int LogData::getValueAtTime(int tIdx, double time, int vIdx, double* value)
{
    double tVal;
    ...
    {
        fprintf(stderr,
            "ERROR: getValueAtTime received paramIdx %d "
            "out of bounds.\n",
            vIdx);
        exit(-1);
    }
 ...
 * log_Ball1, without the 000, 001 extensions...
 */
char* LogData::getBinaryFileName() { return (binFileName_); }

/** Default LogGroup constructor */
LogGroup::LogGroup()
{
    fprintf(stderr,
        "ERROR: Please use LogGroup::LogGroup(char* headerFile)"
        " constructor.\n");
...
LogGroup::LogGroup(char* path)
{
    ...
    {
        fprintf(stderr,
            "ERROR: Failed parsing log headers "
            "for \"%s\".\n",
            path);
    }
 ...
 */
void LogGroup::getMinMaxStats(const char* param, double* tmin, double* min, double* tmax, double* max)
{
    int xIdx, xLogIdx;
    int yIdx, yLogIdx;
    int ret;
    ...
    ret = getParamIdx("sys.exec.out.time", &xIdx, &xLogIdx, param, &yIdx, &yLogIdx);
    if (ret == -1)
    {
        fprintf(stderr,
            "ERROR: Couldn't get min/max statistics "
...
int LogGroup::setUnit(const char* paramName, std::string u)
{
    ...
    ret = getParamIdx("sys.exec.out.time", &xIdx, &xLogIdx, paramName, &yIdx, &yLogIdx);
    if (ret == -1)
    {
        fprintf(stderr,
            "ERROR: In setUnit() died with faulty "
            "parameter name.\n");
        exit(-1);
    }
 ...
 */
int LogGroup::getValueAtTime(const char* paramName, double time, double* value)
{
    int tIdx, tLogIdx;
    int yIdx, yLogIdx;
    double vv;
    ...
    ret = getParamIdx("sys.exec.out.time", &tIdx, &tLogIdx, paramName, &yIdx, &yLogIdx);
    if (ret == -1)
    {
        fprintf(stderr,
            "ERROR: In getValueAtTime() died with faulty "
            "parameter name.\n");
        exit(-1);
    }

    // Get the value at time
    rr = log[yLogIdx]->getValueAtTime(tIdx, time, yIdx, &vv);
 ...
 * error and exit.
 */
int LogGroup::getParamIdx(
    const char* xVarName, int* xIdx, int* xLogDataIdx, const char* yVarName, int* yIdx, int* yLogDataIdx)
{
    int i;
    int yFoundIdx;
...
int LogGroup::getHeaders()
{
    ...
    while ((dp = readdir(dirp)) != NULL)
    {
        ...
        {
            len              = strlen(dp->d_name) + strlen(pathToBinaryData_) + 2;
            full_header_name = (char*)malloc(len);
            snprintf(full_header_name, len, "%s/%s", pathToBinaryData_, dp->d_name);
            if (!strncmp(dp->d_name, "log_", 4) && is04BinaryHeader_(full_header_name))
            {
                if (strlen(dp->d_name) > maxLengthHeaderName)
                {
                    maxLengthHeaderName = strlen(dp->d_name);
                }
                nHeaders_++;
            }
    ...
    while ((dp = readdir(dirp)) != NULL)
    {
        if (strstr(dp->d_name, ".header") != NULL)
        {
            ...
            {
                headers_[i] = (char*)malloc(maxLengthHeaderName);
...
int LogGroup::getNumGroups() { return (nGroups_); }

/** Returns path to Binary data (RUN_ dir) */
char* LogGroup::getDataDir() { return (pathToBinaryData_); }

void LogGroup::setEOF_() { isEOF_ = 1; }
...
IterMultiRun::IterMultiRun(LogGroup* lg1, set<int>& logIndices1, LogGroup* lg2, set<int>& logIndices2, double frequency,
    double startTime, double stopTime)
{
    // Two iterators for iterating over both runs
    iter1_ = new IterPreserveTime(lg1, logIndices1, frequency, startTime, stopTime);
    iter2_ = new IterPreserveTime(lg2, logIndices2, frequency, startTime, stopTime);
...
IterPreserveTime::IterPreserveTime(
    LogGroup* lg, set<int>& logIndices, double frequency, double startTime, double stopTime)
{
    unsigned int ii;
...
void IterPreserveTime::begin()
{
    ...
    for (logIdx_ = logIndices_.begin(); logIdx_ != logIndices_.end(); logIdx_++)
    {
...
double IterPreserveTime::getTime()
{
    ...
    for (logIdx_ = logIndices_.begin(); logIdx_ != logIndices_.end(); logIdx_++)
    {
        ...
        {
            currTime_[ii] = *((double*)lg_->log[*logIdx_]->getVal(timeIdx_[ii]));
        }
        else
        {
...
#define NEXTFREQ_EQUALS_MINTIME (DPLOG_ABS(nextFrequencyCheck_ - minTime) < 0.000000001)

#define NEXTFREQ_LESSTHAN_MINTIME ((nextFrequencyCheck_ - minTime < 0.0) && !NEXTFREQ_EQUALS_MINTIME)
