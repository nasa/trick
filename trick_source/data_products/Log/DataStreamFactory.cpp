...
DataStream * DataStreamFactory::create(const char * iRunDir, const char * iParamName, const char * timeName)
{
    DataStream * stream = 0;
    DIR * dirp;
    struct dirent * dp;
    ...
    while((dp = readdir(dirp)) != NULL)
    {
        ...
        if(!strcmp(&(dp->d_name[len - 4]), ".mat"))
        {
            ...
            full_path = (char *)malloc(full_path_len);
            snprintf(full_path, full_path_len, "%s/%s", runDir.c_str(), dp->d_name);
            if(MatLabLocateParam(full_path, (char *)paramName.c_str(), (char *)timeName))
            {
                closedir(dirp);
                stream = new MatLab(full_path, (char *)paramName.c_str(), (char *)timeName);
                free(full_path);
                return (stream);
            }
            if(MatLab4LocateParam(full_path, (char *)paramName.c_str(), (char *)timeName))
            {
                closedir(dirp);
                stream = new MatLab4(full_path, (char *)paramName.c_str(), (char *)timeName);
                free(full_path);
                return (stream);
            }
            free(full_path);
...
DataStream * DataStreamFactory::create(const char * Machine, const unsigned short Port, const char * VarName)
{
    DataStream * stream = 0;
    ...
    const char * sharedLibName, int nInputStreams, DataStream ** istreams, int nOutputStreams, int outputIdx)
{
    ...
    stream = new ExternalProgram(sharedLibName, nInputStreams, istreams, nOutputStreams, outputIdx);

    return (stream);
}

int getVariableNames(const char * pathToData, int * numVariables, char *** variableNames)
{
    int len;
