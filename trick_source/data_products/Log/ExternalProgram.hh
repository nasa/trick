...
public:
    ExternalProgram(
        const char * sharedLibName, int numInputs, DataStream ** istreams, int numOutputs, int outputParamIdx);
    ~ExternalProgram();
...
private:
    ...
    // a set of outputs.
    int (*external_program)(double * in, int numIn, double * out, int numOut);
