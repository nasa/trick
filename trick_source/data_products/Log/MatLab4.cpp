...
MatLab4::MatLab4(char* file_name, char* param_name, char* time_name)
{
    ...
    while ((!param_found || !time_found) && (offset < max_offset))
    {
        ...
        if (swap_)
        {
            row = trick_byteswap_int(row);
        }

        fread(&column, 4, 1, fp_);
        if (swap_)
        {
            column = trick_byteswap_int(column);
        }

        fread(&imaginary, 4, 1, fp_);
        if (swap_)
        {
            imaginary = trick_byteswap_int(imaginary);
        }
...
int MatLab4::end() { return (curr_row_ >= y_value_.rows_ || curr_row_ >= time_value_.rows_); }
