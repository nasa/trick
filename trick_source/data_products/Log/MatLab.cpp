...
int MatLab::get(double* time, double* value)
{
    ...
    {
        offset = time_value_.data_offset_ + time_value_.dims_[0] * time_value_.field_num_ * time_value_.size_
            + curr_row_ * time_value_.size_;
        fseek(fp_, offset, SEEK_SET);
        *time = getvalue(&time_value_);

        offset = y_value_.data_offset_ + y_value_.dims_[0] * y_value_.field_num_ * y_value_.size_
...
int MatLab::end() { return (curr_row_ >= y_value_.dims_[0] || curr_row_ >= time_value_.dims_[0]); }
