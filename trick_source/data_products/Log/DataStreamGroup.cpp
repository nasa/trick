...
void DataStreamGroup::setTimeMatchTolerance(double tol) { timeMatchTolerance_ = tol; }

void DataStreamGroup::setFrequency(double freq) { frequency_ = freq; }

void DataStreamGroup::setStartTime(double time) { startTime_ = time; }

void DataStreamGroup::setStopTime(double time) { stopTime_ = time; }
...
void DataStreamGroup::clear() { dataStreams_.clear(); }

void DataStreamGroup::setPreserveTimeOn() { preserveTime_ = 1; }

void DataStreamGroup::setPreserveTimeOff() { preserveTime_ = 0; }
...
void DataStreamGroup::begin()
{
    ...
    for (ii = 0; ii < dataStreams_.size(); ii++)
    {
        dataStreams_[ii]->peek(&lastRead_[dataStreams_[ii]].time, &lastRead_[dataStreams_[ii]].value);
    }
}

bool DataStreamGroup::end() { return isEOF_; }
...
#define NEXTFREQ_EQUALS_MINTIME (DPLOG_ABS(nextFrequencyCheck_ - minTime) < timeMatchTolerance_)
...
int DataStreamGroup::matchTimeStamps()
{
    ...
    while (!frequency_match)
    {
        ...
        while (!matched_time_stamps)
        {
            ...
            for (ii = 0; ii < dataStreams_.size(); ii++)
            {
                if (!dataStreams_[ii]->get(&lastRead_[dataStreams_[ii]].time, &lastRead_[dataStreams_[ii]].value))
                {
                    isEOF_ = 1;
                    return (0);
                }
            ...
            for (ii = 0; ii < dataStreams_.size(); ii++)
            {
                while (maxTime - lastRead_[dataStreams_[ii]].time > timeMatchTolerance_)
                {
                    if (!dataStreams_[ii]->get(&lastRead_[dataStreams_[ii]].time, &lastRead_[dataStreams_[ii]].value))
                    {
                        isEOF_ = 1;
