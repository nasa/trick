...
char* measure_name[] = {"Time", "Length", "Angle", "Mass", "Force", "Voltage", "Current", "Resistance", "Temperature",
    "Sound", "Energy", "Power", "Pressure", "Volume", "Unitless", "Frequency"};
...
static Conv_Status_t next_lexeme(parse_context_t* context)
{
    ...
    default:
    {
        ...
        for (i = 0; i < N_conversions; i++)
        {
            ...
            while ((context->s[context->n + len] == conv_tab[i].c_unit[len]) && (context->s[context->n + len] != '\0'))
            {
                len++;
            }
...
static void init_stack(UCF_stack_t* stack) { stack->top = 0; }
        ...
        case Units_Primitive:
        {
            ...
            if (conv_tab[f_units->t[i].a1].measure != conv_tab[t_units->t[i].a1].measure)
            {
                ...
                fprintf(stderr, "\"%s\" is a measure of \"%s\", whereas \"%s\" is a measure of \"%s\".\n",
                    conv_tab[f_units->t[i].a1].c_unit, measure_name[conv_tab[f_units->t[i].a1].measure],
            ...
            if (convert_primitive(conv_tab[f_units->t[i].a1].c_unit, conv_tab[t_units->t[i].a1].c_unit, &p) != CONV_OK)
            {
                return (CONV_ERR);
            }
