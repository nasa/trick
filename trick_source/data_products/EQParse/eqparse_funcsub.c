...
#define NUM_FUNCTIONS 38
    char function_names[NUM_FUNCTIONS][32]
        = {"abs", "acosh", "acos", "asinh", "asin", "atanh", "atan2", "atan", "besj0", "besj1", "besy0", "besy1",
            "ceil", "cosh", "cos", "div", "erfc", "erf", "exp", "floor", "gamma", "int", "inverf", "invnorm", "lgamma",
            "log10", "log", "norm", "rand", "sgn", "sinh", "sin", "sqrt", "tanh", "tan", "PI", "M_PI", "M_E"};

    char* function_symbols = (char*)"\xA2\xA4\xA3\xA7\xA6"
                                    "\xA9\xAA\xA8\xAB\xAC"
                                    "\xAD\xAE\xAF\xB2"
    ...
    {
        if ((isalpha(equation[test])) && ((isalpha(equation[test + 1])) || (equation[test + 1] == '_')))
        {
            comp = 1;
            break;
        }
    ...
    while (comp == 1)
    {
        ...
        while (i == 0)
        {
            if ((isalpha(equation[start])) && ((isalpha(equation[start + 1])) || (equation[start + 1] == '_')))
            {
                i = 1;
            }
            else
        ...
        {
            if (!strncmp(function_names[ii], &equation[start], strlen(function_names[ii])))
            {
        ...
        for (j = 0; j < (tlen - 1); j++)
        {
            if ((isalpha(equation[j])) && ((isalpha(equation[j + 1])) || (equation[j + 1] == '_')))
            {
                comp = 1;
                break;
            }
