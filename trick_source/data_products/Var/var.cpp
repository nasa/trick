...
const char* Var::getVarName() { return (varName_.c_str()); }

const char* Var::getTimeName() { return (timeName_.c_str()); }
...
double Var::getScaleFactor() { return (scaleFactor_); }
 ...
 */
double Var::getBias() { return (bias_); }

/** Get variable's minimal possible value
 *
 */
double Var::getMinRange() { return (minRange_); }

/** Get variable's maximal possible value
 *
 */
double Var::getMaxRange() { return (maxRange_); }

/** Get variable's unit
 */
std::string Var::getUnit() { return (unit_); }
...
std::string Var::getTimeUnit() { return (timeUnit_); }
...
const char* Var::getVarNameNoDims()
{
    ...
    for (ii = 0; ii < len; ii++)
    {
        ...
        {
            jj--;
            while (varName_[ii] != ']' && varName_[ii] != ')' && ii < len)
            {
                ii++;
            }
        }
...
int Var::getDimOffset(const char* param)
{
    ...
    for (i = 0; i < v1.numParams; i++)
    {
        ...
        {
            if (v2.dims[i].lower < v1.dims[i].lower || v2.dims[i].upper > v1.dims[i].upper)
            {
                VAR_DELETE;
 ...
 *
 */
int Var::getSize() { return (byteSize_); }
...
Var::enumType Var::getType() { return (varType_); }
...
int Var::calcNumDimensions()
{
    ...
    for (i = 0; i < stringLen; i++)
    {
        if (varName_[i] == ']')
        {
            cerr << "ERROR: Missing open bracket in" << varName_ << endl;
            delete[] dim1;
            delete[] dim2;
            return (-1);
        }
        ...
        if (varName_[i] == '[')
        {
            while (varName_[i] != ']')
            {
                ...
                if (i == stringLen)
                {
                    cerr << "ERROR: Missing close bracket in" << varName_ << endl;
                    delete[] dim1;
                    delete[] dim2;
                    return (-1);
                }
            ...
            if (dim1[0] == '\0' || dim2[0] == '\0')
            {
                cerr << "ERROR: Bad syntax in dimension "
                     << "specification for " << varName_ << endl;
                delete[] dim1;
                delete[] dim2;
                return (-1);
            }
...
void Var::setMinRange(double minRange) { minRange_ = minRange; }

/** Set variable's maximum range */
void Var::setMaxRange(double maxRange) { maxRange_ = maxRange; }

/** Set variable's scale factor */
void Var::setScaleFactor(double scaleFactor) { scaleFactor_ = scaleFactor; }

/** Set variable's bias @see getBias() */
void Var::setBias(double bias) { bias_ = bias; }
...
const Var& Var::operator=(Var& right)
{
    ...
    {
        fprintf(stderr,
            "Attempted self assignment in Xvar "
            "operator=. \n");
        return *this;
    }
...
const char* Var::getLabel() { return (label_.c_str()); }
