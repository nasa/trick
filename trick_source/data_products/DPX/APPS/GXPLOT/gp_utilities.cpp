...
int filterPostScriptBW(const char * psName)
{
    ...
    buf.subst_g("/LTa { AL [1 dl 2 dl]", "/LTa { AL [1 dl 2 div 2 dl 2 div]");
    buf.subst_g("/LTa { AL [1 udl mul 2 udl mul]", "/LTa { AL [1 dl 2 div 2 dl 2 div]");
...
int filterPostScriptColor(const char * psName, GPViewPageNode * gp_view_page_node)
{
    ...
    if(!page_fg_color.empty())
    {
        if(page_fg_color[0] == '#')
        {
            ...
            buf.subst_g("/LTa { AL [1 dl 2 dl] 0 setdash 0 0 0", tmp_stream.str().c_str());
            buf.subst_g("/LTa { AL [1 udl mul 2 udl mul] 0 setdash 0 0 0", tmp_stream.str().c_str());
