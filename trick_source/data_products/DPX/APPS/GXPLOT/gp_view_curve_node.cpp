...
// CONSTRUCTOR
GPViewCurveNode::GPViewCurveNode(Widget Toplevel, DPV_TextBuffer * Textbuf, DPC_curve * Curve, int Curve_number)
{
    ...
    curve_gp_line_style = (dpc_curve->getYAttribute("gnuplot_line_style"))
                              ? dpc_curve->getYAttribute("gnuplot_line_style")
                              : "lines";
    ...
    if((presentation != NULL) && strcasecmp(presentation, "stream") != 0)
    {
        ...
        if(!access("/tmp", W_OK))
        {
            strncpy(curve_data_file_name, "/tmp/dpx_gp_curve_XXXXXX", sizeof(curve_data_file_name));
        }
        else if(!access("/var/tmp", W_OK))
        {
            strncpy(curve_data_file_name, "/var/tmp/dpx_gp_curve_XXXXXX", sizeof(curve_data_file_name));
        }
        else
        {
