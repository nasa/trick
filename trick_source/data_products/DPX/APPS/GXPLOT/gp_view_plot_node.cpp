...
// CONSTRUCTOR
GPViewPlotNode::GPViewPlotNode(Widget Toplevel, DPV_TextBuffer * Textbuf, DPC_plot * Plot, int Plot_number)
{
    ...
    {
        textbuf->print("set title \"<PLOT_TITLE>\" offset plot_title_xpos, plot_title_ypos <PLOT_FONT_SPEC> "
                       "<PLOT_TEXT_COLOR_SPEC>\n");
        textbuf->print("set xlabel \"<PLOT_X_AXIS_LABEL>\" offset x_axis_label_xpos, x_axis_label_ypos "
                       "<PLOT_FONT_SPEC> <PLOT_TEXT_COLOR_SPEC>\n");
        textbuf->print("set ylabel \"<PLOT_Y_AXIS_LABEL>\" offset y_axis_label_xpos, y_axis_label_ypos "
                       "<PLOT_FONT_SPEC> <PLOT_TEXT_COLOR_SPEC>\n");
    }
    else
    {
...
void GPViewPlotNode::finalize()
{
     ...
     */
    tmp_stream.str("");
    if(GnuplotVersion() < 4.2 || plot_fg_color.empty() || !color_is_valid(&plot_fg_color))
    {
    ...
    else
    {
         ...
         * (Feature only available in Gnuplot version 4.2, or later.)
         */
        if(GnuplotVersion() >= 4.2 && !grid_color.empty() && color_is_valid(&grid_color))
        {
            tmp_stream.str("");
