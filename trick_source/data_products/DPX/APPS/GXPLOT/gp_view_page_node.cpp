...
GPViewPageNode::~GPViewPageNode()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (gp_view_plot_node)
        {
            delete gp_view_plot_node;
        }
    }
}
...
void GPViewPageNode::finalize()
{
     ...
     */
    string plotColors[] = {"black", "red", "blue", "green", "magenta", "yellow", "cyan", "pink", "dark-green",
    ...
    if ((plot_node_list.size() % 5 == 2 || plot_node_list.size() % 5 == 3) && plot_node_list.size() > 2)
    {
        xx_geom = 600;
        yy_geom = 720;
    }
    ...
    if (GnuplotVersion() >= 4.2 && !page_fg_color.empty() && color_is_valid(&page_fg_color))
    {
        tmp_stream.str("");
    ...
    tmp_stream.str("");
    if (GnuplotVersion() < 4.2 || page_fg_color.empty() || !page_fg_color.empty())
    {
    ...
    if (!access("/tmp", W_OK))
    {
        strncpy(gp_template_file_name, "/tmp/dpx_gp_page_XXXXXX", sizeof(gp_template_file_name));
    }
    else if (!access("/var/tmp", W_OK))
    {
        strncpy(gp_template_file_name, "/var/tmp/dpx_gp_page_XXXXXX", sizeof(gp_template_file_name));
    }
    else
    {
    ...
    case GPViewPageNode::DISK:
        ...
        if (diskFileName.empty())
        {
            cerr << "ERROR: GPViewPageNode::finalize() called without "
                 ...
                 << "to disk, you must set output file name." << endl;
            exit(-1);
        }
    ...
    case GPViewPageNode::PRINTER:
        ...
        if (!access("/tmp", W_OK))
        {
            strncpy(outFileName, "/tmp/dpx_gxplot_XXXXXX", sizeof(outFileName));
        }
        else if (!access("/var/tmp", W_OK))
        {
            strncpy(outFileName, "/var/tmp/dpx_gxplot_XXXXXX", sizeof(outFileName));
        }
        else
        {
            std::cerr << "Unable to access /tmp or /var/tmp, where a temporary\n"
