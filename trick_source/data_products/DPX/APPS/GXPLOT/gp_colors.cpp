...
int color_is_valid(string * color)
{
    ...
    if(color_stream.str().find(tmp_string) != string::npos || tmp_string.compare(0, 1, "#") == 0)
    {
    ...
    {
        cout << "warning: \"" << color_orig << "\" is not a known color name in Gnuplot." << endl;
...
int convert_to_hexcolor(string * color)
{
    ...
    for(int i = 0; i < num_colors; i++)
    {
        if(strcasecmp(duplicate.c_str(), (const char *)color_trans[i].color) == 0)
        {
            ...
            cout << "gp_colors.cpp: converting \"" << color_orig << "\" to \"" << tmp_string << "\"." << endl;
            *color = tmp_string;
            return (0);
        }
    }
