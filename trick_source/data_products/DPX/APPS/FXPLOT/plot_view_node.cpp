...
PlotViewNode::PlotViewNode(Widget Toplevel, Widget Parent_form, DPC_plot* Plot)
{
    ...
    main_form_widget = XtVaCreateWidget(
        "PlotForm", xmFormWidgetClass, Parent_form, XmNresizePolicy, XmRESIZE_GROW, XmNfractionBase, 10, NULL);

    ctrls_form_widget = XtVaCreateManagedWidget("CtrlsForm", xmFormWidgetClass, main_form_widget, XmNresizePolicy,
        XmRESIZE_GROW, XmNfractionBase, 10, XmNtopAttachment, XmATTACH_NONE, XmNleftAttachment, XmATTACH_FORM,
    ...
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        toggle1 = XtVaCreateManagedWidget(
            "X", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment, XmATTACH_FORM, NULL);
    }
    else
    {
        toggle1 = XtVaCreateManagedWidget("X", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
    ...
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        toggle2 = XtVaCreateManagedWidget("V", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, toggle1, NULL);
    }
    else
    {
        toggle2 = XtVaCreateManagedWidget("V", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, toggle1, XmNlabelType, XmPIXMAP, XmNlabelPixmap, pixmap, NULL);
    }

    pixmap = XmGetPixmap(XtScreen(toplevel), (char*)"toggle_grid.xpm", fg, bg);
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        toggle3 = XtVaCreateManagedWidget("G", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, toggle2, NULL);
    }
    else
    {
        toggle3 = XtVaCreateManagedWidget("G", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, toggle2, XmNlabelType, XmPIXMAP, XmNlabelPixmap, pixmap, NULL);
    }

    pixmap = XmGetPixmap(XtScreen(toplevel), (char*)"select.xpm", fg, bg);
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        toggle4 = XtVaCreateManagedWidget("Sel", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, toggle3, NULL);
    }
    else
    {
        toggle4 = XtVaCreateManagedWidget("Sel", xmToggleButtonGadgetClass, ctrls_form_widget, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, toggle3, XmNlabelType, XmPIXMAP, XmNlabelPixmap, pixmap, NULL);
    }
    pixmap = XmGetPixmap(XtScreen(toplevel), (char*)"print_plot.xpm", fg, bg);
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        print_button = XtVaCreateManagedWidget(
            "PrintPlot", xmPushButtonWidgetClass, ctrls_form_widget, XmNrightAttachment, XmATTACH_FORM, NULL);
    }
    else
    {
        print_button
            = XtVaCreateManagedWidget("PrintPlot", xmPushButtonWidgetClass, ctrls_form_widget, XmNleftAttachment,
                XmATTACH_WIDGET, XmNleftWidget, toggle4, XmNlabelType, XmPIXMAP, XmNlabelPixmap, pixmap, NULL);
    }

    XtManageChild(ctrls_form_widget);

    xy_widget = XtVaCreateManagedWidget(charbuf, xyWidgetClass, main_form_widget, XmNxAxisLabel, X_label, XmNyAxisLabel,
        Y_label, XmNshowLegend, 1, XmNdoubleBuffer, 1, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment,
        XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
        ctrls_form_widget, NULL);
    ...
    if ((font = plot->getAttribute("font")) != NULL)
    {
        ...
        Arg wargs[3];
        int nargs = 0;
        XtSetArg(wargs[nargs], XmNfontName, font);
        nargs++;
        XtSetArg(wargs[nargs], XmNfontType, XmFONT_IS_FONT);
        nargs++;
        XtSetArg(wargs[nargs], XmNloadModel, XmLOAD_IMMEDIATE);
        nargs++;
...
void PlotViewNode::finalize()
{
    ...
    if (n_curves > 1)
    {
        ...
        for (curv_ix = 0; curv_ix < n_curves; curv_ix++)
        {
            ...
            if (XParseColor(XtDisplay(toplevel), DefaultColormapOfScreen(XtScreen(toplevel)), line_color, &color_def))
            {
                ...
                if ((curves[curv_ix].linePixel == 0)
                    && XAllocColor(XtDisplay(toplevel), DefaultColormapOfScreen(XtScreen(toplevel)), &color_def))
                {
    ...
    XYSetContents(xy_widget, curves, n_curves, XY_RESCALE);

    XYGetVisibleRange(xy_widget, &cxmin, &cymin, &cxmax, &cymax);

    XYSetVisibleRange(xy_widget, dMax(cxmin, plot->getXMin()), dMax(cymin, plot->getYMin()),
        dMin(cxmax, plot->getXMax()), dMin(cymax, plot->getYMax()));
}
