...
// DESTRUCTOR
CurveViewNode::~CurveViewNode() { std::cout << "CurveViewNode destructor." << std::endl; }

// CONSTRUCTOR
CurveViewNode::CurveViewNode(Widget Toplevel, XYCurve* Fermi_curve, DPC_curve* Curve)
{
    ...
    if ((fermi_curve->points = (XYPoint*)malloc(n_allocated_points * sizeof(XYPoint))) == NULL)
    {
    ...
    while (Curve->getXY(&x, &y))
    {
        ...
        {
            n_allocated_points += NumberOfPointsPerMalloc;
            if ((fermi_curve->points = (XYPoint*)realloc(fermi_curve->points, n_allocated_points * sizeof(XYPoint)))
                == NULL)
            {
                std::cerr << "OUT_OF_MEMORY in fermi_view::render_curve." << std::endl;
            }
