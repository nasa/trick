...
TableViewNode::TableViewNode(Widget Toplevel, DPC_table* Table, int Xpos, int Ypos, int Width, int Height)
{
    ...
    // --------------------------------------
    nargs = 0;
    XtSetArg(wargs[nargs], XmNtitle, table_title);
    nargs++;
    XtSetArg(wargs[nargs], XmNx, 100);
    nargs++;
    XtSetArg(wargs[nargs], XmNy, 100);
    nargs++;
    ...
    main_form_widget = XtVaCreateWidget(charbuf, xmFormWidgetClass, dialog_shell_widget, NULL);
    ...
    save_button_widget = XtVaCreateManagedWidget(
        "Save", xmPushButtonWidgetClass, ctrls_form_widget, XmNleftAttachment, XmATTACH_FORM, NULL);
    ...
    // -------------------------------------------------------
    nargs = 0;
    XtSetArg(wargs[nargs], XmNeditable, False);
    nargs++;
    XtSetArg(wargs[nargs], XmNeditMode, XmMULTI_LINE_EDIT);
    nargs++;
    XtSetArg(wargs[nargs], XmNcursorPositionVisible, False);
    nargs++;

    text_widget = XmCreateScrolledText(main_form_widget, (char*)"scrolledtext", wargs, nargs);

    XtVaSetValues(XtParent(text_widget), XmNwidth, 500, XmNheight, 400, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
        ctrls_form_widget, XmNbottomAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNleftAttachment,
        XmATTACH_FORM, NULL);
    ...
    table_text_buf = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, (char*)"Title: %s\n", table_title);
    table_text_buf = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, (char*)"Run: %s\n", run_dir);
    ...
    for (colix = 0; colix < n_columns; colix++)
    {
        ...
        if (column_label.empty())
        {
            ...
            table_text_buf
                = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, column_heading_format[colix], charbuf);
        }
        else
        {
    ...
    for (colix = 0; colix < n_columns; colix++)
    {
        ...
        table_text_buf
            = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, column_heading_format[colix], col_units);
    }
    table_text_buf = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, (char*)"\n");
    ...
    while (table->getRow(&time, row_values))
    {
        for (colix = 0; colix < n_columns; colix++)
        {
            ...
            table_text_buf = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, column_data_format[colix],
                (row_values[colix] * var_scale_val + var_bias_val));
        }
        table_text_buf = twprint(table_text_buf, &table_buf_size, &table_insertion_pos, (char*)"\n");
    }
