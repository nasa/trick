...
static void layout_page(PageViewNode* page_view_node)
{
    ...
    for (i = 0; i < number_of_plots; i++)
    {
        ...
        if ((number_of_selected_plots == 0) || plot_view_node->selected)
        {
            ...
            XtVaSetValues(plot_view_node->main_form_widget, XmNtopAttachment, XmATTACH_POSITION, XmNtopPosition,
                y * cell_height, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, x * cell_width,
                XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, right_pos, XmNbottomAttachment,
                XmATTACH_POSITION, XmNbottomPosition, (y + 1) * cell_height, NULL);
...
PageViewNode::PageViewNode(Widget Toplevel, DPC_page* Page, int Xpos, int Ypos, int Width, int Height)
{
    ...
    // --------------------------------------
    nargs = 0;
    XtSetArg(wargs[nargs], XmNtitle, page_title);
    nargs++;
    XtSetArg(wargs[nargs], XmNmappedWhenManaged, False);
    nargs++;
    ...
    // ----------------------------------------------
    main_form_widget = XtVaCreateWidget(
        "PageForm", xmFormWidgetClass, dialog_shell_widget, XmNresizePolicy, XmRESIZE_GROW, XmNfractionBase, 10, NULL);

    ctrls_form_widget = XtVaCreateManagedWidget("CtrlsForm", xmFormWidgetClass, main_form_widget, XmNresizePolicy,
        XmRESIZE_GROW, XmNfractionBase, 10, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
        XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_NONE, NULL);
    ...
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        print_button_widget = XtVaCreateManagedWidget(
            "PrintPage", xmPushButtonWidgetClass, ctrls_form_widget, XmNleftAttachment, XmATTACH_FORM, NULL);
        XtVaSetValues(print_button_widget, XmNheight, 34, NULL);
    }
    else
    {
        print_button_widget = XtVaCreateManagedWidget("PrintPage", xmPushButtonWidgetClass, ctrls_form_widget,
            XmNleftAttachment, XmATTACH_FORM, XmNlabelType, XmPIXMAP, XmNlabelPixmap, pixmap, NULL);
    }
    ...
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        expand_plot_button_widget = XtVaCreateManagedWidget("Expand", xmPushButtonWidgetClass, ctrls_form_widget,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, print_button_widget, NULL);
        XtVaSetValues(expand_plot_button_widget, XmNheight, 34, NULL);
    }
    else
    {
        expand_plot_button_widget = XtVaCreateManagedWidget("Expand", xmPushButtonWidgetClass, ctrls_form_widget,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, print_button_widget, XmNlabelType, XmPIXMAP,
            XmNlabelPixmap, pixmap, NULL);
    }

    pdf_button_widget = XtVaCreateManagedWidget("PDF", xmPushButtonWidgetClass, ctrls_form_widget, XmNleftAttachment,
        XmATTACH_WIDGET, XmNleftWidget, expand_plot_button_widget, NULL);
    XtVaSetValues(pdf_button_widget, XmNwidth, 34, XmNheight, 34, NULL);

    pixmap = XmGetPixmap(XtScreen(toplevel), (char*)"trick_small.xpm", fg, bg);
    if (pixmap == XmUNSPECIFIED_PIXMAP)
    {
        trick_label = XtVaCreateManagedWidget(
            "Trick", xmLabelGadgetClass, ctrls_form_widget, XmNrightAttachment, XmATTACH_FORM, NULL);
    }
    else
    {
        trick_label = XtVaCreateManagedWidget("Trick", xmLabelGadgetClass, ctrls_form_widget, XmNrightAttachment,
            XmATTACH_FORM, XmNlabelType, XmPIXMAP, XmNlabelPixmap, pixmap, NULL);
    }

    XtManageChild(ctrls_form_widget);

    plots_form_widget = XtVaCreateManagedWidget("PlotsForm", xmFormWidgetClass, main_form_widget, XmNresizePolicy,
        XmRESIZE_GROW, XmNfractionBase, FRACTION_BASE, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
        ctrls_form_widget, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
        XmATTACH_FORM, NULL);
    ...
    XtVaSetValues(dialog_shell_widget, XmNx, Xpos, XmNy, Ypos, XmNwidth, Width, XmNheight, Height, NULL);

    instance_count++;
}

void PageViewNode::finalize() { layout_page(this); }
...
void PageViewNode::get_cell_geometry(int number_of_display_plots, int& n_horizontal_cells, int& n_vertical_cells)
{
    ...
    if (number_of_display_plots > (n_horizontal_cells * n_vertical_cells))
    {
        if (number_of_display_plots < 9)
        {
            ...
            while ((n_horizontal_cells * n_vertical_cells) < number_of_display_plots)
            {
