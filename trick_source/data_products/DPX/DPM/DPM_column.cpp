...
int DPM_column::Initialize(xmlNode* base_node)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                std::cerr << "WARNING: <column> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
...
// MEMBER FUNCTION
const char* DPM_column::getUnits() { return ((const char*)unitspec); }

DPM_var* DPM_column::getVar() { return (column_var); }
