...
int DPM_outputs::Initialize(xmlNode* base_node)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                std::cerr << "WARNING: <outputs> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
    ...
    if (n_measurements < 1)
    {
        std::cerr << "ERROR: <outputs> specification requires at least one valid <measurement> specification."
                  << std::endl;
        return -1;
    }
    return 0;
...
// MEMBER FUNCTION
int DPM_outputs::NumberOfOutputs() { return ((int)measurement_list.size()); }
