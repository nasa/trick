...
int DPM_run::Initialize(xmlNode* base_node)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                std::cerr << "WARNING: <run> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
...
// DESTRUCTOR
DPM_run::~DPM_run() { free(dir); }

const char* DPM_run::getDir() { return ((const char*)dir); }

const char* DPM_run::getTimeName() { return time_name; }
