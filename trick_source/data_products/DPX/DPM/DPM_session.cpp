...
int DPM_session::Initialize(xmlNode* SessionNode)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                std::cerr << "WARNING: <session> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
...
DPM_session::Device DPM_session::getDevice() { return sessionDevice; }

char* DPM_session::getDeviceFileName() { return deviceFileName; }
