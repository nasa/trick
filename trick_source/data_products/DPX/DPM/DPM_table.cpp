    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            else if (strcmp((const char*)current_node->name, "column") == 0)
            {
                ...
                {
                    column = NULL;
                    std::cerr << "ERROR: <table> will be incomplete because one of it\'s <column>s is invalid."
                              << std::endl;
                }
                if (column != NULL)
                {
...
// MEMBER FUNCTION
const char* DPM_table::getTitle() { return (title); }

// MEMBER FUNCTION
DPM_time_constraints* DPM_table::getTimeConstraints() { return (time_constraints); }

// MEMBER FUNCTION
int DPM_table::NumberOfColumns() { return ((int)column_list.size()); }
