...
int DPM_page::Initialize(xmlNode* base_node)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                std::cerr << "WARNING: <page> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
...
// MEMBER FUNCTION
const char* DPM_page::getTitle() { return (title); }

// MEMBER FUNCTION
DPM_time_constraints* DPM_page::getTimeConstraints() { return (time_constraints); }

// MEMBER FUNCTION
int DPM_page::NumberOfRelations() { return ((int)relation_list.size()); }
