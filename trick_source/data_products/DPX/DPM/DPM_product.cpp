...
int DPM_product::Initialize(xmlNode* base_node)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            if (strcmp((char*)current_node->name, "page") == 0)
            {
                ...
                {
                    page = NULL;
                    std::cerr << "ERROR: <product> specification contains an invalid <page> specification."
                              << std::endl;
                }
                if (page != NULL)
                {
            ...
            else if (strcmp((char*)current_node->name, "table") == 0)
            {
                ...
                {
                    table = NULL;
                    std::cerr << "ERROR: <product> specification contains an invalid <table> specification."
                              << std::endl;
                }
                if (table != NULL)
                {
            ...
            else if (strcmp((char*)current_node->name, "extfn") == 0)
            {
                ...
                {
                    ext_prog = NULL;
                    std::cerr << "ERROR: <product> specification contains an invalid <extfn> specification."
                              << std::endl;
                }
                if (ext_prog != NULL)
                {
            ...
            {
                std::cerr << "WARNING: <product> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
    ...
    {
        std::cerr << "ERROR: <product> specification requires at least one <page> or <table> specification."
                  << std::endl;
        return -1;
    }
...
// MEMBER FUNCTION
const char* DPM_product::getTitle() { return (title); }

// MEMBER FUNCTION
DPM_time_constraints* DPM_product::getTimeConstraints() { return (time_constraints); }

// MEMBER FUNCTION
int DPM_product::NumberOfTables() { return ((int)table_list.size()); }
...
// MEMBER FUNCTION
int DPM_product::NumberOfPages() { return ((int)page_list.size()); }
...
// MEMBER FUNCTION
int DPM_product::NumberOfExtFns() { return ((int)extfn_list.size()); }
