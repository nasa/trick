...
int DPM_curve::Initialize(xmlNode* base_node)
{
    ...
    while ((current_node != NULL) && (current_node->type != XML_ELEMENT_NODE))
    {
    ...
    if (strcmp((const char*)current_node->name, "var") == 0)
    {
        while (current_node != NULL)
        {
            if (current_node->type == XML_ELEMENT_NODE)
            {
                ...
                {
                    std::cerr << "WARNING: <curve> specification contains an unknown element \"<" << current_node->name
                              << ">\". Skipping." << std::endl;
                }
            }
            current_node = current_node->next;
...
DPM_curve::~DPM_curve()
{
    ...
    if (x_varcase_list.size())
    {
        x_varcase_list.clear();
    }
    if (y_varcase_list.size())
    {
        y_varcase_list.clear();
    }
    if (z_varcase_list.size())
    {
        z_varcase_list.clear();
    }
}
...
// MEMBER FUNCTION
int DPM_curve::NumberOfVarCases() { return ((int)x_varcase_list.size()); }
