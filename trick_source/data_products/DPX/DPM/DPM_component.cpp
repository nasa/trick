...
// DEFAULT CONSTRUCTOR
DPM_component::DPM_component() { parent = NULL; }
    ...
    if (Base_node != NULL)
    {
        ...
        else
        {
            ...
            for (current_attr = Base_node->properties; current_attr; current_attr = current_attr->next)
            {
                ...
                {
                    add_attribute((const char*)current_attr->name, (const char*)current_attr->children->content);
                }
            }
        }
    }
...
// MEMBER FUNCTION
int DPM_component::NumberOfAttributes() { return (int)attribute_list.size(); }
