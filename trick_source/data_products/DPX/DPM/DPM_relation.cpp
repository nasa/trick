...
int DPM_relation::Initialize(xmlNode* base_node)
{
    ...
    while (current_node != NULL)
    {
        if (current_node->type == XML_ELEMENT_NODE)
        {
            ...
            {
                std::cerr << "WARNING: <plot> specification contains an unknown element \"<" << current_node->name
                          << ">\". Skipping." << std::endl;
            }
        }
...
// MEMBER FUNCTION
const char* DPM_relation::getTitle() { return ((const char*)title); }

// MEMBER FUNCTION
DPM_time_constraints* DPM_relation::getTimeConstraints() { return (time_constraints); }
...
// MEMBER FUNCTION
int DPM_relation::NumberOfCurves() { return ((int)curve_list.size()); }
