...
DPC_product::DPC_product(DPM_session* Session, const char* ProductFileName)
{
    ...
    if ((session_mode == NULL) || (strcasecmp(session_mode, "plot") == 0))
    {
        ...
        for (pagix = 0; pagix < n_pages; pagix++)
        {
            ...
            try
            {
                page = new DPC_page(datastream_supplier, &(Session->run_list), page_spec, parentTimeConstraints);
    ...
    if ((session_mode == NULL) || (strcasecmp(session_mode, "table") == 0))
    {
        ...
        for (tabix = 0; tabix < n_tables; tabix++)
        {
            ...
            for (runix = 0; runix < n_runs; runix++)
            {
                ...
                // Make a plot.
                table
                    = new DPC_table(datastream_supplier, Session->run_list[runix], table_spec, &total_time_constraints);
...
// MEMBER FUNCTION
const char* DPC_product::getTitle() { return (product_spec->getTitle()); }

// MEMBER FUNCTION
int DPC_product::NumberOfTables() { return ((int)table_list.size()); }
...
// MEMBER FUNCTION
int DPC_product::NumberOfPages() { return ((int)page_list.size()); }
...
// MEMBER FUNCTION
const char* DPC_product::getAttribute(const char* key) { return (product_spec->AttributeValue(key)); }
