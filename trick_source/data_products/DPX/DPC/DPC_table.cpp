...
// CONSTRUCTOR
DPC_table::DPC_table(DPC_datastream_supplier* DS_Supplier, DPM_run* Run, DPM_table* Table_spec,
    DPM_time_constraints* ParentTimeConstraints)
{
    ...
    if (!DS_Supplier)
    {
        throw std::invalid_argument("DS_Supplier is NULL");
    }
    if (!Run)
    {
        throw std::invalid_argument("DPM_run is NULL");
    }
    if (!Table_spec)
    {
        throw std::invalid_argument("DPM_table is NULL");
    }
    ...
    for (colix = 0; colix < n_columns; colix++)
    {
        ...
        if (strcmp(column_var_name, run->getTimeName()) == 0)
        {
            if ((column_var_units != NULL) && (strcmp(column_var_units, default_time_units) != 0))
            {
                ...
                if (!to)
                {
                    std::cerr << "ERROR: Unable to convert values to invalid units: \"" << column_var_units << "\"."
                              << std::endl;
                }
        ...
        {
            ds = DS_Supplier->getDataStream(
                column_var_name, column_var_units, column_var_from_units, Run, &total_time_constraints);
            if (ds == NULL)
            {
...
// MEMBER FUNCTION
DPV_pointer DPC_table::render(DPV_view* view, DPV_pointer parent_view_data)
{
    if (view != NULL)
    {
...
// MEMBER FUNCTION
const char* DPC_table::getRunDir() { return (run->getDir()); }

// MEMBER FUNCTION
const char* DPC_table::getTitle() { return (table_spec->getTitle()); }

// MEMBER FUNCTION
int DPC_table::getNumColumns() { return (table_spec->NumberOfColumns()); }
...
// MEMBER FUNCTION
const char* DPC_table::getColumnAttribute(unsigned int index, const char* key)
{
...
// MEMBER FUNCTION
const char* DPC_table::getAttribute(const char* key) { return (table_spec->AttributeValue(key)); }
