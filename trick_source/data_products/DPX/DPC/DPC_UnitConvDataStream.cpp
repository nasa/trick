...
DPC_UnitConvDataStream::DPC_UnitConvDataStream(DataStream* ds, const char* ToUnits, const char* FromUnitsHint)
{
    ...
    if (to != NULL)
    {
        ...
        if (recorded_units.empty())
        {
            ...
            if ((FromUnitsHint == NULL) || (strcmp(FromUnitsHint, "") == 0))
            {
                ...
                std::cerr << "ERROR: Unable to to perform units conversion"
                          ...
                          << " units and no @from_units hint is provided." << std::endl;
                std::cerr.flush();
            }
            else
            { // the user did give us a hint.
                ...
                if (!from)
                {
                    std::cerr << "ERROR: Unable to to perform units conversion"
                              ...
                              << "(\"" << FromUnitsHint << "\"), they are invalid." << std::endl;
                    std::cerr.flush();
                }
            }
        }
        else
        { // the recorded data file does "know" the units in which the data was recorded,
            ...
            if (!from)
            {
                std::cerr << "ERROR: Unable to to perform units conversion because the"
                          << " units in the data recording file appear to be corrupt." << std::endl;
                std::cerr.flush();
            }
        }
        ...
        if (from != NULL)
        {
            ...
            if (cf == NULL)
            {
                std::cerr << "ERROR: Unable to convert from \"" << FromUnitsHint << "\" to \"" << to_units
    ...
    { // The user has not specified a units conversion or the units were not valid.
        // If the recorded data file doesn't contain the units in which the data is recorded ...
        if (recorded_units.empty())
        {
            // If the user didn't give us a hint as to what the units are (using var@from_units) ...
...
// MEMBER FUNCTION
std::string DPC_UnitConvDataStream::getFileName() { return (source_ds->getFileName()); }

// MEMBER FUNCTION
std::string DPC_UnitConvDataStream::getUnit() { return to_units; }

// MEMBER FUNCTION
std::string DPC_UnitConvDataStream::getTimeUnit() { return (source_ds->getTimeUnit()); }

// MEMBER FUNCTION
void DPC_UnitConvDataStream::begin() { source_ds->begin(); }

// MEMBER FUNCTION
int DPC_UnitConvDataStream::end() { return (source_ds->end()); }

// MEMBER FUNCTION
int DPC_UnitConvDataStream::step() { return (source_ds->step()); }
