...
DPC_delta_plot::DPC_delta_plot(DPC_datastream_supplier* DS_Supplier, DPM_run* Run1, DPM_run* Run2,
    DPM_relation* Relation, DPM_time_constraints* ParentTimeConstraints)
    : DPC_plot(Relation)
{
    ...
    if (!Run1)
    {
        throw std::invalid_argument("Run1 is NULL.");
    }
    if (!Run2)
    {
        throw std::invalid_argument("Run2 is NULL.");
    }
    if (!Relation)
    {
        throw std::invalid_argument("Relation is NULL.");
    }
    if (!ParentTimeConstraints)
    {
        throw std::invalid_argument("ParentTimeConstraints is NULL.");
    }
    ...
    for (cix = 0; cix < n_curves; cix++)
    {
        ...
        {
            // Make a curve.
            curve = new DPC_delta_curve(curve_spec, Run1, Run2, DS_Supplier, &total_time_constraints);
        }
    ...
    if (getNumCurves() == 0)
    {
        throw std::invalid_argument(
            "DPC_delta_plot: Ack!! I dont have any curves to plot! I have no reason to exist! *POOF!*");
    }
}

// CONSTRUCTOR
DPC_delta_plot::DPC_delta_plot(DPC_datastream_supplier* DS_Supplier, std::vector<DPM_run*>* RunListp,
    DPM_relation* Relation, DPM_time_constraints* ParentTimeConstraints)
    : DPC_plot(Relation)
{
    DPM_time_constraints* my_time_constraints;
    ...
    if (n_runs == 0)
    {
        throw std::invalid_argument("Run List is NULL.");
    }
    if (!Relation)
    {
        throw std::invalid_argument("Relation is NULL.");
