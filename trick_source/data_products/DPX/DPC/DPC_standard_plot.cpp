...
// CONSTRUCTOR
DPC_standard_plot::DPC_standard_plot(DPC_datastream_supplier* DS_Supplier, std::vector<DPM_run*>* RunListp,
    ...
    : DPC_plot(Relation)
{
    ...
    if (!RunListp)
    {
        throw std::invalid_argument("RunListp is NULL.");
    }
    if (!Relation)
    {
    ...
    for (runix = 0; runix < n_runs; runix++)
    {
        ...
        for (cix = 0; cix < n_curves; cix++)
        {
            ...
            {
                // Make a curve.
                curve = new DPC_std_curve(curve_spec, (*RunListp)[runix], DS_Supplier, &total_time_constraints);
            }
