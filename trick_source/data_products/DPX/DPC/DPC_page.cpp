...
DPC_page::DPC_page(DPC_datastream_supplier* DS_Supplier, std::vector<DPM_run*>* RunListp, DPM_page* PageSpec,
    DPM_time_constraints* ParentTimeConstraints)
{
    ...
    if (((presentation != NULL) && (strcasecmp(presentation, "COMPARISON") == 0)) || (contrast_session))
    {
        ...
        for (relix = 0; relix < n_relations; relix++)
        {
            ...
            {
                // Make a plot.
                plot = new DPC_standard_plot(DS_Supplier, RunListp, relation, &total_time_constraints);
                if (contrast_session)
                {
    ...
    if (((presentation != NULL) && (strcasecmp(presentation, "DELTA") == 0)) || (contrast_session))
    {
        // Foreach of the Variable Relations.
        for (relix = 0; relix < n_relations; relix++)
        {
            ...
            if (contrast_session)
            {
                ...
                {
                    // Make a plot.
                    plot = new DPC_delta_plot(DS_Supplier, RunListp, relation, &total_time_constraints);
                    // Put the new plot in the plot list.
                    plot->setTitle("Contrast");
                    plot_list.push_back(plot);
                }
            ...
            else
            {
                ...
                for (i = 0; i < n_runs - 1; i++)
                {
                    for (j = i + 1; j < n_runs; j++)
                    {
                        ...
                        {
                            // Make a plot.
                            plot = new DPC_delta_plot(DS_Supplier, Run1, Run2, relation, &total_time_constraints);
                            // Put the new plot in the plot list.
                            plot_list.push_back(plot);
                        }
                        catch (const std::invalid_argument& error)
    ...
    if (plot_list.size() == 0)
    {
        ...
        for (relix = 0; relix < n_relations; relix++)
        {
            ...
            for (runix = 0; runix < n_runs; runix++)
            {
                ...
                {
                    // Make a plot.
                    plot = new DPC_standard_plot(DS_Supplier, run_list_of_one, relation, &total_time_constraints);
                    // Put the new plot in the plot list.
                    plot_list.push_back(plot);
                }
                catch (const std::invalid_argument& error)
...
// MEMBER FUNCTION
DPV_pointer DPC_page::render(DPV_view* view, DPV_pointer parent_view_data)
{
    if (view != NULL)
    {
        DPC_plot* plot;
...
// MEMBER FUNCTION
const char* DPC_page::getAttribute(const char* key) { return (page_spec->AttributeValue(key)); }
...
// MEMBER FUNCTION
const char* DPC_page::getTitle() { return (page_spec->getTitle()); }
