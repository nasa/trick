...
// CONSTRUCTOR
DPC_delta_curve::DPC_delta_curve(DPM_curve* Curve_spec, DPM_run* RUN1, DPM_run* RUN2,
    DPC_datastream_supplier* ds_supplier, DPM_time_constraints* Time_constraints)
    : DPC_curve()
{
    char work[255];
    char* temp_s;
    ...
    while ((RUN1_case_index < 0) && (case_index < number_of_cases))
    {
        ...
        if (strcmp(run1_x_var->getName(), RUN1->getTimeName()) == 0)
        {
            ...
            ds[0] = ds_supplier->getDataStream(run1_y_var->getName(), run1_y_var->AttributeValue("units"),
                run1_y_var->AttributeValue("from_units"), RUN1, Time_constraints);
            if (ds[0] != NULL)
            {
    ...
    if (RUN1_case_index < 0)
    {
        std::cerr << "ERROR: For RUN1, the X variable either does not represent time, a requirement for delta plots\n"
                  << "       or the Y variable could not be found in RUN1." << std::endl;
        std::cerr.flush();
    ...
    while ((RUN2_case_index < 0) && (case_index < number_of_cases))
    {
        ...
        if (strcmp(run2_x_var->getName(), RUN2->getTimeName()) == 0)
        {
            ...
            // Create DataStream #1 if Y-variable is also found
            ds[1] = ds_supplier->getDataStream(run2_y_var->getName(), run2_y_var->AttributeValue("units"),
                run2_y_var->AttributeValue("from_units"), RUN2, Time_constraints);
            if (ds[1] != NULL)
            {
    ...
    if (RUN2_case_index < 0)
    {
        std::cerr << "ERROR: For RUN2, the X variable does not represent time, a requirement for delta plots\n"
                  << "       or the Y variable could not be found in RUN2." << std::endl;
        std::cerr.flush();
        throw(std::logic_error("Unable to create DPC_delta_curve."));
    }
...
// MEMBER FUNCTION
const char* DPC_delta_curve::getXUnits() { return (x_actual_units); }

// MEMBER FUNCTION
const char* DPC_delta_curve::getYUnits() { return (y_actual_units); }

// MEMBER FUNCTION
const char* DPC_delta_curve::getDataSrcLbl() { return (data_src_label); }
