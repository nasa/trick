...
// DESTRUCTOR
DPC_datastream_supplier::~DPC_datastream_supplier() { delete data_stream_factory; }

// MEMBER FUNCTION
DataStream* DPC_datastream_supplier::getDataStream(const char* VarName, const char* ToUnits, const char* FromUnitsHint,
    DPM_run* Run, DPM_time_constraints* in_time_constraints)
{
    ...
    for (extfn_index = 0; extfn_index < n_extfns; extfn_index++)
    {
        ...
        if (output_index >= 0)
        {
            ...
            {
                instreams[input_ix] = data_stream_factory->create(run_dir, extfn->getInputVar(input_ix), timename);
            }
...
DataStream* DPC_datastream_supplier::getDataStream(const char* VarName, const char* ToUnits, const char* FromUnitsHint,
    const char* Machine, const unsigned short Port, DPM_time_constraints* in_time_constraints)
{
    DataStream* ds   = NULL;
    DataStream* tds  = NULL;
    DataStream* utds = NULL;
