...
DPC_std_curve::DPC_std_curve(
    DPM_curve* Curve_spec, DPM_run* Run, DPC_datastream_supplier* ds_supplier, DPM_time_constraints* Time_constraints)
    : DPC_curve()
{
    char work[255];
    char* temp_s;
    ...
    while (!success && (case_index < number_of_cases))
    {
        ...
        if (strcmp((const char*)x_var_name, Run->getTimeName()) == 0)
        {
            ...
            if ((x_var_units != NULL) && (strcmp(x_var_units, "s") != 0))
            {
                ...
                if (!to)
                {
                    std::cerr << "ERROR: For the curve <X,Y> = <" << x_var_name << ", " << y_var_name << ">, "
                              << "the units (\"" << x_var_units << "\") specified for " << x_var_name << " are invalid."
                              << std::endl;
                }
                ...
                if (to && from)
                {
                    ...
                    {
                        std::cerr << "ERROR: For the curve <X,Y> = <" << x_var_name << ", " << y_var_name << ">, "
                                  << "seconds (\"s\") cannot be converted to (\"" << x_var_units
                                  << "\"), which have been specified for " << x_var_name << "." << std::endl;
            ...
            ds[0] = ds_supplier->getDataStream(y_var_name, y_var_units, y_var_from_units, Run, Time_constraints);
            if (ds[0] != NULL)
            {
...
// MEMBER FUNCTION
const char* DPC_std_curve::getXUnits() { return (x_actual_units); }

// MEMBER FUNCTION
const char* DPC_std_curve::getYUnits() { return (y_actual_units); }

// MEMBER FUNCTION
const char* DPC_std_curve::getDataSrcLbl() { return (data_src_label); }
