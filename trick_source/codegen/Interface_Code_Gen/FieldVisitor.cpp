...
FieldVisitor::FieldVisitor(clang::CompilerInstance& in_ci, HeaderSearchDirs& in_hsd, CommentSaver& in_cs,
    PrintAttributes& in_pa, std::string container_class)
    : ci(in_ci)
    , hsd(in_hsd)
    , cs(in_cs)
    , pa(in_pa)
...
bool FieldVisitor::VisitDeclaratorDecl(clang::DeclaratorDecl* dd)
{
    ...
    if (!file_name.empty())
    {
        if (isInUserOrTrickCode(ci, dd_range.getEnd(), hsd))
        {
            ...
            if (cs.hasTrickHeader(file_name) and !cs.hasICGNoComment(file_name)
                and !hsd.isPathInICGNoComment(file_name))
            {
...
bool FieldVisitor::ProcessTemplate(std::string in_name, clang::CXXRecordDecl* crd)
{
    ...
    if (processed_templates.find(in_name) == processed_templates.end())
    {
        ...
        processed_templates[in_name] = fdes->getContainerClass() + "_" + fdes->getName() + "_" + mangled_name;
            ...
            else
            {
                ...
                if (t != NULL)
                {
                    if (t->getTypeClass() == clang::Type::Record)
                    {
                        ...
                        {
                            return checkForPrivateTemplateArgs(
                                clang::cast<clang::ClassTemplateSpecializationDecl>(crd));
                        }
                    }
                }
            }
...
// Check if any template argument of an STL container is a record type whose copy assignment
// operator is not publicly accessible. If so, checkpoint_stl and restore_stl functions
// generated by ICG will fail to compile because they use operator=.
static bool checkForNonCopyAssignableTemplateArgs(clang::ClassTemplateSpecializationDecl* ctsd)
{
    for (const clang::TemplateArgument& ta : ctsd->getTemplateArgs().asArray())
    {
        if (ta.getKind() == clang::TemplateArgument::Type)
        {
            ...
            if (t != NULL && t->getTypeClass() == clang::Type::Record)
            {
                ...
                if (crd)
                {
                    ...
                    if (crd)
                    {
                        for (clang::CXXMethodDecl* method : crd->methods())
                        {
        ...
        for (std::map<std::string, bool>::iterator it = stl_classes.begin(); it != stl_classes.end(); ++it)
        {
            ...
            if (!tst_string.compare(0, (*it).first.size(), (*it).first))
            {
                ...
                if (rd != NULL and clang::ClassTemplateSpecializationDecl::classof(rd))
                {
                    ...
                    // If any template argument has a non-public (private/protected) or deleted copy
                    // assignment operator, checkpoint_stl and restore_stl cannot compile because they
                    // use operator= to copy elements. Clear only the checkpoint (TRICK_CHKPNT_OUTPUT bit 2)
                    // and restore (TRICK_CHKPNT_INPUT bit 3) bits so the field remains accessible.
                    if (checkForNonCopyAssignableTemplateArgs(ctsd))
                    {
                        fdes->setIO(fdes->getIO() & ~0xC);
                    }
...
bool FieldVisitor::VisitVarDecl(clang::VarDecl* v)
{
    ...
    if (v->isStaticDataMember() and v->getType().isConstQualified() and v->hasInit())
    {
        fdes->setIO(0);
        return false;
    }
    else if (v->isStaticDataMember() and v->getType().isConstQualified())
    {
    ...
    if (!qt.isCanonical())
    {
        ...
        if (debug_level >= 3)
        {
            std::cout << "\033[33mFieldVisitor VisitVarDecl: Processing canonical type " << tst_string << "\033[00m"
                      << std::endl;
            ct.dump();
        }
        TraverseType(ct);
...
FieldDescription* FieldVisitor::get_field_data() { return fdes; }
