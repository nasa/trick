...
bool EnumVisitor::VisitEnumDecl(clang::EnumDecl* ed)
{
    ...
    if (debug_level >= 3)
    {
        ...
        std::cout << "EnumDecl debug: name=\"" << ed->getName().str() << "\" qname=\"" << ed->getQualifiedNameAsString()
                  << "\""
                  << " scoped=" << (ed->isScoped() ? "true" : "false")
                  << " fixed=" << (ed->isFixed() ? "true" : "false") << " " << ex_im << "_underlying=\""
                  << underlying_type.getAsString() << "\""
                  << " unsigned=" << (is_unsigned ? "true" : "false") << std::endl;
    }
...
bool EnumVisitor::VisitEnumType(clang::EnumType* et)
{
    ...
    if (eval.getName().empty())
    {
        ...
        if ((pos = enum_type_name.find("<anonymous")) != std::string::npos or
            ...
            (pos = enum_type_name.find("(anonymous")) != std::string::npos
            or (pos = enum_type_name.find("(unnamed")) != std::string::npos)
        {
            eval.setName("");
            return false;
        }
...
EnumValues* EnumVisitor::get_enum_data() { return &eval; }
