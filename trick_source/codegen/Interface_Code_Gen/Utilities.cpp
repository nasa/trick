...
#else
            const clang::CustomizableOptional<clang::FileEntryRef> cfer
                = ci.getSourceManager().getFileEntryRefForID(fid);
            if (cfer.has_value())
            {
...
char* almostRealPath(const char* in_path)
{
    ...
    if (!final_path)
    {
        std::cout << bold(color(WARNING, "Warning")) << "    ICG could not resolve realpath of " << quote(bold(in_path))
                  << std::endl;
    }
...
std::string bold(const std::string& text) { return boldSequence + text + noBoldSequence; }

std::string underline(const std::string& text) { return underlineSequence + text + noUnderlineSequence; }
...
int gccVersionToIntOrDefault(const char* verno, int def)
{
    ...
    if (result > 999999)
    {
        std::cerr << "Warning: GCC_VERSION is invalid version number: " << result
                  << ". Setting ICG to use default GCC version: " << def << std::endl;
        return def;
    }
    return result;
}
