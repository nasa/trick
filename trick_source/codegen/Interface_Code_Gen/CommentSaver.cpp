...
CommentSaver::CommentSaver(clang::CompilerInstance& in_ci, HeaderSearchDirs& in_hsd)
    : ci(in_ci)
    , hsd(in_hsd)
{
}
...
std::string CommentSaver::getComment(clang::SourceRange sr)
{
    if (sr.isValid())
    {
        ...
        std::string comment_text(
            fsl_begin.getCharacterData(), (size_t)(fsl_end.getCharacterData() - fsl_begin.getCharacterData()));
        return comment_text;
    }
    ...
    if (resolved_path != NULL)
    {
        if (trick_header_comments.find(resolved_path) == trick_header_comments.end())
        {
            ...
            for (cit = comment_map[resolved_path].begin(); cit != comment_map[resolved_path].end(); ++cit)
            {
                ...
                if (comment_str.find("@trick_parse") != std::string::npos
                    or comment_str.find("\\trick_parse") != std::string::npos)
                {
...
void CommentSaver::getICGField(std::string file_name)
{
    ...
    if (!th_str.empty())
    {
        ...
        if ((trick_parse_keyword = th_str.find("trick_parse")) != std::string::npos)
        {
            ...
            {
                if (trick_parse_everything != std::string::npos and trick_parse_everything < closing_brace)
                {
                    icg_no_comment_found[file_name] = false;
                    icg_no_found[file_name]         = false;
                }
                else if (trick_parse_attributes != std::string::npos and trick_parse_attributes < closing_brace)
                {
                    icg_no_comment_found[file_name] = true;
                    icg_no_found[file_name]         = false;
                }
                else if (trick_parse_dep_only != std::string::npos and trick_parse_dep_only < closing_brace)
                {
                    icg_no_comment_found[file_name] = false;
