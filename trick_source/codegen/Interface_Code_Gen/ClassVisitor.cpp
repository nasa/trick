...
CXXRecordVisitor::CXXRecordVisitor(clang::CompilerInstance & in_ci,
                                   CommentSaver & in_cs,
...
bool CXXRecordVisitor::TraverseDecl(clang::Decl * d)
{
    ...
    switch(d->getKind())
    {
        ...
        case clang::Decl::Friend:
        {
            ...
            // Only use namespaces for identifying class name as the class name can't be the same within the same
            // namespace.
        ...
        if(!bcii->isVirtual())
        {
            ...
            if(rt != NULL)
            {
                ...
                if(isInUserOrTrickCode(ci, rd->RBRACELOC(), hsd))
                {
                    ...
                    inherit_class_offset = record_layout.getBaseClassOffset(llvm::cast<clang::CXXRecordDecl>(rd))
                                               .getQuantity();
        ...
        for(bcii = rec->vbases_begin(); bcii != rec->vbases_end(); ++bcii)
        {
            ...
            if(rt != NULL)
            {
                ...
                if(isInUserOrTrickCode(ci, rd->RBRACELOC(), hsd))
                {
                    ...
                    inherit_class_offset = record_layout.getVBaseClassOffset(llvm::cast<clang::CXXRecordDecl>(rd))
                                               .getQuantity();
...
bool CXXRecordVisitor::isPrivateEmbeddedClass(std::string in_name)
{
    ...
    in_name.erase(std::find_if(in_name.rbegin(),
                               in_name.rend(),
                               [](int c)
                               {
