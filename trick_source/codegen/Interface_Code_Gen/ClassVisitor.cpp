...
CXXRecordVisitor::CXXRecordVisitor(clang::CompilerInstance& in_ci, CommentSaver& in_cs, HeaderSearchDirs& in_hsd,
...
// Elaborated is gone in clang 22, so need to check template specialization in a different way.
        ...
        if (type_ptr->getTypeClass() == clang::Type::Elaborated)
        {
            ...
            // std::cout << "\n[32minherited Type = " << et->getNamedType().getTypePtr()->getTypeClassName() << "[00m"
            // << std::endl ;
...
bool CXXRecordVisitor::VisitCXXRecordDecl(clang::CXXRecordDecl* rec)
{
    ...
    // std::cout << "    [34mprocessing inheritance " <<  rec->getNumBases() << " " << rec->getNumVBases() << "[00m"
    // << std::endl ;
    ...
    for (bcii = rec->bases_begin(); bcii != rec->bases_end(); ++bcii)
    {
        if (!bcii->isVirtual())
        {
            ...
            if (rd != NULL)
            {
                ...
                // std::cout << "    [34m" << cval.getName() << " inherits from " << rd->getNameAsString() << "[00m"
                // << std::endl ; rd->dump() ; std::cout << std::endl ;
                if (isInUserOrTrickCode(ci, rd->RBRACELOC(), hsd))
                {
                    ...
                    inherit_class_offset
                        = record_layout.getBaseClassOffset(llvm::cast<clang::CXXRecordDecl>(rd)).getQuantity();
                    ...
                    {
                        cval.addInheritedFieldDescriptions(
                            inherit_cvis.get_class_data()->getFieldDescriptions(), inherit_class_offset, false);
                        ...
                        // If we are inheriting from a template specialization, don't save the inherited class.  This
                        // list is maintained to call init_attr of the inherited classes.  A template specialization
                        // does not havethese attributes.
        ...
        for (bcii = rec->vbases_begin(); bcii != rec->vbases_end(); ++bcii)
        {
            ...
            if (rd != NULL)
            {
                ...
                if (isInUserOrTrickCode(ci, rd->RBRACELOC(), hsd))
                {
                    ...
                    // Virtual base classes have a fixed offset that cannot be accessed at runtime.  We have clang's
                    // calculation to where the base class will be.
                    inherit_class_offset
                        = record_layout.getVBaseClassOffset(llvm::cast<clang::CXXRecordDecl>(rd)).getQuantity();
                    ...
                    cval.addInheritedFieldDescriptions(
                        inherit_cvis.get_class_data()->getFieldDescriptions(), inherit_class_offset, true);
                    // clear the field list in the inherited class so they are not freed when inherit_cvis goes out of
                    // scope.
                    inherit_cvis.get_class_data()->clearFieldDescription();
                    ...
                    {
                        ClassValues* icv = new ClassValues(*(inherit_cvis.get_class_data()));
                        // The inherited classes of this inherted class are not required in the copy, and they are going
                        // out of scope
                        icv->clearInheritedClass();
        ...
        if (nd != NULL and nd->getDeclName().isIdentifier())
        {
            ...
            else if (!friend_str.find("init_attr"))
            {
                std::cerr << bold(color(WARNING, "Warning")) << "    " << cval.getFileName() << ":"
                          << ci.getSourceManager().getSpellingLineNumber(fd->getSourceRange().getBegin()) << ":"
                          << std::endl;
                std::cerr << "           friend " << friend_str << " does not match expected name " << init_func
                          << std::endl;
            }
        }
    }
...
ClassValues* CXXRecordVisitor::get_class_data() { return &cval; }

std::set<std::string> CXXRecordVisitor::private_embedded_classes;

std::set<std::string> CXXRecordVisitor::friend_classes;

void CXXRecordVisitor::addPrivateEmbeddedClass(std::string in_name) { private_embedded_classes.insert(in_name); }
...
bool CXXRecordVisitor::isPrivateEmbeddedClass(std::string in_name)
{
    ...
    in_name.erase(
        std::find_if(in_name.rbegin(), in_name.rend(), [](int c) { return !std::isspace(c); }).base(), in_name.end());
...
{
    if (cval.getFileName() == "S_source.hh" || !clang::ClassTemplateSpecializationDecl::classof(rec))
    {
        return;
    }
    ...
    {
        if (arg.getKind() != clang::TemplateArgument::ArgKind::Type || !arg.getAsType().getTypePtrOrNull())
        {
            continue;
        }
        ...
        if (debug_level >= 2)
        {
            std::cout << "\n\033[34mCXXRecordVisitor addTemplateArgumentDependencies " << arg_header_file << "\033[0m"
                      << std::endl;
        }
    }
}
