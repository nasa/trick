...
ClassTemplateVisitor::ClassTemplateVisitor(
    clang::CompilerInstance& in_ci, CommentSaver& in_cs, HeaderSearchDirs& in_hsd, PrintAttributes& in_pa)
    : ci(in_ci)
    , cs(in_cs)
    , hsd(in_hsd)
    , pa(in_pa)
    ...
    case clang::Decl::CXXRecord:
    {
        ...
        if (rd != NULL)
        {
            ...
            if (rd->getAccess() == clang::AS_protected || rd->getAccess() == clang::AS_private)
            {
                ...
                // std::cout << "marking private " << temp_cv.getFullyQualifiedName() + crd->getNameAsString() <<
                // std::endl ;
