...
void PrintFileContents10::print_enum_attr(std::ostream& ostream, EnumValues* e)
{
    ...
    {
        ostream << "{\"" << name << pair.first << "\", " << pair.second << ", 0x" << std::hex << mods_value << std::dec
                << "},\n";
    }
    ostream << "{\"\", 0, 0x0}\n};\n";
...
void PrintFileContents10::print_field_attr(std::ostream& ostream, FieldDescription& fdes, ClassValues* cv)
{
    ...
    ostream << ", "
            << fdes.isReference() + (fdes.isStatic() << 1) + (fdes.isDashDashUnits() << 2)
            + (fdes.isReference() ? (((fdes.getFieldWidth() / 8) & 0x3F) << 3) : 0)
            << "," << std::endl; // mods
    if (fdes.isBitField())
    {
...
void PrintFileContents10::print_stl_declarations(std::ostream& ostream, ClassValues* c)
{
    ...
    for (FieldDescription* field : fieldDescriptions)
    {
                ...
                || (field->getSTLTypeEnumString() == "TRICK_STL_ARRAY")))
        {
            ...
            ostream << "size_t get_stl_size_stl_" << className << "_" << fieldName << "(void* start_address);"
                    << std::endl;
            ostream << "void* get_stl_element_stl_" << className << "_" << fieldName
                    << "(void* start_address, size_t index);" << std::endl;
        }
    }
}
...
void PrintFileContents10::print_field_init_attr_stmts(
    std::ostream& ostream, FieldDescription* fdes, ClassValues* cv, unsigned int index)
{
    ...
    else if (global_compat15 or cv->isCompat15())
    {
        ...
        else if (fdes->isVirtualInherited())
        {
            ...
            {
                ostream << prefix << "offset = " << fdes->getBaseClassOffset() << " + offsetof("
                        << fdes->getContainerClass() << ", " << fieldName << ") ;\n";
            }
        }
    ...
    if (fdes->isSTL())
    {
        ...
        {
            ostream << prefix << field << " = " << field << "_"
                    << fullyQualifiedMangledClassNameUnderscores + "_" + sanitize(fieldName) + " ;\n";
        };
        ...
        if (fdes->getSTLTypeEnumString() == "TRICK_STL_VECTOR" || fdes->getSTLTypeEnumString() == "TRICK_STL_DEQUE"
            || fdes->getSTLTypeEnumString() == "TRICK_STL_ARRAY")
        {
            ostream << prefix << "get_stl_size = get_stl_size_stl_"
                    << fullyQualifiedMangledClassNameUnderscores + "_" + sanitize(fieldName) + " ;\n";
            ostream << prefix << "get_stl_element = get_stl_element_stl_"
                    << fullyQualifiedMangledClassNameUnderscores + "_" + sanitize(fieldName) + " ;\n";
            // Only vector<bool> needs a setter function
            if (fdes->getSTLTypeEnumString() == "TRICK_STL_VECTOR" && fdes->getSTLElementTypeName() == "bool")
            {
                ostream << prefix << "set_stl_element = set_stl_element_stl_"
                        << fullyQualifiedMangledClassNameUnderscores + "_" + sanitize(fieldName) + " ;\n";
            }
        }
    }
    ...
    {
        ostream << "    trick_MM->add_attr_info(std::string(attr" << fullyQualifiedMangledClassNameUnderscores << "["
                << index << "].type_name) , &attr" << fullyQualifiedMangledClassNameUnderscores << "[" << index
                << "], __FILE__ , __LINE__ ) ;\n";
    }
}
...
void PrintFileContents10::print_inherited_add_attr_info(std::ostream& ostream, ClassValues* cv)
{
    ...
    {
        ostream << "    trick_MM->add_attr_info( std::string(\"" << inheritedClass
                << "\"), &temp_attr , __FILE__ , __LINE__ ) ;\n";
    }
}
...
void PrintFileContents10::print_io_src_destruct(std::ostream& ostream, ClassValues* cv)
{
    ...
    {
        ostream << "void io_src_destruct_" << cv->getFullyQualifiedMangledTypeName("__")
                << "(void* address __attribute__((unused)), int num __attribute__((unused))) {\n";
        if (!cv->isPOD())
        {
...
void PrintFileContents10::print_checkpoint_stl(std::ostream& ostream, FieldDescription* fdes, ClassValues* cv)
{
    printStlFunction("checkpoint", "void* start_address, const char* obj_name , const char* var_name",
        "checkpoint_stl(*stl, obj_name, var_name)", ostream, *fdes, *cv);
}
...
void PrintFileContents10::print_post_checkpoint_stl(std::ostream& ostream, FieldDescription* fdes, ClassValues* cv)
{
    printStlFunction("post_checkpoint", "void* start_address, const char* obj_name , const char* var_name",
        "delete_stl(*stl, obj_name, var_name)", ostream, *fdes, *cv);
}

void PrintFileContents10::print_restore_stl(std::ostream& ostream, FieldDescription* fdes, ClassValues* cv)
{
    printStlFunction("restore", "void* start_address, const char* obj_name , const char* var_name",
        "restore_stl(*stl, obj_name, var_name)", ostream, *fdes, *cv);
}
...
void PrintFileContents10::print_get_stl_element(std::ostream& ostream, FieldDescription* fdes, ClassValues* cv)
{
    ...
    if (fdes->getSTLTypeEnumString() == "TRICK_STL_VECTOR" && fdes->getSTLElementTypeName() == "bool")
    {
        ...
        element_access = "static thread_local std::vector<char> temp; if (temp.size() < stl->size()) { "
                         "temp.resize(stl->size()); } temp[index] = (*stl)[index]; return (void*)&temp[index]";
    }
    else
    {
    ...
    printStlFunction(
        "get_stl_element", "void* start_address, size_t index", element_access, ostream, *fdes, *cv, "void*");
}
...
void PrintFileContents10::print_set_stl_element(std::ostream& ostream, FieldDescription* fdes, ClassValues* cv)
{
    ...
    if (fdes->getSTLTypeEnumString() == "TRICK_STL_VECTOR" && fdes->getSTLElementTypeName() == "bool")
    {
        ...
        printStlFunction("set_stl_element", "void* start_address, size_t index, void* value_ptr", element_write,
            ostream, *fdes, *cv);
    }
}
...
void PrintFileContents10::print_stl_helper(std::ostream& ostream, ClassValues* cv)
{
    ...
    fieldDescriptions.erase(std::remove_if(fieldDescriptions.begin(), fieldDescriptions.end(),
                                [](FieldDescription* field) { return !field->isSTL(); }),
        fieldDescriptions.end());
    ...
    for (FieldDescription* field : fieldDescriptions)
    {
        ...
        if (field->getSTLTypeEnumString() == "TRICK_STL_VECTOR" || field->getSTLTypeEnumString() == "TRICK_STL_DEQUE"
            || field->getSTLTypeEnumString() == "TRICK_STL_ARRAY")
        {
...
void PrintFileContents10::printClassMap(std::ostream& ostream, ClassValues* cv)
{
    ...
    ostream << "    // " << cv->getFileName() << std::endl
            ...
            << "    class_attribute_map->add_attr(\"" << cv->getFullyQualifiedMangledTypeName() << "\" , attr" << name
            << ") ;" << std::endl;
}

void PrintFileContents10::printClassMapFooter(std::ostream& ostream) { ostream << "}" << std::endl << std::endl; }

void PrintFileContents10::printEnumMapHeader(std::ostream& ostream, std::string function_name)
{
    ostream << "void " << function_name << "() {\n"
            << "    Trick::EnumAttributesMap* enum_attribute_map __attribute__((unused)) = "
               "Trick::EnumAttributesMap::attributes_map();\n\n";
}
...
void PrintFileContents10::printEnumMap(std::ostream& ostream, EnumValues* ev)
{
    ...
    ostream << "    extern ENUM_ATTR enum" << name << "[];" << std::endl
            << "    enum_attribute_map->add_attr(\"" << ev->getFullyQualifiedTypeName() << "\", enum" << name << ");"
            << std::endl;
}

void PrintFileContents10::printEnumMapFooter(std::ostream& ostream) { ostream << "}" << std::endl << std::endl; }

void PrintFileContents10::printStlFunction(const std::string& name, const std::string& parameters,
    const std::string& call, std::ostream& ostream, FieldDescription& fieldDescription, ClassValues& classValues,
    const std::string& returnType)
{
    const std::string typeName     = fieldDescription.getTypeName();
    const std::string functionName = name + "_stl";
    ostream << returnType << " " << functionName << "_" << classValues.getFullyQualifiedMangledTypeName("__") << "_"
            << sanitize(fieldDescription.getName()) << "(" << parameters << ") {" << std::endl
...
void PrintFileContents10::print_template_argument_header_dependencies(
