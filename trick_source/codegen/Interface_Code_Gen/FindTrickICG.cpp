...
void FindTrickICG::FileSkipped(
    const clang::FileEntry& SkippedFile, const clang::Token& FilenameTok, clang::SrcMgr::CharacteristicKind FileType)
{
    std::string file_name = SkippedFile.getName();
#else
void FindTrickICG::FileSkipped(
    const clang::FileEntryRef& SkippedFile, const clang::Token& FilenameTok, clang::SrcMgr::CharacteristicKind FileType)
{
...
void FindTrickICG::If(
    clang::SourceLocation Loc, clang::SourceRange ConditionRange, clang::PPCallbacks::ConditionValueKind ConditionValue)
#else
...
#endif
{
    if (ConditionRange.isValid())
    {
        ...
        llvm::StringRef ref = clang::Lexer::getSourceText(
            clang::CharSourceRange::getCharRange(ConditionRange), ci.getSourceManager(), clang::LangOptions());
        ...
        if (ref.str().find("TRICK_ICG") != std::string::npos)
        {
            ...
            if (print_msgs)
            {
                ...
                {
                    std::cout << color(
                        WARNING, std::string("Warning: TRICK_ICG used in preprocessor conditional ") + rpath)
                              << std::endl;
                }
                else
                {
                    std::cout << color(
                        WARNING, std::string("Warning: TRICK_ICG used in preprocessor conditional ") + loc_str)
                              << std::endl;
                }
            }
        }
    ...
    if (!header_printed)
    {
        std::cout << color(WARNING,
            std::string(
                ...
                "some C++ class/structures in these headers.  Consider removing the TRICK_ICG preprocessor "
                "conditionals\n"));
        header_printed = true;
    }
}
