...
void FindTrickICG::FileChanged(clang::SourceLocation Loc,
...
#else
void FindTrickICG::FileSkipped(const clang::FileEntryRef & SkippedFile,
                               const clang::Token & FilenameTok,
                               clang::SrcMgr::CharacteristicKind FileType)
{
...
void FindTrickICG::If(clang::SourceLocation Loc,
...
#endif
{
    if(ConditionRange.isValid())
    {
        ...
        llvm::StringRef ref = clang::Lexer::getSourceText(clang::CharSourceRange::getCharRange(ConditionRange),
                                                          ci.getSourceManager(),
                                                          clang::LangOptions());
            ...
            if(print_msgs)
            {
                ...
                if(rpath)
                {
                    std::cout << color(WARNING,
                                       std::string("Warning: TRICK_ICG used in preprocessor conditional ") + rpath)
                              << std::endl;
                }
                else
                {
                    std::cout << color(WARNING,
                                       std::string("Warning: TRICK_ICG used in preprocessor conditional ") + loc_str)
                              << std::endl;
                }
            }
        }
...
#if (LIBCLANG_MAJOR > 3) || ((LIBCLANG_MAJOR == 3) && (LIBCLANG_MINOR >= 5))
void FindTrickICG::ElIf(clang::SourceLocation Loc,
                        clang::SourceRange ConditionRange,
                        clang::PPCallbacks::ConditionValueKind ConditionValue)
#else
...
void FindTrickICG::Ifdef(clang::SourceLocation Loc,
...
#if (LIBCLANG_MAJOR > 3) || ((LIBCLANG_MAJOR == 3) && (LIBCLANG_MINOR >= 7))
void FindTrickICG::Ifndef(clang::SourceLocation Loc,
                          const clang::Token & MacroNameTok,
                          const clang::MacroDefinition & MD)
#else
void FindTrickICG::Ifndef(clang::SourceLocation Loc,
