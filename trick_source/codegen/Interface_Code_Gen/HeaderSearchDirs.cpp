...
HeaderSearchDirs::HeaderSearchDirs(
    clang::HeaderSearch& in_hs, clang::HeaderSearchOptions& in_hso, clang::Preprocessor& in_pp, bool in_sim_services)
    : hs(in_hs)
    , hso(in_hso)
    , pp(in_pp)
    ...
    if (env_var_contents != NULL)
    {
        ...
        while (std::getline(ss, item, ':'))
        {
            ...
            if (!item.empty())
            {
                ...
                {
                    std::cout << bold(color(WARNING, "Warning")) << "    Cannot find " << env_var << " path "
                              << quote(bold(item)) << std::endl;
                }
            }
        }
    }
...
void HeaderSearchDirs::addSearchDirs(std::vector<std::string>& include_dirs, std::vector<std::string>& isystem_dirs)
{
...
bool HeaderSearchDirs::isPathInUserDir(const std::string& in_dir)
{
    ...
    if (!sim_services
        and (!in_dir.compare(0, trick_source_dir.size(), trick_source_dir)
            or !in_dir.compare(0, trick_include_dir.size(), trick_include_dir)))
    {
        return false;
    }
