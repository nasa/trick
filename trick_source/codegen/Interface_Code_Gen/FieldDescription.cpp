...
FieldDescription::FieldDescription(std::string in_container_class)
...
void FieldDescription::parseComment(std::string comment)
{
    ...
    if (!ret_str.empty())
    {
        chkpnt_io = io_map[ret_str];
        if (debug_level >= 4)
            std::cout << "go for cio " << io << std::endl;
        chkpnt_io_found = true;
    ...
    if (!ret_str.empty())
    {
        io = io_map[ret_str];
        if (debug_level >= 4)
            std::cout << "go for io " << io << std::endl;
        io_found = true;
    ...
    if (units_found and io != 0 and (valid_units.find(units) == valid_units.end()))
    {
        ...
        else
        {
            ...
            {
                std::cout << bold(color(WARNING, "Warning    ") + file_name + ":" + std::to_string(line_no))
                          << std::endl
                          << "           Units converted from " << quote(units) << " to " << quote(new_units)
                          << std::endl;
                units = new_units;
            }
            ...
            if (test_units == NULL)
            {
                ...
                std::cout << bold(color(WARNING, "Warning    ") + file_name + ":" + std::to_string(line_no))
                          << std::endl
                          << "           Invalid units specification. Changing " << quote(units) << " to " << quote("1")
                          << std::endl;
                units = "1";
            }
            else
...
std::string FieldDescription::getContainerClass() { return container_class; }

void FieldDescription::setContainerClass(std::string in_name) { container_class = in_name; }

void FieldDescription::setBaseClassOffset(unsigned int in_offset) { base_class_offset = in_offset; }

unsigned int FieldDescription::getBaseClassOffset() { return base_class_offset; }

void FieldDescription::setFieldOffset(unsigned int in_offset) { field_offset = in_offset; }

unsigned int FieldDescription::getFieldOffset() { return field_offset; }

void FieldDescription::setFieldWidth(unsigned int in_width) { field_width = in_width; }

unsigned int FieldDescription::getFieldWidth() { return field_width; }

void FieldDescription::setNonCanonicalTypeName(std::string in_val) { non_canonical_type_name = in_val; }

std::string FieldDescription::getNonCanonicalTypeName() { return non_canonical_type_name; }

void FieldDescription::setTypeName(std::string in_val) { type_name = in_val; }

std::string FieldDescription::getTypeName() { return type_name; }
 ...
 */
std::string FieldDescription::getSTLElementTypeName() { return stl_element_type_name; }

void FieldDescription::setLineNo(unsigned int in_val) { line_no = in_val; }

unsigned int FieldDescription::getLineNo() { return line_no; }

void FieldDescription::setMangledTypeName(std::string in_val) { mangled_type_name = in_val; }
...
std::string FieldDescription::getUnits() { return units; }

bool FieldDescription::isDashDashUnits() { return is_dashdash; }

void FieldDescription::setIO(unsigned int in_io) { io = in_io; }

unsigned int FieldDescription::getIO() { return io; }

unsigned int FieldDescription::getChkpntIO() { return io >> 2 & 3; }

bool FieldDescription::isWriteable() { return io & 1; }

bool FieldDescription::isReadable() { return io & 2; }

bool FieldDescription::isCheckpointable() { return io & 4; }

bool FieldDescription::isRestorable() { return io & 8; }

std::string FieldDescription::getDescription() { return description; }

void FieldDescription::setInherited(bool in_inherited) { inherited = in_inherited; }

bool FieldDescription::isInherited() { return inherited; }

void FieldDescription::setVirtualInherited(bool in_inherited) { virtual_inherited = in_inherited; }

bool FieldDescription::isVirtualInherited() { return virtual_inherited; }

void FieldDescription::setAccess(clang::AccessSpecifier in_val) { access = in_val; }

clang::AccessSpecifier FieldDescription::getAccess() { return access; }

void FieldDescription::addOffset(unsigned int offset) { field_offset += offset; }

void FieldDescription::setEnumString(std::string in_str) { type_enum_string = in_str; }

std::string FieldDescription::getEnumString() { return type_enum_string; }

void FieldDescription::setBitField(bool yes_no) { is_bitfield = yes_no; }

bool FieldDescription::isBitField() { return is_bitfield; }

void FieldDescription::setBitFieldWidth(unsigned int len) { bitfield_width = len; }

void FieldDescription::setBitFieldStart(unsigned int sb) { bitfield_start_bit = sb; }

void FieldDescription::setBitFieldByteOffset(unsigned int wo) { bitfield_word_offset = wo; }

unsigned int FieldDescription::getBitFieldWidth() { return bitfield_width; }

unsigned int FieldDescription::getBitFieldStart() { return bitfield_start_bit; }

unsigned int FieldDescription::getBitFieldByteOffset() { return bitfield_word_offset; }

void FieldDescription::setEnum(bool yes_no) { is_enum = yes_no; }

bool FieldDescription::isEnum() { return is_enum; }

void FieldDescription::setRecord(bool yes_no) { is_record = yes_no; }

bool FieldDescription::isRecord() { return is_record; }

void FieldDescription::setSTL(bool yes_no) { is_stl = yes_no; }

bool FieldDescription::isSTL() { return is_stl; }

void FieldDescription::setSTLTemplateName(const std::string& template_name) { stl_template_name = template_name; }

std::string FieldDescription::getSTLTemplateName() const { return stl_template_name; }
...
void FieldDescription::setSTLElementEnum(bool yes_no) { is_stl_elem_enum = yes_no; }

bool FieldDescription::isSTLElementEnum() const { return is_stl_elem_enum; }
...
bool FieldDescription::isSTLContainer(const std::string& type_name)
{
    ...
    return (type_name.find("std::vector<") == 0 || type_name.find("std::array<") == 0
        || type_name.find("std::list<") == 0 || type_name.find("std::deque<") == 0 || type_name.find("std::set<") == 0
        || type_name.find("std::multiset<") == 0 || type_name.find("std::map<") == 0
        || type_name.find("std::multimap<") == 0 || type_name.find("std::unordered_set<") == 0
        || type_name.find("std::unordered_multiset<") == 0 || type_name.find("std::unordered_map<") == 0
        || type_name.find("std::unordered_multimap<") == 0 || type_name.find("std::stack<") == 0
        || type_name.find("std::queue<") == 0 || type_name.find("std::priority_queue<") == 0
...
bool FieldDescription::isUserDefinedType(const std::string& type_name)
{
    ...
    if (base_type == "char" || base_type == "unsigned char" || base_type == "short" || base_type == "short int"
        || base_type == "unsigned short" || base_type == "unsigned short int" || base_type == "int"
        || base_type == "unsigned int" || base_type == "unsigned" || base_type == "long" || base_type == "long int"
        || base_type == "unsigned long" || base_type == "unsigned long int" || base_type == "long long"
        || base_type == "long long int" || base_type == "unsigned long long" || base_type == "unsigned long long int"
        || base_type == "float" || base_type == "double" || base_type == "bool" || base_type == "wchar_t")
    {
        return false;
    }
...
void FieldDescription::setSTLClear(bool yes_no) { has_stl_clear = yes_no; }

bool FieldDescription::hasSTLClear() { return has_stl_clear; }

void FieldDescription::setStatic(bool yes_no) { is_static = yes_no; }

bool FieldDescription::isStatic() { return is_static; }

void FieldDescription::setReference(bool yes_no) { is_reference = yes_no; }

bool FieldDescription::isReference() { return is_reference; }

unsigned int FieldDescription::getNumDims() { return num_dims; }
...
void FieldDescription::addArrayDim(int in_dim) { array_sizes[num_dims++] = in_dim; }
