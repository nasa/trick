...
TypedefVisitor::TypedefVisitor(clang::CompilerInstance& in_ci, CommentSaver& in_cs, HeaderSearchDirs& in_hsd,
    PrintAttributes& in_pa, std::set<std::string>& fdc)
    : ci(in_ci)
    , cs(in_cs)
    , hsd(in_hsd)
    , pa(in_pa)
...
// ElaboratedType is a wrapper type in older LLVM for structs, unions, and classes that includes the elaborated keyword
// (e.g. struct MyStruct). However, it no longer exists in clang 22 and is desugared away during parsing. The
// VisitRecordType and VisitTemplateSpecializationType functions are still called for typedefs of structs, unions, and
// classes and the code in those functions should be sufficient to handle forward declaration typedefs without needing
// to check for ElaboratedType.
...
bool TypedefVisitor::VisitTypedefDecl(clang::TypedefDecl* td)
{
    ...
    if (const clang::RecordType* rt = underlying->getAs<clang::RecordType>())
    {
        ...
        if (rd)
        {
            ...
            if (fwd_declared_classes.find(type_name) != fwd_declared_classes.end() || rt->isIncompleteType())
            {
