...
PrintAttributes::PrintAttributes(int in_attr_version,
                                 HeaderSearchDirs & in_hsd,
                                 CommentSaver & in_cs,
                                 clang::CompilerInstance & in_ci,
                                 bool in_force,
                                 bool in_sim_services_flag,
    ...
    if(hsd.isPathInUserDir(dir_name))
    {
        ...
        else
        {
            ...
            if(sim_services_flag)
            {
                ...
                {
                    if(dir_name.length() < 13 or
                       dir_name.compare(dir_name.size() - 13, dir_name.size(), "trick/include"))
                    {
                ...
                {
                    io_file_name = std::string(getenv("TRICK_HOME")) + "/trick_source/sim_services/include/io_src/" +
                                   base_name;
                }
                else
                {
...
{
    return hasBeenProcessed(classValues.getFullyQualifiedMangledTypeName(),
                            processed_classes[classValues.getFileName()]);
}
