...
PrintAttributes::PrintAttributes(int in_attr_version, HeaderSearchDirs& in_hsd, CommentSaver& in_cs,
    clang::CompilerInstance& in_ci, bool in_force, bool in_sim_services_flag, std::string in_output_dir)
    : hsd(in_hsd)
    , cs(in_cs)
    , ci(in_ci)
    , force(in_force)
        ...
        if (*p == '/')
        {
            *p = 0;
            if (stat(tmp, &buf) != 0)
            {
                ...
                if (returnValue)
                {
                    std::cerr << bold(color(ERROR, "Error")) << "      Unable to create " << quote(bold(tmp))
            ...
            if (sim_services_flag)
            {
                if (dir_name.length() >= 8 and !dir_name.compare(dir_name.size() - 8, dir_name.size(), "/include"))
                {
                    if (dir_name.length() < 13
                        or dir_name.compare(dir_name.size() - 13, dir_name.size(), "trick/include"))
                    {
                ...
                {
                    io_file_name
                        = std::string(getenv("TRICK_HOME")) + "/trick_source/sim_services/include/io_src/" + base_name;
                }
                else
                {
...
void PrintAttributes::printSieClass(ClassValues* cv)
{
    ...
    for (FieldDescription* fdes : printer->getPrintableFields(*cv))
    {
        ...
        {
            ostream << "      <dimension>" << (fdes->getArrayDim(i) != -1 ? fdes->getArrayDim(i) : 0)
                    << "</dimension>\n";
        }
        ostream << "    </member>\n";
    }
...
void PrintAttributes::createMapFiles()
{
    ...
    if (stat(map_dir.c_str(), &buf) != 0)
    {
        if (mkdir(map_dir.c_str(), 0755) != 0)
        {
            ...
            std::cout << bold(color(WARNING, "Warning")) << "    Unable to create " << quote(bold(map_dir))
...
bool PrintAttributes::hasBeenProcessed(ClassValues& classValues)
{
    return hasBeenProcessed(
        classValues.getFullyQualifiedMangledTypeName(), processed_classes[classValues.getFileName()]);
}
...
bool PrintAttributes::isIgnored(ConstructValues& constructValues)
{
    ...
    const bool ignored = constructs.find(constructValues.getName()) != constructs.end()
        or constructs.find(constructValues.getFullyQualifiedName()) != constructs.end()
        or global_ignore_types.find(constructValues.getName()) != global_ignore_types.end()
        or global_ignore_types.find(constructValues.getFullyQualifiedName()) != global_ignore_types.end();

    if (ignored and verboseBuild)
    {
        std::cout << skipping << "ICG Ignore Type: " << constructValues.getName() << " (from " << fileName << ")"
                  << std::endl;
    }
...
bool PrintAttributes::isHeaderExcluded(const std::string& header, bool exclude_ext_libs)
{
    ...
    if (hsd.isPathInExclude(path))
    {
        if (verboseBuild)
        {
            std::cout << skipping << "TRICK_EXCLUDE: " << underline(path, hsd.getPathInExclude(path).size())
                      << std::endl;
        }
        return true;
    }
    ...
    if (hsd.isPathInICGExclude(path))
    {
        if (verboseBuild)
        {
            std::cout << skipping << "TRICK_ICG_EXCLUDE: " << underline(path, hsd.getPathInICGExclude(path).size())
                      << std::endl;
        }
        return true;
    }
    ...
    if (hsd.isPathInExtLib(path) && exclude_ext_libs)
    {
        if (verboseBuild)
        {
            std::cout << skipping << "TRICK_EXT_LIB_DIRS: " << underline(path, hsd.getPathInExtLib(path).size())
                      << std::endl;
        }
    ...
    if (temp)
    {
        ...
        if (real_path.compare(path))
        {
            if (hsd.isPathInExclude(real_path))
            {
                ...
                {
                    std::cout << skipping
                              << "TRICK_EXCLUDE: " << underline(real_path, hsd.getPathInExclude(real_path).size())
                              << std::endl;
                }
                return true;
            }
            ...
            if (hsd.isPathInICGExclude(real_path))
            {
                if (verboseBuild)
                {
                    std::cout << skipping << "TRICK_ICG_EXCLUDE: "
                              << underline(real_path, hsd.getPathInICGExclude(real_path).size()) << std::endl;
                }
                return true;
            }
            ...
            if (hsd.isPathInExtLib(real_path) && exclude_ext_libs)
            {
                ...
                {
                    std::cout << skipping
                              << "TRICK_EXT_LIB_DIRS: " << underline(real_path, hsd.getPathInExtLib(real_path).size())
                              << std::endl;
                }
                ext_lib_io_files.insert(header);
                return true;
...
void PrintAttributes::markHeaderAsVisited(const std::string& header) { visited_files.insert(header); }
