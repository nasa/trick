...
{ // TODO: Make this display correctly
    ...
    mg_printf(conn, "HTTP/1.1 %i Method Not Allowed\r\nConnection: close\r\n", error_code);
...
int parent_http_handler(struct mg_connection* conn, void* data)
{
    ...
    if (server->debug)
    {
        message_publish(MSG_INFO, "Trick Webserver: HTTP_REQUEST: URI = \"%s\".\n", uri.c_str());
    }
    std::string httpType = "";
    ...
    if (httpType != "")
    {
        if (server->debug)
        {
            message_publish(MSG_DEBUG, "HTTP_REQUEST: METHOD = \"%s\"\n", ri->request_method);
        }
        ...
        if (method == "GET")
        {
            if (server->debug)
            {
                message_publish(MSG_DEBUG, "HTTP_REQUEST: HANDLER = \"%s\"\n", httpType.c_str());
            }
...
int ws_connect_handler(const struct mg_connection* conn, void* ignore)
{
    int ret_val = 0;
    return ret_val;
}
...
void ws_ready_handler(struct mg_connection* conn, void* my_server)
{
    ...
    if (server->debug)
    {
        message_publish(MSG_INFO, "Trick Webserver: WEBSOCKET_REQUEST: URI = \"%s\".\n", uri.c_str());
    }
    std::string wsType = "";
    ...
    if (session != NULL)
    {
        if (server->debug)
        {
            message_publish(MSG_INFO, "Trick Webserver: WEBSOCKET[%p] OPENED. URI=\"%s\".\n", (void*)conn, uri.c_str());
        }
...
int ws_data_handler(struct mg_connection* conn, int bits, char* data, size_t data_len, void* my_server)
{
    int rvalue            = 1;
    MyCivetServer* server = (MyCivetServer*)my_server;
    ...
    if (server->debug)
    {
        message_publish(MSG_INFO, "Trick Webserver: WEBSOCKET[%p] RECIEVED: \"%s\".\n", (void*)conn, msg.c_str());
    }
...
void ws_close_handler(const struct mg_connection* conn, void* my_server)
{
    MyCivetServer* server = (MyCivetServer*)my_server;
    server->deleteWebSocketSession(const_cast<mg_connection*>(conn));
    if (server->debug)
    {
        message_publish(MSG_INFO, "Trick Webserver: WEBSOCKET[%p] CLOSED.\n", (void*)conn);
    }
}
