...
static const char* style_css = "h1 {"
...
void* main_loop(void* S)
{
    ...
    while (1)
    {
        ...
        if (messageSent)
        { // If any  message was sent we say the data is now not marshalled.
...
int MyCivetServer::init()
{
    if (enable)
    {
        if (debug)
        {
            message_publish(MSG_DEBUG, "Trick Webserver: Debug logging is enabled.\n");
        }
        ...
        if (ssl_enable)
        {
            ...
            message_publish(MSG_INFO,
                "Trick Webserver: Current path to ssl certificate is %s. To change this put "
                "\"web.server.path_to_ssl_cert = \'/path/to/cert\'\" in your input file.\n",
                path_to_ssl_cert);
        }
        else
        {
            message_publish(MSG_INFO,
                "Trick Webserver: SSL is not enabled. To enable put \"web.server.ssl_enable = True\" in your input "
                "file.\n");
            port_str = std::to_string(port);
        }
        const char* options[] = {"listening_ports", port_str.c_str(), "ssl_certificate", path_to_ssl_cert,
            "document_root", document_root, "enable_directory_listing", "yes", "error_log_file", error_log_file, 0};
        ...
        if (ctx == NULL)
        {
            message_publish(MSG_ERROR,
                "Trick Webserver: Failed to create listener, exiting Simulation.\n"
        ...
        mg_set_websocket_handler(
            ctx, "/api/ws", ws_connect_handler, ws_ready_handler, ws_data_handler, ws_close_handler, (void*)this);
        ...
        {
            message_publish(
                MSG_ERROR, "Trick Webserver: Failed to create main loop.  Web socket connections will not work.");
            exit(-1);
        }
    }
    else
    {
        message_publish(MSG_INFO,
            "Trick Webserver: DISABLED. To enable, add "
...
void MyCivetServer::handleHTTPGETrequest(
    struct mg_connection* conn, const struct mg_request_info* ri, std::string handlerName)
{
