...
static int confirmDocumentRoot(std::string documentRoot)
{
    ...
    else
    {
        message_publish(MSG_INFO,
                        "Trick Webserver: Document root \"%s\" doesn't exist, so we'll create it.\n",
                        documentRoot.c_str());
...
int MyCivetServer::init()
{
    if(enable)
    {
        ...
        {
            message_publish(MSG_INFO,
                            "Trick Webserver: SSL is not enabled. To enable put \"web.server.ssl_enable = True\" in "
                            "your input file.\n");
        ...
        {
            message_publish(MSG_ERROR,
                            "Trick Webserver: Failed to create main loop.  Web socket connections will not work.");
            exit(-1);
        }
    }
    else
...
void MyCivetServer::handleHTTPGETrequest(struct mg_connection * conn,
