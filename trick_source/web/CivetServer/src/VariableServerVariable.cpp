...
const char* VariableServerVariable::getName() { return varInfo->reference; }

const char* VariableServerVariable::getUnits() { return varInfo->attr->units; }
...
void VariableServerVariable::writeValue(std::ostream& outs)
{
    ...
    case TRICK_DOUBLE:
        ...
        //                     sbf = extract_bitfield_any( *(int*)src_addr, attr->size, attr->index[0].start,
        //                     attr->index[0].size);
        //                } else if (attr->size == sizeof(short)) {
        //                     sbf = extract_bitfield_any( *(short*)src_addr, attr->size, attr->index[0].start,
        //                     attr->index[0].size);
        //                } else if (attr->size == sizeof(char)) {
        //                     sbf = extract_bitfield_any( *(char*)src_addr, attr->size, attr->index[0].start,
        //                     attr->index[0].size);
        //                } else {
        ...
        //                src_addr = (char*)stageBuffer + offset * (size_t)attr->size;
        //                if (attr->size == sizeof(int)) {
        //                     bf = extract_unsigned_bitfield_any( *(unsigned int*)src_addr, attr->size,
        //                     attr->index[0].start, attr->index[0].size);
        //                } else if (attr->size == sizeof(short)) {
        //                     bf = extract_unsigned_bitfield_any( *(unsigned short*)src_addr, attr->size,
        //                     attr->index[0].start, attr->index[0].size);
        //                } else if (attr->size == sizeof(char)) {
        //                     bf = extract_unsigned_bitfield_any( *(unsigned char*)src_addr, attr->size,
        //                     attr->index[0].start, attr->index[0].size);
        //                } else {
        //                     message_publish(MSG_ERROR, "Checkpoint Agent INTERNAL ERROR:\n"
        //                                                "Unsupported bitfield size (%d) bytes.\n", attr->size) ;
