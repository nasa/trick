...
// CONSTRUCTOR
VariableServerSession::VariableServerSession(struct mg_connection* nc)
    : WebSocketSession(nc)
{
...
// DESTRUCTOR
VariableServerSession::~VariableServerSession() { clear(); }
...
WebSocketSession* makeVariableServerSession(struct mg_connection* nc) { return new VariableServerSession(nc); }
...
int VariableServerSession::sendUnitsMessage(const char* vname)
{
    ...
    for (it = sessionVariables.begin(); it != sessionVariables.end(); ++it)
    {
        ...
        {
            ss << ((((*it)->getUnits() != NULL) && ((*it)->getUnits()[0] != '\0')) ? (*it)->getUnits() : "--") << "\"}";
            std::string tmp     = ss.str();
            const char* message = tmp.c_str();
            mg_websocket_write(connection, MG_WEBSOCKET_OPCODE_TEXT, message, strlen(message));
            return 0;
    ...
    sendErrorMessage("Variable Server: var_units cannot get units for \"%s\" because it must be added to the variable "
                     "server first\n",
        vname);
    return 0;
}
