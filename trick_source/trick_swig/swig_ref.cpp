            ...
            if (elem != NULL)
            {
                ...
                if (SWIG_IsOK(SWIG_ConvertPtr(elem, &argp, SWIG_TypeQuery("_p_swig_ref"), 0)))
                {
                    ...
                    else
                    {
                        ...
                        Trick::PythonPrint::write_rvalue(
                            os, elem_ref_copy.address, elem_ref_copy.attr, 0, 0, true, true, &startingIndex);
                    }
                }
                else
                {
...
char* swig_ref::__repr__() { return (__str__()); }
...
template <class TYPE> void* assign_array(PyObject* obj1, REF2& ref)
{
    void* argp2;
    int ret;
    TYPE* temp_ptr;
...
int swig_ref::__setitem__(int ii, PyObject* obj1)
{
    ...
    else if (PyTuple_Check(obj1))
    {
        ...
        // cout << "HERE in swig_ref set_item with list type!! ref->num_index " << ref.num_index << " attr->num_index =
        // " << ref.attr->num_index << endl ; cout << "    last index size = " << ref.attr->index[ref.attr->num_index -
        // 1].size << endl ;
        last_index = ref_copy.attr->index[ref_copy.attr->num_index - 1].size;

        if (last_index)
        {
            // The LHS is a fixed-size array. Fill out a V_TREE structure with the RHS data to be used with
            // ref_assignment.
        ...
        if (swig_obj != NULL)
        {
            ...
            else
            {
                std::cout << "\033[31mswig_ref::__setitem__ could not convert  "
                          << SWIG_TypePrettyName(SWIG_Python_GetSwigThis(obj1)->ty) << " to " << ref.attr->type_name
                          << "\033[0m" << std::endl;
...
template <class VALUE_TYPE> VALUE_TYPE getValue(void* address, Py_ssize_t index)
{
...
PyObject* swig_ref::__getitem__(int ii)
{
    ...
    if (temp_ref.num_index_left == 0)
    {
        ...
        case TRICK_STRING:
        {
            std::string* str_ptr = (std::string*)temp_ref.address;
            ret                  = PyUnicode_FromString(str_ptr->c_str());
        }
        break;
        case TRICK_WSTRING:
        {
...
PyObject* swig_ref::__getitem__(PyObject* index)
{
    ...
    else if (PySlice_Check(index))
    {
        ...
        {
            std::cout << "Slice parameters do not fit within list length of " << temp_ref.attr->index[0].size
                      << std::endl;
        }
    }
    else
