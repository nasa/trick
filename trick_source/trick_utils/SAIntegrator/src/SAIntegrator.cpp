...
static void nil_dfunc(
    double x, double state[] __attribute__((unused)), double derivs[], void* udata __attribute__((unused)))
{
    derivs[0] = 0.0;
}
static void nil_gfunc(double t, double x[] __attribute__((unused)), double v[] __attribute__((unused)), double derivs[],
...
void SA::Integrator::advanceIndyVar() { X_out = X_in + default_h; }
void SA::Integrator::step() { advanceIndyVar(); }
void SA::Integrator::load() { X_in = X_out; }
void SA::Integrator::unload() { std::cout << "SA::Integrator : Nothing to unload." << std::endl; }
    ...
    : Integrator(h, udata)
{
    ...
    outState   = new double[state_size];
    if (dfunc == NULL)
        throw std::invalid_argument("dfunc must be non-NULL.");
    derivs_func = dfunc;
}
// Copy Constructor
...
// Assignment Operator
SA::FirstOrderODEVariableStepIntegrator& SA::FirstOrderODEVariableStepIntegrator::operator=(
    const SA::FirstOrderODEVariableStepIntegrator& rhs)
{
...
void SA::FirstOrderODEVariableStepIntegrator::add_Rootfinder(
    double tolerance, SlopeConstraint constraint, RootErrorFunc rfunc)
{
...
// Constructor
SA::EulerIntegrator::EulerIntegrator(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
// Constructor
SA::HeunsMethod::HeunsMethod(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
SA::RK2Integrator::RK2Integrator(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
// Constructor
SA::RK4Integrator::RK4Integrator(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
void SA::RK4Integrator::variable_step(double h)
{
    ...
    for (unsigned int i = 0; i < state_size; i++)
    {
        outState[i] = inState[i]
            + ((1 / 6.0) * derivs[0][i] + (1 / 3.0) * derivs[1][i] + (1 / 3.0) * derivs[2][i]
                  + (1 / 6.0) * derivs[3][i])
                * h;
    }
    advanceIndyVar(h);
...
// Constructor
SA::RK3_8Integrator::RK3_8Integrator(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
void SA::RK3_8Integrator::variable_step(double h)
{
    ...
    {
        outState[i] = inState[i]
            + ((1 / 8.0) * derivs[0][i] + (3 / 8.0) * derivs[1][i] + (3 / 8.0) * derivs[2][i]
                  + (1 / 8.0) * derivs[3][i])
                * h;
    }
    advanceIndyVar(h);
...
// Constructor
SA::RKF45Integrator::RKF45Integrator(
    double eps, double h, unsigned int N, double* in_vars[], double* out_vars[], DerivsFunc dfunc, void* udata)
    : FirstOrderODEIntegrator(h, N, in_vars, out_vars, dfunc, udata)
{
    epsilon = eps;
    next_h  = h;
...
// Constructor
SA::RKF45Integrator::RKF45Integrator(
    double eps, double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
void SA::RKF45Integrator::step() { adaptive_step(next_h); }
...
double SA::RKF45Integrator::adaptive_step(double h)
{
    ...
    do
    {
        ...
        for (unsigned int i = 0; i < state_size; i++)
        {
            wstate[1][i] = inState[i] + h * ((3 / 32.0) * derivs[0][i] + (9 / 32.0) * derivs[1][i]);
        }
        (*derivs_func)(X_in + (3 / 8.0) * h, wstate[1], derivs[2], user_data);
        for (unsigned int i = 0; i < state_size; i++)
        {
            wstate[2][i] = inState[i]
                + h
                    * ((1932 / 2197.0) * derivs[0][i] + (-7200 / 2197.0) * derivs[1][i]
                        + (7296 / 2197.0) * derivs[2][i]);
        }
        (*derivs_func)(X_in + (12 / 13.0) * h, wstate[2], derivs[3], user_data);
        for (unsigned int i = 0; i < state_size; i++)
        {
            wstate[3][i] = inState[i]
                + h
                    * ((439 / 216.0) * derivs[0][i] + (-8.0) * derivs[1][i] + (3680 / 513.0) * derivs[2][i]
                        + (-845 / 4104.0) * derivs[3][i]);
        }
        (*derivs_func)(X_in + h, wstate[3], derivs[4], user_data);
        for (unsigned int i = 0; i < state_size; i++)
        {
            wstate[4][i] = inState[i]
                + h
                    * ((-8 / 27.0) * derivs[0][i] + (2.0) * derivs[1][i] + (-3544 / 2565.0) * derivs[2][i]
                        + (1859 / 4104.0) * derivs[3][i] + (-11 / 40.0) * derivs[4][i]);
        }
        (*derivs_func)(X_in + (1 / 2.0) * h, wstate[4], derivs[5], user_data);
        for (unsigned int i = 0; i < state_size; i++)
        {
            outState[i] = inState[i]
                + ((25 / 216.0) * derivs[0][i] + (1408 / 2565.0) * derivs[2][i] + (2197 / 4104.0) * derivs[3][i]
                      + (-1 / 5.0) * derivs[4][i])
                    * h;
        }
        for (unsigned int i = 0; i < state_size; i++)
        {
            w2[i] = inState[i]
                + ((16 / 135.0) * derivs[0][i] + (6656 / 12825.0) * derivs[2][i] + (28561 / 56430.0) * derivs[3][i]
                      + (-9 / 50.0) * derivs[4][i] + (2 / 55.0) * derivs[5][i])
                    * h;
        }
        last_h = h;
        R      = 0.0;
...
void SA::EulerCromerIntegrator::step() { step(default_h); }
void SA::EulerCromerIntegrator::load()
{
    ...
    {
        std::cerr << "Error: SA::EulerCromerIntegrator::load(). Input variable pointers (pos_in, vel_in) aren't set."
                  << std::endl;
    }
}
...
double SA::EulerCromerIntegrator::undo_integrate()
{
    ...
    {
        std::cerr << "Error: SA::EulerCromerIntegrator::undo_integrate(). Input variable pointers (pos_in, vel_in) "
                     "aren't set."
                  << std::endl;
    }
    return (last_h);
...
// Constructor
SA::ABMIntegrator::ABMIntegrator(unsigned int history_size, double h, unsigned int N, double* in_vars[],
    double* out_vars[], DerivsFunc func, void* udata)
    : FirstOrderODEIntegrator(h, N, in_vars, out_vars, func, udata)
{
    ...
    bufferSize           = algorithmHistorySize
        + 1; // The size of the buffer needs to be one more than the history so that an integration step can be reset.
    hix                = 0;
    currentHistorySize = 0; // How much derivative history is currently stored int the history buffer. Initially there
                            // will be none until we've primed the integrator.
...
void SA::ABMIntegrator::step()
{
    ...
    (*derivs_func)(
        X_in, inState, deriv_history[hix], user_data); // Calculated and store the deriv for current, corrected state.
    ...
    : FirstOrderODEIntegrator(h, N, in_vars, out_vars, dfunc, udata)
{
    ...
    priming_integrator
        = new SA::RK2Integrator(h, N, priming_integrator_in_vars, priming_integrator_out_vars, dfunc, udata);
}
// Constructor
SA::ABM2Integrator::ABM2Integrator(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
...
void SA::ABM2Integrator::step()
{
    ...
    else
    {
        ...
        // Move history index backward, so we over-write the predicted state with the corrected state on our next
        // step().
        hix = (hix + bufferSize - 1) % bufferSize;
    }
    advanceIndyVar();
    ...
    : FirstOrderODEIntegrator(h, N, in_vars, out_vars, dfunc, udata)
{
    ...
    priming_integrator
        = new SA::RK4Integrator(h, N, priming_integrator_in_vars, priming_integrator_out_vars, dfunc, udata);
}
// Constructor
SA::ABM4Integrator::ABM4Integrator(double h, unsigned int N, double* in_out_vars[], DerivsFunc dfunc, void* udata)
