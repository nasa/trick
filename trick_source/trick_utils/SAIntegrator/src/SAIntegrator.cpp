...
void SA::FirstOrderODEVariableStepIntegrator::add_Rootfinder(double tolerance,
                                                             SlopeConstraint constraint,
                                                             RootErrorFunc rfunc)
{
...
void SA::RK4Integrator::variable_step(double h)
{
    ...
    for(unsigned int i = 0; i < state_size; i++)
    {
        outState[i] = inState[i] + ((1 / 6.0) * derivs[0][i] + (1 / 3.0) * derivs[1][i] + (1 / 3.0) * derivs[2][i] +
                                    (1 / 6.0) * derivs[3][i]) *
                                       h;
    }
    advanceIndyVar(h);
...
void SA::RK3_8Integrator::variable_step(double h)
{
    ...
    for(unsigned int i = 0; i < state_size; i++)
    {
        outState[i] = inState[i] + ((1 / 8.0) * derivs[0][i] + (3 / 8.0) * derivs[1][i] + (3 / 8.0) * derivs[2][i] +
                                    (1 / 8.0) * derivs[3][i]) *
                                       h;
    }
    advanceIndyVar(h);
...
double SA::RKF45Integrator::getLastStepSize()
{
    return last_h;
}
...
double SA::RKF45Integrator::adaptive_step(double h)
{
    ...
    do
    {
        ...
        for(unsigned int i = 0; i < state_size; i++)
        {
            wstate[2][i] = inState[i] + h * ((1932 / 2197.0) * derivs[0][i] + (-7200 / 2197.0) * derivs[1][i] +
                                             (7296 / 2197.0) * derivs[2][i]);
        }
        (*derivs_func)(X_in + (12 / 13.0) * h, wstate[2], derivs[3], user_data);
        for(unsigned int i = 0; i < state_size; i++)
        {
            wstate[3][i] = inState[i] + h * ((439 / 216.0) * derivs[0][i] + (-8.0) * derivs[1][i] +
                                             (3680 / 513.0) * derivs[2][i] + (-845 / 4104.0) * derivs[3][i]);
        }
        (*derivs_func)(X_in + h, wstate[3], derivs[4], user_data);
        for(unsigned int i = 0; i < state_size; i++)
        {
            wstate[4][i] = inState[i] +
                           h * ((-8 / 27.0) * derivs[0][i] + (2.0) * derivs[1][i] + (-3544 / 2565.0) * derivs[2][i] +
                                (1859 / 4104.0) * derivs[3][i] + (-11 / 40.0) * derivs[4][i]);
        }
        (*derivs_func)(X_in + (1 / 2.0) * h, wstate[4], derivs[5], user_data);
        for(unsigned int i = 0; i < state_size; i++)
        {
            outState[i] = inState[i] + ((25 / 216.0) * derivs[0][i] + (1408 / 2565.0) * derivs[2][i] +
                                        (2197 / 4104.0) * derivs[3][i] + (-1 / 5.0) * derivs[4][i]) *
                                           h;
        }
        for(unsigned int i = 0; i < state_size; i++)
        {
...
// Constructor
SA::ABMIntegrator::ABMIntegrator(unsigned int history_size,
                                 double h,
                                 unsigned int N,
                                 double * in_vars[],
                                 double * out_vars[],
    ...
    : FirstOrderODEIntegrator(h, N, in_vars, out_vars, func, udata)
{
    ...
    bufferSize = algorithmHistorySize + 1; // The size of the buffer needs to be one more than the history so that an
                                           // integration step can be reset.
    hix = 0;
...
void SA::ABMIntegrator::step()
{
    ...
    (*derivs_func)(X_in, inState, deriv_history[hix], user_data); // Calculated and store the deriv for current,
                                                                  // corrected state.
    ...
    : FirstOrderODEIntegrator(h, N, in_vars, out_vars, dfunc, udata)
{
    ...
    bufferSize = 2 + 1; // The size of the buffer needs to be one more than the history so that an integration step can
                        // be reset.
    hix = 0;
    currentHistorySize = 0; // How much derivative history is currently stored int the history buffer. Initially there
                            // will be none until we've primed the integrator.
    ...
    priming_integrator = new SA::RK2Integrator(h,
                                               N,
                                               priming_integrator_in_vars,
                                               priming_integrator_out_vars,
                                               dfunc,
                                               udata);
}
...
{
    // The amount of history that we intend to use in this ABM integrator.
    bufferSize = 4 + 1; // The size of the buffer needs to be one more than the history so that an integration step can
                        // be reset.
    hix = 0;
    currentHistorySize = 0; // How much derivative history is currently stored int the history buffer. Initially there
                            // will be none until we've primed the integrator.
    ...
    priming_integrator = new SA::RK4Integrator(h,
                                               N,
                                               priming_integrator_in_vars,
                                               priming_integrator_out_vars,
                                               dfunc,
