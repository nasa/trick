...
void RootFinder::init() { init(0.00000000001, Unconstrained); }
// Default Constructor
RootFinder::RootFinder() { init(); }
// Constructor
RootFinder::RootFinder(double tolerance, SlopeConstraint constraint) { init(tolerance, constraint); }
...
{
    if ((fabs(f_error) < f_error_tol) || ((iterations > 0) && (fabs(prev_f_error - f_error) < f_error_tol)))
    {
