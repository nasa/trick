...
namespace SA
{
    ...
    class FirstOrderODEIntegrator : public Integrator
    {
        protected:
            ...
            double** outVars; // Array of pointers to the variables to which the output state vector values are to be
                              // disseminated.
    ...
    class RK2Integrator : public FirstOrderODEVariableStepIntegrator
    {
        public:
            RK2Integrator();
            RK2Integrator(
                double h, unsigned int N, double* in_vars[], double* out_vars[], DerivsFunc derivs_func, void* udata);
    ...
    class RKF45Integrator : public FirstOrderODEIntegrator
    {
        ...
        public:
            RKF45Integrator();
            RKF45Integrator(double epsilon, double h, unsigned int N, double* in_vars[], double* out_vars[],
                DerivsFunc dfunc, void* udata);
            RKF45Integrator(
                double epsilon, double h, unsigned int N, double* in_out_vars[], DerivsFunc derivs_func, void* udata);
    ...
    class ABMIntegrator : public FirstOrderODEIntegrator
    {
        ...
        public:
            ABMIntegrator(unsigned int history_size, double h, unsigned int N, double* in_vars[], double* out_vars[],
                DerivsFunc dfunc, void* udata);
            ABMIntegrator(unsigned int history_size, double h, unsigned int N, double* in_out_vars[],
                DerivsFunc derivs_func, void* udata);
