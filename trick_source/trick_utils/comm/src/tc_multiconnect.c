...
int tc_multiconnect(TCDevice* dev_ptr, char* my_tag, char* other_tag, TrickErrorHndlr* not_used)
{
    ...
    {
        trick_error_report(dev_ptr->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__,
            "[33m%s| cannot connect disabled device %s[00m\n", my_tag, other_tag);
        return (TC_SUCCESS);
    }

    trick_error_report(dev_ptr->error_handler, TRICK_ERROR_ADVISORY, __FILE__, __LINE__,
    ...
    {
        trick_error_report(
            server_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "could not open listen device!\n");
        return (status);
    }
    ...
    while (found_conn == 0)
    {
        ...
        if (nbytes == sizeof(read_me))
        {
            ...
            if (curr_pid != read_me.pid && strcmp(bc_info.send_me.my_tag, read_me.my_tag)
                && !strcmp(bc_info.send_me.other_tag, read_me.other_tag))
            {
                ...
                if (i_am_client == 1)
                {
                    ...
                    sendto(bc_info.fd, (char*)&read_me, sizeof(SEND_ME), 0, (struct sockaddr*)&bc_info.addr,
                        (socklen_t)sizeof(bc_info.addr));
                    found_conn = 1;
                }
            }
            else if (curr_pid == read_me.pid && !strcmp(bc_info.send_me.addr, read_me.addr)
    ...
    {
        trick_error_report(
            server_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "could not open connection!\n");
        return (status);
    }
