    ...
    while((attr[i].name != NULL) && (attr[i].name[0] != '\0'))
    {
        ...
        else if(attr[i].type == TRICK_BITFIELD || attr[i].type == TRICK_UNSIGNED_BITFIELD)
        {
            ...
            if((local_order == TRICK_LITTLE_ENDIAN && tofrom == 1) || (local_order == TRICK_BIG_ENDIAN && tofrom == 0))
            {
                ...
                mask = (~(0xffffffff << attr[i].index[0].size))
                       << (32 - attr[i].index[0].start - attr[i].index[0].size);
                ti &= mask;
        ...
        else
        {
            switch(attr[i].size)
            {
                case 1:
                    for(j = 0; j < num; j++)
                    {
                        *(unsigned char *)((char *)out + attr[i].offset + j) = *(unsigned char *)((char *)in +
                                                                                                  attr[i].offset + j);
                    }
                    break;
                case 2:
