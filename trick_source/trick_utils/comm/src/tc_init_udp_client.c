...
int tc_init_udp_client(TCDevice* udp_client_device)
{
    ...
    {
        trick_error_report(NULL, TRICK_ERROR_ALERT, __FILE__, __LINE__, "UDP device is null.");
        return (-1);
    }

    if (udp_client_device->disabled == TC_COMM_TRUE)
    {
        trick_error_report(udp_client_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__,
            "Attempted to connect a disabled connection\n");
    ...
    if (the_socket == TRICKCOMM_INVALID_SOCKET)
    {
        ...
        trick_error_report(
            udp_client_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "Error creating UDP socket.");
