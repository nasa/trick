...
int tc_init_mcast_server(TCDevice* mcast_server_device)
{
    ...
    if (mcast_server_device->socket == TRICKCOMM_INVALID_SOCKET)
    {
        ...
        trick_error_report(
            mcast_server_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "Error creating mcast socket.");
    ...
    if (setsockopt(mcast_server_device->socket, SOL_SOCKET, SO_REUSEADDR, &yes, (socklen_t)sizeof(yes)) < 0)
    {
        ...
        trick_error_report(mcast_server_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__,
