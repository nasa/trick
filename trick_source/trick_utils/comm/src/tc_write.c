...
int tc_write_(TCDevice* device, char* buffer, int size, const char* file, int line)
{
    ...
    {
        trick_error_report(
            device->error_handler, TRICK_ERROR_ALERT, file, line, "Trying to write to a disabled device");
        return (-1);
    }
    ...
    {
        trick_error_report(
            device->error_handler, TRICK_ERROR_ALERT, file, line, "Trying to write to an invalid socket");
        return (-1);
    }
    ...
    while (nbytes != size)
    {
        /* Send will return -1 with tc_errno set to EINTR if it was interrupted by the delivery of a signal.  Re-send
           data if this occurs. */
        while ((tmp_nbytes = sendto(device->socket, tmp_data, (size_t)tmp_size, TC_NOSIGNAL,
                    (struct sockaddr*)&device->remoteServAddr, (socklen_t)sizeof(struct sockaddr_in)))
                < 0
        ...
        if (tmp_nbytes < 0)
        {
            ...
            {
                snprintf(error_str, sizeof(error_str), "tc_write: %s %s (tc_errno = %d)", client_str, strerror(error),
                    error);
    ...
    case TC_READWRITE_TIMEOUT:
        trick_error_report(device->error_handler, TRICK_ERROR_ADVISORY, file, line,
            ...
            "time limit of %f seconds. delta = %f ref_time = %f",
            client_str, device->blockio_limit, delta, ref_time);
        break;
    case TC_EWOULDBLOCK:
        ...
        break;
    case TC_SUCCESS:
        trick_error_report(device->error_handler, TRICK_ERROR_ALL, file, line, "%s: %d bytes successfully written\n",
            client_str, nbytes);
        break;
    }
