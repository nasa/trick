...
int tc_write_(TCDevice * device, char * buffer, int size, const char * file, int line)
{
    ...
    if(device->disabled)
    {
        trick_error_report(device->error_handler,
                           ...
                           file,
                           line,
                           "Trying to write to a disabled device");
        return (-1);
    }
    ...
    if(device->socket == TRICKCOMM_INVALID_SOCKET)
    {
        trick_error_report(device->error_handler,
                           ...
                           file,
                           line,
                           "Trying to write to an invalid socket");
        return (-1);
    }
    ...
    while(nbytes != size)
    {
        /* Send will return -1 with tc_errno set to EINTR if it was interrupted by the delivery of a signal.  Re-send
           data if this occurs. */
        while((tmp_nbytes = sendto(device->socket,
                                   tmp_data,
                                   (size_t)tmp_size,
                                   TC_NOSIGNAL,
    ...
    switch(error)
    {
        ...
        case TC_EWOULDBLOCK:
            trick_error_report(device->error_handler,
                               ...
                               file,
                               line,
                               "%s No data written during non-blocking write.",
                               client_str);
            break;
        case TC_SUCCESS:
            trick_error_report(device->error_handler,
                               ...
                               file,
                               line,
                               "%s: %d bytes successfully written\n",
                               client_str,
                               nbytes);
            break;
    }
