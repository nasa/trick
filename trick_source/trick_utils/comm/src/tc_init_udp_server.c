...
{ /* INOUT: -- Device structure for the UDP server */
    ...
    if (udp_server_device->socket == TRICKCOMM_INVALID_SOCKET)
    {
        ...
        trick_error_report(
            udp_server_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "Error creating UDP socket.");
    ...
    if (setsockopt(udp_server_device->socket, SOL_SOCKET, SO_REUSEADDR, &yes, (socklen_t)sizeof(yes)) < 0)
    {
        ...
        trick_error_report(udp_server_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__,
