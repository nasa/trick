...
int tc_set_blockio_timeout_limit(TCDevice* device, double limit)
{
    if (!device)
    {
        ...
        trick_error_report(temp_error_hndlr, TRICK_ERROR_ALERT, __FILE__, __LINE__,
            "Trying to tc_set_blockio_timeout_limit "
            "to a NULL device");
        return (-1);
    }

    trick_error_report(device->error_handler, TRICK_ERROR_ALL, __FILE__, __LINE__,
