...
int tc_accept_(TCDevice* listen_device, TCDevice* device, const char* file, int line)
{
    ...
    snprintf(
        client_str, sizeof(client_str), "(ID = %d  tag = %s)", listen_device->client_id, listen_device->client_tag);

    /* Check for error conditon on accept */
    if (the_socket == TRICKCOMM_INVALID_SOCKET)
    {
        trick_error_report(listen_device->error_handler, TRICK_ERROR_ALERT, file, line,
    ...
    trick_error_report(listen_device->error_handler, TRICK_ERROR_ADVISORY, file, line,
