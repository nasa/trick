...
int tc_read_(TCDevice* device, char* buffer, int size, const char* file, int line)
{
    ...
    if (!device)
    {
        ...
        trick_error_report(
            temp_error_hndlr, TRICK_ERROR_ALERT, file, line, "tc_read: Trying to read from a NULL device");
        return (-1);
    }
    ...
    {
        trick_error_report(
            device->error_handler, TRICK_ERROR_ALERT, file, line, "tc_read: Trying to read from a disabled device");
        return (-1);
    }
    ...
    {
        trick_error_report(
            device->error_handler, TRICK_ERROR_ALERT, file, line, "tc_read: Trying to read from an invalid socket");
        return (-1);
    }
    ...
    trick_error_report(
        device->error_handler, TRICK_ERROR_ALL, file, line, "tc_read: %s reading %d bytes\n", client_str, size);
    ...
    while (nbytes != size)
    {
         ...
         */
        while ((tmp_nbytes = recvfrom(
                    device->socket, data, (size_t)tmp_size, TC_NOSIGNAL, (struct sockaddr*)&device->cliAddr, &cliLen))
                < 0
        ...
        else if (tmp_nbytes == -1)
        {
            ...
            {
                snprintf(
                    error_str, sizeof(error_str), "tc_read: %s %s (tc_errno = %d)", client_str, strerror(error), error);
    ...
    case TC_READWRITE_TIMEOUT:
        trick_error_report(device->error_handler, TRICK_ERROR_ADVISORY, file, line,
            "tc_read: %s Failed to read within the specified "
            "time limit of %f seconds. delta = %f",
            client_str, device->blockio_limit, delta);
        break;
    case TC_EWOULDBLOCK:
        trick_error_report(device->error_handler, TRICK_ERROR_ALL, file, line,
            "tc_read: %s %d of %d bytes read during "
            "non-blocking read.",
            client_str, nbytes, size);
        break;
    case TC_SUCCESS:
        trick_error_report(device->error_handler, TRICK_ERROR_ALL, file, line,
