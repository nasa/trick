...
int tc_init_mcast_client(TCDevice* mcast_client_device)
{
    ...
    if (mcast_client_device->socket == TRICKCOMM_INVALID_SOCKET)
    {
        ...
        trick_error_report(mcast_client_device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__,
    ...
    if (setsockopt(mcast_client_device->socket, IPPROTO_IP, IP_MULTICAST_TTL, &mcast_client_device->ttl,
            (socklen_t)sizeof(mcast_client_device->ttl))
        < 0)
    {
