...
int tc_blockio(TCDevice* device, TCCommBlocking blockflag)
{
    ...
    trick_error_report(
        device->error_handler, TRICK_ERROR_TRIVIAL, __FILE__, __LINE__, "%s blockflag = %d\n", client_str, blockflag);
    ...
    case TC_COMM_BLOCKIO:
        ...
        {
            trick_error_report(
                device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "could not set socket to blocking\n");
            return (TC_EWOULDBLOCK);
        }
    ...
    case TC_COMM_NOBLOCKIO:
        ...
        {
            trick_error_report(
                device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__, "could not set socket to non-blocking\n");
            return (TC_EWOULDBLOCK);
        }
        device->blockio_type = blockflag;
        break;
    ...
    case TC_COMM_TIMED_BLOCKIO:
        ...
        if (IOCTL_SOCKET(device->socket, (unsigned long)FIONBIO, &on) < 0)
        {
            trick_error_report(device->error_handler, TRICK_ERROR_ALERT, __FILE__, __LINE__,
