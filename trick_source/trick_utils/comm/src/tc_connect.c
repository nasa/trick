...
int tc_connect_(TCDevice* device, const char* file, int line)
{
    ...
    {
        trick_error_report(
            device->error_handler, TRICK_ERROR_ALERT, file, line, "Attempted to connect a disabled connection\n");
    ...
    snprintf(client_str, sizeof(client_str), "(ID = %d tag = %s hostname = %s port = %d)", device->client_id,
        device->client_tag, device->hostname, device->port);
    ...
    if (!device->disable_handshaking)
    {
        ...
        {
            trick_error_report(device->error_handler, TRICK_ERROR_ALERT, file, line,
                "%s: could not send client id to server\n", client_str);
            CLOSE_SOCKET(the_socket);
            return (TC_COULD_NOT_CONNECT);
        }
        ...
        {
            trick_error_report(device->error_handler, TRICK_ERROR_ALERT, file, line,
                "%s: could not send client tag to server\n", client_str);
            CLOSE_SOCKET(the_socket);
            return (TC_COULD_NOT_CONNECT);
        }
        ...
        {
            trick_error_report(device->error_handler, TRICK_ERROR_ALERT, file, line,
                "%s: could not read byte info from client\n", client_str);
            CLOSE_SOCKET(the_socket);
            return (TC_COULD_NOT_ACCEPT);
        }
