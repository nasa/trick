...
size_t ucodepoint_to_utf32(unsigned int codePoint, int32_t* out)
{
    ...
    {
        fprintf(
            stderr, "%s:ERROR: 0x%08x is reserved for UTF-16, as a surrogate codepoint.\n", __FUNCTION__, codePoint);
    }
    return 0;
}
...
size_t escape_to_ascii(const char* in, char* out, size_t outSize)
{
    ...
    while ((*in != 0) && (state != ERROR_STATE))
    {
        ...
        case 0:
        {
            ...
            { // We should never find a continuation byte in isolation.
        ...
        break;
        case 1:
        { // Expecting one continuation byte.
            if ((ch & 0xc0) == 0x80)
            { // If the next char is a continuation byte ..
        ...
        break;
        case 2:
        { /* Expecting two continuation bytes. */
            if ((ch & 0xc0) == 0x80)
            {
        ...
        break;
        case 3:
        { /* Expecting three continuation bytes. */
            if ((ch & 0xc0) == 0x80)
            {
                codePoint = (codePoint << 6) | (ch & 0x3f);
                state     = 2;
    ...
    if (out != NULL)
        out[out_len] = 0; /* NULL termination of string. */
    return out_len;
}

size_t escape_to_ascii_len(const char* in) { return escape_to_ascii(in, NULL, (size_t)0); }
...
size_t unescape_to_utf8(const char* in, char* out, size_t outSize)
{
    ...
    while ((*in != 0) && (state != ERROR_STATE))
    {
        ...
        { // Normal State
            ...
            { // We should never find a UTF-8 continuation byte in isolation.
        ...
        break;
        case 1:
        { // Escaped State ( that is: we've found a '\' character.)
            switch (ch)
            {
            case '\'':
            case '\"':
        ...
        break;
        case 2:
        { // Escaped Unicode ( that is: we've found '\x', '\u' or '\U'.)
            int digit = -1;
            if (ch >= '0' && ch <= '9')
            {
                digit = ch - (int)'0';
            ...
            {
                fprintf(stderr,
                    "%s:ERROR: Insufficient hexidecimal digits following"
        ...
        case 3:
        { /* Expecting 3 UTF-8 continuation bytes. */
            if ((ch & 0xc0) == 0x80)
            {
                if (out != NULL)
                {
        ...
        case 4:
        { /* Expecting 2 UTF-8 continuation bytes. */
            if ((ch & 0xc0) == 0x80)
            {
                if (out != NULL)
                {
        ...
        case 5:
        { /* Expecting 1 UTF-8 continuation byte. */
            if ((ch & 0xc0) == 0x80)
            {
                if (out != NULL)
                {
...
size_t unescape_to_utf8_len(const char* in) { return unescape_to_utf8(in, NULL, (size_t)0); }
...
size_t utf8_to_wchar(const char* in, wchar_t* out, size_t outSize)
{
    ...
    if (state != 0)
    { /* If we didn't finish in state 0, it's an error. */
        out_len = 0;
    }
    if (out != NULL)
        out[out_len] = 0; /* NULL termination of string. */
...
size_t utf8_to_wchar_len(const char* in) { return utf8_to_wchar(in, NULL, (size_t)0); }
...
size_t wchar_to_utf8_len(const wchar_t* in) { return wchar_to_utf8(in, NULL, (size_t)0); }
