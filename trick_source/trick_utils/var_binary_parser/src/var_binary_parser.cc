...
TRICK_TYPE Var::getType() const { return _trick_type; }

std::vector<unsigned char> Var::getRawBytes() const { return value_bytes; }
...
template <> char Var::getValue<char>() const
{
...
template <> unsigned char Var::getValue<unsigned char>() const
{
...
template <> short Var::getValue<short>() const
{
...
template <> unsigned short Var::getValue<unsigned short>() const
{
...
template <> unsigned int Var::getValue<unsigned int>() const
{
...
template <> int Var::getValue<int>() const
{
...
template <> long Var::getValue<long>() const
{
...
template <> unsigned long Var::getValue<unsigned long>() const
{
...
template <> unsigned long long Var::getValue<unsigned long long>() const
{
...
template <> float Var::getValue<float>() const
{
...
template <> double Var::getValue<double>() const
{
...
template <> wchar_t Var::getValue<wchar_t>() const
{
...
{
    public:
        MessageIterator(const std::vector<unsigned char>& container)
...
int ParsedBinaryMessage::parse(const std::vector<unsigned char>& bytes)
{
    if (bytes.size() < header_size)
    {
        throw MalformedMessageException(
            std::string("Not enough bytes in message to contain header: expected at least 12, got "
    ...
    {
        std::string error_message = "Message size in header (" + std::to_string(_message_size)
            + ") does not match size of message received (" + std::to_string(bytes.size() - message_indicator_size)
            + ")";
...
void ParsedBinaryMessage::combine(const ParsedBinaryMessage& other)
{
    ...
    {
        std::string error_message = "Trying to combine two messages with different message indicators ("
            + std::to_string(_message_type) + " and " + std::to_string(other._message_type) + ")";
        throw MalformedMessageException(error_message);
    }
...
int ParsedBinaryMessage::getMessageType() const { return _message_type; }

unsigned int ParsedBinaryMessage::getMessageSize() const { return _message_size; }

unsigned int ParsedBinaryMessage::getNumVars() const { return _num_vars; }
