...
char * unit_system_name[] = {"Metric", "English", "Standard"};
    ...
    switch(context->s[context->n])
    {
        ...
        default:
        {
            ...
            else
            {
                ...
                fprintf(stderr,
                        "Syntax error: Expected a units primitive in \"%s\" at character position [%d].\n",
                        context->s,
                        context->n);
                status = CONV_ERR;
            }
...
static Conv_Status_t units_factor(parse_context_t * context)
{
    ...
    {
        fprintf(stderr,
                "Syntax error: Expected a units primitive in \"%s\" before character position [%d].\n",
                context->s,
                context->n);
        return (CONV_ERR);
    }
        ...
        switch(f_units->t[i].op)
        {
            case Units_Primitive:
            {
                ...
                if(conv_tab[f_units->t[i].a1].measure != conv_tab[t_units->t[i].a1].measure)
                {
                    ...
                    fprintf(stderr,
                            "\"%s\" is a measure of \"%s\", whereas \"%s\" is a measure of \"%s\".\n",
                            conv_tab[f_units->t[i].a1].c_unit,
                            measure_name[conv_tab[f_units->t[i].a1].measure],
                            conv_tab[t_units->t[i].a1].c_unit,
                            measure_name[conv_tab[t_units->t[i].a1].measure]);
                    return (CONV_ERR);
                }
                ...
                if(convert_primitive(conv_tab[f_units->t[i].a1].c_unit, conv_tab[t_units->t[i].a1].c_unit, &p) !=
                   CONV_OK)
                {
                    return (CONV_ERR);
                }

                switch(f_units->t[i].a2)
                { /* the exponent */
                    case 3:
                        if(p.C[0] != 0.0)
                        {
                            fprintf(stderr,
                                    "Unable represent the conversion to %s3 as a linear function of %s3.\n",
                                    conv_tab[t_units->t[i].a1].c_unit,
                                    conv_tab[f_units->t[i].a1].c_unit);
                            return (CONV_ERR);
                        };
                    ...
                    case 2:
                        ...
                        {
                            fprintf(stderr,
                                    "Unable represent the conversion to %s2 as a linear function of %s2.\n",
                                    conv_tab[t_units->t[i].a1].c_unit,
                                    conv_tab[f_units->t[i].a1].c_unit);
                            return (CONV_ERR);
                        };
...
void list_units(void)
{
    ...
    for(i = 0; i < N_conversions; i++)
    {
        ...
        printf("%-10s %-20s %-10s\n", conv_tab[i].c_unit, conv_tab[i].desc, unit_system_name[conv_tab[i].system]);
    }
}
