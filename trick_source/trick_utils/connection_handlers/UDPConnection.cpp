...
Trick::UDPConnection::UDPConnection()
    : UDPConnection(new SystemInterface())
{
}
Trick::UDPConnection::UDPConnection(SystemInterface* system_interface)
    : _started(false)
    , _initialized(false)
    , _port(0)
    , _hostname("")
...
int Trick::UDPConnection::initialize(const std::string& in_hostname, int in_port)
{
    ...
    if (_system_interface->setsockopt(
            _socket, SOL_SOCKET, SO_REUSEADDR, (const char*)&option_val, (socklen_t)sizeof(option_val))
        != 0)
    {
    ...
    if (in_hostname == "" || in_hostname == "localhost" || in_hostname == "127.0.0.1"
        || strcmp(in_hostname.c_str(), name) == 0)
    {
...
int Trick::UDPConnection::write(const std::string& message)
{
    ...
    return _system_interface->sendto(
        _socket, send_buf, message.length(), 0, (struct sockaddr*)&_remote_serv_addr, sock_size);
}
...
int Trick::UDPConnection::read(std::string& message, int max_len)
{
    ...
    if (last_newline != NULL)
    {
        ...
        nbytes   = _system_interface->recvfrom(
            _socket, incoming_msg, size, 0, (struct sockaddr*)&_remote_serv_addr, &sock_size);
    }
    else
    {
        nbytes = 0;
...
int Trick::UDPConnection::setBlockMode(bool blocking)
{
    ...
    if (_system_interface->fcntl(_socket, F_SETFL, flag) == -1)
    {
        std::string error_message
            = "Unable to set fd " + std::to_string(_socket) + " block mode to " + std::to_string(blocking);
        perror(error_message.c_str());
        return -1;
    }
...
bool Trick::UDPConnection::isInitialized() { return _started; }

int Trick::UDPConnection::getPort() { return _port; }

std::string Trick::UDPConnection::getHostname() { return _hostname; }

std::string Trick::UDPConnection::getClientTag() { return _client_tag; }
