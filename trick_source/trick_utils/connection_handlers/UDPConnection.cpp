...
int Trick::UDPConnection::initialize(const std::string & in_hostname, int in_port)
{
    ...
    if(_system_interface->setsockopt(_socket,
                                     SOL_SOCKET,
                                     SO_REUSEADDR,
                                     (const char *)&option_val,
                                     (socklen_t)sizeof(option_val)) != 0)
    {
...
int Trick::UDPConnection::read(std::string & message, int max_len)
{
    ...
    if(last_newline != NULL)
    {
        ...
        nbytes = _system_interface
                     ->recvfrom(_socket, incoming_msg, size, 0, (struct sockaddr *)&_remote_serv_addr, &sock_size);
    }
    else
    {
        nbytes = 0;
...
int Trick::UDPConnection::setBlockMode(bool blocking)
{
    ...
    {
        std::string error_message = "Unable to set fd " + std::to_string(_socket) + " block mode to " +
