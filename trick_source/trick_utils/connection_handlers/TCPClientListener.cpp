...
Trick::TCPClientListener::TCPClientListener()
    : TCPClientListener(new SystemInterface())
{
}
Trick::TCPClientListener::TCPClientListener(SystemInterface* system_interface)
    : _listen_socket(-1)
    , _hostname("")
    , _port(0)
    , _client_tag("<empty>")
...
int Trick::TCPClientListener::initialize(std::string in_hostname, int in_port)
{
    ...
    if (_system_interface->setsockopt(
            _listen_socket, SOL_SOCKET, SO_REUSEADDR, (const char*)&option_val, (socklen_t)sizeof(option_val))
        != 0)
    {
    ...
    if (_system_interface->setsockopt(
            _listen_socket, IPPROTO_TCP, TCP_NODELAY, (const void*)&option_val, (socklen_t)sizeof(option_val))
        != 0)
    {
...
int Trick::TCPClientListener::initialize() { return initialize("", 0); }
...
int Trick::TCPClientListener::setBlockMode(bool blocking)
{
    ...
    if (flag == -1)
    {
        std::string error_message = "Unable to get flags for fd " + std::to_string(_listen_socket) + " block mode to "
    ...
    if (_system_interface->fcntl(_listen_socket, F_SETFL, flag) == -1)
    {
        std::string error_message
            = "Unable to set fd " + std::to_string(_listen_socket) + " block mode to " + std::to_string(blocking);
        perror(error_message.c_str());
        return -1;
    }
...
bool Trick::TCPClientListener::isInitialized() { return _initialized; }
