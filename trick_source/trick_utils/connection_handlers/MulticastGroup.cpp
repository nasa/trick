...
int Trick::MulticastGroup::broadcast(std::string message)
{
    ...
    {
        int status = _system_interface->sendto(_socket,
                                               message_send,
                                               strlen(message_send),
                                               0,
...
int Trick::MulticastGroup::initialize()
{
    ...
    int option_value = 1;
    if(_system_interface
           ->setsockopt(_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&option_value, (socklen_t)sizeof(option_value)) < 0)
    {
...
int Trick::MulticastGroup::write(const std::string & message)
{
    ...
    return _system_interface
        ->sendto(_socket, send_buf, message.length(), 0, (struct sockaddr *)&_remote_serv_addr, sock_size);
}
...
int Trick::MulticastGroup::read(std::string & message, int max_len)
{
    ...
    int nbytes = _system_interface
                     ->recvfrom(_socket, incoming_msg, MAX_CMD_LEN, MSG_PEEK, (struct sockaddr *)&s_in, &sock_size);
