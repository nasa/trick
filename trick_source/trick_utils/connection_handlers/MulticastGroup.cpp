...
Trick::MulticastGroup::MulticastGroup()
    : MulticastGroup(new SystemInterface)
{
}

Trick::MulticastGroup::MulticastGroup(SystemInterface* system_interface)
    : _initialized(false)
...
int Trick::MulticastGroup::broadcast(std::string message)
{
    ...
    {
        int status = _system_interface->sendto(
            _socket, message_send, strlen(message_send), 0, (struct sockaddr*)&address, (socklen_t)sizeof(address));
        if (status == -1)
        {
...
bool Trick::MulticastGroup::isInitialized() { return _initialized; }
...
int Trick::MulticastGroup::initialize()
{
    ...
    if (_system_interface->setsockopt(
            _socket, SOL_SOCKET, SO_REUSEADDR, (char*)&option_value, (socklen_t)sizeof(option_value))
        < 0)
    {
...
#ifdef SO_REUSEPORT
    if (_system_interface->setsockopt(_socket, SOL_SOCKET, SO_REUSEPORT, (char*)&option_value, sizeof(option_value))
        < 0)
    {
...
int Trick::MulticastGroup::read(std::string& message, int max_len)
{
    ...
    int nbytes = _system_interface->recvfrom(
        _socket, incoming_msg, MAX_CMD_LEN, MSG_PEEK, (struct sockaddr*)&s_in, &sock_size);
