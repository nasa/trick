...
TEST_F(TCPClientListenerTest, initialized) { EXPECT_EQ(listener.isInitialized(), false); }
...
TEST_F(TCPClientListenerTest, checkForNewConnections_select_error)
{
    // ARRANGE
    system_context->register_select_impl(
        [](int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, struct timeval* timeout) { return -1; });
    listener.initialize();
