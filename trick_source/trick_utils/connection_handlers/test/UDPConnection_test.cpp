...
TEST_F(UDPConnectionTest, write_string)
{
    ...
    system_context->register_sendto_impl(
        [&](int socket, const void* buffer, size_t length, int flags, const struct sockaddr* dest_addr,
...
TEST_F(UDPConnectionTest, read_nonewline)
{
    ...
    system_context->noop_fcntl_impl();
    system_context->register_recvfrom_impl(
        [&](int socket, void* buffer, size_t length, int flags, struct sockaddr* address,
