...
class SystemInferfaceMock : public SystemInterface
{
    ...
    public:
        ...
        {
            socket_impl = [](int domain, int type, int protocol) -> int { return ::socket(domain, type, protocol); };
        }
        void noop_socket_impl()
        {
            socket_impl = [](int domain, int type, int protocol) -> int { return 0; };
        }
    ...
    public:
        virtual int setsockopt(
            int socket, int level, int option_name, const void* option_value, socklen_t option_len) override
        {
            return setsockopt_impl(socket, level, option_name, option_value, option_len);
        }
        void register_setsockopt_impl(setsockopt_func_type impl) { setsockopt_impl = impl; }
        void real_setsockopt_impl()
        {
            setsockopt_impl
                = [](int socket, int level, int option_name, const void* option_value, socklen_t option_len) -> int
            { return ::setsockopt(socket, level, option_name, option_value, option_len); };
        }
        void noop_setsockopt_impl()
        {
            setsockopt_impl = [](int socket, int level, int option_name, const void* option_value,
                                  socklen_t option_len) -> int { return 0; };
        }
    ...
    private:
        ...
        // bind implementation
    public:
        virtual int bind(int socket, const struct sockaddr* address, socklen_t address_len) override
        {
            return bind_impl(socket, address, address_len);
        }
        void register_bind_impl(bind_func_type impl) { bind_impl = impl; }
        void real_bind_impl()
        {
            bind_impl = [](int socket, const struct sockaddr* address, socklen_t address_len) -> int
            { return ::bind(socket, address, address_len); };
        }
        void noop_bind_impl()
        {
            bind_impl = [](int socket, const struct sockaddr* address, socklen_t address_len) -> int { return 0; };
        }
    ...
    public:
        virtual int getsockname(int socket, struct sockaddr* address, socklen_t* address_len) override
        {
            return getsockname_impl(socket, address, address_len);
        }
        void register_getsockname_impl(getsockname_func_type impl) { getsockname_impl = impl; }
        void real_getsockname_impl()
        {
            getsockname_impl = [](int socket, struct sockaddr* address, socklen_t* address_len) -> int
            { return ::getsockname(socket, address, address_len); };
        }
        void noop_getsockname_impl()
        {
            getsockname_impl = [](int socket, struct sockaddr* address, socklen_t* address_len) -> int { return 0; };
        }
    ...
    public:
        ...
        void real_listen_impl()
        {
            listen_impl = [](int socket, int backlog) -> int { return ::listen(socket, backlog); };
        }
        void noop_listen_impl()
        {
            listen_impl = [](int socket, int backlog) -> int { return 0; };
        }
    ...
    public:
        virtual int select(
            int nfds, fd_set* readfds, fd_set* writefds, fd_set* errorfds, struct timeval* timeout) override
        {
        ...
        void real_select_impl()
        {
            select_impl
                = [](int nfds, fd_set* readfds, fd_set* writefds, fd_set* errorfds, struct timeval* timeout) -> int
            { return ::select(nfds, readfds, writefds, errorfds, timeout); };
        }
        void noop_select_impl()
        {
            select_impl = [](int nfds, fd_set* readfds, fd_set* writefds, fd_set* errorfds,
                              struct timeval* timeout) -> int { return 0; };
        }
    ...
    public:
        ...
        void real_close_impl()
        {
            close_impl = [](int fildes) -> int { return ::close(fildes); };
        }
        void noop_close_impl()
        {
    ...
    public:
        virtual int getaddrinfo(
            const char* hostname, const char* servname, const struct addrinfo* hints, struct addrinfo** res) override
        {
            return getaddrinfo_impl(hostname, servname, hints, res);
        }
        void register_getaddrinfo_impl(getaddrinfo_func_type impl) { getaddrinfo_impl = impl; }
        void real_getaddrinfo_impl()
        {
            getaddrinfo_impl
                = [](const char* hostname, const char* servname, const struct addrinfo* hints,
                      struct addrinfo** res) -> int { return ::getaddrinfo(hostname, servname, hints, res); };
        }
        void noop_getaddrinfo_impl()
        {
            getaddrinfo_impl = [](const char* hostname, const char* servname, const struct addrinfo* hints,
                                   struct addrinfo** res) -> int { return 0; };
        }
    ...
    public:
        ...
        void real_fcntl_impl()
        {
            fcntl_impl = [](int fildes, int cmd, int arg) -> int { return ::fcntl(fildes, cmd, arg); };
        }
        void noop_fcntl_impl()
        {
    ...
    public:
        ...
        void real_shutdown_impl()
        {
            shutdown_impl = [](int socket, int how) -> int { return ::shutdown(socket, how); };
        }
        void noop_shutdown_impl()
        {
            shutdown_impl = [](int socket, int how) -> int { return 0; };
        }
    ...
    public:
        virtual int accept(int socket, struct sockaddr* address, socklen_t* address_len) override
        {
            return accept_impl(socket, address, address_len);
        }
        void register_accept_impl(accept_func_type impl) { accept_impl = impl; }
        void real_accept_impl()
        {
            accept_impl = [](int socket, struct sockaddr* address, socklen_t* address_len) -> int
            { return ::accept(socket, address, address_len); };
        }
        void noop_accept_impl()
        {
            accept_impl = [](int socket, struct sockaddr* address, socklen_t* address_len) -> int { return 0; };
        }
    ...
    private:
        ...
        // send implementation
    public:
        virtual ssize_t send(int socket, const void* buffer, size_t length, int flags) override
        {
            return send_impl(socket, buffer, length, flags);
        }
        void register_send_impl(send_func_type impl) { send_impl = impl; }
        void real_send_impl()
        {
            send_impl = [](int socket, const void* buffer, size_t length, int flags) -> ssize_t
            { return ::send(socket, buffer, length, flags); };
        }
        void noop_send_impl()
        {
            send_impl = [](int socket, const void* buffer, size_t length, int flags) -> ssize_t { return 0; };
        }
    ...
    public:
        virtual ssize_t sendto(int socket, const void* buffer, size_t length, int flags,
            const struct sockaddr* dest_addr, socklen_t dest_len) override
        {
            return sendto_impl(socket, buffer, length, flags, dest_addr, dest_len);
        }
        void register_sendto_impl(sendto_func_type impl) { sendto_impl = impl; }
        void real_sendto_impl()
        {
            sendto_impl = [](int socket, const void* buffer, size_t length, int flags, const struct sockaddr* dest_addr,
                              socklen_t dest_len) -> ssize_t
            { return ::sendto(socket, buffer, length, flags, dest_addr, dest_len); };
        }
        void noop_sendto_impl()
        {
            sendto_impl = [](int socket, const void* buffer, size_t length, int flags, const struct sockaddr* dest_addr,
                              socklen_t dest_len) -> ssize_t { return 0; };
        }
    ...
    private:
        ...
        // recv implementation
    public:
        virtual ssize_t recv(int socket, void* buffer, size_t length, int flags) override
        {
            return recv_impl(socket, buffer, length, flags);
        }
        void register_recv_impl(recv_func_type impl) { recv_impl = impl; }
        void real_recv_impl()
        {
            recv_impl = [](int socket, void* buffer, size_t length, int flags) -> ssize_t
            { return ::recv(socket, buffer, length, flags); };
        }
        void noop_recv_impl()
        {
            recv_impl = [](int socket, void* buffer, size_t length, int flags) -> ssize_t { return 0; };
        }
    ...
    public:
        virtual ssize_t recvfrom(int socket, void* buffer, size_t length, int flags, struct sockaddr* address,
            socklen_t* address_len) override
        {
            return recvfrom_impl(socket, buffer, length, flags, address, address_len);
        }
        void register_recvfrom_impl(recvfrom_func_type impl) { recvfrom_impl = impl; }
        void real_recvfrom_impl()
        {
            recvfrom_impl = [](int socket, void* buffer, size_t length, int flags, struct sockaddr* address,
                                socklen_t* address_len) -> ssize_t
            { return ::recvfrom(socket, buffer, length, flags, address, address_len); };
        }
        void noop_recvfrom_impl()
        {
            recvfrom_impl = [](int socket, void* buffer, size_t length, int flags, struct sockaddr* address,
    ...
    public:
        ...
        void real_inet_addr_impl()
        {
            inet_addr_impl = [](const char* cp) -> in_addr_t { return ::inet_addr(cp); };
        }
        void noop_inet_addr_impl()
        {
            inet_addr_impl = [](const char* cp) -> in_addr_t { return 0; };
        }
