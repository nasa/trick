...
DLLPOS DLL_FindIndex(int n, DLLIST * list)
{
    ...
    {
        fprintf(stderr,
                "Error (%s): Index is either negative or exceeds the number of elements in the list.\n",
                __FUNCTION__);
        fflush(stderr);
        return NULL;
    }
...
void * DLL_RemoveAt(DLLPOS pos, DLLIST * list)
{
    ...
    {                           // pos is the head, check whether it's also the tail
        list->head = pos->next; // pos not the tail
        pos->next->prev = NULL;
    }
    else
    { // pos is both the head and the tail
        list->head = NULL;
        list->tail = NULL;
    }
