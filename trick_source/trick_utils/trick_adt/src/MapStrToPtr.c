...
void SMI_Delete(StrMapIterator* pIter) { free(pIter); }
...
void* SMI_GetFirst(char** key, StrMapIterator* pIter)
{
    ...
    if (pIter->m_pTable->m_Count == 0) /* make sure at least one element is in the string map */
        return NULL;
    for (i = 0; i < pIter->m_pTable->m_Size; i++)
    { /* find the first list in the list array that has an element */
...
void* SMI_GetNext(char** key, StrMapIterator* pIter)
{
    ...
    if (pIter->m_ListPos != NULL)
    { /* found the next entry in the current list */
    ...
    else if (pIter->m_ListPos == NULL)
    { /* if we are already at the end of the current list then find the next
...
void* SMI_GetPrev(char** key, StrMapIterator* pIter)
{
    ...
    if (pIter->m_ListPos == NULL)
    { /* if we are already at the beginning of the list the check the previous lists */
...
void SM_Delete(MapStrToPtr* Map)
{
    ...
    for (i = 0; i < Map->m_Size; i++)
    {
        ...
        while (pos != NULL)
        { /* delete each table entry from each list */
...
void* SM_Map(char* key, void* data, MapStrToPtr* Map)
{
    ...
    if (pEntry == NULL)
    { /* if no other items match this key */
    ...
    { /* otherwise replace the current key's data with new data */
