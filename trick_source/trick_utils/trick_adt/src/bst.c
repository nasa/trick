...
void* bstDelete(bstNode* node, BST* bst)
{
    ...
    { /* if node is not the root, */ /* then get the address of the parent's pointer to node */
    ...
    { /* neither left nor right is null */
        ...
        { /* if left is null then inner child is found */
    ...
    { /* using left leaf deletion to keep from getting unbalanced due to deletions */

        bst->deltype = 1 - bst->deltype; /* toggle deltype back to right leaf deletion */
        current      = node->left;
        if (current->right == NULL)
        { /* do the same right leaf deletion except in other direction */
            *parentspointer        = current;
            current->parent        = node->parent;
            current->right         = node->right;
            current->right->parent = current;
...
bstNode* bstInsert(void* info, BST* bst)
{
    ...
    if (bst->root == NULL)
    { /* check if this is the only node in the tree */
        bst->root       = newNode;
        newNode->parent = NULL;
    }
    else
    { /* else find a location to insert the node */
        ...
        do
        {
            ...
            if (compval >= 0)
            { /* new node is greater than the current node */
                if (current->right == NULL)
                { /* if right is null then a location has been found */ /* for the new node */
            ...
            { /* the new node is less than the current node */
                ...
                if (current->left == NULL)
                { /* if left is null then put new node to the left */
...
void bstPrintAscending(FILE* output, BST* bst)
{
    ...
    {
        switch (st)
        { /* switch based upon the current state of traversing the tree */
...
void DefTraverseProc(void* info) { (void)info; /* unused */ }

/* default delete function, will be overridden by the user */

void DefDeleteProc(void* info) { (void)info; /* unused */ }
...
void bstSetPrintFunc(BST* bst, void (*printfunc)(FILE* output, void* info)) { bst->print_node = printfunc; }

/* override the default traverse function with a user defined print function */

void bstSetTraverseFunc(BST* bst, void (*traversefunc)(void* info)) { bst->traverse_node = traversefunc; }

/* override the default traverse function with a user defined print function */

void bstSetDeleteFunc(BST* bst, void (*deletefunc)(void* info)) { bst->delete_node = deletefunc; }

/* override the default compare function with a user defined compare function */

void bstSetCompareFunc(BST* bst, int (*compfunc)(void* left, void* right)) { bst->compare = compfunc; }
