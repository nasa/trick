/*
   PURPOSE: (Use the Primal-Dual Interior Point Method to optimize a linear programming problem subject to a set of
   equality constraints given by [A]x = b and inequality constraints of the form X1 > 0, X2 > 0, etc., for each variable
   in the problem.

   ASSUMPTIONS AND LIMITATIONS: ((The problem is in standard form with the inequality constraints being trivial
   inequalities, that is, each variable is greater than 0) (The problem is formulated as a minimization problem) ([A] is
   a square matrix) (The constant term in the objective function is not passed as an argument) (initial guess is in the
   feasible region))
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            for (i = 0; i < num_variables; i++)
            {
                ...
                norm_rdual
                    += (objective_fn[i] - updatedLambda[i] + loopsum) * (objective_fn[i] - updatedLambda[i] + loopsum);
            }
...
void mark_nonzeros(int r, int c, double** A, int* rflags, int* cflags, int* nzrows, int* nzcols)
{
...
int reduce_and_solve(int num_vars, double* minfn, int num_eq, double** Amat, double* B, double* X, double* lambdas,
    double* nus, double epsilon, double alpha, double beta, double mu, double mu_scaler, int maxiter, int maxbacktrack)
{
    int count = 0;
    ...
    count = dPDIP_linear_solver(nzcols, reducedminfn, nzrows, Cmat, dvals, yvals, reducedlambdas, reducednus, epsilon,
