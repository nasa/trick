...
#else
    message_publish(
        MSG_WARNING, "A suitable GSL library was not found, using Trick's built in random number generators\n");
        ...
        do
        {
            if (loops++ == 1000)
            {
                message_publish(MSG_ERROR,
                    "trick_gsl_rand(%s:%d): Gaussian distribution is unable to produce a random number between %f and "
                    "%f"
