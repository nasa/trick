...
{ /* Inout: Generator parameters */
    ...
    message_publish(MSG_WARNING,
                    "A suitable GSL library was not found, using Trick's built in random number generators\n");
...
{ /* Inout: Generator parameters */
    ...
    if(G->min >= G->max)
    {
        message_publish(MSG_ERROR,
                        "trick_gsl_rand(%s:%d): min (%f) must be less than max (%f)\n",
                        __FILE__,
                        __LINE__,
                        G->min,
                        G->max);
    ...
    switch(G->type)
    {
        case TRICK_GSL_GAUSS:
            do
            {
                ...
                {
                    message_publish(MSG_ERROR,
                                    "trick_gsl_rand(%s:%d): Gaussian distribution is unable to produce a random number "
                                    "between %f and %f"
