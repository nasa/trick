...
/* 
 * $Id: dLU_Choleski.c 49 2009-02-02 22:37:59Z lin $
 */
...
int dLU_Choleski(double **A,    /* In: The input matrix [A] */
...
{                                      /* In: Solution mode: 0 = Solve [A]x = b for x 1 = Compute [L] Only 2 = Already
    ...
    volatile int i, j, k;
    volatile double sum, sum_jk, sum_ijk;
    volatile double *Li;
    volatile double *Lj;
    ...
    if (mode == 0 || mode == 1) {
        ...
        if (-TINY < A[0][0] && A[0][0] < TINY) {
            fprintf(stderr, "ERROR: Diagonal Element A[0][0] = %.17g " "is too small...\n", A[0][0]);
