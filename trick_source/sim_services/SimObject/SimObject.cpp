...
Trick::JobData* Trick::SimObject::add_job(int in_thread, int in_id, const char* in_job_class_name, void* sup_class_data,
    double in_cycle, const char* in_name, const char* in_tag, int in_phase, double in_start, double in_stop)
{
    /** @par Detailed Design */
    /** @li create a new job with the incoming characteristics */
    Trick::JobData* job_ptr;
    job_ptr = new Trick::JobData(
        in_thread, in_id, in_job_class_name, sup_class_data, in_cycle, in_name, in_tag, in_phase, in_start, in_stop);
