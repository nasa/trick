...
Trick::JobData::JobData(int in_thread, int in_id, std::string in_job_class_name, void* in_sup_class_data,
    double in_cycle, std::string in_name, std::string in_tag, int in_phase, double in_start, double in_stop)
{
    /** @par Detailed Design */
...
void Trick::JobData::enable() { disabled = false; }

void Trick::JobData::disable() { disabled = true; }

bool Trick::JobData::get_handled() { return (handled); }
