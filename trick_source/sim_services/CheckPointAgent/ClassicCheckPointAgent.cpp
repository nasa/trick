...
// MEMBER FUNCTION
bool Trick::ClassicCheckPointAgent::input_perm_check(ATTRIBUTES* attr) { return (attr->io & TRICK_CHKPNT_INPUT); }

bool Trick::ClassicCheckPointAgent::output_perm_check(ATTRIBUTES* attr) { return (attr->io & TRICK_CHKPNT_OUTPUT); }
...
static int getCompositeSubReference(void* reference_address, /* Address we are looking for */
...
)
{
    ...
    if (Ai->type == TRICK_STRUCTURED && traversalResult.num_computed_indices > 0)
    {
        ...
        char buf[REFNAME_MAXSIZE];
        ret = getCompositeSubReference(
            rAddr, left_type, sAddr + Ai->offset + traversalResult.offset_from_found_attr, (ATTRIBUTES*)Ai->attr, buf);
...
std::string Trick::ClassicCheckPointAgent::get_var_name(
    void* addr, ATTRIBUTES* A, void* struct_addr, std::string name, ATTRIBUTES** left_type)
{
    ...
    {
        std::stringstream ss;
        ss << "Checkpoint Agent ERROR: Unable to create a subreference of variable \"" << name << "\"." << std::endl;
...
int Trick::ClassicCheckPointAgent::restore(std::istream* checkpoint_stream)
{
    ...
    else if ((context->bad_declaration_count > 0) || (context->bad_assignment_count > 0))
    {
        std::stringstream ss;
        ss << "Checkpoint Agent ERROR: " << context->bad_declaration_count << " invalid declaration(s) "
           << "and " << context->bad_assignment_count << " invalid assignment(s)." << std::endl;
        message_publish(MSG_ERROR, ss.str().c_str());
        status = 1;
    }
    ...
    {
        std::stringstream ss;
        ss << "Checkpoint Agent ERROR: Checkpoint restore failed." << std::endl;
...
// MEMBER FUNCTION
int Trick::ClassicCheckPointAgent::is_nil_valued(void* address, ATTRIBUTES* attr, int curr_dim, int offset)
{
    char* test_addr;
    ...
    if (remaining_dimensions == 0)
    {
        ...
        case TRICK_FLOAT:
            ...
            if (fpclassify(*(float*)test_addr) == FP_ZERO)
                return (1);
            break;
        case TRICK_DOUBLE:
            test_addr = (char*)address + offset * sizeof(double);
            if (fpclassify(*(double*)test_addr) == FP_ZERO)
                return (1);
            break;
        case TRICK_LONG_LONG:
        case TRICK_UNSIGNED_LONG_LONG:
            test_addr = (char*)address + offset * sizeof(long long);
            if (*(long long*)test_addr == 0)
                return (1);
            break;
        case TRICK_BITFIELD:
        case TRICK_UNSIGNED_BITFIELD:
            ...
            else
            {
                message_publish(MSG_ERROR,
                    "Checkpoint Agent INTERNAL ERROR:\n"
        ...
        case TRICK_STRING:
            ...
            if (*(std::string*)test_addr == "")
                return (1);
            break;
        case TRICK_WSTRING:
            test_addr = (char*)address + offset * sizeof(std::wstring);
            if (*(std::wstring*)test_addr == L"")
                return (1);
            break;
        case TRICK_STL:
...
void Trick::ClassicCheckPointAgent::write_singleton(
    std::ostream& chkpnt_os, void* address, ATTRIBUTES* attr, int offset)
{
    void* src_addr;
    ...
    case TRICK_UNSIGNED_BITFIELD:
    {
        ...
        if (attr->size == sizeof(int))
        {
            bf = extract_unsigned_bitfield_any(
                *(unsigned int*)src_addr, attr->size, attr->index[0].start, attr->index[0].size);
        }
        else if (attr->size == sizeof(short))
        {
            bf = extract_unsigned_bitfield_any(
                *(unsigned short*)src_addr, attr->size, attr->index[0].start, attr->index[0].size);
        }
        else if (attr->size == sizeof(char))
        {
            bf = extract_unsigned_bitfield_any(
                *(unsigned char*)src_addr, attr->size, attr->index[0].start, attr->index[0].size);
        }
        else
        {
            message_publish(MSG_ERROR,
    ...
    case TRICK_OPAQUE_TYPE:
        chkpnt_os << std::endl
                  << "// ERROR - OPAQUE data type (" << attr->type_name << ") cannot be checkpointed." << std::endl;
        message_publish(
            MSG_ERROR, "Checkpoint Agent ERROR: OPAQUE data type (%s) cannot be checkpointed.\n", attr->type_name);
        break;
    default:
        chkpnt_os << "0";
        message_publish(MSG_ERROR, "Checkpoint Agent file %s: Unhandled Type (%d).\n", __FILE__, attr->type);
...
std::string Trick::ClassicCheckPointAgent::ref_string_from_ptr(void* pointer, ATTRIBUTES* attr, int curr_dim)
{
    ...
    else
    {
        ...
        if (alloc_info != NULL)
        {
            ...
            if ((attr != NULL) && (attr->type == TRICK_CHARACTER || attr->type == TRICK_UNSIGNED_CHARACTER)
                && ((curr_dim + 1) == attr->num_index))
            {
                std::stringstream ss;
            ...
            else
            {
                    ...
                    && ((attr->type != TRICK_STRUCTURED) || (attr->attr == alloc_info->attr)))
                {
                    ...
                    else
                    {
                        if (misalignment == 0)
                        {
                            ...
                            {
                                std::stringstream ss;
                                ss << "Checkpoint Agent ERROR: The name of the allocation at " << alloc_info->start
                                   << " is NULL."
                                   << "Therefore, Trick::ClassicCheckPointAgent::ref_string_from_ptr() can't generate "
                                      "a textual reference to it."
                                   << std::endl;
                                message_publish(MSG_ERROR, ss.str().c_str());
                        ...
                        {
                            message_publish(MSG_ERROR,
                                "Checkpoint Agent ERROR: Badly aligned pointer.\n"
                ...
                else if (alloc_info->type == TRICK_STRUCTURED)
                {
                    ...
                    if (alloc_info->name != NULL)
                    {
                        ...
                        rightside        = get_var_name(pointer, alloc_info->attr,
                            (char*)alloc_info->start + (alloc_elem_index * alloc_info->size), element_name.str(),
                            &attr);
                    ...
                    else
                    {
                        ...
                        ss << "Checkpoint Agent ERROR: The name of the allocation at " << alloc_info->start
                           << " is NULL."
                           << "Therefore, Trick::ClassicCheckPointAgent::ref_string_from_ptr() can't generate a "
                              "textual reference to it."
                           << std::endl;
                        message_publish(MSG_ERROR, ss.str().c_str());
                        reference_string = "ERROR - Allocation name is NULL";
                    }
                ...
                { // The type specifications don't match, and the right hand side is not structured
                    ...
                    if (attr != NULL)
                    {
                        ...
                        message_publish(MSG_ERROR,
                            "Checkpoint Agent ERROR: Type mismatch. Type specifications disagree.\n"
        ...
        else if ((attr != NULL) && ((curr_dim + 1) == attr->num_index))
        {
            ...
            else
            {
                ...
                message_publish(MSG_ERROR,
                    "Checkpoint Agent ERROR: Pointer <%p> in \"%s\" is not in Trick managed memory\n"
...
// MEMBER FUNCTION
void Trick::ClassicCheckPointAgent::write_rvalue(
    std::ostream& chkpnt_os, void* address, ATTRIBUTES* attr, int curr_dim, int offset)
{
    ...
    else if (curr_dim < attr->num_index)
    {
        ...
        { // Fixed dimension
            ...
            if (((curr_dim + 1) == attr->num_index) || (attr->index[curr_dim + 1].size == 0))
            {
    ...
    else
    {
        ...
        message_publish(MSG_ERROR,
            "Checkpoint Agent ERROR: The specified current dimension \"%d\" is greater\n"
...
void Trick::ClassicCheckPointAgent::assign_rvalue(
    std::ostream& chkpnt_os, void* address, ATTRIBUTES* attr, int curr_dim, int offset)
{
    std::string lname = left_side_name();
    ...
    if (!output_perm_check(attr))
    {
        if (debug_level)
        {
            message_publish(MSG_DEBUG,
                "Checkpoint Agent INFO: No assignment generated for \"%s\" "
    ...
    {
        message_publish(
            MSG_DEBUG, "Checkpoint Agent INFO: Generating assignment for [%p] %s.\n", (void*)address, lname.c_str());
    }
    ...
    else
    {
        ...
        bool should_add_decimal_comment = hexfloat_decimal_comment_checkpoint && hexfloat_checkpoint
            && (attr->type == TRICK_FLOAT || attr->type == TRICK_DOUBLE);
