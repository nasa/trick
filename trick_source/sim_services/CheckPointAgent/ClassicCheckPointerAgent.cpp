...
static ATTRIBUTES * findMember(ATTRIBUTES * A, long referenceOffset)
{
    ...
    while((A[i].name[0] != '\0'))
    {
        ...
        if((referenceOffset < (long)(A[i].offset)) || (referenceOffset >= (long)(A[i].offset + temp_size)))
        {
            i++;
        }
        else
...
void Trick::ClassicCheckPointAgent::write_singleton(std::ostream & chkpnt_os,
                                                    void * address,
                                                    ATTRIBUTES * attr,
                                                    int offset)
{
    ...
    switch(attr->type)
    {
        ...
        case TRICK_UNSIGNED_BITFIELD:
        {
            ...
            if(attr->size == sizeof(int))
            {
                bf = extract_unsigned_bitfield_any(*(unsigned int *)src_addr,
                                                   attr->size,
                                                   attr->index[0].start,
                                                   attr->index[0].size);
            }
            ...
            {
                bf = extract_unsigned_bitfield_any(*(unsigned short *)src_addr,
                                                   attr->size,
                                                   attr->index[0].start,
                                                   attr->index[0].size);
            }
            ...
            {
                bf = extract_unsigned_bitfield_any(*(unsigned char *)src_addr,
                                                   attr->size,
                                                   attr->index[0].start,
                                                   attr->index[0].size);
            }
        ...
        case TRICK_OPAQUE_TYPE:
            ...
            message_publish(MSG_ERROR,
                            "Checkpoint Agent ERROR: OPAQUE data type (%s) cannot be checkpointed.\n",
                            attr->type_name);
            break;
        default:
            chkpnt_os << "0";
            ...
            else
            {
                ...
                { // The type specifications don't match, and the right hand side is not structured
                    ...
                    if(attr != NULL)
                    {
                        ...
                        const char * right_type_spec = trickTypeCharString(alloc_info->type,
                                                                           alloc_info->user_type_name);
                        message_publish(MSG_ERROR,
    ...
    std::ostream & chkpnt_os, void * address, ATTRIBUTES * attr, int curr_dim, int offset)
{
    ...
    {
        message_publish(MSG_DEBUG,
                        "Checkpoint Agent INFO: Generating assignment for [%p] %s.\n",
                        (void *)address,
                        lname.c_str());
    }
