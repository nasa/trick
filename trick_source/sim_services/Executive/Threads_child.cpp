...
*/
static int call_next_job(Trick::JobData * curr_job,
                         Trick::ScheduledJobQueue & job_queue,
                         bool rt_nap,
                         long long curr_time_tics)
{
...
-# The child enters an infinite loop
    ...
    -# Switch if the child is a synchronous thread
        -# For each scheduled jobs whose next call time is equal to the current simulation time [@ref ScheduledJobQueue]
            -# Call call_next_job(Trick::JobData * curr_job, Trick::ScheduledJobQueue & job_queue, bool rt_nap, long
long curr_time_tics)
                ...
                switch(process_type)
                {
                    ...
                    case PROCESS_TYPE_AMF_CHILD:
                        ...
                        while((curr_job = top_of_frame_queue.get_next_job()) != NULL)
                        {
                            ...
                            if(ret != 0)
                            {
                                exec_terminate_with_return(ret,
                                                           curr_job->name.c_str(),
                                                           0,
                                                           " top_of_frame job did not return 0");
                            }
                        }
                        ...
                        while((curr_job = end_of_frame_queue.get_next_job()) != NULL)
                        {
                            ...
                            if(ret != 0)
                            {
                                exec_terminate_with_return(ret,
                                                           curr_job->name.c_str(),
                                                           0,
                                                           " end_of_frame job did not return 0");
                            }
                        }
                        break;
