...
void Trick::Executive::signal_handler(int sig)
{
    ...
    if (!debugger_command.empty())
    {
        ...
        {
            snprintf(
                command, sizeof(command), "%s -silent /proc/%d/exe %d", debugger_command.c_str(), getpid(), getpid());
            system(command);
        }
        else if (stack_trace == true)
        {
            snprintf(command, sizeof(command),
                ...
                "/proc/%d/exe %d",
                debugger_command.c_str(), getpid(), getpid());
            system(command);
        }
//=========================================================================================`
#elif __APPLE__
...
void Trick::Executive::ctrl_c_handler()
{
    ...
    {
        write(2,
            "\033[31mSetting exec_command = ExitCmd to shutdown simulation.\nIf process still unresponsive, type "
            "<CTRL-C> again for immediate exit.\033[0m\n",
            135);
