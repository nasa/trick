 ...
 * @return Executive
 */
Trick::Executive* exec_get_exec_cpp() { return (the_exec); }
 ...
 * C wrapper for Trick::Executive::add_depends_on_job
 */
extern "C" int exec_add_depends_on_job(
    const char* target_job_string, unsigned int t_instance, const char* depend_job_string, unsigned int d_instance)
{
    if (the_exec != NULL)
    {
        return the_exec->add_depends_on_job(
            std::string(target_job_string), t_instance, std::string(depend_job_string), d_instance);
    }
    return -1;
}
