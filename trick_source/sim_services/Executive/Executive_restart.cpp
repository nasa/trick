...
int Trick::Executive::restart()
{
    ...
    std::pair<std::multimap<std::string, Trick::JobData *>::iterator,
              std::multimap<std::string, Trick::JobData *>::iterator>
        ret;
    ...
    for(ii = 1; ii < threads.size(); ii++)
    {
           ...
           (curr_thread->process_type == PROCESS_TYPE_ASYNC_CHILD))
        {
            ...
            {
                curr_thread->amf_next_tics = time_tics + curr_thread->amf_cycle_tics -
                                             (time_tics % curr_thread->amf_cycle_tics);
            }
            else
            {
