...
int Trick::Executive::shutdown()
{
    ...
    if (prev_mode != Initialization)
    {
        ...
        {
            if ((threads[ii]->process_type == Trick::PROCESS_TYPE_ASYNC_CHILD
                    || threads[ii]->process_type == Trick::PROCESS_TYPE_AMF_CHILD)
                && threads[ii]->shutdown_wait_async)
            {
...
/* Get memory usage in MB for the calling process. Note that ru_maxrss returns long value in bytes on Mac and kilobytes
 * on Linux. */
#if __APPLE__
    ...
    message_publish(MSG_NORMAL,
        ...
        process_id, except_file.c_str(), except_message.c_str(), sim_start, get_sim_time(), sim_elapsed_time,
        user_cpu_time, kernal_cpu_time, sim_to_cpu, user_cpu_init, kernal_cpu_init, sim_mem, v_context_switch_init,
        iv_context_switch_init, v_context_switch_run, iv_context_switch_run);
