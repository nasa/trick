...
*/
int Trick::Executive::add_depends_on_job(std::string target_job_string,
                                         ...
                                         unsigned int d_instance)
{
    ...
    if(target_job == NULL)
    {
        message_publish(MSG_ERROR,
                        "add_depends_on_job: Could not find target job %s instance %d\n",
                        target_job_string.c_str(),
                        t_instance);
        return (-1);
    }
    ...
    {
        message_publish(MSG_ERROR,
                        "add_depends_on_job: Could not find depend job %s instance %d\n",
    ...
    {
        message_publish(MSG_ERROR,
                        "add_depends_on_job: target job %s and depend job %s on the same thread",
                        target_job_string.c_str(),
                        depend_job_string.c_str());
        ;
        return (-3);
