...
public:
    ...
    virtual int call_function(Trick::JobData *)
    {
        return 0;
    }
...
protected:
    ...
    int get_class_map_value(std::string job_class)
    {
        return exec.class_map[job_class];
    }

    Trick::ScheduledJobQueue * get_default_data_queue()
    {
        return &exec.default_data_queue;
    }

    Trick::ScheduledJobQueue * get_input_processor_queue()
    {
        return &exec.input_processor_queue;
    }

    Trick::ScheduledJobQueue * get_initialization_queue()
    {
        return &exec.initialization_queue;
    }

    Trick::ScheduledJobQueue * get_scheduled_queue()
    {
        return &exec.scheduled_queue;
    }

    Trick::ScheduledJobQueue * get_end_of_frame_queue()
    {
        return &exec.end_of_frame_queue;
    }

    Trick::ScheduledJobQueue * get_shutdown_queue()
    {
        return &exec.shutdown_queue;
    }

    Trick::ScheduledJobQueue * get_freeze_init_queue()
    {
        return &exec.freeze_init_queue;
    }

    Trick::ScheduledJobQueue * get_freeze_queue()
    {
        return &exec.freeze_queue;
    }

    Trick::ScheduledJobQueue * get_unfreeze_queue()
    {
        return &exec.unfreeze_queue;
    }

    Trick::JobData * get_all_jobs()
    {
        return exec.all_jobs_for_checkpoint;
    }

    Trick::JobData * get_job_from_all_jobs(unsigned int ii)
    {
        return &(exec.all_jobs_for_checkpoint[ii]);
    }

    Trick::SimObject * get_sim_object(unsigned int ii)
    {
        return exec.sim_objects[ii];
    }

    Trick::Threads * get_thread(unsigned int ii)
    {
        return exec.threads[ii];
    }
};
...
{
    // req.add_requirement("r_exec_jobs");
    //"The Executive Scheduler shall assign job class id of the following classes < 1000, default_data , input_processor
    //, initialization , top_of_frame , end_of_frame , shutdown , freeze_init , freeze_scheduled , freeze_automatic,
    // freeze , unfreeze, and exec_time_tic_changed"
...
TEST_F(ExecutiveTest, AdvanceSimTimeToNextJob)
{
    ...
    //"The Executive Scheduler shall advance simulation time to the time of the next lowest job call time if the next
    // lowest job call time is less than the next software frame boundary ") ;
...
TEST_F(ExecutiveTest, AdvanceSimTimeToBoundary)
{
    ...
    //  "The Executive Scheduler shall advance simulation time to the time of the next software boundary if the next
    //  lowest job call time is greater than or equal to the next software frame boundary ") ;
    Trick::Threads * curr_thread;
...
TEST_F(ExecutiveTest, Checkpoint)
{
    ...
    //"The Executive Scheduler shall copy all job information to a checkpointable array before a checkpoint is
    // executed") ;
