...
int Trick::Executive::add_jobs_to_queue(Trick::SimObject * in_sim_object, bool restart_flag)
{
    ...
    for(jj = 0; jj < in_sim_object->jobs.size(); jj++)
    {
        ...
        if(!temp_job->job_class_name.compare(0, 7, "system_") || !temp_job->job_class_name.compare(0, 9, "automatic") ||
        ...
        if((!temp_job->job_class_name.compare("instrumentation")) || (!temp_job->job_class_name.compare("malfunction")))
        {
