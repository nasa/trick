...
int Trick::Executive::check_all_job_cycle_times()
{
    ...
    for(ii = 0; ii < all_jobs_vector.size(); ii++)
    {
        ...
        if(temp_job->cycle < (1.0 / time_tic_value))
        {
            message_publish(MSG_WARNING,
                            "Cycle for (%s) is less than time tic value. cycle = %16.12f, time_tic = %16.12f\n",
                            temp_job->name.c_str(),
                            temp_job->cycle,
                            1.0 / time_tic_value);
    ...
    for(ii = 0; ii < all_jobs_vector.size(); ii++)
    {
        ...
        {
            message_publish(MSG_WARNING,
                            "Cycle for (%s) cannot be exactly scheduled with time tic value. "
    ...
    for(ii = 1; ii < threads.size(); ii++)
    {
        ...
        if(curr_thread->process_type == PROCESS_TYPE_AMF_CHILD and curr_thread->amf_cycle_tics == 0)
        {
            message_publish(MSG_ERROR,
                            "Asynchronous must finish thread %d has no cycle time set\n"
