...
Trick::ThreadTriggerBase::ThreadTriggerBase(ThreadTriggerType in_tt_type)
    : tt_type(in_tt_type)
{
}
...
/* ThreadTriggerMutex */
Trick::ThreadTriggerMutex::ThreadTriggerMutex()
    : ThreadTriggerBase(TT_MUTEX)
{
    pthread_cond_init(&go_cv, NULL);
    pthread_mutex_init(&go_mutex, NULL);
}

void Trick::ThreadTriggerMutex::init() { pthread_mutex_lock(&go_mutex); }
...
void Trick::ThreadTriggerMutex::wait() { pthread_cond_wait(&go_cv, &go_mutex); }

void Trick::ThreadTriggerMutex::dump(std::ostream& oss) { oss << "    trigger type = mutex" << std::endl; }

/* ThreadTriggerFlag */
Trick::ThreadTriggerFlag::ThreadTriggerFlag()
    : ThreadTriggerBase(TT_FLAG)
    , frame_trigger(false)
{
}

void Trick::ThreadTriggerFlag::fire() { frame_trigger = true; }
...
void Trick::ThreadTriggerFlag::dump(std::ostream& oss) { oss << "    trigger type = flag" << std::endl; }
...
Trick::ThreadTriggerEventFD::ThreadTriggerEventFD()
    : ThreadTriggerBase(TT_EVENTFD)
{
    efd = eventfd(0, 0);
}
...
void Trick::ThreadTriggerEventFD::dump(std::ostream& oss) { oss << "    trigger type = eventfd" << std::endl; }

#include <linux/futex.h>
#include <syscall.h>

/* ThreadTriggerFutex */
Trick::ThreadTriggerFutex::ThreadTriggerFutex()
    : ThreadTriggerBase(TT_FUTEX)
    , futex_addr(0)
{
}

void Trick::ThreadTriggerFutex::fire() { syscall(SYS_futex, &futex_addr, FUTEX_WAKE, 1, NULL, NULL, 0); }
...
void Trick::ThreadTriggerFutex::dump(std::ostream& oss) { oss << "    trigger type = futex" << std::endl; }
#else
...
Trick::ThreadTriggerContainer::ThreadTriggerContainer()
    : ttBase(&ttMutex)
{
}

Trick::ThreadTriggerBase* Trick::ThreadTriggerContainer::getThreadTrigger() { return ttBase; }
