...
int Trick::Executive::remove_jobs(Trick::SimObject * in_sim_object)
{
    ...
    for(std::multimap<std::string, Trick::JobData *>::iterator mit = all_tagged_jobs.begin();
        mit != all_tagged_jobs.end();)
    {
        if((*mit).second->parent_object == in_sim_object)
        {
    ...
    for(std::vector<Trick::JobData *>::iterator jit = in_sim_object->jobs.begin(); jit != in_sim_object->jobs.end();
        ++jit)
    {
