...
int Trick::Executive::call_input_processor()
{
    ...
    /* A restart can be called by a call to the input processor.  If that happens the Executive restart job will clear
       this queue and restart it at the beginning.  We don't want to call the input processor jobs again in this case.
    ...
    while(!restart_called and (curr_job = input_processor_queue.get_next_job()) != NULL)
    {
        ...
        {
            init_log_stream << "input_processor," << curr_job->name << ','
                            << (double)(end - start) / clock_tics_per_sec() << '\n';
        }
        if(ret != 0)
        {
