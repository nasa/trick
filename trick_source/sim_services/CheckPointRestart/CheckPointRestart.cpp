...
const char* Trick::CheckPointRestart::get_output_file() { return output_file.c_str(); }

const char* Trick::CheckPointRestart::get_load_file() { return load_checkpoint_file_name.c_str(); }
...
int Trick::CheckPointRestart::do_checkpoint(std::string file_name, bool print_status)
{
    ...
    if (print_status)
    {
        ...
        {
            message_publish(
                MSG_INFO, "Dumped Checkpoint (floating point values in Hexadecimal) %s.\n", file_name.c_str());
        }
        else
        {
...
void Trick::CheckPointRestart::load_checkpoint(std::string file_name) { load_checkpoint_file_name = file_name; }
...
int Trick::CheckPointRestart::instrument_job_before(Trick::JobData* instrument_job)
{
    ...
    /** @li if target_job specified, instrument job will only be inserted before it
        note: when more than one target job with same name, this will insert instrument job for 1st occurrence of
       target_job in each queue
     */
