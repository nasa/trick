...
bool ParsedDeclaration::parseDeclaration()
{
    ...
    if (!errorCondition)
    {
        ...
        if ((token == Token::Asterisk) || (token == Token::Identifier) || (token == Token::LeftParen)
...
std::string ParsedDeclaration::getTypeSpecifier() { return typeSpec; }

std::string ParsedDeclaration::getVariableName() { return varName; }

unsigned int ParsedDeclaration::getNumberOfDimensions() { return ndims; }
