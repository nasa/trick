...
bool ArrayDataType::initArrayDataType(
...
DataType* ArrayDataType::clone() const { return new ArrayDataType(*this); }
...
// MEMBER FUNCTION
unsigned int ArrayDataType::getElementCount() const { return elementCount; }

// MEMBER FUNCTION
const DataType* ArrayDataType::getSubType() const { return subType; }
    ...
    if ((index >= 0) && (index < elementCount))
    {
        ...
        if (nextToken == Token::RightBracket)
        {
            ...
            else if (nextToken == Token::period)
            {
                ...
                {
                    errorCondition
                        |= ((const CompositeDataType*)searchType)->getMemberInfo(lexer, elementAddress, varAccessInfo);
                }
                else
                {
                    errorCondition = true;
            ...
            else if (nextToken == Token::LeftBracket)
            {
                ...
                {
                    errorCondition
                        |= ((const ArrayDataType*)searchType)->getElementInfo(lexer, elementAddress, varAccessInfo);
                }
                else
                {
                    errorCondition = true;
