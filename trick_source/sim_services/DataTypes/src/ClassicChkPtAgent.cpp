...
// MEMBER FUNCTION
bool ClassicChkPtAgent::writePointerValue(
    std::ostream& out_s, MemMgr* memMgr, void* address, const PointerDataType* pointerDataType)
{
    bool errorCondition        = false;
...
// MEMBER FUNCTION
bool ClassicChkPtAgent::writeArrayAssignments(
    std::ostream& out_s, MemMgr* memMgr, void* address, const ArrayDataType* arrayDataType)
{
    bool errorCondition         = false;
    ...
    if (makeCompactArrays
        && ((elementTypeClass == TypeClass::PRIMITIVE) || (elementTypeClass == TypeClass::ENUMERATION)
            || (elementTypeClass == TypeClass::POINTER)))
    {
        ...
        for (int ii = 0; ii < elementCount; ii++)
        {
            ...
            {
                errorCondition
                    |= writePointerValue(rightHandSide, memMgr, elementAddress, (const PointerDataType*)elementType);
            }
            else
            {
...
// MEMBER FUNCTION
bool ClassicChkPtAgent::writeCompositeAssignments(
    std::ostream& out_s, MemMgr* memMgr, void* address, const CompositeDataType* compDataType)
{
    bool errorCondition = false;
...
// MEMBER FUNCTION
bool ClassicChkPtAgent::getArraySubName(std::ostream& nameStream, void* searchAddress, const DataType* searchType,
    void* baseAddress, const ArrayDataType* baseType)
{
    bool errorCondition = false;
...
// MEMBER FUNCTION
bool ClassicChkPtAgent::getCompositeSubName(std::ostream& nameStream, void* searchAddress, const DataType* searchType,
    void* baseAddress, const CompositeDataType* baseType)
{
    bool errorCondition = false;
    size_t baseTypeSize = baseType->getSize();
...
// MEMBER FUNCTION
bool ClassicChkPtAgent::getSubName(std::ostream& nameStream, void* searchAddress, const DataType* searchType,
    void* baseAddress, const DataType* baseType)
{
    bool errorCondition = false;
    size_t baseTypeSize = baseType->getSize();
    ...
    if ((searchAddress >= (char*)baseAddress) && (searchAddress < (char*)baseAddress + baseTypeSize))
    {
        ...
        else
        {
            ...
            {
                errorCondition = getCompositeSubName(
                    nameStream, searchAddress, searchType, baseAddress, (const CompositeDataType*)baseType);
            }
            else if (typeClass == TypeClass::ARRAY)
            {
                errorCondition = getArraySubName(
                    nameStream, searchAddress, searchType, baseAddress, (const ArrayDataType*)baseType);
            }
            else
            {
                errorCondition = true;
...
// MEMBER FUNCTION
void ClassicChkPtAgent::setReducedCheckpoint(bool flag) { reducedCheckpoint = flag; }

// MEMBER FUNCTION
void ClassicChkPtAgent::setHexfloatCheckpoint(bool flag) { hexfloatCheckpoint = flag; }

// MEMBER FUNCTION
void ClassicChkPtAgent::setMakeCompactArrays(bool flag) { makeCompactArrays = flag; }
