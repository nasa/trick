...
// MEMBER FUNCTION
void* MemMgr::declare_var(const std::string& typeSpecifier, const std::string& variableName,
    unsigned int dimensionsCount, int* dimensions, void* suppliedAllocation)
{
    ...
    try
    {
        ...
        newAllocInfo = new AllocInfo(
            variableName, typeSpecifier, dimensionsCount, dimensions, typeDictionary, suppliedAllocation);
...
// MEMBER FUNCTION
void* MemMgr::declare_var(const std::string& declaration, void* suppliedAllocation)
{
...
// MEMBER FUNCTION
void* MemMgr::declare_var(
    const std::string& typeSpecName, const std::string& variableName, int n, void* suppliedAllocation)
{
...
// MEMBER FUNCTION
void* MemMgr::declare_var(const std::string& typeSpecName, int n, void* suppliedAllocation)
{
...
void MemMgr::write_checkpoint(std::ostream& outStream, std::vector<AllocInfo*>& allocInfoList)
{
    ...
    for (int ii = 0; ii < itemCount; ii++)
    {
        ...
        if (storageClass == StorageClass::LOCAL)
        {
            ...
            {
                currentCheckPointAgent->writeDeclaration(
                    outStream, allocInfo->getSerialName(), allocInfo->getDataType());
            }
        }
    }
...
// MEMBER FUNCTION
const DataType* MemMgr::getDataType(std::string typeName) { return typeDictionary->getDataType(typeName); }
