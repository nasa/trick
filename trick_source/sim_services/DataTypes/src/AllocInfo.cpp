...
void AllocInfo::initialize(const std::string& varName, const std::string& typeSpecName, unsigned int dimsCount,
    int* dims, TypeDictionary* typeDictionary, void* suppliedAllocation)
{
    ...
    {
        if (ownDataType->getSize() > 0)
        { // FIXME: Is this correct? Shouldn't it be: ( ownDataType->getSubType()->getSize() > 0 ) ?
        ...
        else
        {
            ...
            errorStream << "ERROR: Data type \"" << ownDataType->makeDeclaration(name, true)
                        << "\" is a useless type because it's size can only ever be 0." << std::endl;
            delete ownDataType;
    ...
    else
    {
        std::stringstream errorStream;
        errorStream << "ERROR: Data type \"" << ownDataType->makeDeclaration(name, true)
                    << "\" can not be validated. Perhaps the typespecifier \"" << typeSpecifierName
                    << "\" is mis-spelled?" << std::endl;
        delete ownDataType;
        throw std::logic_error(errorStream.str());
    }
...
// CONSTRUCTOR
AllocInfo::AllocInfo(const std::string& varName, const std::string& typeSpecName, unsigned int dimsCount, int* dims,
    TypeDictionary* typeDictionary, void* suppliedAllocation)
{
    initialize(varName, typeSpecName, dimsCount, dims, typeDictionary, suppliedAllocation);
}

// CONSTRUCTOR
AllocInfo::AllocInfo(const std::string& declaration, TypeDictionary* typeDictionary, void* suppliedAllocation)
{
...
std::string AllocInfo::getName() const { return name; }
...
// PUBLIC MEMBER FUNCTION
unsigned int AllocInfo::getSerialNumber() const { return serialNumber; }

// PUBLIC MEMBER FUNCTION
void* AllocInfo::getStart() const { return start; }

// PUBLIC MEMBER FUNCTION
int AllocInfo::getSize() const { return dataType->getSize(); }

// PUBLIC MEMBER FUNCTION
std::string AllocInfo::getTypeSpecifierName() const { return typeSpecifierName; }

// PUBLIC MEMBER FUNCTION
unsigned int AllocInfo::getDimensionsCount() const { return dimensionsCount; }
...
// PUBLIC MEMBER FUNCTION
StorageClass::e AllocInfo::getStorageClass() const { return storageClass; }

// PUBLIC MEMBER FUNCTION
void AllocInfo::clear() const { dataType->clearValue(start); }
...
// PUBLIC MEMBER FUNCTION
void AllocInfo::appendDependenciesfromComposite(
    const CompositeDataType* compDataType, void* address, MemMgr* memMgr, std::vector<AllocInfo*>& dependencies)
{
    ...
    for (int ii = 0; ii < memberCount; ii++)
    {
        ...
        if (ioControl == 1 || ioControl == 3)
        {
            ...
            if (memberAddress != NULL)
            {
                ...
                {
                    const CompositeDataType* memberCompositeDataType
                        = dynamic_cast<const CompositeDataType*>(memberDataType);
...
// PUBLIC MEMBER FUNCTION
void AllocInfo::appendDependenciesfromArray(
    const ArrayDataType* arrayDataType, void* address, MemMgr* memMgr, std::vector<AllocInfo*>& dependencies)
{
