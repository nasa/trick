...
// CONSTRUCTOR
CompositeDataType::CompositeDataType(TypeDictionary* typeDictionary, std::string name, size_t structSize,
    void* (*allocator)(int), void (*deAllocator)(void*))
{
    this->is_valid       = false;
    this->name           = name;
...
DataType* CompositeDataType::clone() const { return new CompositeDataType(*this); }
...
// MEMBER FUNCTION
size_t CompositeDataType::getSize() const { return structSize; }
...
void CompositeDataType::assignValue(void* address, Value* value) const
{
    ...
    if (composite_value)
    {
        ...
        {
            std::cerr << "ERROR: number of members (" << memberCount << ") and number of values (" << num_values
                      << ") is not the same.";
        }
...
// MEMBER FUNCTION
std::string CompositeDataType::getTypeSpecName() const { return name; }

// MEMBER FUNCTION
void CompositeDataType::addRegularMember(
    std::string memberName, int member_offset, std::string typeSpecName, unsigned int n_dims, int dims[])
{
    int memberCount = memberList.size();
    for (int ii = 0; ii < memberCount; ii++)
    {
    ...
    memberList.push_back(new NormalStructMember(memberName, member_offset, typeDictionary, typeSpecName, n_dims, dims));
}

// MEMBER FUNCTION
void CompositeDataType::addStaticMember(
    std::string memberName, void* memberAddress, std::string typeSpecName, unsigned int n_dims, int dims[])
{
    ...
    memberList.push_back(new StaticStructMember(memberName, memberAddress, typeDictionary, typeSpecName, n_dims, dims));
}

// MEMBER FUNCTION
int CompositeDataType::getMemberCount() const { return ((int)memberList.size()); }

// MEMBER FUNCTION
StructMember* CompositeDataType::getStructMember(const int index) const { return (memberList[index]); }
...
bool CompositeDataType::getMemberInfo(LexicalAnalyzer* lexer, void* baseAddress, VarAccessInfo& varAccessInfo)
{
    ...
    if (nextToken == Token::Identifier)
    {
        ...
        int memberCount        = memberList.size();
        int ii                 = 0;
        while ((ii < memberCount) && (memberName != memberList[ii]->getName()))
        {
            ii++;
        }
        if (ii < memberCount)
        {
            ...
            if (!errorCondition)
            {
                ...
                else if (nextToken == Token::period)
                {
                    ...
                    {
                        errorCondition |= ((const CompositeDataType*)memberDataType)
                                              ->getMemberInfo(lexer, memberAddress, varAccessInfo);
                    }
                    else
                    {
                        errorCondition = true;
                ...
                else if (nextToken == Token::LeftBracket)
                {
                    ...
                    {
                        errorCondition
                            |= ((const ArrayDataType*)searchType)->getElementInfo(lexer, memberAddress, varAccessInfo);
                    }
                    else
                    {
                        errorCondition = true;
