...
// CONSTRUCTOR
StaticStructMember::StaticStructMember(std::string memberName, void* memberAddress, TypeDictionary* typeDictionary,
...
StructMember* StaticStructMember::clone() const { return new StaticStructMember(*this); }
...
bool StaticStructMember::validate()
{
    if (!is_valid)
    {
        ...
        else
        {
            ...
            {
                std::cerr << "ERROR: Type \"" << typeSpecName << "\" not found in the TypeDictionary." << std::endl;
            }
        }
    }
    return is_valid;
...
// MEMBER FUNCTION
const DataType* StaticStructMember::getDataType() { return subType; }

// MEMBER FUNCTION
void* StaticStructMember::getAddress() { return memberAddress; }
