...
public:
    ...
    void * declare_var(const std::string & typeSpecName,
    ...
    //    void* declare_extern_var( void* address, TRICK_TYPE type, std::string class_name, int n_stars, std::string
    //    var_name, int n_cdims, int *cdims);
    ...
    //    int add_attr_info(std::string& type_name , ATTRIBUTES* attr , const char * file_name = "None" , unsigned int
    //    line_num = 0 ) ; int add_template_name_trans(std::string template_name , std::string attr_name) ; void*
    //    add_var( TRICK_TYPE   type, const char*  class_name, VAR_DECLARE* var_declare, char* units); int add_vars(
    //    TRICK_TYPE  type, const char* class_name, VAR_LIST*   var_list, char*       units); int ref_allocate( REF2* R,
    //    int num ) ; REF2 *ref_attributes( const char* name); int ref_assignment( REF2* R, V_TREE* V); int ref_var(
    //    REF2* R, char* name); int ref_dim( REF2* R, V_DATA* V); int ref_name(REF2 * R, char *name); int
    //    get_enumerated(const char* name, V_DATA* v_data); (see EnumDictionary.hh) int get_size(char *ptr); int
    //    get_truncated_size(char *ptr); int io_get_fixed_truncated_size(char *ptr, ATTRIBUTES * A, char *str, int dims,
