...
Trick::RtiStager::~RtiStager() { the_rtis = NULL; }
...
int Trick::RtiStager::Fire(unsigned int thread_id)
{
    ...
    {
        message_publish(
            MSG_ERROR, "%s:%s:%d RTI Fire failed, thread %d out of range\n", __FILE__, __func__, __LINE__, thread_id);
        return -1;
    }
...
void Trick::RtiStager::AddInjectorExecutor(Trick::RtiExec* rtie) { executors.push_back(rtie); }
