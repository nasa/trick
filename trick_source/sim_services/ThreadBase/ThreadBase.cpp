...
std::string Trick::ThreadBase::get_name() { return name; }

void Trick::ThreadBase::set_name(std::string in_name) { name = in_name; }

pthread_t Trick::ThreadBase::get_pthread_id() { return pthread_id; }

pid_t Trick::ThreadBase::get_pid() { return pid; }
...
#if __linux__
cpu_set_t* Trick::ThreadBase::get_cpus() { return cpus; }
...
#endif
#if __APPLE__
void* Trick::ThreadBase::get_cpus() { return NULL; }
        ...
        else
        {
            ...
            if (proc_priority < min_priority)
            {
                message_publish(MSG_WARNING,
                    ...
                    rt_priority, (max_priority - min_priority) + 2);
...
int Trick::ThreadBase::execute_priority()
{
    ...
    if (rt_priority > 0)
    {
        ...
        if (proc_priority < min_priority)
        {
            ...
            message_publish(
                MSG_WARNING, "The Darwin thread priority range is %d:%d (min:max).\n", min_priority, max_priority);
            message_publish(
                MSG_WARNING, "The corresponding minimum Trick priority is %d.\n", (max_priority - min_priority) + 1);
        ...
        if (ret != 0)
        {
            ...
            message_publish(
                MSG_ERROR, "The Darwin thread priority range is %d:%d (min:max).\n", min_priority, max_priority);
        }
        else
        {
            ...
            message_publish(
                MSG_INFO, "The Darwin thread priority range is %d:%d (min:max).\n", min_priority, max_priority);
        }
    }
...
int Trick::ThreadBase::create_thread()
{
    ...
    {
        message_publish(
            MSG_ERROR, "create_thread called on thread %s (%p) which has already been started.\n", name.c_str(), this);
        return 0;
    }
...
void Trick::ThreadBase::test_shutdown() { test_shutdown(NULL, NULL); }
...
void Trick::ThreadBase::thread_shutdown() { thread_shutdown(NULL, NULL); }
