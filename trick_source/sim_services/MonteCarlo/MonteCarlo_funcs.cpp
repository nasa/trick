...
void Trick::MonteCarlo::set_enabled(bool in_enabled) { this->enabled = in_enabled; }

bool Trick::MonteCarlo::get_enabled() { return enabled; }

void Trick::MonteCarlo::set_dry_run(bool in_dry_run) { this->dry_run = in_dry_run; }

bool Trick::MonteCarlo::get_dry_run() { return dry_run; }
...
bool Trick::MonteCarlo::get_localhost_as_remote() { return localhost_as_remote; }
...
bool Trick::MonteCarlo::get_custom_slave_dispatch() { return custom_slave_dispatch; }

void Trick::MonteCarlo::set_timeout(double in_timeout) { this->timeout = in_timeout; }

double Trick::MonteCarlo::get_timeout() { return timeout; }

void Trick::MonteCarlo::set_max_tries(unsigned int in_max_tries) { this->max_tries = in_max_tries; }

unsigned int Trick::MonteCarlo::get_max_tries() { return max_tries; }
...
std::string Trick::MonteCarlo::get_user_cmd_string() { return user_cmd_string; }

const char* Trick::MonteCarlo::get_user_cmd_string_c_str() { return user_cmd_string.c_str(); }
...
std::string Trick::MonteCarlo::get_custom_pre_text() { return custom_pre_text; }

const char* Trick::MonteCarlo::get_custom_pre_text_c_str() { return custom_pre_text.c_str(); }
...
std::string Trick::MonteCarlo::get_custom_post_text() { return custom_post_text; }

const char* Trick::MonteCarlo::get_custom_post_text_c_str() { return custom_post_text.c_str(); }

void Trick::MonteCarlo::set_verbosity(Trick::MonteCarlo::Verbosity in_verbosity) { this->verbosity = in_verbosity; }

Trick::MonteCarlo::Verbosity Trick::MonteCarlo::get_verbosity() { return verbosity; }
...
unsigned int Trick::MonteCarlo::get_num_runs() { return num_runs; }

unsigned int Trick::MonteCarlo::get_num_results() { return num_results; }

unsigned int Trick::MonteCarlo::get_slave_id() { return slave_id; }
...
void Trick::MonteCarlo::get_ranges(std::vector<MonteRange*>& ranges) { ranges = run_ranges; }
    ...
    for (std::vector<Trick::MonteVar*>::const_iterator i = variables.begin(); i != variables.end(); ++i)
    {
        if ((*i)->name.compare(variable->name) == 0)
        {
            message_publish(MSG_WARNING,
                "Monte WARNING: Cannot add new MonteVar \"%s\", variable of that name already exists.\n",
                variable->name.c_str());
            return;
        }
    }
...
const std::vector<Trick::MonteVar*>& Trick::MonteCarlo::get_variables() { return variables; }
 ...
 * the list, ensuring that every slave has a unique id. This function has no effect if the slave has been previously
 * added.
 */
...
/**
 * @par Detailed Design:
 * This function has an effect only if the slave exists and is in the MC_STOPPING, MC_UNRESPONSIVE_STOPPING, or
 * MC_STOPPED state.
 */
...
/**
 * @par Detailed Design:
 * This function has an effect only if the slave exists and is in the MC_READY, MC_RUNNING, or MC_UNRESPONSIVE_RUNNING
 * state.
 */
    ...
    if (enabled && is_slave())
    {
        ...
        else
        {
            if (verbosity >= MC_ERROR)
                message_publish(
                    MSG_ERROR, "Monte [%s:%d] Failed to connect to master.\n", machine_name.c_str(), slave_id);
        }
    }
    return 0;
}
    ...
    else
    {
        if (verbosity >= MC_ERROR)
        {
            message_publish(MSG_ERROR,
                "Monte [Master] Run %d has reached its maximum allowed tries and has been skipped.\n",
                slave.current_run->id);
        }
...
void Trick::MonteCarlo::resolve_run(MonteSlave& slave, MonteRun::ExitStatus exit_status)
{
    ...
    {
        message_publish(
            MSG_INFO, "Monte [Master] Run %d has been resolved as: %d.\n", slave.current_run->id, exit_status);
    }
}
...
void Trick::MonteCarlo::check_timeouts()
{
    ...
    for (std::vector<Trick::MonteSlave*>::size_type i = 0; i < slaves.size(); ++i)
    {
        ...
        {
            /**
             * <ul><li> This run might have been redispatched due to a previous timeout for which the slave actually
             * returned data later. Only process this timeout if the run hasn't been resolved yet.
             */
            ...
            slaves[i]->state = slaves[i]->state == MonteSlave::MC_RUNNING ? MonteSlave::MC_UNRESPONSIVE_RUNNING
                                                                          : MonteSlave::MC_UNRESPONSIVE_STOPPING;
        }
    }
}
...
Trick::MonteRun* Trick::MonteCarlo::get_next_dispatch()
{
    ...
    while (!runs.empty())
    {
        ...
        else
        {
            ...
            {
                message_publish(
                    MSG_WARNING, "Monte [Master] Run %d is out of range and has been skipped.\n", curr_run->id);
            }
    ...
    if (curr_run->num_tries == 0)
    {
        ...
        for (std::vector<std::string>::size_type i = 0; i < variables.size(); ++i)
        {
            ...
            if (curr_run->variables.back() == "EOF")
            {
                ...
                {
                    message_publish(MSG_WARNING,
                        "Monte [Master] File variable '%s' reached end-of-file. Reducing number of runs to %d.\n",
...
unsigned int Trick::MonteCarlo::get_current_run() { return (current_run); }

void Trick::MonteCarlo::set_current_run(int run_num) { current_run = run_num; }

void Trick::MonteCarlo::set_listen_device_port(int port_number) { listen_device.port = port_number; }
...
int Trick::MonteCarlo::get_listen_device_port() { return listen_device.port; }

int Trick::MonteCarlo::get_connection_device_port() { return connection_device.port; }
...
int Trick::MonteCarlo::read(char* data, int size) { return tc_read(&connection_device, data, size); }
