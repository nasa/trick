        ...
        if(tc_connect(&connection_device) == TC_SUCCESS)
        {
            ...
            if(verbosity >= MC_ALL)
            {
                message_publish(MSG_INFO,
                                "Monte [%s:%d] Sending run exit status to master: %d\n",
                                machine_name.c_str(),
                                slave_id,
                                exit_status);
            }
...
void Trick::MonteCarlo::resolve_run(MonteSlave & slave, MonteRun::ExitStatus exit_status)
{
    ...
    {
        message_publish(MSG_INFO,
                        "Monte [Master] Run %d has been resolved as: %d.\n",
                        slave.current_run->id,
                        exit_status);
    }
}
    ...
    for(std::vector<Trick::MonteSlave *>::size_type i = 0; i < slaves.size(); ++i)
    {
               ...
               timeout)
        {
            ...
            if(slaves[i]->current_run->exit_status == MonteRun::MC_RUN_INCOMPLETE)
            {
                ...
                {
                    message_publish(MSG_ERROR,
                                    "Monte [Master] %s:%d has not responded for run %d.\n",
                                    slaves[i]->machine_name.c_str(),
                                    slaves[i]->id,
                                    slaves[i]->current_run->id);
                }
...
Trick::MonteRun * Trick::MonteCarlo::get_next_dispatch()
{
    ...
    while(!runs.empty())
    {
        ...
        else
        {
            ...
            {
                message_publish(MSG_WARNING,
                                "Monte [Master] Run %d is out of range and has been skipped.\n",
                                curr_run->id);
            }
