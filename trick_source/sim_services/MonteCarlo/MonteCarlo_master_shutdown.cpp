...
void Trick::MonteCarlo::master_shutdown()
{
    ...
    FILE* file_ptr;
    if (open_file(run_directory + std::string("/run_summary"), &file_ptr) == -1)
        exit(-2);
...
void Trick::MonteCarlo::print_statistics(FILE** fp)
{
    fprintf(*fp,
        ...
        num_runs, num_results - failed_runs.size() - error_runs.size(), failed_runs.size(), error_runs.size(),
    ...
    for (std::vector<MonteSlave*>::size_type j = 0; j < slaves.size(); ++j)
    {
        fprintf(*fp, "%3d.  %20s %7u  (%6.2f%%)  %12.2lf  %12.2lf\n", slaves[j]->id, slaves[j]->machine_name.c_str(),
            slaves[j]->num_results, num_results ? slaves[j]->num_results / (float)num_results * 100 : 0,
            slaves[j]->cpu_time, slaves[j]->num_results ? slaves[j]->cpu_time / slaves[j]->num_results : 0);
    ...
    if (error_runs.size())
    {
        ...
        {
            fprintf(
                *fp, "RUN_%05d MonteRun::ExitStatus = %s (%d)\n", run->id, run->exit_status_string(), run->exit_status);
        }
    }
}
