...
void Trick::MonteCarlo::print_statistics(FILE ** fp)
{
    fprintf(
        *fp,
        ...
        num_runs,
        num_results - failed_runs.size() - error_runs.size(),
        failed_runs.size(),
        error_runs.size(),
    ...
    {
        fprintf(*fp,
                "%3d.  %20s %7u  (%6.2f%%)  %12.2lf  %12.2lf\n",
                slaves[j]->id,
                slaves[j]->machine_name.c_str(),
    ...
    if(error_runs.size())
    {
        ...
        {
            fprintf(*fp,
                    "RUN_%05d MonteRun::ExitStatus = %s (%d)\n",
                    run->id,
                    run->exit_status_string(),
                    run->exit_status);
        }
    }
}
