...
Trick::MonteVarFile::~MonteVarFile() { delete input_file_stream; }
...
std::string Trick::MonteVarFile::get_next_value()
{
    ...
    if (input_file_stream->good())
    {
        ...
        if ((column == 0) || (column > ntokens))
        {
            ...
            snprintf(string, sizeof(string),
                "Trick:MonteVarFile An invalid column number %u, valid column numbers are 1 - %u", column, ntokens);
    ...
    char string[100];
    snprintf(
        string, sizeof(string), "Trick:MonteVarFile the input file \"%s\" is not open for reading", file_name.c_str());
    exec_terminate_with_return(-1, __FILE__, __LINE__, string);
...
void Trick::MonteVarFile::set_column(unsigned int in_column) { column = in_column; }
