...
int Trick::MonteCarlo::slave_process_run() {
    ...
    /** <li>Parent process: */
        ...
        MonteRun::ExitStatus exit_status = signal == SIGALRM ? MonteRun::RUN_TIMED_OUT : MonteRun::RUN_DUMPED_CORE;
        if (verbosity >= ERROR) {
    ...
    /** <li> Child process: */
        ...
        input[size] = '\0';
        if ( ip_parse(input) != 0 ) {
            exit(MonteRun::PROBLEM_PARSING_INPUT);
        }
        ...
        if (access(output_dir.c_str(), F_OK) != 0) {
            if (mkdir(output_dir.c_str(), 0775) == -1) {
                exit(MonteRun::CANT_CREATE_OUTPUT_DIR);
            }
        }
