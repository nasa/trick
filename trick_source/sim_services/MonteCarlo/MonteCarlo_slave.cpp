    ...
    while(true)
    {
        ...
        if(tc_accept(&listen_device, &connection_device) != TC_SUCCESS)
        {
            if(verbosity >= MC_ERROR)
            {
                message_publish(MSG_ERROR,
                                "Monte [%s:%d] Lost connection to Master. Shutting down.\n",
                                machine_name.c_str(),
                                slave_id);
            }
            slave_shutdown();
        ...
        if(tc_read(&connection_device, (char *)&command, (int)sizeof(command)) != (int)sizeof(command))
        {
            if(verbosity >= MC_ERROR)
            {
                message_publish(
                    MSG_ERROR,
                    "Monte [%s:%d] Lost connection to Master while receiving instructions. Shutting down.\n",
                    machine_name.c_str(),
                    slave_id);
            }
            slave_shutdown();
        ...
        switch(command = ntohl(command))
        {
            ...
            case MonteSlave::MC_PROCESS_RUN:
                /**
                 * <ul><li> MonteSlave::MC_PROCESS_RUN: Call #slave_process_run. This will return a non-zero value when
                 * run in a child process to indicate that this function should return so that the sim can complete.
                 */
            ...
            case MonteSlave::MC_SHUTDOWN:
                ...
                if(verbosity >= MC_INFORMATIONAL)
                {
                    message_publish(MSG_INFO,
                                    "Monte [%s:%d] Shutdown command received from Master. Shutting down.\n",
                                    machine_name.c_str(),
                                    slave_id);
                }
                slave_shutdown();
            ...
            default:
                ...
                {
                    message_publish(MSG_ERROR,
                                    "Monte [%s:%d] Unrecognized command %d received from Master. Shutting down.\n",
                                    machine_name.c_str(),
                                    slave_id,
                                    command);
                }
