...
void Trick::MonteCarlo::initialize_slave(Trick::MonteSlave * slave_to_init)
{
    ...
    ss << slave_to_init->S_main_name << " " << command_line_args_get_input_file() << " --monte_host " << machine_name
       << " --monte_sync_port " << listen_device.port << " --monte_client_id " << slave_to_init->id << " -O "
       << slave_output_directory;
    buffer += ss.str();
    ...
    if(verbosity >= MC_INFORMATIONAL)
    {
        message_publish(MSG_INFO,
                        ...
                        slave_to_init->machine_name.c_str(),
                        slave_to_init->id,
                        buffer.c_str());
    }
...
void Trick::MonteCarlo::default_slave_dispatch_pre_text(Trick::MonteSlave * slave_to_init, std::string & buffer)
{
    ...
    if(!localhost_as_remote && (equals_ignore_case(slave_to_init->machine_name, machine_name) ||
                ...
                if(slave_to_init->user_remote_shell.empty())
                {
                    ...
                    if(verbosity >= MC_ERROR)
                    {
                        message_publish(MSG_WARNING,
                                        "Monte: TRICK_USER_REMOTE_SH specified for Slave %s:%d, but no shell given.\n"
                                        "Defaulting to %s.\n",
                                        machine_name.c_str(),
                                        slave_to_init->id,
                                        slave_to_init->user_remote_shell.c_str());
                    }
                }
