...
Trick::MonteVarRandom::MonteVarRandom(
    std::string in_name, Distribution in_distribution, std::string in_unit, StlEngine in_engine)
    : engineType(in_engine)
    , randist()
    , stlGenPtr(0)
{
    this->name         = in_name;
    ...
    double unused                               = 0.0;
    StlRandomGenerator::StlDistribution stlDist = static_cast<StlRandomGenerator::StlDistribution>(distribution);
    StlRandomGenerator::StlEngine stlEngine     = static_cast<StlRandomGenerator::StlEngine>(engineType);
    if (engineType != NO_ENGINE)
    {
        ...
        {
        case GAUSSIAN:
            stlGenPtr = StlRandomGeneratorFactory::newGenerator(randist.mu, randist.sigma, seed, stlDist, stlEngine);
            break;
        case FLAT:
            // StlRandomGenerator min,max are always absolute
            stlGenPtr = StlRandomGeneratorFactory::newGenerator(
                get_absolute_min(), get_absolute_max(), seed, stlDist, stlEngine);
            break;
        case POISSON:
            stlGenPtr = StlRandomGeneratorFactory::newGenerator(randist.mu, unused, seed, stlDist, stlEngine);
            break;
        default:
            break;
        }
...
Trick::MonteVarRandom::~MonteVarRandom() { delete stlGenPtr; }
...
void Trick::MonteVarRandom::set_sigma_range(int sigma_range) { randist.sigma_range = sigma_range; }
...
void Trick::MonteVarRandom::set_uniform_generator(uniform_generator uniform) { randist.uniform = uniform; }
    ...
    if (stlGenPtr)
    {
        ...
        if (count >= 100)
        {
            ...
            snprintf(string, sizeof(string),
                "Trick:MonteVarRandom failed to generate a random value for variable \"%s\"\n", name.c_str());
...
/// are called.)
void Trick::MonteVarRandom::updateStlRandom()
{
    if (stlGenPtr)
    {
        switch (randist.type)
...
const TRICK_GSL_RANDIST& Trick::MonteVarRandom::get_random_distribution() { return randist; }
