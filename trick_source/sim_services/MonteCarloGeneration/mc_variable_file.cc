...
MonteCarloVariableFile::MonteCarloVariableFile(
    const std::string& var_name, const std::string& filename_, size_t column_number_, size_t first_column_number_)
...
*****************************************************************************/
void MonteCarloVariableFile::initialize_file()
{
    ...
    if (first_column_number > first_var->get_column_number())
    {
        std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__)
            + ",  Configuration Error\nIn configuring " + "the file for variable "
            + first_var->get_variable_name().c_str()
    ...
    do
    {
        ...
        line.empty() || line.front() == '#' || line.front() == '/' || std::all_of(line.begin(), line.end(), isspace));
    // Rewind the file
...
*****************************************************************************/
void MonteCarloVariableFile::generate_assignment()
{
...
*****************************************************************************/
void MonteCarloVariableFile::register_dependent(MonteCarloVariableFile* new_var)
{
    if (new_var == NULL)
    {
        std::string message = std::string("File: ") + __FILE__ + ", line " + std::to_string(__LINE__)
    ...
    {
        std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__)
            + ", Invalid configuration\nError in " + "attempting to make " + new_var->get_variable_name().c_str()
            + " be dependent on " + variable_name.c_str() + ".\n" + new_var->get_variable_name().c_str()
            + " cannot be marked as " + "dependent when it has dependencies of its own.\nThe dependency "
    ...
    if (new_var->max_skip != max_skip)
    {
        std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__)
            ...
            + "as a setting for the maximum number of lines to skip of " + std::to_string(max_skip) + "\n";
        message_publish(MSG_ERROR, message.c_str());
    }
...
*****************************************************************************/
void MonteCarloVariableFile::process_line()
{
    size_t skip_count = 0;
    if (max_skip > 0)
    {
    ...
    for (auto it = dependents.begin(); it != dependents.end(); ++it)
    {
        ...
        {
            std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__)
                + ", Malformed data file\nData file " + "for variable " + variable_name.c_str()
                + " includes this line:\n" + line.c_str() + "\nWhich has only "
                + std::to_string(current_column_number - 1) + " values.\nVariable " + variable_name.c_str()
                + "  uses the value from position " + std::to_string(column_number)
...
*****************************************************************************/
bool MonteCarloVariableFile::sort_by_col_num(MonteCarloVariableFile* left, MonteCarloVariableFile* right)
{
...
std::string MonteCarloVariableFile::summarize_variable() const
{
    ...
    ss << MonteCarloVariable::summarize_variable() << ", max_skip=" << max_skip << ", is_dependent=" << is_dependent
