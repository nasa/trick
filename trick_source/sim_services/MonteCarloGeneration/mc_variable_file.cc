...
Constructor
*****************************************************************************/
MonteCarloVariableFile::MonteCarloVariableFile(const std::string & var_name,
    ...
    do
    {
        ...
        if(file.eof())
        {
            std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__) +
                                  ...
                                  " contains no recognized lines of data\n" + "Required for variable " +
                                  variable_name.c_str() + ".\n";
            message_publish(MSG_ERROR, message.c_str());
            exec_terminate_with_return(1, __FILE__, __LINE__, message.c_str());
        }
...
void MonteCarloVariableFile::register_dependent(MonteCarloVariableFile * new_var)
{
    ...
    {
        std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__) +
                              ", Invalid configuration\nError in " + "attempting to make " +
                              new_var->get_variable_name().c_str() + " be dependent on " + variable_name.c_str() +
    ...
    for(auto it = dependents.begin(); it != dependents.end(); ++it)
    {
        ...
        if(current_column_number < next_column_needed)
        {
            std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__) +
                                  ...
                                  std::to_string(column_number) + ", which does not exist in this line\n";
            exec_terminate_with_return(1, __FILE__, __LINE__, message.c_str());
        }
