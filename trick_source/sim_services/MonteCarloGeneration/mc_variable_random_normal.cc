...
Constructor
*****************************************************************************/
MonteCarloVariableRandomNormal::MonteCarloVariableRandomNormal(const std::string & var_name,
                                                               unsigned int seed,
                                                               double mean,
                                                               double stdev)
...
*****************************************************************************/
void MonteCarloVariableRandomNormal::generate_assignment()
{
    ...
    if(truncated_low && truncated_high)
    {
        if(min_value > max_value)
        {
            std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__) +
                                  ...
                                  " the specified minimum allowable value (" + std::to_string(min_value) +
                                  ") >= the specified maximum allowable" + +" value (" + std::to_string(max_value) +
                                  ").\nOne or both of the"
    ...
    size_t num_tries = 0;
    while((truncated_high && assignment_d > max_value) || (truncated_low && assignment_d < min_value))
    {
        ...
        {
            std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__) +
                                  " Random value truncation failure\n" + "Could not generate a value for " +
                                  variable_name.c_str() +
                                  "  within the specified domain within\nthe specified maximum " + "number of tries (" +
            ...
            if(!truncated_high)
            { // i.e. truncated-low only
                assignment_d = min_value;
            }
            else if(!truncated_low)
            { // i.e. truncated-high only
                assignment_d = max_value;
            }
            else
            { // i.e. truncated both sides
...
std::string MonteCarloVariableRandomNormal::summarize_variable() const
{
    ...
    ss << MonteCarloVariable::summarize_variable() << std::string(", dispersion=Normal")
       << ", mean=" << distribution.mean() << ", stddev=" << distribution.stddev() << ", min_value=" << min_value
       << ", max_value=" << max_value << ", seed=" << seed_m;
    return (ss.str());
}
...
*****************************************************************************/
void MonteCarloVariableRandomNormal::truncate(double limit, TruncationType truncType)
{
    if(limit < 0)
    {
        std::string message = std::string("File: ") + __FILE__ + ", Line " + std::to_string(__LINE__) +
                              ", Out-of-domain error\nNegative " + "double-sided truncation specified for variable " +
    ...
    {
        std::string message = std::string("File: ") + __FILE__ + ", Line " + std::to_string(__LINE__) +
                              ",  Configuration error\nZero truncation " + "for specified for variable " +
                              variable_name.c_str() + "  which " + "will produce a fixed point\n";
        message_publish(MSG_WARNING, message.c_str());
    }
...
*****************************************************************************/
void MonteCarloVariableRandomNormal::truncate(double min, double max, TruncationType truncType)
{
...
*****************************************************************************/
void MonteCarloVariableRandomNormal::truncate_low(double min, TruncationType truncType)
{
    switch(truncType)
    {
        ...
        default:
            std::string message = std::string("File: ") + __FILE__ + ", Line " + std::to_string(__LINE__) +
                                  ", Invalid TruncationType\nInvalid " +
                                  "truncation type passed to truncate_low for variable " + variable_name.c_str() +
...
*****************************************************************************/
void MonteCarloVariableRandomNormal::truncate_high(double max, TruncationType truncType)
{
    switch(truncType)
    {
        case StandardDeviation:
        ...
        default:
            std::string message = std::string("File: ") + __FILE__ + ", Line: " + std::to_string(__LINE__) +
                                  ", Invalid TruncationType\nInvalid " +
                                  "truncation type passed to truncate_high for variable " + variable_name.c_str() +
...
*****************************************************************************/
void MonteCarloVariableRandomNormal::untruncate()
{
    truncated_low = false;
    truncated_high = false;
}
