    ...
    switch(remote_shell)
    {
        ...
        case TRICK_USER_REMOTE_SH:
            if(user_remote_shell.empty())
            {
                message_publish(
                    MSG_WARNING,
                    "TRICK_USER_REMOTE_SH specified for Slave startup, but no shell given.\nDefaulting to %s.\n",
                    unix_ssh);
...
int Trick::SlaveInfo::read_slave_status()
{
    ...
    if(activated == true)
    {
        ...
        // fixup: is it possible we won't get slave's Exit command over socket when it terminates?, set it here if that
        // happens
        ...
        if(exec_command != MS_ExitCmd)
        {
            ...
            {
                case(MS_ErrorCmd):
                    /** @li if the user has set a reconnect_wait_limit, continue on if we are still under that limit,
                       otherwise if the current slave mode cannot be read, exit the master if sync_error_terminate ==
                       true, otherwise set the activated flag to false */
                ...
                case(MS_ExitCmd):
                    ...
                    if(sync_error_terminate == true)
                    {
                        message_publish(MSG_WARNING,
                                        "sync_error_terminate is true: Slave is exiting, so master is terminating.\n");
                    ...
                    else
                    {
                        ...
                        {
                            message_publish(MSG_WARNING,
                                            "Master will wait %f seconds for slave to reconnect.\n",
                                            reconnect_wait_limit);
                            // make reads (shared mem connection) return quickly so we don't overrun waiting for
                            // reconnect
                        ...
                        {
                            message_publish(MSG_WARNING,
                                            "reconnect_wait_limit: 0.0 - Master will stop communicating with slave.\n");
                            activated = false;
                        }
                        return (0);
                    }
    ...
    if(activated == true)
    {
        ...
        if(full_path_name.length() > sizeof(chkpnt_name) - 1)
        {
            message_publish(MSG_ERROR,
                            "Master could not send checkpoint name to slave because name too long (max = %d).\n",
                            sizeof(chkpnt_name));
    ...
    if(enabled)
    {
        for(ii = 0; ii < slaves.size(); ii++)
        {
            ...
            slaves[ii]->connection->write_time((long long)((get_checkpoint_pre_init() << 2) +
                                                           (get_checkpoint_post_init() << 1) + (get_checkpoint_end())));
        }
