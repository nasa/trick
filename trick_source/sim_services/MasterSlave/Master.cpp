        ...
        if (exec_command != MS_ExitCmd)
        {
            ...
            case (MS_ErrorCmd):
                /** @li if the user has set a reconnect_wait_limit, continue on if we are still under that limit,
                   otherwise if the current slave mode cannot be read, exit the master if sync_error_terminate == true,
            ...
            case (MS_ExitCmd):
                ...
                {
                    message_publish(
                        MSG_WARNING, "sync_error_terminate is true: Slave is exiting, so master is terminating.\n");
                ...
                else
                {
                    ...
                    {
                        message_publish(
                            MSG_WARNING, "Master will wait %f seconds for slave to reconnect.\n", reconnect_wait_limit);
                    ...
                    {
                        message_publish(
                            MSG_WARNING, "reconnect_wait_limit: 0.0 - Master will stop communicating with slave.\n");
                        activated = false;
                    }
                    return (0);
                }
...
int Trick::Master::process_sim_args() { return (0); }
