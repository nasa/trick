    ...
    if ((enabled) and (activated))
    {
        ...
        if (master_time == MS_ERROR_TIME)
        {
            if (sync_error_terminate == true)
            {
                ...
                message_publish(
                    MSG_ERROR, "Slave lost sync with master. sync_error_terminate is true: Slave is terminating.\n");
        ...
        case (MS_ErrorCmd):
            ...
            else
            {
                if (slave_command != MS_FreezeCmd)
                {
                    /** @li if reading the master mode command returned an error freeze the sim if sync_error_terminate
                     * == false */
                    activated = false;
                    message_publish(MSG_ERROR,
                        "Slave lost sync with master. sync_error_terminate is false: Slave is entering Freeze mode.\n");
