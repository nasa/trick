    ...
    if((enabled) and (activated))
    {
        ...
        if(master_time == MS_ERROR_TIME)
        {
            if(sync_error_terminate == true)
            {
                ...
                message_publish(MSG_ERROR,
                                "Slave lost sync with master. sync_error_terminate is true: Slave is terminating.\n");
            ...
            else
            {
                if(slave_command != MS_FreezeCmd)
                {
                    ...
                    message_publish(
                        MSG_ERROR,
                        "Slave lost sync with master. sync_error_terminate is false: Slave is entering Freeze mode.\n");
        ...
        switch(command)
        {
            case(MS_ErrorCmd):
                if(sync_error_terminate == true)
                {
                    /** @li if reading the master mode command returned an error exit the sim if sync_error_terminate ==
                     * true */
                    message_publish(
                        MSG_ERROR,
                        "Slave lost sync with master. sync_error_terminate is true: Slave is terminating.\n");
                ...
                else
                {
                    if(slave_command != MS_FreezeCmd)
                    {
                        /** @li if reading the master mode command returned an error freeze the sim if
                         * sync_error_terminate == false */
                        activated = false;
                        message_publish(MSG_ERROR,
                                        "Slave lost sync with master. sync_error_terminate is false: Slave is entering "
                                        "Freeze mode.\n");
                        exec_set_exec_command(FreezeCmd);
                        connection->write_command(MS_FreezeCmd);
                        return (0);
            ...
            case(MS_ChkpntLoadAsciiCmd): // Master tells slave to load an ascii checkpoint
                ...
                break;
            default:
                /** @li if reading the master mode command returned an Executive mode, set the slave mode command to the
                 * master mode command */
