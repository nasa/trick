...
int Trick::EventManager::add_event(Trick::Event* in_event)
{
    ...
    else
    {
        message_publish(MSG_WARNING,
            "Event thread number is not in range: thread %d requested, only %d child threads in simulation.\n",
...
*/
void Trick::EventManager::add_event_processor(Trick::EventProcessor* in_ep) { event_processors.push_back(in_ep); }
...
int Trick::EventManager::restart()
{
    ...
    for (ii = 0; ii < num_active_events; ii++)
    {
        ...
        if (active_events[ii]->get_before_after() == Trick::EVENT_BEFORETARGET)
        {
            ...
            add_event_before(
                active_events[ii], active_events[ii]->get_target_name(), active_events[ii]->get_target_inst());
        }
