...
void Trick::FrameLog::add_recording_vars_for_frame()
{
    ...
    for(ii = 0; ii < num_threads; ii++)
    {
        ...
        {
            asprintf(&job_name,
                     "JOB_data_record_group_frame_userjobs_C%d.data_record.%2.2f(end_of_frame)",
                     ii,
        ...
        {
            asprintf(&job_name,
                     "JOB_data_record_group_frame_userjobs.data_record.%2.2f(end_of_frame)",
                     drg_users[ii]->write_job->frame_id);
        }
        trick_jobs.push_back(std::string(job_name));
        new_ref->reference = job_name;
...
int Trick::FrameLog::set_max_samples(int num)
{
    ...
    if(num > 0)
    {
        ...
        for(ii = 0; ii < num_threads; ii++)
        {
            ...
            timeline_other[ii] = (Trick::timeline_t *)realloc(timeline_other[ii],
                                                              tl_max_samples * sizeof(Trick::timeline_t));
        }
...
int Trick::FrameLog::create_DP_Product_dir()
{
    ...
    if(ret == -1)
    {
        message_publish(MSG_WARNING,
                        "Could not create DP_Product directory for Frame Logging. DP files not created.\n");
        perror("mkdir");
    }
    return ret;
}
...
int Trick::FrameLog::create_DP_job_files()
{
    ...
    fprintf(fpx, "            <curve>\n                <var>sys.exec.out.time</var>\n");
    fprintf(fpx,
            "                <var line_color=\"red\" label=\"Frame Sched Time\">%s.rt_sync.frame_time</var>\n",
            rt_sim_object_name.c_str());
    fprintf(fpx, "            </curve>\n");
    fprintf(fpx, "        </plot>\n");
    fprintf(fpx, "    </page>\n");
    ...
    for(page_count = 0; page_count < total_pages; page_count++)
    {
        ...
        for(ii = 0; ii < plots_per_page and (page_count * plots_per_page + ii + 1) < drg_users.size(); ii++)
        {
            fprintf(fpx, "        <plot grid=\"yes\">\n");
            fprintf(fpx,
                    "            <title>Child thread %u Frame Scheduled Jobs</title>\n",
            ...
            fprintf(fpx,
                    "                <var line_color=\"red\" label=\"Frame Sched Time\">%s.frame_time</var>\n",
    ...
    for(ii = 0; ii < num_threads; ii++)
    {
        ...
        for(drb_it = drg_users[ii]->rec_buffer.begin() + 1; drb_it != drg_users[ii]->rec_buffer.end(); ++drb_it)
        {
            ...
            fprintf(fpx,
                    "        <plot grid=\"yes\" background_color=\"%s\">\n            <title>%s</title>\n",
                    bg_color,
                    (*drb_it)->name.c_str());
            fprintf(fpx, "            <xaxis> <label>Time</label> <units>s</units> </xaxis>\n");
