...
void Trick::FrameLog::add_recording_vars_for_jobs()
{
    ...
    for (ii = 0; ii < all_jobs_vector.size(); ii++)
    {
                ...
                || (!all_jobs_vector[ii]->job_class_name.compare("end_of_frame"))))
        {
            ...
            /** @li add job frame id, job class and cycle time to displayed job name, and prepend "JOB_" so it stands
             * out in quickplot */
            if (all_jobs_vector[ii]->job_class >= exec_get_scheduled_start_index())
            {
                ...
                { // add child thread # to name
                    asprintf(&job_name, "JOB_%s_C%d.%2.2f(%s_%2.3f)", all_jobs_vector[ii]->name.c_str(),
                        all_jobs_vector[ii]->thread, all_jobs_vector[ii]->frame_id,
                        all_jobs_vector[ii]->job_class_name.c_str(), all_jobs_vector[ii]->cycle);
                }
            }
            else
            { // non-scheduled class
...
void Trick::FrameLog::add_recording_vars_for_frame()
{
    ...
    for (ii = 0; ii < num_threads; ii++)
    {
        ...
        if (ii > 0)
        {
            asprintf(&job_name, "JOB_data_record_group_frame_userjobs_C%d.data_record.%2.2f(end_of_frame)", ii,
...
int Trick::FrameLog::set_max_samples(int num)
{
    ...
    if (num > 0)
    {
        ...
        for (ii = 0; ii < num_threads; ii++)
        {
            ...
            timeline_other[ii]
                = (Trick::timeline_t*)realloc(timeline_other[ii], tl_max_samples * sizeof(Trick::timeline_t));
        }
...
int Trick::FrameLog::shutdown()
{
    ...
    for (int thread_num = 0; thread_num < num_threads; thread_num++)
    {
        ...
        {
            snprintf(log_buff, sizeof(log_buff), "%s/log_newtimelineC%d.csv", command_line_args_get_output_dir(),
                thread_num);
        }
    ...
    if (fp_time_main == NULL)
    {
        ...
        fprintf(fp_time_other,
            "trick_frame_log.frame_log.job_trickinit_id {--},trick_frame_log.frame_log.job_userinit_id {--}\n");
    }
...
void Trick::FrameLog::set_clock(Trick::Clock& in_clock) { clock = in_clock; }
...
int Trick::FrameLog::create_DP_Product_dir()
{
    ...
    {
        message_publish(
            MSG_WARNING, "Could not create DP_Product directory for Frame Logging. DP files not created.\n");
        perror("mkdir");
    }
    return ret;
}
...
int Trick::FrameLog::create_DP_job_files()
{
    ...
    fprintf(fpx,
        "                <var line_color=\"darkgreen\" label=\"Overrun/Underrun\">%s.rt_sync.frame_overrun</var>\n",
    ...
    for (ii = 0; ii < num_threads; ii++)
    {
        ...
        {
            if (!(*drb_it)->name.compare(0, 5, "frame"))
                continue;
...
int Trick::FrameLog::create_DP_timeline_files()
{
    ...
    fprintf(fpx, "            <curve>\n                <var>sys.exec.out.time</var>\n");
    fprintf(fpx,
        "                <var line_color=\"red\" label=\"User Job Id\">trick_frame_log.frame_log.job_user_id</var>\n");
    fprintf(fpx, "            </curve>\n");
    fprintf(fpx, "        </plot>\n");
    ...
    fprintf(fpx, "            <curve>\n                <var>sys.exec.out.time</var>\n");
    fprintf(fpx,
        "                <var line_color=\"darkgreen\" label=\"Trick Job "
        "Id\">trick_frame_log.frame_log.job_trick_id</var>\n");
    fprintf(fpx, "            </curve>\n");
    fprintf(fpx, "        </plot>\n");
    ...
    if (num_threads > 1)
    {
        ...
        for (jj = 1; jj < num_threads; jj++)
        {
            fprintf(fpx, "            <curve>\n                <var>sys.exec.out.time</var>\n");
            fprintf(fpx,
                "                <var line_color=\"blue\" label=\"Child%d Job "
                "Id\">trick_frame_log.frame_log.job_userC%d_id</var>\n",
                jj, jj);
            fprintf(fpx, "            </curve>\n");
        }
        fprintf(fpx, "        </plot>\n");
    ...
    fprintf(fpx, "            <curve>\n                <var>sys.exec.out.time</var>\n");
    fprintf(fpx,
        "                <var line_color=\"red\" label=\"User Job "
        "Id\">trick_frame_log.frame_log.job_userinit_id</var>\n");
    fprintf(fpx, "            </curve>\n");
    fprintf(fpx, "        </plot>\n");
    fprintf(fpx, "        <plot grid=\"yes\">\n            <title>Trick Job Timeline</title>\n");
    fprintf(fpx, "            <xaxis> <label>Time</label> <units>s</units> </xaxis>\n");
    ...
    fprintf(fpx,
        "                <var line_color=\"darkgreen\" label=\"Trick Job "
        "Id\">trick_frame_log.frame_log.job_trickinit_id</var>\n");
    fprintf(fpx, "            </curve>\n");
    fprintf(fpx, "        </plot>\n");
    fprintf(fpx, "    </page>\n");
    ...
    fprintf(fpx, "            <curve>\n                <var>sys.exec.out.time</var>\n");
    fprintf(fpx,
        "                <var line_color=\"red\" label=\"Trick Job "
        "Id\">trick_frame_log.frame_log.job_userinit_id</var>\n");
    fprintf(fpx, "            </curve>\n");
    fprintf(fpx, "        </plot>\n");
    fprintf(fpx, "    </page>\n</product>");
