...
-# Sets the real_time clock to the incoming class [@ref disable]
*/
const char* Trick::RealtimeSync::clock_get_name() { return rt_clock->get_name(); }
...
void Trick::RealtimeSync::get_sim_start_time() { sim_start_time = default_clock->wall_clock_time(); }

void Trick::RealtimeSync::get_sim_end_init_time() { sim_end_init_time = default_clock->wall_clock_time(); }

void Trick::RealtimeSync::get_sim_end_time() { sim_end_time = default_clock->wall_clock_time(); }
...
template <size_t N> class Run_Ratio
{
    public:
        Run_Ratio()
            : num_samples(0)
        ...
        operator double() const
        {
            ...
            else if (num_samples < N)
            {
                ...
                {
                    return round(exec_get_software_frame() * (num_samples - 1) * exec_get_time_tic_value() * rt_ratio
                               / (double)(deltaVal) * 100.0)
                        / 100.0;
                }
            }
            else
            {
                ...
                {
                    return round(exec_get_software_frame() * (N - 1) * exec_get_time_tic_value() * rt_ratio
                               / (double)(deltaVal) * 100.0)
                        / 100.0;
                }
            }
        ...
        if (frame_overrun_cnt >= rt_max_overrun_cnt || frame_overrun_time >= rt_max_overrun_time_tics)
        {
            ...
            if (rt_overrun_freeze == true)
            {
                ...
                message_publish(MSG_ERROR,
                    ...
                    frame_overrun_cnt, rt_max_overrun_cnt, (double)(frame_overrun_time / tics_per_sec),
                    rt_max_overrun_time);
                exec_freeze();
            }
            else
...
bool Trick::RealtimeSync::is_active() { return active; }
