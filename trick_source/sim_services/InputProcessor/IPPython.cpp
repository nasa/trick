...
// Constructor
Trick::IPPython::IPPython()
    : Trick::InputProcessor::InputProcessor()
    , units_conversion_msgs(true)
{
    the_pip = this;
    return;
}
...
bool Trick::IPPython::get_units_conversion_msgs() { return units_conversion_msgs; }
...
int Trick::IPPython::init()
{
    ...
    PyRun_SimpleString("import sys\n"
                       ...
                       "    sys.path.append(os.path.join(os.environ['VIRTUAL_ENV'], \"lib\", "
    ...
    if (verify_input)
    {
        ...
        ss << "print('{0} SHA1: {1}'.format(input_file,hashlib.sha1(open(input_file, 'rb').read()).hexdigest()))"
           << std::endl;
        PyRun_SimpleString(ss.str().c_str());
