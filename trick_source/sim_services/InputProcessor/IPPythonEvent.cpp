...
int Trick::IPPythonEvent::condition_var(int num, std::string varname, std::string comment)
{
    ...
    if(ref == NULL)
    {
        message_publish(MSG_WARNING,
                        "Event condition variable %s not found. Setting condition to False.\n",
                        varname.c_str());
...
int Trick::IPPythonEvent::condition_job(int num, std::string jobname, std::string comment)
{
    ...
    {
        message_publish(MSG_WARNING,
                        "Event condition job %s not found. Setting condition to False.\n",
                        jobname.c_str());
        condition(num, "False", comment, NULL, NULL);
    }
    else
...
bool Trick::IPPythonEvent::condition_fired(int num)
{
    ...
    {
        message_publish(MSG_WARNING,
                        "Event condition fired state not returned. Condition number %d is invalid.\n",
                        num);
    }
    return (false);
}
...
int Trick::IPPythonEvent::condition_fired_count(int num)
{
    ...
    {
        message_publish(MSG_WARNING,
                        "Event condition fired count not returned. Condition number %d is invalid.\n",
                        num);
    }
    return (0);
}
        ...
        for(ii = 0; ii < condition_count; ii++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        exec_terminate_with_return(python_ret,
                                                   __FILE__,
                                                   __LINE__,
                                                   "Python error in event condition processing");
                    }
                }
                if(return_val)
                {
