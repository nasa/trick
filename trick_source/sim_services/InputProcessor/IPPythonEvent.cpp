...
void Trick::IPPythonEvent::set_python_processor(Trick::IPPython* in_ip) { ip = in_ip; }

void Trick::IPPythonEvent::set_mtv(Trick::MTV* in_mtv) { mtv = in_mtv; }
...
// Command to manually fire the event next cycle and hold it fired (enter manual mode, bypasses normal condition
// processing).
...
void Trick::IPPythonEvent::add() { added = true; }

void Trick::IPPythonEvent::remove() { added = false; }

// Command to turn on info messages
void Trick::IPPythonEvent::set_event_info_msg_on() { info_msg = true; }

// Command to turn off info messages
void Trick::IPPythonEvent::set_event_info_msg_off() { info_msg = false; }

void Trick::IPPythonEvent::terminate_on_event_parse_error(bool on_off) { terminate_sim_on_event_python_error = on_off; }
...
void Trick::IPPythonEvent::restart()
{
    ...
    {
        if (condition_list[jj]->cond_type == 1)
        { // condition variable
            condition_list[jj]->ref = ref_attributes(condition_list[jj]->str.c_str());
        }
        if (condition_list[jj]->cond_type == 2)
        { // condition job
...
// Command to create a new condition using a model variable (or reset an existing condition variable), num is index
// starting at 0.
int Trick::IPPythonEvent::condition_var(int num, std::string varname, std::string comment)
{
    ...
    {
        message_publish(
            MSG_WARNING, "Event condition variable %s not found. Setting condition to False.\n", varname.c_str());
...
int Trick::IPPythonEvent::condition_job(int num, std::string jobname, std::string comment)
{
    ...
    {
        message_publish(
            MSG_WARNING, "Event condition job %s not found. Setting condition to False.\n", jobname.c_str());
        condition(num, "False", comment, NULL, NULL);
    }
    else
    {
...
// Command to create a new condition and set its input string (or reset an existing condition string), num is index
// starting at 0.
int Trick::IPPythonEvent::condition(int num, std::string str, std::string comment, REF2* ref, JobData* job)
{
    ...
    if (num == condition_count)
    {
        ...
        {
            condition_list = (Trick::condition_t**)TMM_declare_var_s("Trick::condition_t*[1]");
        }
        else
        {
            condition_list = (Trick::condition_t**)TMM_resize_array_1d_a(condition_list, condition_count);
        }
        condition_list[num] = (Trick::condition_t*)TMM_declare_var_s("Trick::condition_t");
    ...
    if ((num >= 0) && (num < condition_count))
    {
        ...
        else if (job != NULL)
        {
            ...
            condition_list[num]->cond_type = 0;
...
bool Trick::IPPythonEvent::condition_fired(int num)
{
    ...
    {
        message_publish(
            MSG_WARNING, "Event condition fired state not returned. Condition number %d is invalid.\n", num);
    }
    return (false);
}
...
int Trick::IPPythonEvent::condition_fired_count(int num)
{
    ...
    {
        message_publish(
            MSG_WARNING, "Event condition fired count not returned. Condition number %d is invalid.\n", num);
    }
    return (0);
}
...
// Command to create a new action and set its input string (or reset an existing action string), num is index starting
// at 0.
int Trick::IPPythonEvent::action(int num, std::string str, std::string comment, JobData* job, int act_type)
{
    ...
    if (num == action_count)
    {
        ...
        action_list[num] = (Trick::action_t*)TMM_declare_var_s("Trick::action_t");
                ...
                else
                {
                    ...
                    if (python_ret != 0 && terminate_sim_on_event_python_error)
                    {
                        exec_terminate_with_return(
                            python_ret, __FILE__, __LINE__, "Python error in event condition processing");
                    }
                }
                if (return_val)
                {
