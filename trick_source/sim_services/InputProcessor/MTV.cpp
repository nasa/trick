...
int Trick::MTV::send_event_data() {
    ...
    for (ii=0; ii < mtv_count; ii++) {
        ...
        for (jj=0; jj< mtv_list[ii]->condition_count; jj++) {
            snprintf(varname, sizeof(varname), "%s[%u][0].condition_list[%d].comment", mtv_name, ii, jj);
            var_add(varname);
        }
        for (jj=0; jj< mtv_list[ii]->action_count; jj++) {
            snprintf(varname, sizeof(varname), "%s[%u][0].action_list[%d].comment", mtv_name, ii, jj);
            var_add(varname);
        }
