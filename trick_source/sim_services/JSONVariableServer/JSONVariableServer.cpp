...
const char* Trick::JSONVariableServer::get_hostname() { return listen_dev.hostname; }

unsigned short Trick::JSONVariableServer::get_port() { return port; }
...
std::string Trick::JSONVariableServer::get_source_address() { return source_address; }
...
int Trick::JSONVariableServer::restart()
{
    ...
    if (user_port_requested)
    {
        ...
        {
            message_publish(
                MSG_ERROR, "ERROR: Could not establish listen port %d for JSON Variable Server. Aborting.\n", port);
            return (-1);
        }
    }
    else
