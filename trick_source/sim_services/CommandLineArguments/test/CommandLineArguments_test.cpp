...
TEST_F(CommandLineArgumentsTest, GetArgcInitiallyZero) { EXPECT_EQ(cmd_args.get_argc(), 0); }
...
TEST_F(CommandLineArgumentsTest, GetArgcMultipleArgs)
{
    ...
    char* args[] = {(char*)getTestExecutable(), (char*)"RUN_test/input.py", (char*)"-O",
        (char*)output_dir->path().c_str(), (char*)"--some-flag"};
    int nargs    = 5;
...
{
    char* args[]
        = {(char*)"./S_main", (char*)"RUN_test/input.py", (char*)"-O", (char*)"output1", (char*)"-O", (char*)"output2"};
    int nargs = 6;

    // EXPECT_EXIT checks that the code calls exit() with status 1
    // and that stderr contains the expected error message
    EXPECT_EXIT(
        cmd_args.process_sim_args(nargs, args), ::testing::ExitedWithCode(1), "ERROR: Multiple -O or -OO flags found");
}
...
{
    char* args[] = {
        (char*)"./S_main", (char*)"RUN_test/input.py", (char*)"-OO", (char*)"output1", (char*)"-OO", (char*)"output2"};
    int nargs = 6;
...
{
    char* args[] = {
        (char*)"./S_main", (char*)"RUN_test/input.py", (char*)"-O", (char*)"output1", (char*)"-OO", (char*)"output2"};
    int nargs = 6;
...
TEST_F(CommandLineArgumentsTest, OutputFlagWithoutDirDeathTest)
{
    ...
    EXPECT_EXIT(cmd_args.process_sim_args(nargs, args), ::testing::ExitedWithCode(1),
