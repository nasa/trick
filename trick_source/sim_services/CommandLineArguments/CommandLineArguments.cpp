...
int Trick::CommandLineArguments::get_argc() { return (argc); }

std::vector<std::string>& Trick::CommandLineArguments::get_argv() { return (argv); }

std::string Trick::CommandLineArguments::get_output_dir() { return (output_dir); }

std::string& Trick::CommandLineArguments::get_output_dir_ref() { return (output_dir); }

std::string Trick::CommandLineArguments::get_user_output_dir() { return (user_output_dir); }

std::string& Trick::CommandLineArguments::get_user_output_dir_ref() { return (user_output_dir); }

std::string Trick::CommandLineArguments::get_input_file() { return (input_file); }

std::string& Trick::CommandLineArguments::get_input_file_ref() { return (input_file); }

std::string Trick::CommandLineArguments::get_default_dir() { return (default_dir); }

std::string& Trick::CommandLineArguments::get_default_dir_ref() { return (default_dir); }

std::string Trick::CommandLineArguments::get_cmdline_name() { return (cmdline_name); }

std::string& Trick::CommandLineArguments::get_cmdline_name_ref() { return (cmdline_name); }
    ...
    while (cur_index != full_dir.size())
    {
        ...
        else
        {
            ...
            if (info.st_mode & S_IFDIR)
            {
                ...
                {
                    // Not writeable
                    std::cerr << "Intermediate directory " << cur_dir
                              << " is not writable, unable to create output directory." << std::endl;
                    return 1;
                }
            }
            else
            {
                ...
                std::cerr << "Intermediate directory " << cur_dir
                          << " is not a directory, unable to create output directory." << std::endl;
                return 1;
            }
        }
    }
...
int Trick::CommandLineArguments::process_sim_args(int nargs, char** args)
{
    ...
    if (argc > 1)
    {
        ...
        {
            std::cerr << "\nERROR: while accessing input file directory \"" << run_dir
        ...
        if (access(output_dir.c_str(), F_OK) != 0)
        {
            ...
            {
                std::cerr << "\nERROR: While trying to create output directory \"" << output_dir
                          << "\" : " << std::strerror(errno) << std::endl;
                exit(1);
            }
        }
        else
        {
            ...
            {
                std::cerr << "\nERROR: while writing to output directory \"" << output_dir
                          << "\" : " << std::strerror(errno) << std::endl;
                exit(2);
            }
        }
...
int Trick::CommandLineArguments::output_dir_timestamped_on()
{
    ...
    snprintf(temp_str, sizeof(temp_str), "DATA_%4d_%02d_%02d_%02d_%02d_%02d", curr_time->tm_year + 1900,
        curr_time->tm_mon + 1, curr_time->tm_mday, curr_time->tm_hour, curr_time->tm_min, curr_time->tm_sec);
...
*/
void Trick::CommandLineArguments::set_output_dir(std::string output_directory) { output_dir = output_directory; }
