...
int Trick::RKF45_Integrator::integrate()
{
    ...
    case 2:
        ...
        b_3 = b4_45[3] * dt;
        for (i = 0; i < num_state; i++)
            state_ws[3][i] = state_ws[0][i] + deriv[0][i] * b_1 + deriv[1][i] * b_2 + deriv[2][i] * b_3;
    ...
    case 3:
        ...
        for (i = 0; i < num_state; i++)
            state_ws[4][i]
                = state_ws[0][i] + deriv[0][i] * b_1 + deriv[1][i] * b_2 + deriv[2][i] * b_3 + deriv[3][i] * b_4;
    ...
    case 4:
        ...
        b_5 = b6_45[5] * dt;
        for (i = 0; i < num_state; i++)
            state_ws[5][i] = state_ws[0][i] + deriv[0][i] * b_1 + deriv[1][i] * b_2 + deriv[2][i] * b_3
                + deriv[3][i] * b_4 + deriv[4][i] * b_5;
    ...
    case 5:
        ...
        for (i = 0; i < num_state; i++)
            state_ws[0][i]
                += (deriv[0][i] * c_1 + deriv[2][i] * c_3 + deriv[3][i] * c_4 + deriv[4][i] * c_5 + deriv[5][i] * c_6);
        time              = time_0 + dt;
        intermediate_step = 0;
        break;
    }
