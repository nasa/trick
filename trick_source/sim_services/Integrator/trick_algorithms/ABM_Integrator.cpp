...
Trick::ABM_Integrator::~ABM_Integrator()
{
    ...
    for(i = 0; i < 8; i++)
    {
        if(stored_data[i])
        {
            INTEG_FREE(stored_data[i]);
        }
    }
    if(stored_data)
    {
        INTEG_FREE(stored_data);
    }
}
    ...
    if(!primed)
    {
        ...
        switch(intermediate_step)
        {
            ...
            case 3:
                ...
                for(i = 0; i < num_state; i++)
                {
                    state_ws[0][i] += ((deriv[0][i] + deriv[3][i]) * dto6 + (deriv[1][i] + deriv[2][i]) * dto3);
    ...
    else
    {
        switch(intermediate_step)
        {
            case 0:
                ...
                {
                    state_ws[0][i] = state[i];
                    state_ws[1][i] = state[i] + dt / 24.0 *
                                                    (55.0 * stored_data[7][i] - 59.0 * stored_data[6][i] +
                                                     37.0 * stored_data[5][i] - 9.0 * stored_data[4][i]);
                }
                intermediate_step = 1;
                break;
            case 1:
                ...
                for(i = 0; i < num_state; i++)
                {
                    state_ws[2][i] = state_ws[0][i] + dt / 24.0 *
                                                          (9.0 * deriv[1][i] + 19.0 * stored_data[7][i] -
                                                           5.0 * stored_data[6][i] + stored_data[5][i]);
                }
                intermediate_step = 2;
                break;
            case 2:
