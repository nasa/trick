 ...
 */
Trick::IntegLoopScheduler::IntegLoopScheduler(double in_cycle, Trick::SimObject* in_parent_so)
    : Scheduler()
    ,
 ...
 */
Trick::IntegLoopScheduler::SimObjectVector::iterator Trick::IntegLoopScheduler::find_sim_object(
...
/**
 ...
 Deprecated!
 */
int Trick::IntegLoopScheduler::add_integ_jobs_from_sim_object(Trick::SimObject* in_obj)
{
    unsigned int ii;
...
int Trick::IntegLoopScheduler::add_sim_object(Trick::SimObject* sim_obj __attribute((unused))) { return 0; }
 ...
 */
void Trick::IntegLoopScheduler::restart_checkpoint() { manager.clear_sim_object_info(); }
...
void Trick::IntegLoopScheduler::rebuild_jobs()
{
    ...
    for (SimObjectVector::iterator so_iter = sim_objects.begin(); so_iter != sim_objects.end(); ++so_iter)
    {
    ...
    for (std::map<std::string, int>::iterator id_iter = class_map.begin(); id_iter != class_map.end(); ++id_iter)
    {
        ...
        {
            Trick::SimObject* sim_object = *so_iter;
            manager.add_jobs_to_queue(job_class_name, job_class_id, sim_object, queue);
        }
    }
}
 ...
 */
void Trick::IntegLoopScheduler::clear_queue(Trick::ScheduledJobQueue& job_queue)
{
    if (job_queue.size() != 0)
    {
        Trick::JobData* curr_job;
 ...
 */
void Trick::IntegLoopScheduler::call_deriv_jobs() { call_jobs(deriv_jobs); }
    ...
    else
    {
        ...
        while ((curr_job = integ_jobs.get_next_job()) != NULL)
        {
            if ((curr_job->job_class == Trick::IntegrationJobClassId)
                ...
                && ((trick_integrator = *(static_cast<Trick::Integrator**>(sup_class_data))) != NULL)
...
bool Trick::IntegLoopScheduler::get_last_step_deriv()
{
    ...
    if (last_step_deriv || ((integ_ptr != NULL) && integ_ptr->last_step_deriv))
    {
        return true;
    }
        ...
        while ((curr_job = integ_jobs.get_next_job()) != NULL)
        {
            ...
            if (verbosity || trick_curr_integ->verbosity)
            {
                message_publish(MSG_DEBUG,
                    "Job: %s, target_integ_time: %20.16f, integ_time: %20.16f, dt: %20.16f, ipass = %d\n",
            ...
            {
                message_publish(MSG_ERROR, "Integ Scheduler ERROR: Integrators not in sync.\n");
                return 1;
            }
        }
    } while (ipass);
 ...
 */
Trick::Integrator* Trick::IntegLoopScheduler::getIntegrator(Integrator_type alg, unsigned int state_size)
{
 ...
 */
int Trick::IntegLoopScheduler::set_integ_cycle(double in_cycle)
{
    ...
    for (std::vector<Trick::JobData*>::iterator vit = parent_sim_object->jobs.begin();
        vit != parent_sim_object->jobs.end(); ++vit)
    {
        JobData* vit_job = *vit;
    ...
    if (loops_found != 1)
    {
        message_publish(MSG_ERROR, "Integ Scheduler ERROR: %s\n",
    ...
    else
    {
        ...
        nominal_cycle = in_cycle;
        next_cycle    = double((found_job->next_tics - prev_tics)) / double(found_job->time_tic_value);
        return 0;
    }
}
 ...
 */
int Trick::IntegLoopScheduler::instrument_job_before(Trick::JobData* instrument_job)
{
    int count = 0;
    /** @par Detailed Design */
 ...
 */
int Trick::IntegLoopScheduler::instrument_job_after(Trick::JobData* instrument_job)
{
    int count = 0;
