...
extern "C" int integrate() { return (trick_curr_integ->integrate()); }
...
extern "C" double get_integ_time() { return (trick_curr_integ->time); }

extern "C" double get_integ_dt(void) { return (trick_curr_integ->dt); }

extern "C" double get_integ_target_time(void) { return (trick_curr_integ->target_integ_time); }
...
extern "C" void load_indexed_state(unsigned int index, double state)
{
    if (trick_curr_integ != NULL)
    {
        if (trick_curr_integ->verbosity)
            message_publish(MSG_DEBUG, " LOAD INDEXED STATE: %f\n", state);
...
extern "C" void load_indexed_deriv(unsigned int index, double deriv)
{
    ...
    {
        if (trick_curr_integ->verbosity)
            message_publish(MSG_DEBUG, "LOAD INDEXED DERIV: %f\n", deriv);
...
extern "C" void load_indexed_deriv2(unsigned int index, double deriv2)
{
    ...
    {
        if (trick_curr_integ->verbosity)
            message_publish(MSG_DEBUG, "LOAD INDEXED DERIV2: %f\n", deriv2);
...
extern "C" double unload_indexed_state(unsigned int index)
{
    ...
    {
        if (trick_curr_integ->verbosity)
            message_publish(MSG_DEBUG, "UNLOAD INDEXED STATE: %u\n", index);
...
extern "C" int get_intermediate_step() { return (trick_curr_integ->intermediate_step); }
...
extern "C" int get_integ_type() { return (trick_curr_integ->get_Integrator_type()); }
