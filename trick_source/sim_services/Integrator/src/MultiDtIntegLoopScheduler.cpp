...
namespace Trick
{
        ...
        for (size_t ii = 0; ii < integ_rates.size(); ++ii)
        {
            ...
            if (test_rem > 0.001)
            {
                message_publish(MSG_WARNING,
                    "Integ Scheduler ERROR: Cycle for %lu integ rate idx cannot be exactly scheduled with time tic "
                    "value. "
        ...
        if (found_job != NULL)
        {
            if (integRateIn < (1.0 / exec_get_time_tic_value()))
            {
                message_publish(MSG_ERROR,
                    "Integ Scheduler ERROR: Cycle for %lu integ rate idx is less than time tic value. cycle = "
                    "%16.12f, time_tic = %16.12f\n",
                    rate_idx, integRateIn, exec_get_time_tic_value());
                return 1;
            }
     ...
     */
    int MultiDtIntegLoopScheduler::set_integ_cycle(double in_cycle) { return set_integ_rate(0, in_cycle); }
     ...
     */
    size_t MultiDtIntegLoopScheduler::get_num_rates() { return integ_rates.size(); }
