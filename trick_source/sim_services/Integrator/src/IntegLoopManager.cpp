...
void Trick::IntegrationManager::build_sim_object_info()
{
    ...
    for (SimObjectVector::iterator iter = sim_object_vector.begin(); iter != sim_object_vector.end(); ++iter)
    {
    ...
    for (SimObjectIntegInfoVector::iterator obj_iter = sim_objects_info.begin(); obj_iter != sim_objects_info.end();
        ++obj_iter)
    {
 ...
 */
void Trick::IntegrationManager::create_sim_object_info(Trick::SimObject* sim_object)
{
    sim_objects_info.push_back(SimObjectIntegInfo(sim_object));
    SimObjectIntegInfo& sim_obj_info = sim_objects_info.back();

    for (std::vector<Trick::JobData*>::iterator iter = sim_object->jobs.begin(); iter != sim_object->jobs.end(); ++iter)
    {
        ...
        if (class_iter != sim_obj_info.job_class_to_info.end())
        {
            ...
            if ((assign_to == NULL) || (job_class_name->compare("integration") == 0))
            {
 ...
 */
void Trick::IntegrationManager::transfer_relegated_jobs(SimObjectIntegInfo& sim_obj_info)
{
    ...
    for (SimObjectIntegInfo::JobClassMap::iterator map_iter = obj_map.begin(); map_iter != obj_map.end(); ++map_iter)
    {
        ...
        for (JobVector::iterator job_iter = jobs->relegated_jobs.begin(); job_iter != jobs->relegated_jobs.end();
            ++job_iter)
        {
            ...
            SimObjectIntegInfo* assign_to = find_sim_object_info(job->sup_class_data, false);
            if (assign_to != NULL)
            {
            ...
            {
                message_publish(MSG_ERROR, "job %s lost it relegated sim object\n", job->name.c_str());
            }
        }
    }
}
 ...
 */
void Trick::IntegrationManager::clear_integrated_by(IntegLoopScheduler* integrator)
{
    for (SimObjectIntegInfoVector::iterator obj_iter = sim_objects_info.begin(); obj_iter != sim_objects_info.end();
        ++obj_iter)
    {
 ...
 */
Trick::IntegLoopScheduler* Trick::IntegrationManager::get_integrated_by(Trick::SimObject* sim_object)
{
    SimObjectIntegInfo* sim_obj_info = find_sim_object_info(sim_object, false);
 ...
 */
void Trick::IntegrationManager::set_integrated_by(Trick::SimObject* sim_object, Trick::IntegLoopScheduler* integrator)
{
    // Find the SimObjectIntegInfo that represents the sim object.
    SimObjectIntegInfo* sim_obj_info = find_sim_object_info(sim_object, true);
 ...
 */
Trick::SimObject* Trick::IntegrationManager::find_sim_object(const void* derived_sim_object)
{
    SimObjectIntegInfo* sim_obj_info = find_sim_object_info(derived_sim_object, true);
 ...
 */
Trick::IntegrationManager::SimObjectIntegInfo* Trick::IntegrationManager::find_sim_object_info(
    const Trick::SimObject* sim_object, bool allow_rebuild)
{
    FindSimObject search_predicate(sim_object);

    SimObjectIntegInfoVector::iterator found_iter
        = std::find_if(sim_objects_info.begin(), sim_objects_info.end(), search_predicate);
    if (allow_rebuild && (found_iter == sim_objects_info.end()))
    {
        build_sim_object_info();
        found_iter = std::find_if(sim_objects_info.begin(), sim_objects_info.end(), search_predicate);
    }
 ...
 */
Trick::IntegrationManager::SimObjectIntegInfo* Trick::IntegrationManager::find_sim_object_info(
    const void* derived_sim_object, bool allow_rebuild)
{
 ...
 */
void Trick::IntegrationManager::add_jobs_to_queue(
    const std::string& job_class_name, int job_class_id, Trick::SimObject* sim_object, Trick::ScheduledJobQueue& queue)
{
    ...
    SimObjectIntegInfo::JobClassMap::iterator map_iter = sim_obj_info->job_class_to_info.find(job_class_name);
    ...
    JobClassJobs* jobs = map_iter->second;
    for (JobVector::iterator job_iter = jobs->owned_jobs.begin(); job_iter != jobs->owned_jobs.end(); ++job_iter)
    {
        Trick::JobData* job = *job_iter;
