...
protected:
    ...
    {
        memmgr = new Trick::MemoryManager;
        IntegLoop = new Trick::IntegLoopScheduler(0.01, &uno);
    }
    ...
    Trick::ScheduledJobQueue * get_derivative_queue()
    {
        return &IntegLoop->deriv_jobs;
    }

    Trick::ScheduledJobQueue * get_integration_queue()
    {
        return &IntegLoop->integ_jobs;
    }

    Trick::ScheduledJobQueue * get_dynamic_event_queue()
    {
        return &IntegLoop->dynamic_event_jobs;
    }

    Trick::ScheduledJobQueue * get_post_integration_queue()
    {
        return &IntegLoop->post_integ_jobs;
    }

    Trick::ScheduledJobQueue * get_pre_integration_queue()
    {
        return &IntegLoop->pre_integ_jobs;
    }
};
...
void verify_ball_sim_results(
    BALL * ball, const double tolerance, const double pos0, const double pos1, const double vel0, const double vel1)
{
...
TEST_F(IntegratorTest, Ball_Euler_Cromer)
{
    ...
    Trick::Euler_Cromer_Integrator * integrator = (Trick::Euler_Cromer_Integrator *)Trick::getIntegrator(Euler_Cromer,
                                                                                                         4,
                                                                                                         0.01);
    ASSERT_TRUE((void *)integrator != NULL);
    EXPECT_EQ(integrator->num_state, 4);
