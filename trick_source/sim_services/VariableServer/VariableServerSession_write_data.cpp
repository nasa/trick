...
int Trick::VariableServerSession::write_binary_data(const std::vector<VariableReference *> & given_vars,
    ...
    for(int i = 0; i < given_vars.size(); i++)
    {
        ...
        if(header_size + total_var_size > MAX_MSG_LEN)
        {
            message_publish(MSG_WARNING,
                            "tag=<%s> Variable Server buffer[%d] too small (need %d) for symbol %s, SKIPPING IT.\n",
                            _connection->getClientTag().c_str(),
                            MAX_MSG_LEN,
                            header_size + total_var_size,
                            var->getName().c_str());
        ...
        if(total_size + total_var_size > MAX_MSG_LEN)
        {
            ...
            if(_debug >= 2)
            {
                message_publish(
                    MSG_DEBUG,
                    "%p tag=<%s> var_server buffer[%d] too small (need %d), sending multiple binary packets.\n",
                    _connection,
                    _connection->getClientTag().c_str(),
                    MAX_MSG_LEN,
    ...
    for(const auto & message_info : message_sizes_and_vars)
    {
        ...
        if(_debug >= 2)
        {
            message_publish(MSG_DEBUG,
                            "%p tag=<%s> var_server sending %u binary bytes containing %d variables.\n",
                            _connection,
                            _connection->getClientTag().c_str(),
                            curr_message_size,
                            curr_message_vars.size());
        }
...
int Trick::VariableServerSession::write_ascii_data(const std::vector<VariableReference *> & given_vars,
                                                   VS_MESSAGE_TYPE message_type)
{
    ...
    for(int i = 0; i < given_vars.size(); i++)
    {
        ...
        {
            message_publish(MSG_WARNING,
                            "tag=<%s> Variable Server buffer[%d] too small for symbol %s, TRUNCATED IT.\n",
                            _connection->getClientTag().c_str(),
                            MAX_MSG_LEN,
                            given_vars[i]->getName().c_str());
        ...
        if(message_size + var_size + 2 > MAX_MSG_LEN)
        {
            ...
            if(_debug >= 2)
            {
                message_publish(
                    MSG_DEBUG,
                    "%p tag=<%s> var_server buffer[%d] too small (need %d), sending multiple ascii packets.\n",
                    _connection,
                    _connection->getClientTag().c_str(),
                    MAX_MSG_LEN,
                    message_size + var_size + 2);

                message_publish(MSG_DEBUG,
                                "%p tag=<%s> var_server sending %d ascii bytes:\n%s\n",
                                _connection,
                                _connection->getClientTag().c_str(),
                                message_size,
                                message.c_str());
