...
int Trick::VariableServerThread::copy_sim_data(std::vector<VariableReference *> given_vars) {
    ...
    if ( pthread_mutex_trylock(&copy_mutex) == 0 ) {
        ...
        for (int i = 0 ; i < given_vars.size() ; i++ ) {
            copy_sim_data_single(given_vars[i]);
        }
...
// Copy mutex should be held during this method
// Way to assert? idk
int Trick::VariableServerThread::copy_sim_data_single(VariableReference * curr_var) {
