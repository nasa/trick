...
void setup_default_connection_mocks(MockClientConnection * connection)
{
    ...
    ON_CALL(*connection, start()).WillByDefault(Return(0));

    // We should always get a disconnect call
    ON_CALL(*connection, disconnect()).WillByDefault(Return(0));
}

void setup_default_session_mocks(MockVariableServerSession * session, bool command_exit = true)
{
    ON_CALL(*session, handle_message()).WillByDefault(Return(0));

    ON_CALL(*session, copy_and_write_async()).WillByDefault(Return(0));

    ON_CALL(*session, get_pause()).WillByDefault(Return(false));

    ON_CALL(Const(*session), get_update_rate()).WillByDefault(Return(0.001));

    ON_CALL(*session, get_exit_cmd()).WillByDefault(Return(false));
}
...
void setup_normal_connection_expectations(MockClientConnection * connection)
{
    ...
    // We should always get a disconnect call
    EXPECT_CALL(*connection, disconnect()).Times(1);
}
...
TEST_F(VariableServerSessionThread_test, DISABLED_exit_if_handle_message_fails)
{
    ...
    EXPECT_CALL(*session, handle_message()).Times(1).WillOnce(Return(-1));
...
TEST_F(VariableServerSessionThread_test, DISABLED_exit_if_write_fails)
{
    ...
    // Write out data
    EXPECT_CALL(*session, copy_and_write_async()).WillOnce(Return(-1));
...
TEST_F(VariableServerSessionThread_test, throw_trick_executive_exception)
{
    ...
    EXPECT_CALL(*session, get_exit_cmd()).WillRepeatedly(Return(false));
    ...
    EXPECT_CALL(*session, handle_message())
        .WillOnce(Throw(
            Trick::ExecutiveException(-1, __FILE__, __LINE__, "Trick::ExecutiveException Error message for testing")));
...
TEST_F(VariableServerSessionThread_test, throw_exception)
{
    ...
    EXPECT_CALL(*session, handle_message()).WillOnce(Throw(std::logic_error("Error message for testing")));
    EXPECT_CALL(message_publisher, publish(MSG_ERROR, _));
