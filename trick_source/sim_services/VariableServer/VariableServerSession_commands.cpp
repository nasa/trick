    ...
    else
    {
        ...
        if (_debug >= 2)
        {
            message_publish(MSG_DEBUG, "%p tag=<%s> var_server sending:\n%s\n", _connection,
                _connection->getClientTag().c_str(), buf1);
        }
...
bool Trick::VariableServerSession::get_send_stdio() { return _send_stdio; }
    ...
    if (_binary_data)
    {
        ...
        if (_debug >= 2)
        {
            message_publish(MSG_DEBUG, "%p tag=<%s> var_server sending %d event variables\n", _connection,
                _connection->getClientTag().c_str(), var_count);
        }
    ...
    else
    {
        ...
        if (_debug >= 2)
        {
            message_publish(MSG_DEBUG, "%p tag=<%s> var_server sending number of event variables:\n%s\n", _connection,
                _connection->getClientTag().c_str(), write_string.str().c_str());
        }
...
int Trick::VariableServerSession::transmit_file(std::string filename)
{
    ...
    if (_debug >= 2)
    {
        message_publish(MSG_DEBUG, "%p tag=<%s> var_server opening %s.\n", _connection,
            _connection->getClientTag().c_str(), filename.c_str());
    }
...
int Trick::VariableServerSession::send_file(std::string file_name) { return transmit_file(file_name); }
