...
Trick::VariableReference::VariableReference(std::string var_name, double* time)
    : _staged(false)
    , _write_ready(false)
{
    if (var_name != "time")
        ...
        else
        {
            ...
            if ((_var_info->attr->num_index - _var_info->num_index) > 1)
            {
                message_publish(MSG_ERROR,
                    "Variable Server Error: var_add(%s) requests more than one dimension of dynamic array.\n",
                    _var_info->reference);
...
std::string Trick::VariableReference::getName() const { return _name; }

int Trick::VariableReference::getSizeBinary() const { return _size; }

TRICK_TYPE Trick::VariableReference::getType() const { return _trick_type; }

std::string Trick::VariableReference::getBaseUnits() const { return _base_units; }
...
int Trick::VariableReference::setRequestedUnits(std::string units_name)
{
    ...
    if (units_name.compare("--"))
    {
        ...
        if (units_name.compare(new_units))
        {
            ...
            std::ostringstream oss;
            oss << "[" << getName() << "] old-style units converted from [" << units_name << "] to [" << new_units
                << "]";
        ...
        {
            std::ostringstream oss;
            oss << "[" << getName() << "] cannot convert units from [" << getBaseUnits() << "] to [" << new_units
                << "]";
            publish(MSG_ERROR, oss.str());
            return -1;
        }
...
bool Trick::VariableReference::validate()
{
    ...
    if ((_trick_type != TRICK_STRING) and (_trick_type != TRICK_WSTRING) and (_var_info->address != &_bad_ref_int)
        and (get_alloc_info_of(_address) == NULL))
    {
...
int Trick::VariableReference::writeValueAscii(std::ostream& out) const
{
    ...
    while (bytes_written < _size)
    {
        ...
        case TRICK_BITFIELD:
            out << (GET_BITFIELD(
                buf_ptr, _var_info->attr->size, _var_info->attr->index[0].start, _var_info->attr->index[0].size));
            break;

        case TRICK_UNSIGNED_BITFIELD:
            out << (GET_UNSIGNED_BITFIELD(
                buf_ptr, _var_info->attr->size, _var_info->attr->index[0].start, _var_info->attr->index[0].size));
            break;
...
bool Trick::VariableReference::isStaged() const { return _staged; }

bool Trick::VariableReference::isWriteReady() const { return _write_ready; }
...
void Trick::VariableReference::byteswap_var(char* out, char* in) const { byteswap_var(out, in, *this); }
...
int Trick::VariableReference::writeValueBinary(std::ostream& out, bool byteswap) const
{
    ...
    {
        int temp_i = GET_BITFIELD(
            _write_buffer, _var_info->attr->size, _var_info->attr->index[0].start, _var_info->attr->index[0].size);
    ...
    {
        int temp_unsigned = GET_UNSIGNED_BITFIELD(
            _write_buffer, _var_info->attr->size, _var_info->attr->index[0].start, _var_info->attr->index[0].size);
