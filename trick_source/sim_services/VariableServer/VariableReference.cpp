    ...
    else if(_var_info->attr)
    {
        if(_var_info->attr->type == TRICK_STRUCTURED)
        {
            ...
            message_publish(MSG_ERROR,
                            "Variable Server: var_add cant add \"%s\" because its a composite variable.\n",
                            var_name.c_str());
...
bool Trick::VariableReference::validate()
{
    ...
    if((_trick_type != TRICK_STRING) and (_trick_type != TRICK_WSTRING) and (_var_info->address != &_bad_ref_int) and
...
int Trick::VariableReference::writeValueAscii(std::ostream & out) const
{
    ...
    while(bytes_written < _size)
    {
        ...
        switch(local_type)
        {
            ...
            case TRICK_BITFIELD:
                out << (GET_BITFIELD(buf_ptr,
                                     _var_info->attr->size,
                                     _var_info->attr->index[0].start,
                                     _var_info->attr->index[0].size));
                break;

            case TRICK_UNSIGNED_BITFIELD:
                out << (GET_UNSIGNED_BITFIELD(buf_ptr,
                                              _var_info->attr->size,
                                              _var_info->attr->index[0].start,
                                              _var_info->attr->index[0].size));
                break;
...
int Trick::VariableReference::writeValueBinary(std::ostream & out, bool byteswap) const
{
    ...
    {
        int temp_i = GET_BITFIELD(_write_buffer,
                                  _var_info->attr->size,
                                  _var_info->attr->index[0].start,
                                  _var_info->attr->index[0].size);
    ...
    {
        int temp_unsigned = GET_UNSIGNED_BITFIELD(_write_buffer,
                                                  _var_info->attr->size,
                                                  _var_info->attr->index[0].start,
                                                  _var_info->attr->index[0].size);
