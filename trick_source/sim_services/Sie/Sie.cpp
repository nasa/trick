...
void Trick::Sie::sie_append_runtime_objs()
{
    ...
    // loop through the file looking for the marker comment.  The comment could straddle 2 reads.  Copy the last bytes
    // of the previous read to start the next search.
    ...
    if(!find_str)
    {
        std::cerr << "Warning: Cannot add runtime/dynamic allocations to S_sie.resource. S_sie.resource is corrupted, "
                     "outdated, or missing. Please be sure that SIM_*/S_sie.resource is preserved after build time if "
                     "needed at runtime for trick-tv or other variable server clients. Please also rerun trick-CP."
                  << std::endl;
        delete[] buff;
        return;
    }
