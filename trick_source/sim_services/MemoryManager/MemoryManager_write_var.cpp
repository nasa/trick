...
// MEMBER FUNCTION
void Trick::MemoryManager::write_composite_var(std::ostream & out_s, void * address, ATTRIBUTES * attr_list)
{
    if(attr_list == NULL)
    {
...
// MEMBER FUNCTION
void Trick::MemoryManager::write_array_var(
    std::ostream & out_s, void * address, ATTRIBUTES * attr, int curr_dim, int offset)
{
    if(attr == NULL)
    {
    ...
    { // This is a pointer (a.k.a: an unconstrained array).
        currentCheckPointAgent->assign_rvalue(out_s, address, attr, curr_dim, offset);
    }
    else
    { // This is a contrained array.
