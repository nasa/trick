...
int Trick::MemoryManager::delete_var(void * address)
{
    ...
    else
    {
        ...
        if(resetting_memory == true)
        {
            ...
            std::list<void *>::iterator iter = std::find(deleted_addr_list.begin(), deleted_addr_list.end(), address);
            // If we didn't recently delete it, then there's a problem.
            if(iter == deleted_addr_list.end())
            {
                std::stringstream message;
                message << "The MemoryManager cannot delete memory at address [" << address
                        << "] because it has no record of it. Furthermore,"
...
int Trick::MemoryManager::delete_var(std::string name)
{
    ...
    {
        std::stringstream message;
        message << "Cannot delete variable \"" << name << "\" because it doesn't exist.";
