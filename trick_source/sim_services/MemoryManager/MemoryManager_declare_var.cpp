...
// PUBLIC MEMBER FUNCTION: void* Trick::MemoryManager::declare_var(TRICK_TYPE type,std::string user_type_name, int
// n_stars, std::string var_name, int n_cdims, int *cdims);
void* Trick::MemoryManager::declare_var(
    TRICK_TYPE type, std::string user_type_name, int n_stars, std::string var_name, int n_cdims, int* cdims)
{
    int size;
    ...
    if ((type == TRICK_STRUCTURED) && (sub_attr->language == Language_CPP) && (n_stars == 0))
    {
...
void* Trick::MemoryManager::declare_var(const char* alloc_definition)
{
    ...
    if (context != NULL)
    {
        if (ADEF_parse(context) == 0)
        {
            ...
            address = declare_var(context->type, context->user_type_name, context->n_stars, context->var_name,
                context->n_cdims, context->cdims);
...
void* Trick::MemoryManager::declare_var(const char* element_definition, int n_elems)
{
    ...
    if (context != NULL)
    {
        /** @li Parse the allocation definition and ensure that the dimension is at least one less than the maximum
         * of 8. */
        if ((ADEF_parse(context) == 0) && (context->n_cdims < 8))
        {
            ...
            /** @li Call the general form of declare_var to perform the allocation. */
            address = declare_var(
                context->type, context->user_type_name, context->n_stars, context->var_name, n_cdims, cdims);
...
// PUBLIC MEMBER FUNCTION: void* Trick::MemoryManager::declare_operatornew_var(std::string user_type_name, unsigned int
// alloc_size , unsigned int element_size, std::string alloc_name );

void* Trick::MemoryManager::declare_operatornew_var(
    std::string user_type_name, unsigned int alloc_size, unsigned int element_size, std::string alloc_name)
{
    ...
    {
        std::stringstream message;
        message << "get_type_attributes failed for type: " << TRICK_STRUCTURED << " " << user_type_name.c_str() << ".";
        emitError(message.str());
        return ((void*)NULL);
    }
    ...
    if (debug_level)
    {
        ...
        std::cout << "), size(" << new_alloc->size << ") @ addr(" << address
                  << ") sentinel_bytes = " << new_alloc->sentinel_bytes;
        std::cout << std::endl << std::endl;
        std::cout.flush();
    }
