...
void Trick::MemoryManager::recursive_array_copy(
    void* s_base, void* d_base, int* s_cdims, int* d_cdims, size_t size, int n, int cur, int s_off, int d_off)
{
    ...
    else
    {
        ...
        {
            recursive_array_copy(s_base, d_base, s_cdims, d_cdims, size, n, cur + 1, s_off * s_cdims[cur] + ii,
...
// PRIVATE MEMBER FUNCTION
void Trick::MemoryManager::array_copy(void* s_base, void* d_base, int* s_cdims, int* d_cdims, size_t size, int n)
{
...
void* Trick::MemoryManager::resize_array(void* address, int n_cdims, int* cdims)
{
    ...
    if (n_cdims != pre_n_cdims)
    {
        ...
        else
        {
            ...
            message << "resize_array: The number of dimensions specified, [" << n_cdims
                    << "], doesn't match the number of dimensions of the object at " << address << ", [" << pre_n_cdims
                    << "].";
            emitError(message.str());
        }
        return ((void*)NULL);
    ...
    pthread_mutex_lock(&mm_mutex);
    if ((alloc_info->type == TRICK_STRUCTURED) && (alloc_info->language == Language_CPP)
        && (alloc_info->num_index == n_cdims))
    {
        if ((new_address = io_src_allocate_class(alloc_info->user_type_name, new_n_elems)) == NULL)
        {
            std::stringstream message;
            message << "io_src_allocate_class(" << alloc_info->user_type_name << "," << new_n_elems
    ...
    array_copy(alloc_info->start, new_address, alloc_info->index, cdims, alloc_info->size, n_cdims);
