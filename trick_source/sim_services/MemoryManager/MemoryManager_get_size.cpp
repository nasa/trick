...
int Trick::MemoryManager::io_get_fixed_truncated_size(
    char* ptr, ATTRIBUTES* A, char* str, int dims, ATTRIBUTES* left_type)
{
#if 1
    int i = 0, j;
    long offset;
    ...
    while ((A[i].name[0] != '\0') && (long)ptr >= (long)((long)str + A[i].offset))
    {
        i++;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            ret = io_get_fixed_truncated_size(ptr, (ATTRIBUTES*)A[i].attr, str + A[i].offset + offset, dims, left_type);
        }
