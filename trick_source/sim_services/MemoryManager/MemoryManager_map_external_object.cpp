...
// MEMBER FUNCTION
void* Trick::MemoryManager::declare_extern_var(void* address, TRICK_TYPE type, std::string user_type_name, int n_stars,
    std::string var_name, int n_cdims, int* cdims)
{
    ...
    {
        std::stringstream message;
        message << "get_type_attributes failed for type " << type << " \"" << user_type_name << "\".";
        emitError(message.str());
        return ((void*)NULL);
    }
...
// MEMBER FUNCTION
void* Trick::MemoryManager::declare_extern_var(void* address, const char* alloc_definition)
{
    ...
    if (context != NULL)
    {
        if (ADEF_parse(context) == 0)
        {
            ...
            res_address = declare_extern_var(address, context->type, context->user_type_name, context->n_stars,
                context->var_name, context->n_cdims, context->cdims);
...
void* Trick::MemoryManager::declare_extern_var(void* address, const char* element_definition, int n_elems)
{
    ...
    if (context != NULL)
    {
        ...
        if ((ADEF_parse(context) == 0) && (context->n_cdims < 8))
        {
            ...
            res_address = declare_extern_var(
                address, context->type, context->user_type_name, context->n_stars, context->var_name, n_cdims, cdims);
        ...
        {
            std::stringstream message;
            message << "declare_extern_var( \"" << element_definition << "\"," << n_elems << ") failed.";
            emitError(message.str());
        }
    }
    /** @li Return the address of the allocation. */
