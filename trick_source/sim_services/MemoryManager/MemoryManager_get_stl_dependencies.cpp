...
void Trick::MemoryManager::get_stl_dependencies(ALLOC_INFO * alloc_info)
{
    ...
    if(debug_level)
    {
        std::cout << __FUNCTION__ << ": Begin scan of allocation @" << (void *)alloc_info->start << " for dependencies."
                  << std::endl;
    }
    ...
    std::string name, char * address, ATTRIBUTES * attr, int curr_dim, int offset)
{
    ...
    else
    {
        ...
        for(ii = 0; ii < curr_dim_size; ii++)
        {
            ...
            {
                get_stl_dependencies_in_arrayed_class(name + index,
                                                      address,
                                                      attr,
                                                      curr_dim + 1,
                                                      offset * curr_dim_size + ii);
            }
            else
            {
                char * elem_addr;
...
// MEMBER FUNCTION
void Trick::MemoryManager::get_stl_dependencies_in_intrinsic(
    ...
    if(curr_dim < attr->num_index)
    {
        ...
        { // otherwise its a constrained array.
            ...
            for(ii = 0; ii < attr->index[curr_dim].size; ii++)
            {
                char index[16];
                snprintf(index, sizeof(index), "[%d]", ii);
                // get_stl_dependencies_in_intrinsic( name + index , address, attr, curr_dim + 1, offset *
                // attr->index[curr_dim].size + ii);
            }
        }
    }
}
