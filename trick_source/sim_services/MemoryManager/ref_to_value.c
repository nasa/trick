...
int ref_to_value(REF2 * R, V_DATA * V)
{
    ...
    //    if (R->deprecated) {
    //        /* Removed input_processor as a parameter to ref_to_value for simplification but that means we no longer
    //        know
    ...
    {
        case TRICK_VOID_PTR:
            // if ((R->num_index < R->attr->num_index - 1) || ((R->address_req == 0) && (R->num_index <
            // R->attr->num_index))) {
            //     V->value.vp = address;
            // } else {
