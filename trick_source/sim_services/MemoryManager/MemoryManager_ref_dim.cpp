...
int Trick::MemoryManager::ref_dim( REF2* R, V_DATA* V) {
    ...
    // Special handling for STL vectors - treat vec[i] as accessing the internal data array
    if (R->attr->type == TRICK_STL && R->attr->stl_type == TRICK_STL_VECTOR && R->num_index == 0) {
        // Use the generated accessor functions to get vector size and element address
        // These functions are generated by ICG and know the actual vector type
        ...
        if (index_value < 0 || (size_t)index_value >= num_elements) {
            if (debug_level > 1) {
                ...
                message << index_value << " is out of bounds for STL vector " << R->reference
                        << " (size=" << num_elements << ").";
