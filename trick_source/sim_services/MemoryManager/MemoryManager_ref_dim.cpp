...
int Trick::MemoryManager::ref_dim(REF2* R, V_DATA* V)
{
    ...
    if (R->attr->type == TRICK_STL
        && (R->attr->stl_type == TRICK_STL_VECTOR || R->attr->stl_type == TRICK_STL_DEQUE
        ...
        && R->num_index == 0)
    {
        ...
        static constexpr std::array<TRICK_TYPE, 14> supported_stl_elem_types = {TRICK_STRING,
            // TRICK_CHARACTER,
            // TRICK_UNSIGNED_CHARACTER,
            TRICK_ENUMERATED, TRICK_SHORT, TRICK_UNSIGNED_SHORT, TRICK_INTEGER, TRICK_UNSIGNED_INTEGER, TRICK_LONG,
            TRICK_UNSIGNED_LONG, TRICK_FLOAT, TRICK_DOUBLE, TRICK_LONG_LONG, TRICK_UNSIGNED_LONG_LONG, TRICK_BOOLEAN,
            TRICK_STRUCTURED};

        // Check if element type is supported
        if (std::find(supported_stl_elem_types.begin(), supported_stl_elem_types.end(), R->attr->stl_elem_type)
        ...
        if (R->attr->stl_elem_type == TRICK_ENUMERATED && R->attr->stl_elem_type_name != NULL)
        {
            ...
            if (enum_pos != enumeration_map.end())
            {
                ...
                temp_enum_attr.size
                    = (enum_size > 0) ? enum_size : sizeof(int); // Fall back to sizeof(int) if not found
    ...
    else
    {
        ...
        if (index_value >= (get_size(*(void**)(R->address))) || index_value < 0)
        {
            if (index_value >= 0 && get_size(*(void**)(R->address)) == 0)
            {
                ...
                // If a pointer is not allocated regardless of regular pointer or array pointer, an error message will
                // be emitted below
                //   However, if the pointer is already assigned to a valid address, the error message will NOT be
                //   emitted below
            }
            else
            {
        ...
        if (alloc_info != NULL && alloc_info->name != NULL)
        {
            ...
            if (!ref_name.empty() && ref_name.front() == '&')
            {
                ...
                if (ref2->num_index > 0)
                {
                    // Check if the pointer points to a static array or a dynamic array and if array index is out of
                    // bounds
                    if (ref2->attr && ref2->attr->index[ref2->attr->num_index - 1].size != 0)
                    { // Static array case
