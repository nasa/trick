...
int Trick::MemoryManager::ref_dim(REF2 * R, V_DATA * V)
{
       ...
       R->num_index == 0)
    {
        ...
        if(R->attr->stl_elem_type == TRICK_ENUMERATED && R->attr->stl_elem_type_name != NULL)
        {
            ...
            if(enum_pos != enumeration_map.end())
            {
                ...
                temp_enum_attr.size = (enum_size > 0) ? enum_size
                                                      : sizeof(int); // Fall back to sizeof(int) if not found
    ...
    if(R->create_add_path)
    {
        ...
        if(index_value > 0)
        {
            ...
            {
                case AO_ADDRESS:
                    address_node->operand.address = (void *)((char *)address_node->operand.address +
                                                             index_value * item_size);
                    break;
                case AO_DEREFERENCE:
                    address_node = new ADDRESS_NODE;
