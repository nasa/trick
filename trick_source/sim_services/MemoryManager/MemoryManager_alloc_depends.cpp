...
void Trick::MemoryManager::get_alloc_deps_in_allocation(ALLOC_INFO * alloc_info)
{
    ...
    if(alloc_info == NULL)
    {
        std::cerr << "ERROR: Trick::MemoryManager::get_alloc_deps_in_allocation called with alloc_info == NULL."
                  << std::endl;
        std::cout.flush();
        return;
    }
    ...
    for(int ii = 0; ii < n_depends; ii++)
    {
        if(alloc_info == dependencies[ii])
        {
            ...
            if(debug_level)
            {
                std::cout << __FUNCTION__ << ": Dependencies of allocation @" << (void *)alloc_info->start
                          << " are already known." << std::endl;
            }
            return;
        }
    }
    ...
    if(!found)
    {
        if(debug_level)
        {
            std::cout << __FUNCTION__ << ": Begin scan of allocation @" << (void *)alloc_info->start
                      << " for dependencies." << std::endl;
        }
...
void Trick::MemoryManager::get_alloc_deps_in_allocation(const char * var_name)
{
    ...
    else
    {
        std::cerr << "ERROR: get_alloc_deps_in_allocation(\"" << var_name
                  << "\") failed because the given name is'nt known by the MemoryManager." << std::endl;
        std::cerr.flush();
    }
}
...
// MEMBER FUNCTION
void Trick::MemoryManager::get_alloc_deps_in_arrayed_class(char * address, ATTRIBUTES * attr, int curr_dim, int offset)
{
    int ii;
    ...
    if(attr->index[curr_dim].size == 0)
    {
        ...
        if(pointer != NULL)
        {
            ...
            if(alloc_info != NULL)
            {
                if(debug_level)
                {
                    std::cout << __FUNCTION__ << ": @" << (void *)address << "." << attr->name << "[" << offset
                              << "] points to @" << (void *)pointer << std::endl;
                }
                get_alloc_deps_in_allocation(alloc_info);
            }
            else
            {
                if(debug_level)
                {
                    std::cout << __FUNCTION__ << ": Pointer (" << (void *)pointer
                              << ") refers to memory that the MemoryManager doesn't know about." << std::endl;
                    std::cout.flush();
                }
            }
        }
...
void Trick::MemoryManager::get_alloc_deps_in_intrinsic(void * address, ATTRIBUTES * attr, int curr_dim, int offset)
{
    ...
    if(curr_dim < attr->num_index)
    {
        ...
        { // otherwise its a constrained array.
            int ii;
