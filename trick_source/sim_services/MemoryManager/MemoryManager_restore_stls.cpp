...
void Trick::MemoryManager::restore_stls_in_class(std::string name, char * address, ATTRIBUTES * attr)
{
    ...
    if(attr == NULL)
    {
        std::cerr << "ERROR: Trick::MemoryManager::restore_stls called with name = " << name << " and attr = NULL."
                  << std::endl;
        return;
    }
...
// MEMBER FUNCTION
void Trick::MemoryManager::restore_stls_in_intrinsic(
    ...
    if(curr_dim < attr->num_index)
    {
        ...
        { // otherwise its a constrained array.
            ...
            for(ii = 0; ii < attr->index[curr_dim].size; ii++)
            {
                char index[16];
                snprintf(index, sizeof(index), "[%d]", ii);
                // restore_stls_in_intrinsic( name + index , address, attr, curr_dim + 1, offset *
