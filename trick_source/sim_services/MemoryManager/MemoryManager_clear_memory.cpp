    ...
    if(remaining_dimensions == 0)
    {
        switch(attr->type)
        {
            ...
            case TRICK_UNSIGNED_BITFIELD:
                ...
                {
                    *(unsigned int *)final_address = insert_bitfield_any(*(unsigned int *)final_address,
                                                                         0,
                                                                         attr->size,
                                                                         attr->index[0].start,
                                                                         attr->index[0].size);
                ...
                {
                    *(unsigned short *)final_address = insert_bitfield_any(*(unsigned short *)final_address,
                                                                           0,
                                                                           attr->size,
                                                                           attr->index[0].start,
                                                                           attr->index[0].size);
                ...
                {
                    *(unsigned char *)final_address = insert_bitfield_any(*(unsigned char *)final_address,
                                                                          0,
                                                                          attr->size,
                                                                          attr->index[0].start,
                                                                          attr->index[0].size);
                ...
                else
                {
                    ...
                    message << "INTERNAL - Unhandled bitfield struct size (" << attr->size
...
void Trick::MemoryManager::clear_var(const char * name)
{
    ...
    {
        std::stringstream message;
        message << "Can't clear variable \"" << name << "\" because it doesn't exist.";
        emitError(message.str());
    }
    pthread_mutex_unlock(&mm_mutex);
}
