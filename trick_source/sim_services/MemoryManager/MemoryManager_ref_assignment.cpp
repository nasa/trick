...
int Trick::MemoryManager::assign_recursive(
    void * base_addr, ATTRIBUTES * attr, int curr_dim, int offset, V_TREE * v_tree, cv_converter * cf)
{
        ...
        switch(local_type)
        {
            ...
            case TRICK_WCHAR:
                ...
                if(debug_level)
                {
                    ...
                    std::cout << std::endl
                              << "Assignment: *(wchar_t*)" << (void *)assign_addr << " = " << buff << ";" << std::endl;
                    std::cout.flush();
                }
                break;
            case TRICK_SHORT:
            case TRICK_UNSIGNED_SHORT:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(short*)" << (void *)assign_addr << " = " << *(short *)assign_addr << ";"
                              << std::endl;
                    std::cout.flush();
                }
                break;
            ...
            case TRICK_UNSIGNED_INTEGER:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(int*)" << (void *)assign_addr << " = " << *(int *)assign_addr << ";"
                              << std::endl;
                    std::cout.flush();
                }
                break;
            case TRICK_BOOLEAN:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(bool*)" << (void *)assign_addr << " = " << *(bool *)assign_addr << ";"
                              << std::endl;
                    std::cout.flush();
                }
                break;
                ...
                if(v_tree && v_tree->v_data)
                {
                    if((size_t)attr->size == sizeof(int))
                    {
                        ...
                        *(int *)assign_addr = vval_int(v_tree->v_data);
                        ...
                        {
                            std::cout << std::endl
                                      << "Assignment (Enum): *(int*)" << (void *)assign_addr << " = "
                    ...
                    else if((size_t)attr->size == sizeof(short))
                    {
                        ...
                        *(short *)assign_addr = vval_short(v_tree->v_data);
                        ...
                        {
                            std::cout << std::endl
                                      << "Assignment (Enum): *(short*)" << (void *)assign_addr << " = "
                    ...
                    else if((size_t)attr->size == sizeof(char))
                    {
                        ...
                        *(char *)assign_addr = vval_char(v_tree->v_data);
                        ...
                        {
                            std::cout << std::endl
                                      << "Assignment (Enum): *(char*)" << (void *)assign_addr << " = "
            ...
            case TRICK_UNSIGNED_LONG:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(long*)" << (void *)assign_addr << " = " << *(long *)assign_addr << ";"
                              << std::endl;
                    std::cout.flush();
                }
                break;
            case TRICK_FLOAT:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(float*)" << (void *)assign_addr << " = " << *(float *)assign_addr << ";"
                              << std::endl;
                    std::cout.flush();
                }
                break;
            case TRICK_DOUBLE:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(double*)" << (void *)assign_addr << " = " << *(double *)assign_addr
                              << ";" << std::endl;
                    std::cout.flush();
                }
                break;
            ...
            case TRICK_UNSIGNED_LONG_LONG:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(long long*)" << (void *)assign_addr << " = "
            ...
            case TRICK_UNSIGNED_BITFIELD:
            {
                ...
                {
                    *(unsigned int *)assign_addr = insert_bitfield_any(*(unsigned int *)assign_addr,
                                                                       input_value,
                                                                       attr->size,
                                                                       attr->index[0].start,
                                                                       attr->index[0].size);
                ...
                {
                    *(unsigned short *)assign_addr = insert_bitfield_any(*(unsigned short *)assign_addr,
                                                                         input_value,
                                                                         attr->size,
                                                                         attr->index[0].start,
                                                                         attr->index[0].size);
                ...
                {
                    *(unsigned char *)assign_addr = insert_bitfield_any(*(unsigned char *)assign_addr,
                                                                        input_value,
                                                                        attr->size,
                                                                        attr->index[0].start,
                                                                        attr->index[0].size);
                ...
                if(debug_level)
                {
                    std::cout << std::endl
                              ...
                              << attr->index[0].start + attr->index[0].size - 1 << "] = " << input_value << ";"
                              << std::endl;
                    std::cout.flush();
                }
            }
            ...
            case TRICK_FILE_PTR:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: TRICK_FILE_PTR assignments not yet implemented." << std::endl;
                    std::cout.flush();
                }
                break;
            case TRICK_VOID_PTR:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: TRICK_VOID_PTR assignments not yet implemented." << std::endl;
                    std::cout.flush();
                }
                break;
            case TRICK_STRING:
                ...
                {
                    std::cout << std::endl
                              << "Assignment: *(std::string)" << (void *)assign_addr << " = \""
    ...
    else if(remaining_dimensions > 0)
    {
        ...
        if(size_of_curr_dim == 0)
        { // the remaining dimensions are pointer dimensions.
            ...
            {
                std::cout << std::endl
                          << "Assignment: *(void**)" << (void *)assign_addr << " = " << *(void **)assign_addr << ";"
                          << std::endl;
                std::cout.flush();
            }
        }
        else
        { // next dimension is fixed.

            if((local_type == TRICK_CHARACTER) && (remaining_dimensions == 1) && (v_tree) && (v_tree->v_data))
            {
                assign_addr = (char *)base_addr + offset * size_of_curr_dim * sizeof(char);

                if((v_tree->v_data->type == TRICK_STRING) && (v_tree->v_data->value.cp != NULL))
                {
            ...
            else if((local_type == TRICK_WCHAR) && (remaining_dimensions == 1))
            {
                assign_addr = (char *)base_addr + offset * size_of_curr_dim * sizeof(wchar_t);

                if((v_tree) && (v_tree->v_data->type == TRICK_WSTRING) && (v_tree->v_data->value.wcp != NULL))
                {
                ...
                else if((v_tree) && (v_tree->v_data->type == TRICK_STRING) && (v_tree->v_data->value.cp != NULL))
                {
...
int Trick::MemoryManager::ref_assignment(REF2 * R, V_TREE * V)
{
    ...
    if(R->ref_attr != NULL && R->ref_attr->type == TRICK_STL && R->ref_attr->stl_elem_type == TRICK_BOOLEAN && V &&
       V->v_data)
    {
