 ...
 *
 */
int Trick::MemoryManager::add_attr_info(
    const std::string& user_type_string, ATTRIBUTES* attr, const char* file_name, unsigned int line_num)
{
    ...
    */
    ...
    std::replace(user_type_name.begin(), user_type_name.end(), ']', '_');
    // remove spaces
    user_type_name.erase(
        std::remove_if(user_type_name.begin(), user_type_name.end(), (int (*)(int))std::isspace), user_type_name.end());
    ...
    if (size_func == NULL)
    {
        ...
        message << "(" << file_name << ":" << line_num << "): Couldn't find an io_src_sizeof_ function for type "
    ...
    if (sub_attr != NULL)
    { // If attributes for the named type were found ...
        ...
        if (init_sub_attr != NULL)
        { // If the initialization function was found,
            (*init_sub_attr)(); // then call it.
        }
        else
        {
            std::stringstream message;
            message << " ATTRIBUTES init routine for type \"" << user_type_name.c_str() << "\" not found.";
            emitWarning(message.str());
            return (1);
        }
    }
    else
    { // The named user type isn't a structure type. Maybe it's an enumeration type.
