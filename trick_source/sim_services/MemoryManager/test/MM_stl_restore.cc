...
TEST_F(MM_stl_restore, i_s_vec)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        ...
        std::string temp_var_name
            = "int my_alloc_i_s_vec_" + std::to_string(i) + "[" + std::to_string(lengths[i]) + "]";
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        ...
        EXPECT_EQ(testbed->i_s_vec[i],
            std::vector<int>(test_data.begin() + start_index, test_data.begin() + start_index + lengths[i]));
        start_index += lengths[i];
    }
...
TEST_F(MM_stl_restore, nested_list_queue)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        ...
        std::string temp_var_name
            = "float my_alloc_nested_list_queue_" + std::to_string(i) + "[" + std::to_string(lengths[i]) + "]";
...
TEST_F(MM_stl_restore, nested_list_stack)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        ...
        std::string temp_var_name
            = "float my_alloc_nested_list_stack_" + std::to_string(i) + "[" + std::to_string(lengths[i]) + "]";
        std::list<float> temp_expected;
        float* temp_arr = (float*)memmgr->declare_var(temp_var_name.c_str());
        for (int j = 0; j < lengths[i]; j++)
        {
...
TEST_F(MM_stl_restore, vector_set)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        ...
        std::string temp_var_name
            = "int my_alloc_vector_set_" + std::to_string(i) + "[" + std::to_string(lengths[i]) + "]";
...
TEST_F(MM_stl_restore, vector_multiset)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        for (unsigned int j = 0; j < 2; j++)
        {
            ...
            std::string temp_var_name
                = "int my_alloc_vector_multiset_" + std::to_string(i * 2 + j) + "[" + std::to_string(lengths[i]) + "]";
            std::vector<int> temp_expected;
            int* temp_arr = (int*)memmgr->declare_var(temp_var_name.c_str());
...
TEST_F(MM_stl_restore, nested_map_multiset)
{
    ...
    for (unsigned int i = 0; i < 6; i++)
    {
        for (unsigned int k = 0; k < 2; k++)
        {
            ...
            std::string vals_var_name
                = "double my_alloc_nested_map_multiset_" + std::to_string(i * 2 + k) + "_data[10]";
...
TEST_F(MM_stl_restore, vec_array)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        ...
        std::string temp_var_name
            = "int my_alloc_vec_array_" + std::to_string(i) + "[" + std::to_string(lengths[i]) + "]";
        int* temp_arr = (int*)memmgr->declare_var(temp_var_name.c_str());
        for (int j = 0; j < lengths[i]; j++)
        {
            temp_arr[j] = test_data[start_index + j];
