...
template<typename T>
void validate_temp_sequence(Trick::MemoryManager * memmgr,
...
void validate_links_sequences(Trick::MemoryManager * memmgr,
                              std::string object_name,
...
void validate_links_pairs(Trick::MemoryManager * memmgr,
                          std::string object_name,
                          std::string top_level_name,
                          int num_pairs)
{
...
template<typename T>
void validate_temp_set(Trick::MemoryManager * memmgr,
                       std::string object_name,
                       std::string var_name,
                       std::set<T> expected_data)
{
    std::string temp_name = object_name + "_" + var_name;
    ASSERT_TRUE(memmgr->var_exists(temp_name) == 1);
...
void validate_temp_pair(Trick::MemoryManager * memmgr,
                        std::string object_name,
                        std::string var_name,
                        std::pair<First, Second> expected_data)
{
    std::string temp_name = object_name + "_" + var_name;
...
void validate_temp_map(Trick::MemoryManager * memmgr,
                       std::string object_name,
                       std::string var_name,
                       std::map<Key, Val> expected_data)
{
    std::string temp_name = object_name + "_" + var_name;
...
TEST_F(MM_stl_checkpoint, s_vec)
{
    ...
    for(unsigned int i = 0; i < test_data.size() / 2; i++)
    {
        ...
        validate_temp_pair<int, int>(memmgr,
                                     std::string("my_alloc"),
                                     var_name,
                                     std::pair<int, int>(test_data[i * 2], test_data[i * 2 + 1]));
    }
...
TEST_F(MM_stl_checkpoint, i_i_pair)
{
    ...
    // ASSERT
    validate_temp_pair<double, float>(memmgr,
                                      std::string("my_alloc"),
                                      vec_attr->name,
                                      std::pair<double, float>(test_data[0], test_data[1]));
}
...
TEST_F(MM_stl_checkpoint, pair_pair)
{
    ...
    validate_temp_pair(memmgr,
                       std::string("my_alloc"),
                       std::string("pair_pair_first_inner"),
...
TEST_F(MM_stl_checkpoint, s_i_map)
{
    ...
    for(unsigned int i = 0; i < lengths.size(); i++)
    {
        test_map[std::set<int>(test_keys.begin() + start_index,
                               test_keys.begin() + start_index + lengths[i])] = test_vals[i];
        testbed->s_i_map[std::set<int>(test_keys.begin() + start_index,
                                       test_keys.begin() + start_index + lengths[i])] = test_vals[i];
        start_index += lengths[i];
    }
...
TEST_F(MM_stl_checkpoint, s_s_map)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        testbed->s_s_map[std::pair<int, int>(test_keys[i * 2], test_keys[(i * 2) + 1])] = std::vector<int>(
            test_vals.begin() + (i * 4), test_vals.begin() + (i + 1) * 4);
        test_map[std::pair<int, int>(test_keys[i * 2], test_keys[(i * 2) + 1])] = std::vector<int>(test_vals.begin() +
                                                                                                       (i * 4),
                                                                                                   test_vals.begin() +
                                                                                                       (i + 1) * 4);
    }
...
TEST_F(MM_stl_checkpoint, s_queue)
{
    ...
    for(unsigned int i = 0; i < test_first.size(); i++)
    {
        ...
        validate_temp_pair(memmgr,
                           std::string("my_alloc"),
                           var_name,
                           std::pair<int, double>(test_first[i], test_second[i]));
    }
...
TEST_F(MM_stl_checkpoint, nested_list_queue)
{
    ...
    for(unsigned int i = 0; i < list_sizes.size(); i++)
    {
        ...
        validate_temp_sequence(memmgr,
                               std::string("my_alloc"),
                               var_name,
                               std::vector<float>(test_data.begin() + data_index,
                                                  test_data.begin() + data_index + list_sizes[i]));
        data_index += list_sizes[i];
    }
...
TEST_F(MM_stl_checkpoint, i_stack)
{
    ...
    validate_temp_sequence<long long>(memmgr,
                                      std::string("my_alloc"),
                                      std::string("i_stack"),
...
TEST_F(MM_stl_checkpoint, s_stack)
{
    ...
    for(unsigned int i = 0; i < test_first.size(); i++)
    {
        ...
        validate_temp_pair(memmgr,
                           std::string("my_alloc"),
                           var_name,
                           std::pair<short, double>(test_first[test_first.size() - 1 - i],
                                                    test_second[test_first.size() - 1 - i]));
    }
...
TEST_F(MM_stl_checkpoint, nested_list_stack)
{
    ...
    validate_links_sequences(memmgr,
                             std::string("my_alloc"),
                             std::string("nested_list_stack"),
...
TEST_F(MM_stl_checkpoint, i_set)
{
    ...
    // ASSERT
    validate_temp_set<char>(memmgr,
                            std::string("my_alloc"),
                            std::string("i_set"),
                            std::set<char>(test_data.begin(), test_data.end()));
}
...
TEST_F(MM_stl_checkpoint, s_set)
{
    ...
    for(std::pair<int, int> expected_pair : expected)
    {
        ...
        validate_temp_pair(memmgr,
                           std::string("my_alloc"),
                           var_name,
                           std::pair<int, int>(expected_pair.first, expected_pair.second));
    }
...
TEST_F(MM_stl_checkpoint, i_multiset)
{
    ...
    // ASSERT
    validate_temp_sequence<int>(memmgr,
                                std::string("my_alloc"),
                                std::string("i_multiset"),
...
TEST_F(MM_stl_checkpoint, pair_array)
{
    ...
    for(unsigned int i = 0; i < test_data.size(); i += 2)
    {
        ...
        validate_temp_pair(memmgr,
                           std::string("my_alloc"),
                           var_name,
                           std::pair<int, int>(test_data[i], test_data[i + 1]));
    }
...
TEST_F(MM_stl_checkpoint, vec_array)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        ...
        testbed->vec_array[i].insert(testbed->vec_array[i].end(),
                                     test_data.begin() + (i * 10),
                                     test_data.begin() + ((i + 1) * 10));
    }
