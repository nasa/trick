...
class MM_stl_checkpoint : public ::testing::Test
{
    protected:
        ...
        ~MM_stl_checkpoint() { delete memmgr; }
        void SetUp() { }
        void TearDown() { }
};
...
template <typename T>
void validate_temp_sequence(
    Trick::MemoryManager* memmgr, std::string object_name, std::string var_name, std::vector<T> expected_data)
{
    std::string temp_name = object_name + "_" + var_name;
    ASSERT_TRUE(memmgr->var_exists(temp_name) == 1);
...
void validate_links_sequences(
    Trick::MemoryManager* memmgr, std::string object_name, std::string top_level_name, std::vector<int> lengths)
{
...
void validate_links_pairs(
    Trick::MemoryManager* memmgr, std::string object_name, std::string top_level_name, int num_pairs)
{
    std::string temp_name = object_name + "_" + top_level_name;
    ASSERT_TRUE(memmgr->var_exists(temp_name) == 1);
...
template <typename T>
void validate_temp_set(
    Trick::MemoryManager* memmgr, std::string object_name, std::string var_name, std::set<T> expected_data)
{
    std::string temp_name = object_name + "_" + var_name;
    ASSERT_TRUE(memmgr->var_exists(temp_name) == 1);
...
void validate_temp_pair(
    Trick::MemoryManager* memmgr, std::string object_name, std::string var_name, std::pair<First, Second> expected_data)
{
    std::string temp_name   = object_name + "_" + var_name;
...
void validate_temp_map(
    Trick::MemoryManager* memmgr, std::string object_name, std::string var_name, std::map<Key, Val> expected_data)
{
    std::string temp_name = object_name + "_" + var_name;
...
TEST_F(MM_stl_checkpoint, s_vec)
{
    ...
    for (unsigned int i = 0; i < test_data.size() / 2; i++)
    {
        ...
        validate_temp_pair<int, int>(
            memmgr, std::string("my_alloc"), var_name, std::pair<int, int>(test_data[i * 2], test_data[i * 2 + 1]));
    }
...
TEST_F(MM_stl_checkpoint, i_i_pair)
{
    ...
    // ASSERT
    validate_temp_pair<double, float>(
        memmgr, std::string("my_alloc"), vec_attr->name, std::pair<double, float>(test_data[0], test_data[1]));
}
...
TEST_F(MM_stl_checkpoint, pair_pair)
{
    ...
    validate_temp_pair(
        memmgr, std::string("my_alloc"), std::string("pair_pair_first_inner"), std::pair<int, int>(1, 2));
...
TEST_F(MM_stl_checkpoint, s_i_map)
{
    ...
    for (unsigned int i = 0; i < lengths.size(); i++)
    {
        test_map[std::set<int>(test_keys.begin() + start_index, test_keys.begin() + start_index + lengths[i])]
            = test_vals[i];
        testbed->s_i_map[std::set<int>(test_keys.begin() + start_index, test_keys.begin() + start_index + lengths[i])]
            = test_vals[i];
        start_index += lengths[i];
    }
...
TEST_F(MM_stl_checkpoint, s_s_map)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        testbed->s_s_map[std::pair<int, int>(test_keys[i * 2], test_keys[(i * 2) + 1])]
            = std::vector<int>(test_vals.begin() + (i * 4), test_vals.begin() + (i + 1) * 4);
        test_map[std::pair<int, int>(test_keys[i * 2], test_keys[(i * 2) + 1])]
            = std::vector<int>(test_vals.begin() + (i * 4), test_vals.begin() + (i + 1) * 4);
    }
...
TEST_F(MM_stl_checkpoint, s_queue)
{
    ...
    for (unsigned int i = 0; i < test_first.size(); i++)
    {
        ...
        validate_temp_pair(
            memmgr, std::string("my_alloc"), var_name, std::pair<int, double>(test_first[i], test_second[i]));
    }
...
TEST_F(MM_stl_checkpoint, nested_list_queue)
{
    ...
    {
        testbed->nested_list_queue.push(
            std::list<float>(test_data.begin() + data_index, test_data.begin() + data_index + list_sizes[i]));
    ...
    for (unsigned int i = 0; i < list_sizes.size(); i++)
    {
        ...
        validate_temp_sequence(memmgr, std::string("my_alloc"), var_name,
            std::vector<float>(test_data.begin() + data_index, test_data.begin() + data_index + list_sizes[i]));
        data_index += list_sizes[i];
    }
...
TEST_F(MM_stl_checkpoint, i_stack)
{
    ...
    validate_temp_sequence<long long>(memmgr, std::string("my_alloc"), std::string("i_stack"),
        std::vector<long long>(test_data.rbegin(), test_data.rend()));
}
...
TEST_F(MM_stl_checkpoint, s_stack)
{
    ...
    for (unsigned int i = 0; i < test_first.size(); i++)
    {
        ...
        validate_temp_pair(memmgr, std::string("my_alloc"), var_name,
            std::pair<short, double>(test_first[test_first.size() - 1 - i], test_second[test_first.size() - 1 - i]));
    }
...
TEST_F(MM_stl_checkpoint, nested_list_stack)
{
    ...
    for (unsigned int i = 0; i < list_sizes.size(); i++)
    {
        testbed->nested_list_stack.push(
            std::list<float>(test_data.begin() + data_index, test_data.begin() + data_index + list_sizes[i]));
        data_index += list_sizes[i];
    }
    ...
    validate_links_sequences(memmgr, std::string("my_alloc"), std::string("nested_list_stack"),
        std::vector<int>(list_sizes.rbegin(), list_sizes.rend()));
}
...
TEST_F(MM_stl_checkpoint, i_set)
{
    ...
    // ASSERT
    validate_temp_set<char>(
        memmgr, std::string("my_alloc"), std::string("i_set"), std::set<char>(test_data.begin(), test_data.end()));
}
...
TEST_F(MM_stl_checkpoint, vector_set)
{
    ...
    // validate_temp_set<char>(memmgr, std::string("my_alloc"), std::string("vector_set"), std::set<char>
...
TEST_F(MM_stl_checkpoint, s_set)
{
    ...
    for (std::pair<int, int> expected_pair : expected)
    {
        ...
        validate_temp_pair(
            memmgr, std::string("my_alloc"), var_name, std::pair<int, int>(expected_pair.first, expected_pair.second));
    }
...
TEST_F(MM_stl_checkpoint, i_multiset)
{
    ...
    // ASSERT
    validate_temp_sequence<int>(
        memmgr, std::string("my_alloc"), std::string("i_multiset"), std::vector<int>(expected.begin(), expected.end()));
}
...
TEST_F(MM_stl_checkpoint, pair_array)
{
    ...
    for (unsigned int i = 0; i < test_data.size(); i += 2)
    {
        ...
        validate_temp_pair(
            memmgr, std::string("my_alloc"), var_name, std::pair<int, int>(test_data[i], test_data[i + 1]));
    }
...
TEST_F(MM_stl_checkpoint, vec_array)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        testbed->vec_array[i].insert(
            testbed->vec_array[i].end(), test_data.begin() + (i * 10), test_data.begin() + ((i + 1) * 10));
    }
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        validate_temp_sequence(memmgr, std::string("my_alloc"), var_name,
            std::vector<int>(test_data.begin() + (i * 10), test_data.begin() + ((i + 1) * 10)));
    }
...
TEST_F(MM_stl_checkpoint, recursive_nightmare)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        {
            testbed->recursive_nightmare[i].insert(
                std::pair<std::pair<int, int>, std::vector<std::stack<std::string>>>(keys[m], vals[m]));
            recursive_nightmare_copy[i].insert(
                std::pair<std::pair<int, int>, std::vector<std::stack<std::string>>>(keys[m], vals[m]));
        }
    }
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        for (auto it = recursive_nightmare_copy[i].begin(); it != recursive_nightmare_copy[i].end(); ++it, j++)
        {
            ...
            {
                std::string val_var_name = "recursive_nightmare_" + std::to_string(i) + "_data_" + std::to_string(j)
                    + "_" + std::to_string(k);
