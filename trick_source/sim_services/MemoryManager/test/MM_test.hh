 ...
 */
template <typename T>
void validate_alloc_info(ALLOC_INFO* alloc_info, T* test_var, TRICK_TYPE trick_type, const char* type_name,
    const char* var_name, int num, int num_index, int* extent)
{
    int ii;
    ...
    {
        EXPECT_TRUE(alloc_info->attr != NULL) << "If alloc_info->type is TRICK_STRUCTURED or TRICK_ENUMERATED then"
    ...
    {
        EXPECT_TRUE(alloc_info->attr == NULL) << "If alloc_info->type is not TRICK_STRUCTURED nor TRICK_ENUMERATED then"
...
template <typename T>
void validate_alloc_info(Trick::MemoryManager* memmgr, T* test_var, TRICK_TYPE trick_type, const char* type_name,
    const char* var_name, int num, int num_index, int* extent)
{
...
template <typename T, typename std::enable_if<!std::is_floating_point<T>::value>::type* = nullptr> T random()
{
    return (T)(rand());
}
...
template <> bool random<bool>() { return (bool)(rand() & 1); }
...
template <typename T> std::vector<T> get_test_data(int num)
{
    std::vector<T> ret;
    for (int i = 0; i < num; i++)
    {
