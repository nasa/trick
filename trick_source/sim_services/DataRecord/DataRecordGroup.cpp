...
Trick::LoggingCycle::LoggingCycle(double rate_in) { set_rate(rate_in); }
...
Trick::DataRecordGroup::~DataRecordGroup() { free((void*)time_value_attr.units); }
...
const std::string& Trick::DataRecordGroup::get_group_name() { return group_name; }
...
int Trick::DataRecordGroup::write_header()
{
    ...
    for (jj = 0; jj < rec_buffer.size(); jj++)
    {
        ...
        out_stream << "log_" << group_name << "\t"
                   << type_string(rec_buffer[jj]->ref->attr->type, rec_buffer[jj]->ref->attr->size) << "\t"
                   << std::setw(6);
...
int Trick::DataRecordGroup::write_data(bool must_write)
{
    ...
    if (record and inited and (buffer_type == DR_No_Buffer or must_write) and (total_bytes_written <= max_file_size))
    {
        ...
        {
            std::cerr << "WARNING: Data record max file size " << (static_cast<double>(max_file_size)) / (1 << 20)
                      << "MB reached.\n"
                         "https://nasa.github.io/trick/documentation/simulation_capabilities/"
                         "Data-Record#changing-the-max-file-size-of-a-data-record-group-ascii-and-binary-only"
                      << std::endl;
