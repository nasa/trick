...
*/
Trick::DRBinary::DRBinary(std::string in_name, bool register_group, Trick::DR_Type dr_type)
...
int Trick::DRBinary::format_specific_write_data(unsigned int writer_offset)
{
    ...
    for(ii = 0; ii < rec_buffer.size(); ii++)
    {
        ...
        switch(rec_buffer[ii]->ref->attr->type)
        {
            ...
            case TRICK_BITFIELD:
                sbf = GET_BITFIELD(address,
                                   rec_buffer[ii]->ref->attr->size,
            ...
            case TRICK_UNSIGNED_BITFIELD:
                bf = GET_UNSIGNED_BITFIELD(address,
                                           rec_buffer[ii]->ref->attr->size,
                                           rec_buffer[ii]->ref->attr->index[0].start,
                                           rec_buffer[ii]->ref->attr->index[0].size);
