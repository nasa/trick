...
Trick::DRHDF5::DRHDF5(std::string in_name, Trick::DR_Type dr_type)
    : Trick::DataRecordGroup(in_name, dr_type)
{
    ...
    for(ii = 0; ii < rec_buffer.size(); ii++)
    {
        ...
        if(param_dataset_ids[ii] == H5I_BADID)
        {
            message_publish(MSG_ERROR,
                            "An error occured in data record group \"%s\" when adding \"%s\".\n",
                            group_name.c_str(),
                            param_names[ii]);
            continue;
        }
    ...
    switch(drb->ref->attr->type)
    {
        ...
        case TRICK_BITFIELD:
            ...
            for(size_t j = 0; j < records; j++)
            {
                ...
                {
                    ((int *)data)[j] = extract_bitfield_any(*(int *)(buf + offset),
                                                            drb->ref->attr->size,
                                                            drb->ref->attr->index[0].start,
                                                            drb->ref->attr->index[0].size);
                }
                else if(drb->ref->attr->size == sizeof(short))
                {
                    ((short *)data)[j] = extract_bitfield_any(*(short *)(buf + offset),
                                                              drb->ref->attr->size,
                                                              drb->ref->attr->index[0].start,
                                                              drb->ref->attr->index[0].size);
                }
                ...
                {
                    ((char *)data)[j] = extract_bitfield_any(*(char *)(buf + offset),
                                                             drb->ref->attr->size,
                                                             drb->ref->attr->index[0].start,
                                                             drb->ref->attr->index[0].size);
                }
        ...
        case TRICK_UNSIGNED_BITFIELD:
            ...
            for(size_t j = 0; j < records; j++)
            {
                ...
                if(drb->ref->attr->size == sizeof(int))
                {
                    ((unsigned int *)data)[j] = extract_unsigned_bitfield_any(*(unsigned int *)(buf + offset),
                                                                              drb->ref->attr->size,
                                                                              drb->ref->attr->index[0].start,
                                                                              drb->ref->attr->index[0].size);
                }
                ...
                {
                    ((unsigned short *)data)[j] = extract_unsigned_bitfield_any(*(unsigned short *)(buf + offset),
                                                                                drb->ref->attr->size,
                                                                                drb->ref->attr->index[0].start,
                                                                                drb->ref->attr->index[0].size);
                }
                ...
                {
                    ((unsigned char *)data)[j] = extract_unsigned_bitfield_any(*(unsigned char *)(buf + offset),
                                                                               drb->ref->attr->size,
                                                                               drb->ref->attr->index[0].start,
                                                                               drb->ref->attr->index[0].size);
                }
                ...
                {
                    ((int *)data)[j] = extract_unsigned_bitfield_any(*(int *)(buf + offset),
                                                                     drb->ref->attr->size,
                                                                     drb->ref->attr->index[0].start,
                                                                     drb->ref->attr->index[0].size);
                }
