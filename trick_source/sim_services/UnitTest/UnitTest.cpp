...
Trick::TestSuite::TestSuite() { num_failures = 0; }
...
int Trick::UnitTest::add_test_results(
    std::string in_test_suite_name, std::string in_test_case, std::string in_failure_string)
{
    if (enabled)
    {
        ...
        if (in_failure_string.compare(""))
        {
            message_publish(MSG_ERROR, "[  FAILED  ] %s:%s %s\n", in_test_suite_name.c_str(), in_test_case.c_str(),
                in_failure_string.c_str());
        }
        else
        {
            message_publish(
                MSG_NORMAL, "\033[32m[       OK ]\033[0m %s:%s\n", in_test_suite_name.c_str(), in_test_case.c_str());
        }
    }
...
int Trick::UnitTest::add_test_requirements(
    std::string in_test_suite_name, std::string in_test_case, std::string par_num)
{
    if (enabled)
    {
