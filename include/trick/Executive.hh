...
namespace Trick
{
    ...
    class Executive : public Trick::Scheduler
    {
        ...
        protected:
            ...
            /** Allows the current thread to give up the cpu during multi process job completion and dependency
             * checking.\n */
            ...
            /** Count of how many sim_objects have been added to the scheduler. May be different than current number of
             * sim_objects\n */
        ...
        public:
            ...
            */
            ...
            /**
             @userdesc Command to get the stack trace toggle value (will the sim attempt stack trace if signal shuts it
             down).
             @par Python Usage:
            ...
            */
            ...
            /**
             ...
             @param on_off - boolean yes (C integer 1) = attach debugger if signal shuts sim down. no (C integer 0) = do
             not attach debugger.
             @return always 0
            */
            ...
            */
            ...
            /**
             ...
             @param on_off - boolean yes (C integer 1) = allow <CTRL-C> freezing. no (C integer 0) = disallow <CTRL-C>
             freezing.
             @return always 0
            */
            ...
            /**
             ...
             @param on_off - boolean yes (C integer 1) = go to freeze mode from initialization. no (C integer 0) = go to
             run mode from initialization
             @return always 0
            */
            ...
            */
            ...
            /**
             @userdesc Command to set the stack trace toggle value (sim will attempt stack trace if signal shuts it
             down).
             @par Python Usage:
            ...
            */
            ...
            /**
             ...
             If on_off is set to false (default), the signal will not be trapped which causes any affected variable to
             contain Nan. Setting on_off to true will trap the operating system floating point exception signal and then
            ...
            */
            ...
            /**
             * @brief Adds JobData pointer to assigned thread queue if scheduled class job.  For non-scheduled jobs i.e.
             * "initialization", all jobs added to main thread queues.
            ...
            /**
             * Adds a "depends on" job to the target job.  The target job will wait for the dependent job to finish
             * before starting.  The target and depends on jobs must be on different threads.
             ...
             * @return 0 if dependency added, -1 if target_job not found, -2 if depend_job not found -3 if target and
             * depend job on same thread
             */
            virtual int add_depends_on_job(
                std::string target_job, unsigned int t_instance, std::string depend_job, unsigned int d_instance);
            ...
            */
            ...
            /**
             @userdesc Command to set the thread scheduling type of a child thread.  Does not affect main thread, which
             is fixed at scheduled.
             @par Python Usage:
            ...
            */
            ...
            /**
             ...
             @param thread_id - thread id as specified in S_define file
             @param process_type - integer representation of enumeration.  0 = scheduled, 1 = asynchronous, 2 =
             asynchronous_must_finish
            ...
            */
            ...
            /**
             @userdesc Command to set a child thread's asynchronous wait at shutdown flag, so that the executive will
             wait for the child to finish before shutting down (before shutdown class jobs are run, etc).
             @par Python Usage:
            ...
            */
            ...
            /**
             ...
             otherwise add in_job before each job in the initialization, integration, scheduled, end_of_frame, &
             shutdown job queues. Requirement [@ref r_exec_instrument_0]
            ...
            */
            ...
            /**
             Actually performs the job insertions described above. Called by the above instrument_job_before with the
             appropriate instrument job pointer. Requirement [@ref r_exec_instrument_0]
            ...
            */
            ...
            /**
             ...
             otherwise add in_job after each job in the initialization, integration, scheduled, end_of_frame, & shutdown
             job queues. Requirement [@ref r_exec_instrument_2]
             @param in_job - name of the instrument job
             @param target_job - name of the target job, empty string means all jobs are to be instrumented
             @param in_event - instrumentation event data to be stored with instrument job
             @return number of times the job was inserted
            */
            ...
            /**
             Actually performs the job insertions described above. Called by the above instrument_job_after with the
             appropriate instrument job pointer. Requirement [@ref r_exec_instrument_2]
             @param instrument_job - pointer to the instrument job
             @param target_job - name of the target job, empty string means all jobs are to be instrumented
             @param in_event - instrumentation event data to be stored with instrument job
             @return number of times the job was inserted
            ...
            */
            ...
            /**
             @brief Writes the S_job_execution file which details the jobs known to the scheduler at the end of
             initialization.
            ...
            */
            ...
            /**
             @userdesc Transition to Shutdown mode.  This matches the calling arguments of the legacy exec_terminate
             call. Same as calling exec_terminate_with_return() with ret_code=-1 and line=0.
