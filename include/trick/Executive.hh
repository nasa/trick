...
public:
    ...
    */
    ...
    /**
     ...
     @param on_off - boolean yes (C integer 1) = attach debugger if signal shuts sim down. no (C integer 0) = do not
     attach debugger.
     @return always 0
    */
    ...
    */
    ...
    /**
     ...
     @param on_off - boolean yes (C integer 1) = go to freeze mode from initialization. no (C integer 0) = go to run
     mode from initialization
     @return always 0
    */
    ...
    */
    ...
    /**
     ...
     If on_off is set to false (default), the signal will not be trapped which causes any affected variable to contain
     Nan. Setting on_off to true will trap the operating system floating point exception signal and then perforam a
     graceful simulation shutdown. Trapping this signal is useful in debugging where floating point exception occurs.
     @par Python Usage:
    ...
    */
     ...
     * @return 0 if dependency added, -1 if target_job not found, -2 if depend_job not found -3 if target and depend job
     * on same thread
     */
    virtual int add_depends_on_job(std::string target_job,
                                   unsigned int t_instance,
                                   std::string depend_job,
    ...
    */
    ...
    /**
     @userdesc Command to set the thread scheduling type of a child thread.  Does not affect main thread, which is fixed
     at scheduled.
     @par Python Usage:
    ...
    */
    ...
    /**
     @userdesc Command to set a child thread's asynchronous wait at shutdown flag, so that the executive will wait for
     the child to finish before shutting down (before shutdown class jobs are run, etc).
     @par Python Usage:
    ...
    */
    ...
    /**
     ...
     otherwise add in_job before each job in the initialization, integration, scheduled, end_of_frame, & shutdown job
     queues. Requirement [@ref r_exec_instrument_0]
    ...
    */
    ...
    /**
     ...
     otherwise add in_job after each job in the initialization, integration, scheduled, end_of_frame, & shutdown job
     queues. Requirement [@ref r_exec_instrument_2]
     @param in_job - name of the instrument job
     @param target_job - name of the target job, empty string means all jobs are to be instrumented
     @param in_event - instrumentation event data to be stored with instrument job
     @return number of times the job was inserted
