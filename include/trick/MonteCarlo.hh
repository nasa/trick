...
static const char* MC_exit_status_string[] = {"Incomplete", "Complete", "Failed", "Core Dumped", "Timed Out",
    "No Permission to Output Directory", "Bad Input", "Unrecognized Return Code"};
...
namespace Trick
{
    ...
    class MonteRange
    {
        ...
        public:
            /**
             * Constructs a MonteRange with the specified inclusive end points. Specifying an end value that is less
             * than the start value results in a range which includes only the start value.
             *
            ...
            /** Gets #end. */
            unsigned int get_end() { return end; }
    };
    ...
    class MonteCarlo : public Trick::Scheduler
    {
        ...
        protected:
            ...
            /**
             * Indicates whether or not this is a dry run. A dry run executes pre run jobs only. Post run jobs and the
             * runs themselves are not executed.
             */
            bool dry_run; /**< \n trick_units(--) */

            /**
             * Indicates whether or not the localhost should be treated as a remote machine. This determines if slaves
             * running locally use remote shells.
             */
            bool localhost_as_remote; /**< \n trick_units(--) */

            /**
             * Indicates how much automation should be employed in forming the commands used to dispatch slaves.
             * - A value of <code>true</code> indicates that Trick should form only the core of the command, which
             * consists of S_main executable with the proper slave-specific arguments. #custom_pre_text and
             * #custom_post_text will then be prepended and appended to the core, respectively, before executing the
             * command. #remote_shell_args, #user_cmd_string, MonteSlave::remote_shell, and MonteSlave::machine_name are
             * ignored.
             * - A value of <code>false</code> indicates that Trick should use the shell specified by
             * MonteSlave::remote_shell with the #remote_shell_args, followed by the slave's machine name,
             * #user_cmd_string, navigation to the correct directory, and finally the core command described above.
             * #custom_pre_text and #custom_post_text are ignored.
             */
        ...
        public:
            ...
            /**
             * Stops the slave with the specified id if it exists. Stopping a slave puts it into a state in which it
             * will not accept new runs from the master.
             *
        ...
        protected:
            ...
            bool equals_ignore_case(
                std::string string1, std::string string2); // I am appalled by having to write this myself.
    };
