...
// queue: intrinsic
template <typename ITEM_TYPE, typename _Sequence, typename std::enable_if<!is_stl_container<ITEM_TYPE>::value>::type*>
...
int checkpoint_stl(
    std::priority_queue<ITEM_TYPE, _Container, _Compare>& in_stl, std::string object_name, std::string var_name)
{
    unsigned int ii;
    unsigned int cont_size;
    std::ostringstream var_declare;
    ...
    if (cont_size > 0)
    {
        ...
        if (items)
        {
            ...
            for (ii = 0; ii < cont_size; ii++)
            {
                ...
                index_string << ii;
                checkpoint_stl(
                    const_cast<ITEM_TYPE&>(temp_queue.top()), object_name + "_" + var_name, index_string.str());
                temp_queue.pop();
            }
        }
    }
