...
#define PUT_BITFIELD(addr, value, size, start, bits)                                                                   \
    if(size == sizeof(int))                                                                                            \
        *(int *)addr = insert_bitfield_any((*(unsigned int *)addr), value, size, start, bits);                         \
    else if(size == sizeof(short))                                                                                     \
        *(short *)addr = (insert_bitfield_any((*(unsigned short *)addr), value, size, start, bits));                   \
    else                                                                                                               \
        *(char *)addr = (insert_bitfield_any((*(unsigned char *)addr), value, size, start, bits))
