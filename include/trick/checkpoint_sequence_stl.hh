...
template <class STL> int checkpoint_sequence_i(STL& in_stl, std::string object_name, std::string var_name)
{
    unsigned int ii;
    ...
    if (cont_size > 0)
    {
        ...
        var_declare << type_string << " " << object_name << "_" << var_name << "[" << cont_size << "]";
...
template <class STL> int checkpoint_sequence_s(STL& in_stl, std::string object_name, std::string var_name)
{
    unsigned int ii;
    unsigned int cont_size;
    std::ostringstream var_declare;
    ...
    if (cont_size > 0)
    {
        ...
        if (items)
        {
            ...
            for (ii = 0, it = in_stl.begin(), end = in_stl.end(); it != end; ++it, ii++)
            {
                ...
                checkpoint_stl(
                    const_cast<typename STL::value_type&>(*it), object_name + "_" + var_name, index_string.str());
            }
        }
    }
    return 0;
...
template <typename ITEM_TYPE, std::size_t N, typename std::enable_if<is_stl_container<ITEM_TYPE>::value>::type*>
...
template <typename ITEM_TYPE, typename _Alloc, typename std::enable_if<is_stl_container<ITEM_TYPE>::value>::type*>
 ...
 */
template <class STL> int restore_sequence_i(STL& in_stl, std::string object_name, std::string var_name)
{
    unsigned int ii;
    unsigned int cont_size;
    std::string temp_str;
...
template <class STL> int restore_sequence_s(STL& in_stl, std::string object_name, std::string var_name)
{
    unsigned int ii;
    unsigned int cont_size;
