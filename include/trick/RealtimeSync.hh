...
namespace Trick
{
    ...
    class RealtimeSync
    {
        public:
            ...
            /**
             @brief @userdesc Start real-time clock and set to the given time. Trick will automatically call this
             routine whenever real-time is enabled by the user. Called as an initialization job (or as a restart job
             after a checkpoint reload) in the S_define file, or by the real-time monitor if the user enables real-time
             while the sim is executing.
             @par Python Usage:
