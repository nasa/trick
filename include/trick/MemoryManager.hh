...
public:
    ...
    /**
     ...
     @return - an address to the allocated memory or NULL on failure.
     */
    void * declare_operatornew_var(std::string class_name,
    ...
    */
     ...
     */
    int add_attr_info(const std::string & type_name,
                      ATTRIBUTES * attr,
    ...
    ALLOC_INFO_MAP_ITER alloc_info_map_begin()
    {
        return alloc_info_map.begin();
    }

    ALLOC_INFO_MAP_ITER alloc_info_map_end()
    {
        return alloc_info_map.end();
    }

    VARIABLE_MAP_ITER variable_map_begin()
    {
        return variable_map.begin();
    }

    VARIABLE_MAP_ITER variable_map_end()
    {
    ...
    static bool restore_stls_default; /**< -- true = restore STL variables on checkpoint restore if user does not
                                         specify option. false = don't */
...
private:
    ...
    bool reduced_checkpoint;  /**< -- true = Don't write zero valued variables in the checkpoint. false= Write all
                                 values. */
    bool hexfloat_checkpoint; /**< -- true = Represent floating point values as hexidecimal to preserve precision.
                                 false= Normal. */
    bool hexfloat_decimal_comment_checkpoint; /**< -- true = Add decimal representation comment for hexfloat values.
                                                 false= no decimal representation comment. */
    ...
    void restore_stls_in_arrayed_class(std::string name, char * address, ATTRIBUTES * attr, int curr_dim, int offset);
     ...
     */
    void recursive_array_copy(
        void * s_base, void * d_base, int * s_cdims, int * d_cdims, size_t size, int n, int cur, int s_off, int d_off);
