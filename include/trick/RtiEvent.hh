...
namespace Trick
{
    ...
    template <class T> class RtiEventInfo : public RtiEventBase
    {
        public:
            RtiEventInfo(REF2* in_ref, T in_val)
                : RtiEventBase(in_ref)
                , value(in_val) { };
    ...
    template <class T> class RtiBitfieldEventInfo : public RtiEventBase
    {
        public:
            RtiBitfieldEventInfo(REF2* in_ref, T in_val)
                : RtiEventBase(in_ref)
                , value(in_val) { };
            virtual void do_assignment(void)
            {
                ...
                {
                    PUT_BITFIELD(
                        ref->address, (int)value, sizeof(T), ref->attr->index[0].start, ref->attr->index[0].size);
                }
            };
            virtual std::string print_val()
            {
    ...
    template <class T> RtiEventBase* createRtiEvent(char* variable, T value)
    {
