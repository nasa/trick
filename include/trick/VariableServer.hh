...
namespace Trick
{
    ...
    class VariableServer
    {
        ...
        Each new session will create a file under RUN_<>/session_logs/ containg the client's IP and port and all
        messages received.
        @par Python Usage:
        @code trick.set_var_server_log_on() @endcode
        @return always 0
        */
            ...
            */
            ...
            /**
             ...
             @return 0 if successful
            */
            int create_multicast_socket(const char* mcast_address, const char* source_address, unsigned short port);
        ...
        protected:
            ...
            /** Pointer to automatic_last job that copies requested variable values to their output buffers in sync
             * mode.\n */
            Trick::JobData* copy_data_job; /**< trick_io(**) trick_units(--) */

            /** Pointer to freeze_automatic job that copies requested variable values to their output buffers in sync
             * mode.\n */
