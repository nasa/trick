...
namespace Trick
{
    class VariableReference
    {
        public:
            ...
            // These variables have 2 staging buffers that we can swap between to allow for different copy and write-out
            // modes stageValue copies data from the simulation into one of the buffers (stage_buffer) prepareForWrite
            // swaps the pointers for stage_buffer and write_buffer This way we can have data ready for writing, but
            // also be copying out from the sim at the same time stageValue must be called first, and then prepare for
            // write, and then writeValue* can be called.
