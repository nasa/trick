...
namespace Trick
{
    ...
    /**
     * A variable whose values are calculated by the user. This class is meant to provide the user with a way to fine
     * tune a Monte Carlo's progression, using results from previous runs to influence the values used in future runs.
     * The value is taken from a simulation variable named by the user. The proper way to update that reference variable
     * is to change its value in a Trick job of type <code>"monte_master_pre"</code> or
     * <code>"monte_master_post"</code>.
     *
     * @author Alex Lin
     * @author Donna Panter
