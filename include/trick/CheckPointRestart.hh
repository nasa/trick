...
public:
    ...
    std::priority_queue<long long, std::vector<long long>, std::greater<long long>> checkpoint_times; /**<
                                                                                                         trick_units(--)
                                                                                                       */
    ...
    */
    ...
    /**
     @brief @userdesc Command to set the CPU to use for checkpoints.  The default is to use the same CPU as the main
     thread. If the main thread of the simulation is running at high real-time priority, it is recommended to choose a
     different CPU to use for checkpointing.
     @par Python Usage:
    ...
    /**
     ...
     safestore_period to the integral time tic value corresponding to the incoming in_time so that checkpoint occurs
     periodically.)
     @par Python Usage:
    ...
    /**
     @brief @userdesc Command to dump a checkpoint at in_time. (Sets checkpoint_time to the integral time tic value
     corresponding to the incoming in_time so that checkpoint occurs once at that time at the end of the execution
     frame.) The checkpointed file name is @e chkpnt_<in_time>.
     @par Python Usage:
    ...
    /**
     @brief @userdesc Command to load a checkpoint file. (Calls the preload_checkpoint jobs, calls the MemoryManager
     restore_managed_memory method, then calls the restart jobs.) This is invoked when the user clicks the "Load ASCII
     Chkpnt" button on the sim control panel.
     @par Python Usage:
    ...
    /**
     @brief @userdesc Command to load a checkpoint file. (Calls the preload_checkpoint jobs, calls the MemoryManager
     restore_managed_memory method, then calls the restart jobs.)
     @par Python Usage:
     @code trick.load_checkpoint("<file_name>") @endcode
     @param file_name - file to read checkpoint data from
     @return always 0
