...
extern "C"
{
    ...
    typedef void (*TrickErrorFuncPtr)(
        struct _TrickErrorHndlr * this_error, TrickErrorLevel level, const char * file, int line, const char * msg);
    ...
    void trick_error_shutdown(TrickErrorHndlr * error_hndlr); /* IN: -- Error object */

    void trick_error_set_func(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err handler data */
                              TrickErrorFuncPtr error_func); /* IN: -- Error function */

    void trick_error_set_data(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err handler data */
                              TrickErrorDataPtr error_data); /* IN: -- Error data */

    void trick_error_set_report_level(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err handler data */
                                      TrickErrorLevel report_level); /* IN: -- Err reporting level */

    void trick_error_set_stream(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err handler data */
                                TrickErrorLevel level,         /* IN: -- Err reporting level
                                                                         stream to set. */
                                FILE * stream);                /* IN: -- Err stream for
                                                                         reporting level. */

    void trick_error_set_all_streams(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err handler data */
                                     FILE * stream);                /* IN: -- Error stream */

    void trick_error_copy_streams(TrickErrorHndlr * dest_error_hndlr, /* OUT: -- Err hdler data*/
    ...
    void trick_error_set_all_send_hs_flags(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err handler data */
                                           int use_send_hs);              /* IN: -- 1= use send_hs;
                                                                                    else use fprintf */

    void trick_error_copy_send_hs_flags(TrickErrorHndlr * dest_error_hndlr, /* OUT: -- Err hdler data */
                                        TrickErrorHndlr * src_error_hndlr); /* IN:  -- Err hdler data */

    TrickErrorFuncPtr trick_error_get_func(TrickErrorHndlr * error_hndlr); /* INOUT: -- Err hdler data */

    TrickErrorDataPtr trick_error_get_data(TrickErrorHndlr * error_hndlr); /* INOUT: -- Error hdler data */

    TrickErrorLevel trick_error_get_report_level(TrickErrorHndlr * error_hndlr); /* INOUT: -- Error hdler data */

    FILE * trick_error_get_stream(TrickErrorHndlr * error_hndlr, /* INOUT: -- Error hdler data */
                                  TrickErrorLevel level);        /* IN: -- Error reporting
                                                                           level stream to get */

    FILE * trick_error_get_curr_stream(TrickErrorHndlr * error_hndlr); /* INOUT: -- Err hdler data */

    int trick_error_get_send_hs_flag(TrickErrorHndlr * error_hndlr, /* INOUT: -- Err hdler data */
                                     TrickErrorLevel level);        /* IN: -- Err report level
                                                                              stream to get. */

    int trick_error_get_curr_send_hs_flag(TrickErrorHndlr * error_hndlr); /* INOUT: -- Err hdler data */
