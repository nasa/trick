...
int PoolTable::state_deriv()
{
    ...
    for (int i = 0; i < numBalls; i++)
    {
        ...
        // Has weird behavior when velocity is very small, so only apply friction if velocity is greater than a
        // tolerance
    ...
    for (ii = 0; ii < numBalls; ii++)
    {
        ...
        for (jj = 0; jj < numBumpers; jj++)
        {
            ...
            if (this_tgo == 0)
            {
                ...
                {
                    // When a ball hits exactly a corner of 2 bumpers, both collisions will be detected and impulses
                    // applied. But since an impulse takes into consideration the original velocity, the second impulse
                    // applied just cancels out the first one most of the time. Both impulses should be identical since
                    // the collision point is the same, so make sure only 1 gets applied. This is kind of hacky
                    continue;
                }
...
void PoolTable::resetCueBall() { resetCueBall(defaultCueBallX, defaultCueBallY); }
...
void PoolTable::applyCueForce(double x_end, double y_end) { applyCueForce(x_end, y_end, 0, 0, 0); }

void PoolTable::applyCueForce(
    double x_end, double y_end, double cueHorizontalDisplacement, double cueVerticalDisplacement, double cueAngle)
{
    ...
    double force = (2 * m * cue_v)
        / (1.0 * (m / cueMass)
            + (5.0 / (2.0 * pow(r, 2)))
                * (pow(a, 2) + pow(b, 2) * pow(cos(theta), 2) + pow(c, 2) * pow(sin(theta), 2)
                    - 2 * b * c * sin(theta) * cos(theta)));
