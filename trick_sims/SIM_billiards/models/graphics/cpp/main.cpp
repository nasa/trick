...
std::vector<Eigen::Vector3d> ball_colors = {
    Eigen::Vector3d(0.0, 0.4, 0.0), // green
...
class Point
{
    public:
        ...
        Point(double x, double y, double layer)
            : point(x, y, layer)
        {
        }

        Eigen::Vector3d toVec3() const { return Eigen::Vector3d(point); }
...
{
    public:
        Polygon(unsigned int verts, double layer)
            : vertexMax(verts)
            , layer(layer)
        {
        }
        ...
        void setColor(double r, double g, double b) { color = Eigen::Vector3d(r, g, b); }

        void setColor(Eigen::Vector3d& c) { color = Eigen::Vector3d(c); }

        void setLayer(double l) { layer = l; }

        Eigen::Vector3d getColor() const { return color; }
        ...
        virtual bool isValid() const { return points.size() <= vertexMax; }
...
{
    public:
        Circle(double x, double y, double r, double layer)
            : Polygon(1, layer)
            , radius(r)
        {
            addPoint(x, y);
        ...
        bool isValid() const { return points.size() == 1 && radius >= 0; }
        ...
        RenderedShape* render() const
        {
            ...
            for (int i = 0; i <= numWedges; i++)
            {
                shape->vertices.emplace_back(points[0].x() + radius * cos(2 * M_PI * i / numWedges),
                    points[0].y() + radius * sin(2 * M_PI * i / numWedges), layer);
            }
...
class Rectangle : public Polygon
{
    public:
        ...
        Rectangle(double layer)
            : Polygon(2, layer)
        {
        }

        bool isValid() const { return points.size() == vertexMax; }

        void addCorner(double x, double y) { addPoint(x, y); }
...
class Table
{
    public:
        ...
        int addShape(
            std::vector<double> shapeData, Eigen::Vector3d color, bool isStatic, PolygonType type, double layer)
        {
            Polygon* shape;
...
void printUsage() { std::cout << "Usage: program <portNumber>" << std::endl; }
...
template <typename T> T stringConvert(const std::string& str)
{
...
template <typename T> std::vector<T> trickResponseConvert(std::string& response)
{
...
template <typename T> T getVar(Socket& socket, std::string varName)
{
...
template <typename T> std::vector<T> getVarList(Socket& socket, std::string varName, int num)
{
...
template <typename T> std::vector<T> fold(const std::vector<T> a, const std::vector<T> b)
{
    std::vector<T> result;
...
int main(int argc, char* argv[])
{
    ...
    {
        table.addShape(std::vector<double>({pocket_x[i], pocket_y[i], pocket_r[i]}), Eigen::Vector3d(0.0, 0.0, 0.0),
            true, CIRCLE, layer_POCKET);
    }
    ...
    view->callback_pre_draw = [&](igl::opengl::glfw::Viewer& viewer) -> bool
    {
        ...
        if (mousePressed)
        {
            ...
            std::vector<double> triangleData
                = {cueBallPos(0), cueBallPos(1), point1(0), point1(1), point2(0), point2(1)};
