...
int sun_pred_cyclic(SUN_PRED* S, double current_sim_time)
{
    ...
    S->utc.hour = (int)(tsec / 3600.0);
    tsec        = tsec - (S->utc.hour * 3600.0);
    S->utc.min  = (int)(tsec / 60.0);
    tsec        = tsec - (S->utc.min * 60.0);
    S->utc.sec  = (int)(tsec);
    ...
    S->local_time.hour = (int)(tsec / 3600.0);
    tsec               = tsec - (S->local_time.hour * 3600.0);
    S->local_time.min  = (int)(tsec / 60.0);
    tsec               = tsec - (S->local_time.min * 60.0);
    ...
    LST_secs_time(S->sidereal_time_at_Greenwich, S->observer_longitude, &S->local_sidereal_time);
    ...
    S->hour_angle = S->sidereal_time_at_Greenwich * (360.0 / 86400.0) - S->observer_longitude - S->right_ascension;

    /* Calculate the local azimuth and elevation of the Sun. */
    equatorial_to_local_horiz(S->observer_latitude, S->hour_angle, S->right_ascension, S->declination,
        &S->solar_azimuth, &S->solar_elevation);
