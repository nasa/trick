...
void amoeba_order(AMOEBA * A)
{
     ...
     */
    qsort(A->vertices, A->num_vertices, sizeof(double *), amoeba_compare_vertices);
 ...
 */
void amoeba_init(AMOEBA * A, int num_dims, double epsilon, int max_steps, double * simplex_point, double simplex_size)
{
    int ii, jj;
    ...
    {
        fprintf(stderr,
                "Initial simplex undefined by user.\n"
    ...
    for(ii = 0; ii < A->num_vertices; ii++)
    {
        A->vertices[ii] = (double *)malloc((A->num_dims + 1) * sizeof(double));
    }
    ...
    if(simplex_point != NULL)
    {
        ...
        for(ii = 1; ii < A->num_vertices; ii++)
        {
            for(jj = 0; jj < A->num_dims; jj++)
            {
                if(jj + 1 == ii)
                {
                    A->vertices[ii][jj] = simplex_point[jj] - simplex_size;
                }
                else
                {
