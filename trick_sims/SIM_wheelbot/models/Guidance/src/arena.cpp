...
int Arena::distanceBetween(GridSquare* orig, GridSquare* dest, int& distance)
{
    Point origPt;
    Point destPt;
    if (((getGridSquareCoordinates(orig, origPt)) == 0) && ((getGridSquareCoordinates(dest, destPt)) == 0))
    {
        distance
            = 10 * sqrt((destPt.x - origPt.x) * (destPt.x - origPt.x) + (destPt.y - origPt.y) * (destPt.y - origPt.y));
        return 0;
    }
