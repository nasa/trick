...
std::vector<Point> FindPath(GridSquare * origin, GridSquare * goal, Arena * arena)
{
    ...
    while(!openSet.empty())
    {
        ...
        for(neighborsIterator = neighbors.begin(); neighborsIterator != neighbors.end(); ++neighborsIterator)
        {
            ...
            std::vector<GridSquare *>::iterator closedSetIterator = find(closedSet.begin(), closedSet.end(), neighbor);
            ...
            if(closedSetIterator == closedSet.end())
            {
                ...
                std::vector<GridSquare *>::iterator openSetIterator = find(openSet.begin(), openSet.end(), neighbor);
