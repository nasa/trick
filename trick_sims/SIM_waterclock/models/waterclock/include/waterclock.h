...
typedef struct
{
    ...
    /*    ASSUMPTIONS
     *         1) For the purpose of determining if a pipe is submerged, it will be treated as a single point at the
     * center of the pipe 2) When the timing bucket is filled, a siphon will drain it instaneously 3) Buckets are
     * cylindrical
     */

    /*    DESCRIPTION
     *         This is a simulation of a simple water clock. The clock consists of two cylinders (buckets).
     *         The first bucket is known as the intake bucket. It receives water from an unspecified external source
     * (like a stream). The intake bucket has two cylindrical spouts extending from its sides, one known as an overflow
     * spout and the other the clock spout. The overflow spout is simply for maintaing an equilibrium of the water
     * level. It keeps the intake bucket from exceeding a certain water level, thus ensuring it's water level is
     * consistent. The clock spout pours water into the second bucket (timer bucket). In an ideal clock, the water level
     * of the intake bucket will remain consistent, meaning the clock spout will output at a constant rate and operate
     * as an accurate clock. The timer bucket receives water from the clock spout. The timer bucket has no spouts, it
     * fills until it is full, upon which a siphon instantaneously drains it. There exists a float in the timer bucket,
     * connected to an arrow which will point to vertical tick marks. As the timer bucket fills, the arrow will move
     * vertically along these ticks, indicating how much time has passed. Depending on the configuration of the clock
     * (size of the buckets, size of the spouts, external water source rate, and the number/spacing of ticks) you can
     * create a clock that tracks a variety of times.
     */
