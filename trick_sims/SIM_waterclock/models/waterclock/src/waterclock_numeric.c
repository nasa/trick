...
int waterclock_deriv(WATERCLOCK * WC)
{
    ...
    // Calculate flow rate using Torricelli's equation ( V = sqrt(2gh) ) to find water velocity. Multiple velocity by
    // spout area to find flow rate. Calculate input bucket spout flow rate.
    if(WC->intake_water_level > WC->intake_clock_spout_height)
    {
        WC->intake_clock_spout_flowrate = spout_area * sqrt(2 * WC->gravity *
                                                            (WC->intake_water_level - WC->intake_clock_spout_height));
    }
    else
    {
    ...
    {
        WC->intake_overflow_flowrate = overflow_area *
                                       sqrt(2 * WC->gravity * (WC->intake_water_level - WC->intake_overflow_height));
    }
    else
    {
...
int waterclock_integ(WATERCLOCK * WC)
{
    ...
    load_state(&WC->intake_bucket_vol, &WC->timer_bucket_vol, NULL);

    load_deriv(&WC->intake_bucket_net_flow, &WC->intake_clock_spout_flowrate, NULL);

    ipass = integrate();

    unload_state(&WC->intake_bucket_vol, &WC->timer_bucket_vol, NULL);
...
// Since we are inetgrating over water volume, we must convert volume to water level in order to track how quickly the
// timer bucket is filling up.
    ...
    if(tgo == 0.0)
    {
        ...
        if((WC->current_tick < WC->total_ticks) && (WC->current_tick >= 0))
        {
            ...
            message_publish(MSG_NORMAL,
                            "Tick %d, Sim Time %f, Water Level %f\n",
                            WC->current_tick,
                            exec_get_sim_time(),
                            WC->timer_water_level);
