...
void Aircraft::set_desired_compass_heading(double compass_heading) { desired_heading = compassToPsi(compass_heading); }
...
double northWestToPsi(double (&p)[2])
{
    ...
    {
        std::cerr << "Error: Arguments to northWestToPsi() are zero." << std::endl;
        psi = 0.0;
    }
    return psi;
}

double vector_magnitude(double (&vector)[2]) { return (sqrt(vector[0] * vector[0] + vector[1] * vector[1])); }
