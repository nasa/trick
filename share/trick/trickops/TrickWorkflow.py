...
class TrickWorkflow(WorkflowCommon):
    ...
    all_possible_phases = range(
        allowed_phase_range["min"], allowed_phase_range["max"] + 1
    )
    ...
    def listify_phase(phase=None):
        ...
        if phase is None:
            ...
            (isinstance(phase, list) or isinstance(phase, range))
            and all(isinstance(p, int) for p in phase)
            and all(p in TrickWorkflow.all_possible_phases for p in phase)
        ):
            phases = list(phase)  # Cast to list to cover range case
        else:
            msg = (
                "ERROR: Given phase %s in listify_given_phase() must be an int,"
                " list of ints, or range() within the bounds [%s, %s]"
                % (
                    phase,
    ...
    def _find_range_string(string):
        ...
        if (len(re.findall(pattern, string))) > 1:
            msg = (
                "ERROR: [min-max] pattern found more than once in %s. Only one instance is"
                " supported." % (string)
            )
            raise RuntimeError(msg)
    ...
    def __init__(
        self, project_top_level, log_dir, trick_dir, config_file, cpus=3, quiet=False
    ):
        """
        ...
        """
        super().__init__(
            project_top_level=project_top_level, log_dir=log_dir, quiet=quiet
        )
        ...
        self.parsing_errors = (
            self.yaml_verifier.parsing_errors
    ...
    def _populate_sims(self):
        ...
        for s in self.config:
            ...
            if not os.path.exists(
                os.path.join(self.project_top_level, self.config[s]["path"])
            ):
                cprint(
                    "ERROR: %s's 'path' %s not found. Continuing but skipping this entire entry from %s."
                    % (s, self.config[s]["path"], self.config_file),
                    "DARK_RED",
                )
                continue
            if self.config[s]["path"] in all_sim_paths:
                cprint(
                    "ERROR: %s's 'path' is not unique in %s. Continuing but ignoring this sim."
                    % (s, self.config_file),
                    "DARK_RED",
                )
                continue
            # Add the CPU format string (won't fail if there is no CPU placeholder)
            self.config[s]["binary"] = self.config[s]["binary"].format(
                cpu=self.trick_host_cpu
            )
            ...
            thisSim = TrickWorkflow.Sim(
                name=s,
                sim_dir=self.config[s]["path"],
                description=self.config[s]["description"],
                labels=self.config[s]["labels"],
                prebuild_cmd=self.env,
                build_cmd=trick_CP,
                cpus=self.cpus,
                size=self.config[s]["size"],
                phase=self.config[s]["phase"],
            ...
            for r in self.config[s]["runs"]:
                ...
                just_RUN_dir = os.path.dirname(
                    just_RUN
                )  # Drop path before     RUN../..py
                ...
                #  continue
                if (
                    just_RUN_dir in all_run_paths
                    and self.config[s]["parallel_safety"] == "strict"
                ):
                    cprint(
                        "ERROR: %s's run directory %s is not unique in %s. With setting "
                        ...
                        % (s, r, self.config_file),
                        "DARK_RED",
                    )
                    continue

                thisRun = TrickWorkflow.Run(
                    sim_dir=self.config[s]["path"],
                    input_file=r,
                    ...
                    phase=self.config[s]["runs"][r]["phase"],
                    log_dir=self.log_dir,
                )
                ...
                try:
                    theseRuns = (
                        thisRun.multiply()
                    )  # If [from-to] notation used, expand runs
                except RuntimeError as e:
                    msg = (
                        "ERROR: Unable to multiply run %s in sim %s. Ignoring this run. "
                ...
                all_run_paths.append(
                    just_RUN_dir
                )  # Keep track of all runs to check parallel safety

            self.sims.append(thisSim)  # Add Sim() to internal list
        if len(self.sims) < 1:  # At minimum, one valid SIM structure must exist
            msg = (
                "ERROR: After validating config file, there is insufficient information to continue."
    ...
    def get_trick_host_cpu(self):
        ...
        result = run_subprocess(
            gte_cmd, m_stdout=subprocess.PIPE, m_stderr=subprocess.PIPE
        )
        self.trick_host_cpu = result.stdout.strip()
        if self.trick_host_cpu == None or self.trick_host_cpu == "":
            tprint(
                "ERROR: Unable to determine TRICK_HOST_CPU, you may encounter problems "
                "if you continue... ",
                "DARK_RED",
            )
    ...
    def get_sim(self, identifier):
        ...
        if type(identifier) != str:
            raise TypeError(
                "get_sim() only accepts a Sim name or Sim path from project top level"
            )
    ...
    def status_summary(self):
        ...
        executed_builds = [
            ...
            if build.get_status() != Job.Status.NOT_STARTED
        ]
        executed_runs = [
            run for run in all_runs if run.get_status() != Job.Status.NOT_STARTED
        ]
        executed_analysis = [
            a for a in all_analysis if a.get_status() != Job.Status.NOT_STARTED
        ]
        executed_comparisons = [
            c for c in all_comparisons if c.status != Job.Status.NOT_STARTED
        ]
        executed_valgrind = [
            v for v in all_valgrind if v.get_status() != Job.Status.NOT_STARTED
        ]

        ok_builds = [
            build
            for build in executed_builds
            if build.get_status() == Job.Status.SUCCESS
        ]
        ok_runs = [
            run for run in executed_runs if run.get_status() == Job.Status.SUCCESS
        ]
        ok_analysis = [
            a for a in executed_analysis if a.get_status() == Job.Status.SUCCESS
        ]
        ok_comparisons = [
            c for c in executed_comparisons if c.status == Job.Status.SUCCESS
        ]
        ok_valgrind = [
            v for v in executed_valgrind if v.get_status() == Job.Status.SUCCESS
        ]

        tprint("SUMMARY:")
        if executed_builds:
            tprint(
                "  {0} out of {1} builds succeeded".format(
                    len(ok_builds), len(executed_builds)
                )
            )
        if executed_runs:
            tprint(
                "  {0} out of {1} runs succeeded".format(
                    len(ok_runs), len(executed_runs)
                )
            )
        if executed_analysis:
            tprint(
                "  {0} out of {1} analyses succeeded".format(
                    len(ok_analysis), len(executed_analysis)
                )
            )
        if executed_comparisons:
            tprint(
                "  {0} out of {1} comparisons succeeded".format(
                    len(ok_comparisons), len(executed_comparisons)
                )
            )
        if executed_valgrind:
            tprint(
                "  {0} out of {1} valgrind runs succeeded".format(
                    len(ok_valgrind), len(executed_valgrind)
                )
            )

        if (
            executed_builds == ok_builds
            and executed_runs == ok_runs
            and executed_analysis == ok_analysis
            and executed_comparisons == ok_comparisons
            and executed_valgrind == ok_valgrind
        ):
            return "SUCCESS"
        else:
            return "FAILURE"
    ...
    def get_jobs(self, kind, phase=None):
        ...
        if kind == "build" or kind == "builds":
            ...
            raise TypeError(
                "get_jobs() only accepts kinds: build, run, valgrind, analysis"
            )
    ...
    def get_koviz_report_job(self, test_dir, baseline_dir, pres=None):
        ...
        for dir in dirs:
            if not os.path.exists(dir):
                msg = (
                    "ERROR: %s not found, Returning None in get_koviz_report_job()"
                    % (dir)
                )
                tprint(msg, "DARK_RED")
                return None, msg
        ...
        cmd += " -pdf %s %s %s" % (
            os.path.join(
                self.log_dir,
                (unixify_string(test_dir) + "_vs_" + unixify_string(baseline_dir))
                + ".pdf",
            ),
            test_dir,
            baseline_dir,
        ...
        return (
            Job(
                name=name,
                command=cmd,
                log_file=os.path.join(self.log_dir, "." + unixify_string(name)),
    ...
    def get_koviz_report_jobs(self):
        ...
        koviz_errors = []
        all_cmp_dirnames = (
            self.get_unique_comparison_dirs()
        )  # List of (test_dir, baseline_dir) tuples
        ...
        for test_dir, baseline_dir in all_cmp_dirnames:
            if test_dir and baseline_dir:  # Will be None if dir is empty
                job, error = self.get_koviz_report_job(
                    test_dir, baseline_dir, pres="error"
                )
                if job:
    ...
    class Sim(object):
        ...
        def __init__(
            ...
            self.labels = (
                labels  # Options list of user-specified labels associated w/ this sim
            )
        ...
        def get_build_job(self):
            ...
            if not self.build_job:
                ...
                self.build_job = FileSizeJob(
                    name=name,
                    command=(
                        "%s cd %s && export MAKEFLAGS=-j%d && %s"
                        % (self.prebuild_cmd, self.sim_dir, self.cpus, self.build_cmd)
                    ),
                    log_file=os.path.join(
                        self.log_dir, unixify_string(self.sim_dir) + "_build.txt"
                    ),
                    size=self.size,
                )
        ...
        def get_run_jobs(self, kind="normal", phase=None):
            ...
            if kind == "valgrind":
                all_jobs = [
                    r.get_run_job() for r in self.valgrind_runs if r.phase in phases
                ]
            else:
        ...
        def get_analysis_jobs(self, phase=None):
            ...
            return [
                r.analysis
                for r in (self.runs + self.valgrind_runs)
                if (r.analysis and r.phase in phases)
            ]
        ...
        def get_run(self, input_file):
            ...
            if type(input_file) != str:
                raise TypeError(
                    "get_run() only accepts the unique key representing the entire input to"
        ...
        def set_phase(self, phase):
            ...
            if (
                ...
                or phase < TrickWorkflow.allowed_phase_range["min"]
                or phase > TrickWorkflow.allowed_phase_range["max"]
            ):
                msg = (
                    "ERROR: set_phase() for SIM %s must be an integer between [%s, %s]"
                    % (
                        self.name,
                        TrickWorkflow.allowed_phase_range["min"],
                        TrickWorkflow.allowed_phase_range["max"],
        ...
        def report(self, indent=""):
            ...
            tprint(
                ...
                + "  Build: %-20s  %-60s  "
                % (
                    self.build_job._translate_status()
                    if self.build_job
                    else printer.colorstr("NOT RUN", "DARK_YELLOW"),
                    self.sim_dir,
                )
            )
            if self.runs:
    ...
    class Run(object):
        ...
        def add_analysis(self, cmd):
            ...
            if self.analysis:
                tprint(
                    "WARNING: Overwriting analysis definition for %s's %s"
                    % (self.sim_dir, self.input_file),
                    "DARK_YELLOW",
                )
            logfile = (
                os.path.join(self.log_dir, unixify_string(self.sim_dir))
                + "_"
                + unixify_string(self.input_file)
                + "_analysis.txt"
            )
            self.analysis = Job(
                name=textwrap.shorten(cmd, width=90),
                command=self.prerun_cmd + " " + cmd,
                log_file=logfile,
                expected_exit_status=0,
            )
        ...
        def set_phase(self, phase):
            ...
            if (
                ...
                msg = (
                    "ERROR: set_phase() for %s/%s must be an integer between [%s, %s]"
                    % (
                        self.sim_dir,
                        self.input_file,
                        TrickWorkflow.allowed_phase_range["min"],
        ...
        def report(self, indent=""):
            ...
            tprint(
                ...
                + "  %-20s       %s"
                % (
                    self.run_job._translate_status()
                    if self.run_job
                    else printer.colorstr("NOT RUN", "DARK_YELLOW"),
                    self.input_file,
                )
        ...
        def get_run_job(self):
            ...
            if not self.run_job:
                ...
                if self.valgrind_flags:
                    cmd += "valgrind %s --log-file=%s " % (
                        self.valgrind_flags,
                        (
                            os.path.join(self.log_dir, sim_name)
                            + "_valgrind_"
                            + unixify_string(self.input_file)
                            + ".valgrind"
                        ),
                ...
                self.run_job = SingleRun(
                    name=name,
                    command=(cmd),
                    expected_exit_status=self.returns,
                    log_file=logfile,
                )
            return self.run_job
        ...
        def _get_range_list(self, pattern):
            ...
            if any([char not in pattern for char in must_exist]):
                msg = (
                    'ERROR: Pattern %s doesn\'t match expected syntax of "[min-max]"'
                    % (pattern)
                )
                raise RuntimeError(msg)
            ...
            try:
                ...
                msg = (
                    "ERROR: Pattern %s minimum cannot be converted to integer. \n%s"
                    % (pattern, e)
                )
                raise RuntimeError(msg)
            try:
                ...
                msg = (
                    "ERROR: Pattern %s maximum cannot be converted to integer. \n%s"
                    % (pattern, e)
                )
                raise RuntimeError(msg)
            if min >= (max):
    ...
    class Comparison(object):
        ...
        def __init__(self, test_data, baseline_data):
            ...
            """
            self.test_data = os.path.expandvars(
                test_data
            )  # Test data file with respect to project top level
            self.baseline_data = os.path.expandvars(
                baseline_data
            )  # Baseline data file with respect to project top level
        ...
        def _translate_status(self):
            ...
            text, color = {
                ...
                Job.Status.FAILED: ("FAIL", "DARK_RED"),
            }[self.get_status()]
        ...
        def pattern_replace(self, expecting_pattern, replace_with):
            ...
            if (rsb and rsb != expecting_pattern) or (rst and rst != expecting_pattern):
                msg = (
                    "ERROR: [min-max] pattern from run (%s) must match pattern in run's"
                    " comparisons test (%s) and baseline (%s) sections, if specified."
                    % (expecting_pattern, rst, rsb)
                )
                raise RuntimeError(msg)
            if rsb:
                self.baseline_data = self.baseline_data.replace(
                    expecting_pattern, replace_with
                )
            if rst:
...
class SingleRun(Job):
    ...
    def __init__(
        self, name, command, log_file, expected_exit_status=0, use_var_server=True
    ):
        """
        Initialize this instance.
    ...
    def start(self):
        ...
        def connect():
            ...
            while self.get_status() is self.Status.RUNNING:
                ...
                try:
                    children = psutil.Process(self._process.pid).children(
                        recursive=True
                    )
                    outerbreak = False
                ...
                try:
                    ...
                    self._variable_server.add_variables(*self._create_variables())
    ...
    def _running_string(self):
        ...
        if self._connected:
            return (
                elapsed_time + self._connected_string() + "\n" + self._connected_bar()
            )
    ...
    def _create_variables(self):
        self._tics = variable_server.Variable("trick_sys.sched.time_tics", type_=float)
    ...
    def _connected_string(self):
        return "      {0}       {1}".format(self._sim_time(), self._average_speed())
    ...
    def _connected_bar(self):
        ...
        return create_progress_bar(progress, "{0:.1f}%".format(100 * progress))
