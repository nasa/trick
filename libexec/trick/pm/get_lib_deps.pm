...
sub get_lib_deps ($$) {
    ...
    @raw_lib_deps =
        ( $contents =~ /LIBRARY[ _]DEPENDENC(?:Y|IES)\s*:(.*?)(?=^[ \t]*\**[ \t]*[A-Z][A-Z _\t]*:|\*\/)/gmsi );
    ...
    foreach (@ordered_resolved_files) {
        if ( my $exclude_path = get_containing_path( $_, @ext_lib_paths ) ) {
            if ( get_containing_path( $_, @ext_lib_paths_overrides ) eq 0 ) {
                print "[95mDep Skip[39m   TRICK_EXT_LIB_DIRS: [4m$exclude_path[24m"
                    . substr( $_, length $exclude_path ) . "\n"
                    if $verbose_build;
                next;
            }
        }
...
sub get_s_source_deps() {
    ...
    my @include_paths = map( "-I$_",
        (
            get_include_paths(),             "$ENV{TRICK_HOME}",
            "$ENV{TRICK_HOME}/include",      "$ENV{TRICK_HOME}/include/trick/compat",
