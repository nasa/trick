...
sub s_source($) {
    ...
    #--------------------------------------------------------------
    # Generate S_source.c
    open S_SOURCE, ">$ENV{'TRICK_BUILD_DIR'}build/S_source.cpp"
        or die "Couldn't open $ENV{'TRICK_BUILD_DIR'}build/S_source.cpp!\n";
    open S_SOURCE_H, ">$ENV{'TRICK_BUILD_DIR'}S_source.hh" or die "Couldn't open $ENV{'TRICK_BUILD_DIR'}S_source.hh!\n";
    open TOP_LEVEL_OBJECTS_RESOURCE, ">$ENV{'TRICK_BUILD_DIR'}build/top_level_objects.resource"
        or die "Couldn't open $ENV{'TRICK_BUILD_DIR'}top_level_objects.resource!";
...
#include \"trick/UnitsMap.hh\"\n\n";
    ...
    foreach my $inst ( @{ $$sim_ref{instances} } ) {
        ...
        $temp_type =~ s/[<>,:*]/_/g;
        print S_SOURCE " " x 4,
            "TMM_declare_ext_var(&$inst, TRICK_STRUCTURED,\"$temp_type\", 0, \"$inst\", 0, NULL) ;\n";
    ...
    foreach my $integ_loop ( sort sim_integ_by_name @{ $$sim_ref{integ_loop} } ) {
        ...
        print S_SOURCE " " x 4,
            "TMM_declare_ext_var(&$$integ_loop{name}, TRICK_STRUCTURED,\"IntegLoopSimObject\", 0, \"$$integ_loop{name}\", 0, NULL) ;\n";
    ...
    open S_INSTANCE, ">$ENV{TRICK_BUILD_DIR}build/CP_instances"
        or die "Couldn't open $ENV{TRICK_BUILD_DIR}build/CP_instances!\n";
