...
sub write_makefile_swig_deps() {
    open DEPENDENCIES_FILE, ">$ENV{TRICK_BUILD_DIR}build/Makefile_swig_deps"
        or die "Could not open $ENV{TRICK_BUILD_DIR}build/Makefile_swig_deps for writing";
...
sub write_makefile_swig() {
    ...
    open MAKEFILE, ">$ENV{TRICK_BUILD_DIR}build/Makefile_swig"
        or die "Could not open $ENV{TRICK_BUILD_DIR}build/Makefile_swig for writing";
    print MAKEFILE
...
ifeq (\$(IS_CC_CLANG), 1)
    ...
    TRICK_SYSTEM_SWIG_CFLAGS += -Wno-unused-but-set-variable -Wno-maybe-uninitialized -Wno-cast-function-type
endif
...
";
    ...
    open INITSWIGFILE, ">$ENV{TRICK_BUILD_DIR}build/init_swig_modules.cpp"
        or die "Could not open $ENV{TRICK_BUILD_DIR}build/init_swig_modules.cpp for writing";
...
sub write_lib_files() {
    open PY_LINK_LIST, ">$ENV{TRICK_BUILD_DIR}build/py_link_list"
        or die "Could not open $ENV{TRICK_BUILD_DIR}build/py_link_list for writing";
    open TRICKIFY_PY_LINK_LIST, ">$ENV{TRICK_BUILD_DIR}build/trickify_py_link_list"
        or die "Could not open $ENV{TRICK_BUILD_DIR}build/trickify_py_link_list for writing";
    open SWIGLIB, ">$ENV{TRICK_BUILD_DIR}build/S_library_swig"
        or die "Could not open $ENV{TRICK_BUILD_DIR}build/S_library_swig for writing";
