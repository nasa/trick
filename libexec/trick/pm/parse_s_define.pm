...
sub check_for_cpp_comment (\$\%\@) {
    ...
    if (    $$text eq "//"
        and ( $context->{"string"} == 0 )
        and ( $context->{"c_comment"} == 0 )
        and ( $context->{"angle"} == 0 ) )
    {
...
sub parse_s_define ($) {
    ...
    @{ $$sim_ref{inc_paths} } = (
        get_include_paths(),
        $ENV{TRICK_SYSTEM_CFLAGS} =~ /-(?:I|isystem)(\S+)/g,
        "$ENV{TRICK_HOME}/trick_source",
        "../include", $ENV{'TRICK_BUILD_DIR'}
    );
...
sub handle_sim_class ($$$$) {
    ...
    ( $full_template_args, $class_name, $base_clause ) =
        $s =~ /(?:\s*template\s*<\s*([^>]+?)\s*>)?\s*class\s+(\w+)\s*(?::\s*(.+?)\s*)?$/;
...
#$constructor_contents =~ s/\{(\s+)/\{$1int ii __attribute__ ((unused)) = $$sim_ref{sim_class_index}{$class_name} ;\n/ ;
        while ( $constructor_contents =~ s/^(.*?)($sim_class_job_def)//s ) {
            ...
            if ( $final_contents !~ /new\s*$/ ) {
                ...
                ( $job_push, $job_call, $is_dynamic_event ) =
                    handle_sim_class_job( $job, $$sim_ref{sim_class_index}{$class_name}, $sim_ref );
                $final_contents .= "\n            $job_push";
                if ( $is_dynamic_event == 1 ) {
                    push @double_job_calls, $job_call;
                    $double_call_functions .=
                        "        case $$sim_ref{sim_class_index}{$class_name}:\n            trick_ret = $job_call ;\n            break ;\n";
                }
                else {
    ...
    if ( $constructor_found == 1 ) {
        ...
        $double_call_functions .=
            "::call_function_double ( Trick::JobData * curr_job __attribute__ ((unused)) ) {return 0.0;}\n";
        $$sim_ref{sim_class_call_functions} .= $int_call_functions . $double_call_functions;
    }
...
sub handle_compiler_directive($$) {
    ...
    if ( $s =~ /^\#include\s+\</ ) {
        ...
        if ( $s =~ s/^\#include \"(\/.*)\"/$1/ ) {
            ...
            if ( $file_name eq "" ) {
                trick_formatted_print( $$sim_ref{fh}, "[91m[1m", "Error      ", "[0m",
            ...
            trick_print( $$sim_ref{fh},
                "ERROR: S_define:$$sim_ref{line_num}: ##include expects \"FILENAME\" (double quotes for FILENAME) \n",
                "title_red", $$sim_ref{args}{v} );
        }
    }
}
